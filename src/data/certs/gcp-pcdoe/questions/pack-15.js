export const GCP_PCDOE_QUESTIONS_15 = [
  {
    ...
    options: [
      { id: 'A', text: "Allow individual engineers to run 'terraform apply' from local laptops against production." },
      { id: 'B', text: "Implement an automated CI/CD pipeline (e.g. Cloud Build or Atlantis) that executes terraform plan on PRs and terraform apply on merge." },
      { id: 'C', text: "Delete the Terraform state file after every infrastructure change." },
      { id: 'D', text: "Store Terraform state files in unversioned email attachments." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Rename the column in-place in production during active daytime user traffic." },
      { id: 'D', text: "Implement the Expand/Contract (Parallel Run) pattern: add the new column (expand), dual-write, backfill data, migrate reads, and drop the old column (contract)." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    question: "Which engineering approach should the DevOps team select to manage this demand reliably without manual intervention?",
    options: [
      { id: 'A', text: "Disable all metric collection to conserve network egress bandwidth." },
      { id: 'B', text: "Require application threads to perform synchronous HTTP POST calls to an external logging SaaS on every incoming request." },
      { id: 'C', text: "Write metrics to flat text files inside containers and manually SCP them periodically to an external server." },
      { id: 'D', text: "Deploy the OpenTelemetry Collector as a sidecar or DaemonSet to scrape Prometheus endpoints and export metrics to Cloud Monitoring using the Google Cloud Monitoring exporter." }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Deploy the OpenTelemetry Collector as a sidecar or DaemonSet to scrape Prometheus endpoints and export metrics to Cloud Monitoring using the Google Cloud Monitoring exporter. Deploying the OpenTelemetry Collector allows standard metric collection from Prometheus endpoints and direct ingestion into Cloud Monitoring via the Google Cloud Monitoring exporter, decoupling instrumentation from storage and keeping latency minimal.",
  ...
  },
  {
    ...
    question: "Which architectural approach or configuration satisfies these operational continuity and resilience objectives?",
    options: [
      { id: 'A', text: "Configure alerts to page on-call engineers immediately whenever a single 500 error response occurs." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Implement multi-window, multi-burn-rate alerting policies in Cloud Monitoring that trigger notifications based on both short-term (fast burn) and long-term (slow burn) error budget consumption." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Set a static CPU threshold alert at 50% utilization regardless of user request success rate." },
