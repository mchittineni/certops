export const GCP_PCDOE_QUESTIONS_15 = [
  {
    ...
    options: [
      { id: 'A', text: "Store Terraform state files in unversioned email attachments." },
      { id: 'B', text: "Delete the Terraform state file after every infrastructure change." },
      { id: 'C', text: "Allow individual engineers to run 'terraform apply' from local laptops against production." },
      { id: 'D', text: "Implement an automated CI/CD pipeline (e.g. Cloud Build or Atlantis) that executes terraform plan on PRs and terraform apply on merge." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Store Terraform state files in unversioned email attachments." },
      { id: 'B', text: "Delete the Terraform state file after every infrastructure change." },
      { id: 'C', text: "Implement an automated CI/CD pipeline (e.g. Cloud Build or Atlantis) that executes terraform plan on PRs and terraform apply on merge." },
      { id: 'D', text: "Allow individual engineers to run 'terraform apply' from local laptops against production." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "Implement an automated CI/CD pipeline (e.g. Cloud Build or Atlantis) that executes terraform plan on PRs and terraform apply on merge. Automating Infrastructure as Code (IaC) via CI/CD pipelines ensures that all infrastructure changes are peer-reviewed, version-controlled, and audited. The pipeline runs `terraform plan` on pull requests for validation and executes `terraform apply` with state locking in Cloud Storage upon merge.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Delete the Terraform state file after every infrastructure change." },
      { id: 'B', text: "Store Terraform state files in unversioned email attachments." },
      { id: 'C', text: "Implement an automated CI/CD pipeline (e.g. Cloud Build or Atlantis) that executes terraform plan on PRs and terraform apply on merge." },
      { id: 'D', text: "Allow individual engineers to run 'terraform apply' from local laptops against production." }
    ],
    correctAnswers: ['C'],
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Hardcode the API credentials directly in the cloudbuild.yaml file." },
      { id: 'B', text: "Commit credentials to a public GitHub repository." },
      { id: 'C', text: "Retrieve secrets from Secret Manager using Cloud Build's native 'availableSecrets' and 'secretEnv' configuration blocks." },
      { id: 'D', text: "Echo the API secret in cleartext to stdout during build execution." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Commit credentials to a public GitHub repository." },
      { id: 'D', text: "Echo the API secret in cleartext to stdout during build execution." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Echo the API secret in cleartext to stdout during build execution." },
      { id: 'B', text: "Retrieve secrets from Secret Manager using Cloud Build's native 'availableSecrets' and 'secretEnv' configuration blocks." },
      { id: 'C', text: "Hardcode the API credentials directly in the cloudbuild.yaml file." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Commit credentials to a public GitHub repository." },
      { id: 'B', text: "Echo the API secret in cleartext to stdout during build execution." },
      { id: 'C', text: "Retrieve secrets from Secret Manager using Cloud Build's native 'availableSecrets' and 'secretEnv' configuration blocks." },
      { id: 'D', text: "Hardcode the API credentials directly in the cloudbuild.yaml file." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "Retrieve secrets from Secret Manager using Cloud Build's native 'availableSecrets' and 'secretEnv' configuration blocks. Cloud Build provides native integration with Secret Manager. Using the `availableSecrets` block in `cloudbuild.yaml`, secrets are fetched securely at build time and injected into step environments (`secretEnv`) without exposing raw credentials in configuration files or build logs.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Echo the API secret in cleartext to stdout during build execution." },
      { id: 'C', text: "Commit credentials to a public GitHub repository." },
      { id: 'D', text: "Retrieve secrets from Secret Manager using Cloud Build's native 'availableSecrets' and 'secretEnv' configuration blocks." }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Retrieve secrets from Secret Manager using Cloud Build's native 'availableSecrets' and 'secretEnv' configuration blocks. Cloud Build provides native integration with Secret Manager. Using the `availableSecrets` block in `cloudbuild.yaml`, secrets are fetched securely at build time and injected into step environments (`secretEnv`) without exposing raw credentials in configuration files or build logs.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Shut down all application servers for 12 hours while running manual database alter scripts." },
      { id: 'B', text: "Implement the Expand/Contract (Parallel Run) pattern: add the new column (expand), dual-write, backfill data, migrate reads, and drop the old column (contract)." },
      { id: 'C', text: "Never alter database schemas to avoid breaking running applications." },
      { id: 'D', text: "Rename the column in-place in production during active daytime user traffic." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Implement the Expand/Contract (Parallel Run) pattern: add the new column (expand), dual-write, backfill data, migrate reads, and drop the old column (contract)." },
      { id: 'D', text: "Rename the column in-place in production during active daytime user traffic." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Rename the column in-place in production during active daytime user traffic." },
      { id: 'B', text: "Shut down all application servers for 12 hours while running manual database alter scripts." },
      { id: 'C', text: "Implement the Expand/Contract (Parallel Run) pattern: add the new column (expand), dual-write, backfill data, migrate reads, and drop the old column (contract)." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Implement the Expand/Contract (Parallel Run) pattern: add the new column (expand), dual-write, backfill data, migrate reads, and drop the old column (contract)." },
      { id: 'D', text: "Shut down all application servers for 12 hours while running manual database alter scripts." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "Implement the Expand/Contract (Parallel Run) pattern: add the new column (expand), dual-write, backfill data, migrate reads, and drop the old column (contract). The Expand/Contract pattern (Parallel Run) enables zero-downtime database refactoring. In the Expand phase, new schema elements are added while maintaining backward compatibility. After application code is updated to write to both schemas, the Contract phase safely removes deprecated structures.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Write metrics to flat text files inside containers and manually SCP them periodically to an external server." },
      { id: 'B', text: "Deploy the OpenTelemetry Collector as a sidecar or DaemonSet to scrape Prometheus endpoints and export metrics to Cloud Monitoring using the Google Cloud Monitoring exporter." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Write metrics to flat text files inside containers and manually SCP them periodically to an external server." },
      { id: 'B', text: "Disable all metric collection to conserve network egress bandwidth." },
      { id: 'C', text: "Deploy the OpenTelemetry Collector as a sidecar or DaemonSet to scrape Prometheus endpoints and export metrics to Cloud Monitoring using the Google Cloud Monitoring exporter." },
      { id: 'D', text: "Require application threads to perform synchronous HTTP POST calls to an external logging SaaS on every incoming request." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "Deploy the OpenTelemetry Collector as a sidecar or DaemonSet to scrape Prometheus endpoints and export metrics to Cloud Monitoring using the Google Cloud Monitoring exporter. Deploying the OpenTelemetry Collector allows standard metric collection from Prometheus endpoints and direct ingestion into Cloud Monitoring via the Google Cloud Monitoring exporter, decoupling instrumentation from storage and keeping latency minimal.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Require application threads to perform synchronous HTTP POST calls to an external logging SaaS on every incoming request." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Write metrics to flat text files inside containers and manually SCP them periodically to an external server." },
      { id: 'B', text: "Deploy the OpenTelemetry Collector as a sidecar or DaemonSet to scrape Prometheus endpoints and export metrics to Cloud Monitoring using the Google Cloud Monitoring exporter." },
      { id: 'C', text: "Disable all metric collection to conserve network egress bandwidth." },
      { id: 'D', text: "Require application threads to perform synchronous HTTP POST calls to an external logging SaaS on every incoming request." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set a static CPU threshold alert at 50% utilization regardless of user request success rate." },
      { id: 'B', text: "Disable all automated alerts and rely on customer tweets and support tickets to detect service degradation." },
      { id: 'C', text: "Configure alerts to page on-call engineers immediately whenever a single 500 error response occurs." },
      { id: 'D', text: "Implement multi-window, multi-burn-rate alerting policies in Cloud Monitoring that trigger notifications based on both short-term (fast burn) and long-term (slow burn) error budget consumption." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Disable all automated alerts and rely on customer tweets and support tickets to detect service degradation." },
      { id: 'B', text: "Implement multi-window, multi-burn-rate alerting policies in Cloud Monitoring that trigger notifications based on both short-term (fast burn) and long-term (slow burn) error budget consumption." },
      { id: 'C', text: "Configure alerts to page on-call engineers immediately whenever a single 500 error response occurs." },
      { id: 'D', text: "Set a static CPU threshold alert at 50% utilization regardless of user request success rate." }
    ],
    correctAnswers: ['B'],
    type: "single",
    explanation: "Implement multi-window, multi-burn-rate alerting policies in Cloud Monitoring that trigger notifications based on both short-term (fast burn) and long-term (slow burn) error budget consumption. Multi-window multi-burn-rate alerts evaluate both short lookback windows (to detect rapid, catastrophic budget depletion) and long lookback windows (to detect slow, continuous erosion). This SRE standard avoids false alarms while ensuring prompt intervention when budgets are at risk.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set a static CPU threshold alert at 50% utilization regardless of user request success rate." },
      { id: 'B', text: "Implement multi-window, multi-burn-rate alerting policies in Cloud Monitoring that trigger notifications based on both short-term (fast burn) and long-term (slow burn) error budget consumption." },
      { id: 'C', text: "Disable all automated alerts and rely on customer tweets and support tickets to detect service degradation." },
      { id: 'D', text: "Configure alerts to page on-call engineers immediately whenever a single 500 error response occurs." }
    ],
    correctAnswers: ['B'],
    type: "single",
    explanation: "Implement multi-window, multi-burn-rate alerting policies in Cloud Monitoring that trigger notifications based on both short-term (fast burn) and long-term (slow burn) error budget consumption. Multi-window multi-burn-rate alerts evaluate both short lookback windows (to detect rapid, catastrophic budget depletion) and long lookback windows (to detect slow, continuous erosion). This SRE standard avoids false alarms while ensuring prompt intervention when budgets are at risk.",
