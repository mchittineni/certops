export const GITHUB_GHAS_QUESTIONS_3 = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Resolve twenty-nine of the alerts as duplicates and remediate only the repository where it was first committed." },
      { id: 'C', text: "Leave the credential in place and add push protection so no further copies are added." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The alert will close itself once the commit is older than two years." },
      ...
      { id: 'D', text: "The alert is valid because the value remains retrievable from the commit history by anyone who can clone the repository, so it must be rotated regardless of whether current files contain it." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Count total bypasses and set a target of zero, since any bypass indicates misuse." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The dependency graph of the producing repository, which records published artifacts." },
      { id: 'B', text: "The image digest recorded in the deployment manifest, which establishes the build origin." },
      { id: 'C', text: "A Dependabot alert, which would be raised if an artifact came from an unexpected source." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Disable security updates and rely on scheduled version updates to pick up the fixes eventually." },
      { id: 'B', text: "Configure grouping so related security updates are combined into a single pull request per group, which removes the mutual conflicts and gives reviewers one change to assess." },
      { id: 'C', text: "Move the schedule to monthly, which is the only lever available for security updates." },
      { id: 'D', text: "Lower the open pull requests limit, which merges the excess updates into the permitted ones." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The suggestion is produced by re-running the analysis in a repair mode that proves the fix is correct." },
      { id: 'B', text: "The suggestion is a template drawn from the query documentation rather than from the code, so it rarely applies." },
      { id: 'C', text: "Autofix generates a suggested change for supported alert types that the developer must review, test and accept, because it is a suggestion informed by the alert rather than a verified fix." },
      { id: 'D', text: "Autofix applies verified fixes automatically once the alert is confirmed, so review is unnecessary." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Compare the alert count against another repository to establish a baseline." },
      { id: 'B', text: "Re-run with the debug option enabled so the workflow uploads the database and the extractor logs as artifacts, then inspect them to confirm whether source files were extracted at all." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Whether the default branch was renamed, which is the only cause of stale analysis." },
      { id: 'B', text: "Whether the workflow is disabled, whether its triggers still match the branches receiving commits, whether recent runs are failing, and whether the analysis is only configured for a branch that is no longer used." },
      { id: 'C', text: "Whether the alerts have exceeded the retention period, which causes the analysis to be marked stale." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Number of repositories with at least one enabled feature, since coverage is the whole programme." },
      { id: 'B', text: "Time to remediate by severity, the age distribution of the open backlog, and coverage of the estate by each feature, since together they show whether findings are being fixed promptly, whether old risk is accumulating, and whether the numbers describe the whole estate." },
      { id: 'C', text: "Total alerts created per month, since a rising number shows the tooling is working." },
