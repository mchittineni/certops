export const GITHUB_GHAS_QUESTIONS_3 = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Export the security overview page on a schedule and ingest the export." },
      { id: 'C', text: "Subscribe to the secret scanning alert webhook at organization level for near real time delivery, and use the alerts API for backfill and periodic reconciliation." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Leave the credential in place and add push protection so no further copies are added." },
      { id: 'B', text: "Use the organization-wide alert view to establish the full set of affected repositories and owners, provision a replacement credential and migrate consumers to it from a managed secret store, then revoke the original and resolve every alert." },
      { id: 'C', text: "Rotate the credential immediately and let each team fix its own breakage, since exposure outweighs availability in all cases." },
      { id: 'D', text: "Resolve twenty-nine of the alerts as duplicates and remediate only the repository where it was first committed." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The average time taken to resolve alerts." },
      ...
      { id: 'D', text: "The total number of repositories with the feature enabled." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Disable bypass entirely, which removes the need for review." },
      { id: 'B', text: "Review only the bypasses where the reason given was that the value is used in tests." },
      { id: 'C', text: "Examine the bypass events and their stated reasons alongside the alerts each bypass raised, looking for repositories or teams with a high bypass rate and for bypasses on credentials that validity checks report as active." },
      { id: 'D', text: "Count total bypasses and set a target of zero, since any bypass indicates misuse." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "They are unrelated: repository advisories are internal notes and never reach the global database." },
      { id: 'D', text: "Global advisories are generated automatically from code scanning alerts across public repositories." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set the check to fail only on critical findings and suppress the display of everything else." },
      { id: 'B', text: "Replace dependency review with Dependabot alerts, which do not block pull requests." },
      { id: 'C', text: "Configure a severity threshold so the check fails only at or above an agreed level, with the remaining findings still reported in the pull request for awareness, and keep the licence policy separate." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The image digest recorded in the deployment manifest, which establishes the build origin." },
      { id: 'B', text: "A Dependabot alert, which would be raised if an artifact came from an unexpected source." },
      { id: 'C', text: "Build provenance attestations generated by the producing workflow and verified by the consumer against the expected repository and workflow identity before the image is deployed." },
      { id: 'D', text: "The dependency graph of the producing repository, which records published artifacts." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Move the schedule to monthly, which is the only lever available for security updates." },
      { id: 'C', text: "Lower the open pull requests limit, which merges the excess updates into the permitted ones." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Severity is derived from the file path, with results in production directories ranked higher." },
      { id: 'B', text: "Severity is assigned by GitHub based on how many alerts of that rule exist across public repositories." },
      { id: 'C', text: "Severity comes from a security severity property carried on the rule in the uploaded results, which CodeQL populates for its own queries and which a third-party tool must set for its findings to be ranked and gated consistently." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Autofix applies verified fixes automatically once the alert is confirmed, so review is unnecessary." },
      { id: 'B', text: "Autofix generates a suggested change for supported alert types that the developer must review, test and accept, because it is a suggestion informed by the alert rather than a verified fix." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Add the affected paths to the ignore list, which removes those findings." },
      { id: 'B', text: "Continue dismissing them, because excluding a security query is never acceptable." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Screen scraping the security overview page, since alert data is not exposed programmatically." },
      { id: 'B', text: "The audit log, which records each alert as it is created." },
      { id: 'C', text: "A scheduled workflow in each repository that writes its own alert count to a shared file." },
      { id: 'D', text: "The code scanning alerts API at organization level, which returns alerts with severity, state and timestamps for aggregation into the dashboard." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Whether the default branch was renamed, which is the only cause of stale analysis." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "All three detect the same findings through different interfaces, so one can be chosen on preference." },
      { id: 'B', text: "They address different classes of problem: code scanning finds flaws in code the team writes, dependency tooling finds known vulnerabilities in components it consumes, and secret scanning finds exposed credentials, so none substitutes for another." },
      { id: 'C', text: "Code scanning subsumes dependency scanning, since vulnerable library calls appear in the analysed code." },
      { id: 'D', text: "Secret scanning subsumes code scanning, since credential exposure is the dominant risk." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Enable detection features only, and defer all preventative controls until the backlog reaches zero." },
      ...
      { id: 'D', text: "Enable every feature everywhere on the same day so the full risk picture is available immediately for prioritisation." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Number of dismissed alerts, since dismissal indicates active triage." },
      { id: 'D', text: "Number of repositories with at least one enabled feature, since coverage is the whole programme." }
    ],
    correctAnswers: ['A'],
    type: "single",
