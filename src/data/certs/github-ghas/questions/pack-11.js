export const GITHUB_GHAS_QUESTIONS_11 = [
  ...
  },
  {
    ...
    correctAnswers: ['A'],
    type: "single",
    explanation: "Rely on secret scanning partner patterns. When a supported partner token is found in a public source, such as a public repository, a public npm package or a public Actions log, GitHub sends the match to the token issuer, which can then revoke it. This happens only for public sources: a match in a private repository alerts the repository administrators and the committer instead and is not sent to the issuer, which is why the requirement is scoped to public repositories. Push protection blocks new pushes but does nothing about a token that is already published, an alert webhook needs your own revocation logic, and validity checks only report whether the token is still active.",
  ...
  },
  {
    ...
    correctAnswers: ['A'],
    type: "single",
    explanation: "Enable validity checks, which ask the token provider whether a detected secret still works. Each alert then shows its validity as active, inactive or unknown (in the UI: 'Active secret', 'Secret inactive' and 'Possibly active secret'), so responders can work active credentials first and deprioritize inactive test tokens. Partner patterns hand a public token to its issuer, push protection stops new pushes, and path-based closing ignores whether a token is live, so none of them sorts alerts by risk.",
