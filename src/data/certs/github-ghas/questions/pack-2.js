export const GITHUB_GHAS_QUESTIONS_2 = [
  {
    ...
    options: [
      { id: 'A', text: "Converting the false positive patterns into custom patterns, which suppresses the block." },
      ...
      { id: 'D', text: "Disabling push protection for the repositories that report the most false positives." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A gitignore entry for the fixtures directory." },
      { id: 'B', text: "A custom pattern that matches the fixtures format and marks it inactive." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Rotate or revoke the credential first, because it must be assumed captured the moment it was pushed, then remove it from the codebase and consider history rewriting as cleanup that cannot recall existing clones." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Nobody is notified automatically; alerts must be polled from the API." },
      { id: 'B', text: "Every repository collaborator is notified, which is why alerts are limited to public repositories." },
      { id: 'C', text: "Only the organization owner is notified, and only in a weekly digest." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Running a repository search for the token prefix, which is equivalent to the scanner behaviour." },
      { id: 'B', text: "Publishing the pattern and reviewing the audit log for bypass events." },
      { id: 'C', text: "Enabling the pattern in alert-only mode for a week, since no dry run capability exists." },
      { id: 'D', text: "A dry run of the pattern, which reports the matches it would have found across the selected scope without creating alerts or blocking anything." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Publish the advisory first so consumers are warned, then develop the fix in the open." },
      { id: 'B', text: "Rely on the dependency graph to detect the vulnerable versions automatically once a patched release is tagged." },
      { id: 'C', text: "Create a repository security advisory as a draft, use its temporary private fork to develop and review the fix, request a CVE identifier if one is needed, then publish the advisory so it reaches the advisory database and generates alerts for consumers." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A workflow that runs before Dependabot to authenticate the runner to the feed." },
      { id: 'B', text: "Nothing; Dependabot cannot resolve private registries and those dependencies must be excluded." },
      { id: 'C', text: "A registries section in the Dependabot configuration describing the feed, referencing credentials held as Dependabot secrets, and the relevant update entry granting access to that registry." },
      { id: 'D', text: "An Actions organization secret holding the feed credentials, which Dependabot reads at run time." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Setting the update schedule to hourly, which causes superseded patch pull requests to merge automatically." },
      ...
      { id: 'D', text: "A workflow triggered by Dependabot pull requests that reads the update metadata, and for patch-level updates enables auto-merge so the pull request merges once required checks pass." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Dependabot alerts, which include licence findings alongside vulnerabilities." },
      { id: 'B', text: "The dependency review action, which can be configured with licence allow or deny lists as well as a severity threshold, failing the check when a newly introduced dependency violates the policy." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Whether the dependency graph has been disabled, which freezes existing alerts." },
      { id: 'B', text: "Whether the release notes are authoritative, since advisories are only updated quarterly." },
      { id: 'C', text: "Whether the vulnerable package is still resolved transitively through another dependency, whether a lockfile still pins the old version, and whether the advisory affects a version range the new release still falls within." },
      { id: 'D', text: "Whether the alert simply needs manual dismissal, since alerts never close automatically." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "CodeQL compares the code against a database of known vulnerable code snippets." },
      { id: 'D', text: "CodeQL applies a larger library of regular expressions than a linter, which is what allows cross-file matches." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "A distinct commit for each upload, since one commit can carry only one analysis." },
      { id: 'D', text: "A separate branch for each tool, which is the only way to keep results apart." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Code scanning results can only be produced by the CodeQL action, so the build must migrate." },
      { id: 'C', text: "Mirror the repository to a second repository where Actions is enabled and analyse the mirror, since results cannot be uploaded from elsewhere." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A single database covering all languages at once, analysed in one step." },
      { id: 'B', text: "A matrix over the languages, so each language gets its own database creation, its own build handling where required and its own analysis, which is reported under its own category." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "It is the call stack captured when the analysis executed the code, showing the runtime sequence." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Default setup disables itself when an unexpected language appears, and must be re-enabled manually." },
      { id: 'B', text: "Default setup detects the languages present and updates the analysed set, so a newly added supported language is picked up without migrating, and the configured languages can also be reviewed and adjusted." },
      { id: 'C', text: "Default setup analyses only the languages present when it was enabled, so any addition requires migrating to advanced setup." },
      { id: 'D', text: "Default setup analyses every supported language regardless of whether it is present, so nothing changes." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Alerts are tracked per analysed ref, so findings that still exist in the merged code are reported against the default branch while findings that were removed or that only existed on the deleted branch no longer appear." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Make the team organization owners, which is the only way to see every repository." },
      { id: 'B', text: "Add the team as an administrator on every repository individually." },
      { id: 'C', text: "Assign the security manager role to the team, which grants read access to every repository and the ability to manage security alerts and settings without conferring organization ownership." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A repository ruleset requiring the features, applied by the enterprise to every repository." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The list of currently open and closed secret scanning alerts." },
      { id: 'B', text: "A scheduled workflow that records the setting daily into a repository file." },
      { id: 'C', text: "The security overview coverage view, which shows the current state and is sufficient as historical evidence." },
