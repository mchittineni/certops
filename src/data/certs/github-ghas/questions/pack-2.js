export const GITHUB_GHAS_QUESTIONS_2 = [
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Commit a generated lockfile listing every transitive dependency, which is the only supported route." },
      ...
      { id: 'D', text: "Submit the resolved dependency set from the build using the dependency submission API, typically through an action for that build tool, so the graph reflects what the build actually resolves." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Reduce the query suite to the default and accept that large codebases cannot be analysed on a schedule." },
      { id: 'B', text: "Split the SARIF upload into smaller files, which reduces analysis memory use." },
      { id: 'C', text: "Give the analysis a larger runner, tune the memory and thread options the analysis step exposes, narrow the scope with path filters or build configuration where parts of the tree need not be analysed, and consider moving the exhaustive run to a schedule with a narrower analysis on pull requests." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Nothing in the file; attribution is configured in the repository security settings." },
      ...
      { id: 'D', text: "Only the severity of each result, since attribution is derived from the uploading workflow name." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Consumption is per repository, so eighty repositories cost eighty units regardless of who commits." },
