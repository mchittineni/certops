export const GITHUB_GHAS_QUESTIONS_9 = [
  ...
  },
  {
    ...
    domainId: "d4",
    domainName: "Configure and use Code Security",
    title: "CodeQL Action: Default CLI Version",
    scenario: "An enterprise development team is managing security policies, vulnerability scans, and supply chain controls on GitHub.",
    question: "Which CodeQL CLI version does github/codeql-action use by default on GitHub-hosted runners?",
    options: [
      { id: 'A', text: "The latest nightly CodeQL bundle, fetched fresh at the start of each run" },
      { id: 'B', text: "Whatever CLI version the repository's own configuration file resolves to" },
      { id: 'C', text: "The recommended CodeQL bundle version for that release of the action" },
      { id: 'D', text: "The latest stable CLI, downloaded when it is absent from the tool cache" }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "By default, github/codeql-action uses the recommended CodeQL bundle version for that release of the action, so the CLI version follows the action version you pin. Setting `tools: linked` selects the version bundled with the action release, and `tools: nightly` opts into the latest nightly bundle. The action does not simply download the latest stable CLI, and it does not read the version from a repository configuration file.",
    referenceUrl: "https://docs.github.com/en/code-security/code-scanning/creating-an-advanced-setup-for-code-scanning/about-code-scanning-with-codeql",
