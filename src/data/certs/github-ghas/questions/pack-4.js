export const GITHUB_GHAS_QUESTIONS_4 = [
  {
    ...
    options: [
      { id: 'A', text: "A repository ruleset containing a push protection rule." },
      { id: 'B', text: "Asking each maintainer to enable it, tracked as a checklist." },
      { id: 'C', text: "A security configuration that enables secret scanning and push protection, applied to the target repositories and set as the default for newly created ones." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Two of the three occurrences were suppressed because they sit in files matching a default exclusion." },
      { id: 'D', text: "An alert represents the exposed secret rather than a single occurrence, and it lists every location where that value was found, so remediation is tracked once against the credential." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Scanning covers every artifact associated with the repository, so a clean queue is conclusive." },
      { id: 'B', text: "Scanning covers everything but reports only high confidence matches, so lowering the confidence threshold would close the gap." },
      { id: 'C', text: "Scanning covers only the default branch, so the gap is limited to other branches." },
      { id: 'D', text: "Scanning covers repository content and its history for patterns it can recognise, so credentials inside opaque or compressed artifacts, in places outside that content, or in formats no pattern matches can still exist; a clean queue is evidence about what is scanned rather than proof of absence." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Only values referenced through the secrets context are scanned; literals are ignored." },
      { id: 'C', text: "Workflow files are repository content like any other, so a recognisable credential in one is detected and push protection would block it, while a value in no known format may not be, which is why review and using the secrets store remain necessary." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A single named individual, so accountability is unambiguous." },
      { id: 'B', text: "Everyone with write access, so a request is never delayed." },
      { id: 'C', text: "The organization owners, since they already hold the highest privilege." },
      { id: 'D', text: "A nominated reviewing group that does not include the requester by default, sized so requests are answered quickly, since a reviewer who is always the same person as the pusher reproduces self-approval." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Push protection is complete for the patterns it supports, so only unusual credential formats remain a training topic." },
      { id: 'B', text: "Push protection blocks recognisable credentials at the point of push and can be bypassed, so it reduces the rate of exposure sharply without eliminating it, and it says nothing about credentials committed before it was enabled." },
      { id: 'C', text: "Push protection also rotates any credential it detects, so handling practices no longer matter." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The open pull requests limit has been reached, so Dependabot raises no further version updates until some are merged or closed." },
      { id: 'B', text: "Dependabot pauses an ecosystem after four weeks without activity and must be resumed manually." },
      { id: 'C', text: "The configuration file has expired and must be re-committed." },
      { id: 'D', text: "Dependabot only raises updates for dependencies with no existing open pull request across the whole organization." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The base repository must add the contributor as a collaborator so the token is elevated." },
      { id: 'B', text: "A fork pull request runs with a read-only token, so a job needing more will fail; the check should be arranged so the comparison runs with the permissions it needs, for example by granting only the read scopes the action requires or by performing the privileged part in a workflow triggered by the completion of the untrusted run." },
      { id: 'C', text: "The fork must enable its own dependency graph before the base repository can compare." },
      { id: 'D', text: "Dependency review cannot run on fork pull requests at all, so external contributions must be reviewed manually." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Alerts close automatically after seven days without action." },
      { id: 'B', text: "The underlying advisory was withdrawn or its affected version range was corrected, so the dependency no longer matches and the alert closed on its own." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Enable grouping, which is the only control over rebase behaviour." },
      { id: 'B', text: "Set the schedule to daily, which prevents rebasing between runs." },
      { id: 'C', text: "Set the rebase strategy to disabled for that ecosystem, so branches are not automatically rebased on every default branch change, accepting that a stale branch may need a manual rebase before merge." },
      { id: 'D', text: "Set the versioning strategy to increase, which avoids branch updates." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Severity already accounts for the consuming application, so the tiered policy is complete as written." },
      { id: 'B', text: "Severity should be ignored entirely in favour of manual assessment of every alert." },
      { id: 'C', text: "Advisory severity is a property of the vulnerability rather than of this deployment, so it is a reasonable default ordering that should be adjusted by exposure and dependency scope, with auto-triage rules encoding the adjustments the team agrees on." },
      { id: 'D', text: "Severity should be recalculated per repository by editing the advisory." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "No action is needed for a transitive dependency, since the project does not call it directly." },
      { id: 'B', text: "Treat it as an incident rather than an upgrade: the package should be removed rather than bumped, anything it could have accessed during builds should be considered compromised including credentials present on the runner, and the path that introduced it should be identified so the same substitution cannot recur." },
      { id: 'C', text: "Upgrade to the latest version of the package, which will have had the malicious code removed." },
      { id: 'D', text: "Dismiss the alert as a false positive if the build succeeded, since malicious packages fail to install." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "For supported compiled languages the analysis can run in a mode that builds no code, extracting from source directly, which removes the build from the scanning path at some cost in fidelity for constructs that depend on build configuration." },
      { id: 'B', text: "The build can be skipped by caching the CodeQL database between runs and re-analysing it." },
      { id: 'C', text: "No: extraction for a compiled language always requires observing a build." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Nothing extra; the reference is always taken from the default branch." },
      { id: 'B', text: "The analysis category only, since the reference is inferred from the tool name." },
      { id: 'C', text: "The reference and commit the results describe, supplied explicitly on the upload, because outside a workflow triggered by that commit the platform cannot infer them." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The uploads are using different categories each run, which resets the alert list." },
      { id: 'B', text: "The results do not carry stable identity information, so the platform cannot recognise a finding across runs; the tool should emit consistent rule identifiers and the fingerprint information used to track a result when its line number moves." },
      { id: 'C', text: "Alert tracking only works for CodeQL, so third-party results always churn." },
      { id: 'D', text: "The alerts are being uploaded for the wrong commit, which recreates them." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Multi-repository variant analysis, which runs a CodeQL query against many repositories at once from the editor tooling and returns the matches." },
      { id: 'B', text: "Exporting every repository CodeQL database and grepping the files." },
      { id: 'C', text: "Enabling the security-and-quality suite everywhere and waiting for the scheduled runs." },
      { id: 'D', text: "Code search with a regular expression matching the vulnerable call." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Workflow analysis requires converting the files to a supported language first." },
      { id: 'B', text: "Secret scanning already covers workflows completely, so no further analysis applies." },
      { id: 'C', text: "CodeQL can analyse workflow definitions themselves, reporting issues such as untrusted input reaching a run step, so the automation is covered by the same alert and triage machinery as application code." },
      { id: 'D', text: "Workflow files can only be reviewed manually, since CodeQL analyses programming languages rather than configuration." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Switch to default setup, which reports the tooling version on each alert and needs no pinning." },
      { id: 'B', text: "Track the default branch of the action so results always reflect the newest queries." },
      { id: 'C', text: "Pin to an immutable reference and leave it, since changing the tooling would invalidate previous results." },
      { id: 'D', text: "Pin the analysis action to an immutable reference and keep it current with automated update pull requests, since an old analysis silently stops detecting newly published query coverage." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Run a narrower, faster analysis on pull requests and the exhaustive one on a schedule, invest in the analysis cost drivers such as machine size and build mode, and gate merges on the pull request analysis so its result is on the path to merge rather than after it." },
      { id: 'B', text: "Reduce the query suite permanently to the smallest available set for every run." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The creation timestamp, since tools run on different schedules." },
      { id: 'B', text: "The tool that reported it and the rule identifier, both available on the alert and in the API, together with the analysis category where several configurations of one tool are in use." },
      { id: 'C', text: "The severity and the file path, since ownership follows the code rather than the tool." },
      { id: 'D', text: "The commit author, since the person who introduced the code owns the finding." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A saved filter in security overview shared with each team." },
      { id: 'B', text: "A scheduled workflow that opens an issue per alert in every repository." },
      { id: 'C', text: "A security campaign, which groups a chosen set of alerts, assigns them to the owning teams with a deadline, and tracks progress against that target in one place." },
      { id: 'D', text: "An organization ruleset blocking merges until the alerts are closed." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Disabling the false positive dismissal reason so only won't fix remains." },
      { id: 'C', text: "Removing write access from developers so only security can touch alerts." },
      { id: 'D', text: "Delegated alert dismissal, which turns a dismissal into a request that a nominated reviewing team approves or denies, leaving ordinary triage unchanged." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Consume the alert APIs for each feature at organization scope, driven by webhooks for timeliness and reconciled periodically, carrying the repository, rule or advisory identity, severity and state so the platform can track a finding rather than re-import it." },
      { id: 'B', text: "Export a CSV from security overview each month and upload it." },
      { id: 'C', text: "Grant the platform organization owner access so it can read the interface directly." },
