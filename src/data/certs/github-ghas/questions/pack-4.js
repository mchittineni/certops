export const GITHUB_GHAS_QUESTIONS_4 = [
  {
    ...
    options: [
      ...
      { id: 'B', text: "A script calling the repository settings API nightly for every repository." },
      { id: 'C', text: "A repository ruleset containing a push protection rule." },
      { id: 'D', text: "Asking each maintainer to enable it, tracked as a checklist." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An alert represents the exposed secret rather than a single occurrence, and it lists every location where that value was found, so remediation is tracked once against the credential." },
      ...
      { id: 'D', text: "Alerts are created per commit, and all three files were changed in one commit." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Scanning covers everything but reports only high confidence matches, so lowering the confidence threshold would close the gap." },
      { id: 'B', text: "Scanning covers repository content and its history for patterns it can recognise, so credentials inside opaque or compressed artifacts, in places outside that content, or in formats no pattern matches can still exist; a clean queue is evidence about what is scanned rather than proof of absence." },
      { id: 'C', text: "Scanning covers only the default branch, so the gap is limited to other branches." },
      { id: 'D', text: "Scanning covers every artifact associated with the repository, so a clean queue is conclusive." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Workflow files are repository content like any other, so a recognisable credential in one is detected and push protection would block it, while a value in no known format may not be, which is why review and using the secrets store remain necessary." },
      { id: 'B', text: "Workflow files are excluded from scanning because they legitimately reference secrets." },
      { id: 'C', text: "Only values referenced through the secrets context are scanned; literals are ignored." },
      { id: 'D', text: "Workflow files are scanned only by code scanning, not by secret scanning." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The organization owners, since they already hold the highest privilege." },
      { id: 'B', text: "Everyone with write access, so a request is never delayed." },
      { id: 'C', text: "A nominated reviewing group that does not include the requester by default, sized so requests are answered quickly, since a reviewer who is always the same person as the pusher reproduces self-approval." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Push protection blocks recognisable credentials at the point of push and can be bypassed, so it reduces the rate of exposure sharply without eliminating it, and it says nothing about credentials committed before it was enabled." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set the versioning strategy to increase, which avoids branch updates." },
      { id: 'B', text: "Set the schedule to daily, which prevents rebasing between runs." },
      { id: 'C', text: "Enable grouping, which is the only control over rebase behaviour." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Severity should be recalculated per repository by editing the advisory." },
      { id: 'C', text: "Severity already accounts for the consuming application, so the tiered policy is complete as written." },
      { id: 'D', text: "Severity should be ignored entirely in favour of manual assessment of every alert." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Upgrade to the latest version of the package, which will have had the malicious code removed." },
      { id: 'C', text: "Dismiss the alert as a false positive if the build succeeded, since malicious packages fail to install." },
      { id: 'D', text: "No action is needed for a transitive dependency, since the project does not call it directly." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The build can be replaced by uploading the compiled artifacts, which the extractor reads instead." },
      { id: 'C', text: "The build can be skipped by caching the CodeQL database between runs and re-analysing it." },
      { id: 'D', text: "No: extraction for a compiled language always requires observing a build." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The reference and commit the results describe, supplied explicitly on the upload, because outside a workflow triggered by that commit the platform cannot infer them." },
      { id: 'B', text: "The workflow run identifier, from which the reference and commit are derived." },
      { id: 'C', text: "The analysis category only, since the reference is inferred from the tool name." },
      { id: 'D', text: "Nothing extra; the reference is always taken from the default branch." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The uploads are using different categories each run, which resets the alert list." },
      { id: 'C', text: "The alerts are being uploaded for the wrong commit, which recreates them." },
      { id: 'D', text: "Alert tracking only works for CodeQL, so third-party results always churn." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Code search with a regular expression matching the vulnerable call." },
      { id: 'C', text: "Enabling the security-and-quality suite everywhere and waiting for the scheduled runs." },
      { id: 'D', text: "Exporting every repository CodeQL database and grepping the files." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The alert must be closed manually, since the platform cannot know a fix was intentional." },
      { id: 'B', text: "The alert remains open until the next scheduled full analysis, then must be dismissed as won't fix." },
      { id: 'C', text: "The alert closes when the pull request is merged, regardless of whether the finding persists." },
      { id: 'D', text: "When a later analysis of the same reference no longer finds the result, the alert moves to fixed automatically, which is why dismissal is reserved for findings that will not be fixed rather than for ones that have been." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Keep the full analysis on pull requests and ask developers to wait, since correctness matters more than speed." },
      { id: 'C', text: "Move the analysis entirely to a nightly schedule, since pull request analysis is optional." },
      { id: 'D', text: "Reduce the query suite permanently to the smallest available set for every run." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The tool that reported it and the rule identifier, both available on the alert and in the API, together with the analysis category where several configurations of one tool are in use." },
      { id: 'B', text: "The severity and the file path, since ownership follows the code rather than the tool." },
      { id: 'C', text: "The commit author, since the person who introduced the code owns the finding." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A security campaign, which groups a chosen set of alerts, assigns them to the owning teams with a deadline, and tracks progress against that target in one place." },
      { id: 'B', text: "A saved filter in security overview shared with each team." },
      { id: 'C', text: "An organization ruleset blocking merges until the alerts are closed." },
      { id: 'D', text: "A scheduled workflow that opens an issue per alert in every repository." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Removing write access from developers so only security can touch alerts." },
      { id: 'B', text: "A weekly report of dismissals for retrospective review." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The two are always identical, so hosted documentation can be followed exactly." },
      { id: 'C', text: "The self-hosted product receives features first, since releases are validated there." },
      { id: 'D', text: "Security features are unavailable on a self-hosted server and require migration to the hosted service." }
    ],
    correctAnswers: ['A'],
    type: "single",
