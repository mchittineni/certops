export const GITHUB_GHAS_QUESTIONS_1 = [
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A code scanning query that matches the prefix in source files." },
      ...
      { id: 'D', text: "A partner pattern request, which is the only way to add a new format." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Enabling push protection, which retroactively resolves alerts for revoked credentials." },
      { id: 'B', text: "Closing every alert older than a year as won't fix, then reviewing only new ones." },
      { id: 'C', text: "Sorting the alerts by commit date, since older credentials are always inactive." },
      { id: 'D', text: "Validity checks, which query the relevant provider to report whether a detected token is still active, so remediation can start with the credentials that currently work." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "The revoked credential as a false positive and the other two as revoked." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Only the tip of the default branch is scanned, so other branches need separate enablement." },
      { id: 'C', text: "Only commits pushed after enablement are scanned, so a historical sweep is required." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Dependabot alerts are only produced for public repositories." },
      { id: 'B', text: "The dependency graph is not enabled for the repository, and Dependabot alerts are derived from it by matching resolved dependencies against published advisories." },
      { id: 'C', text: "Advisories are only matched against direct dependencies listed in the manifest, and this project has none." },
      { id: 'D', text: "Dependabot alerts require a dependabot.yml configuration file to be present." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A CodeQL query written to detect vulnerable package versions in manifests." },
      { id: 'C', text: "Secret scanning push protection, which also covers vulnerable dependency introductions." },
      { id: 'D', text: "Dependabot alerts, which fire during the pull request and block the merge." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Export the Dependabot alert list, which is the recognised bill of materials format." },
      { id: 'B', text: "Export a software bill of materials from the dependency graph in the SPDX format, either from the repository interface or through the API so it can be produced as part of a release." },
      { id: 'C', text: "There is no export capability, so the inventory must be generated by a third-party tool." },
      { id: 'D', text: "Export the dependency review results of the most recent pull request." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Dependabot auto-triage rules, which can automatically dismiss alerts matching criteria such as low severity in development-scoped dependencies while leaving other alerts open." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An ignore entry for that dependency naming the update types to skip, so major version updates are not proposed while patch updates continue." },
      { id: 'B', text: "Setting open-pull-requests-limit to zero for the ecosystem." },
      { id: 'C', text: "Adding the dependency to an allow entry, which pins it to its current version." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Default setup for both, because it accepts a configuration file supplying build commands and query packs." },
      ...
      { id: 'C', text: "Default setup for the first, since it configures and maintains the analysis with no workflow file, and advanced setup for the second, since a workflow file is what allows custom build steps, query selection and scheduling." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Additional queries can only be added by writing custom queries, since the suites are fixed at the default set." },
      ...
      { id: 'D', text: "The security-extended suite adds further security queries beyond the default, and security-and-quality adds maintainability and reliability queries on top of that." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Post the findings as pull request review comments, which code scanning then indexes." },
      { id: 'B', text: "Have the tool emit results in the SARIF format and upload the file with the code scanning upload action, granting the job the security-events: write permission." },
      { id: 'C', text: "Upload the tool output as a build artifact, which code scanning ingests automatically." },
      { id: 'D', text: "Rewrite the tool checks as CodeQL queries, because only CodeQL results can populate code scanning." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The alert should be left open indefinitely, because dismissing any security alert is discouraged." },
      { id: 'C', text: "Used in tests, since the framework validation makes the path safe." },
      { id: 'D', text: "Won't fix, since the code is not going to change." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Convert the queries into SARIF and upload them, since custom queries cannot be executed." },
      { id: 'C', text: "Submit the queries for inclusion in the security-extended suite, which is the only route for custom logic." },
      { id: 'D', text: "Copy the query files into each repository and reference them by relative path from the configuration." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Dismissing each alert as won't fix, since analysis scope cannot be narrowed." },
      { id: 'B', text: "A gitignore entry for the directory, which also excludes it from analysis." },
      ...
      { id: 'D', text: "A paths-ignore entry, which applies uniformly to every language including compiled ones." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    question: "Why?",
    options: [
      { id: 'A', text: "Pull request analysis only examines files under a size threshold, which excluded the rest." },
      ...
      { id: 'C', text: "Only the two highest severity alerts are ever shown in a pull request." },
      { id: 'D', text: "The backlog is hidden because those alerts were dismissed automatically when the pull request was opened." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Push an empty commit whenever a new advisory is published." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The audit log, filtered on security events." },
      { id: 'B', text: "Security overview, whose risk view aggregates open alerts across repositories and whose coverage view shows which security features are enabled where." },
      { id: 'C', text: "The organization insights page, which reports alert counts and feature enablement." },
      { id: 'D', text: "The dependency graph of each repository, reviewed individually." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Add an issue template asking reporters not to include details, relying on reporters to comply." },
      { id: 'C', text: "Disable issues on the repository so nothing can be disclosed publicly." },
      { id: 'D', text: "Enable private vulnerability reporting so researchers can submit confidentially through the Security tab, and add a SECURITY.md describing the policy and expectations, which is surfaced when someone opens an issue." }
    ],
    correctAnswers: ['D'],
    type: "single",
