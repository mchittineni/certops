export const GITHUB_GHAS_FLASHCARDS_1 = [
  ...
  },
  {
    ...
    back: "For supported token types, GitHub checks with the provider whether a detected secret still works and labels the alert <strong>active</strong>, <strong>inactive</strong> or <strong>unknown</strong> (shown as 'Active secret', 'Secret inactive' and 'Possibly active secret').",
  ...
  },
  {
    ...
    back: "When a partner pattern is found in a <strong>public</strong> source (a public repository, public npm package or public Actions log), GitHub sends the token to the issuing provider (e.g. AWS), which can notify the customer or revoke the credential. Matches in private repositories are <strong>not</strong> sent to the partner; they alert the repository administrators and the committer only.",
  ...
  },
  {
    ...
    hint: "Free for public repositories vs. a GitHub Secret Protection licence required for private repositories.",
    back: "Secret scanning and push protection are <strong>free for all public repositories</strong>. Secret scanning, including push protection, on private repositories requires a <strong>GitHub Secret Protection</strong> licence, available on GitHub Team and Enterprise plans.",
  ...
  },
  {
    ...
    certId: "github-ghas",
    domainId: "d2",
    front: "Push Protection Bypass Audit Log Event",
    hint: "Investigating who bypassed push protection and what commit was pushed.",
    back: "The audit log event <strong><code>secret_scanning_push_protection.bypass</code></strong> is recorded when a developer bypasses a push protection block. There is no <code>secret_scanning.bypass</code> event, so filter the audit log on the full action name.",
  ...
  },
  {
    ...
    back: "Secret scanning inspects <strong>git commit history</strong> across all branches, tags, and commits. It also scans <strong>issues, pull request descriptions and comments, discussions, and wikis</strong>, in public repositories and in private repositories with <strong>GitHub Secret Protection</strong>. Private repositories without Secret Protection are not scanned.",
