export const K8S_CKA_QUESTIONS_6 = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Traffic to nodes without a pod is forwarded to other nodes with SNAT applied." },
      { id: 'C', text: "Traffic is dropped permanently and the Service must be recreated as Cluster policy." },
      { id: 'D', text: "Nodes with no local backend fail the load balancer health check and are removed from rotation, so traffic reaches only nodes running a pod." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Create a ClusterRole with get, list, and watch on backups and label it rbac.authorization.k8s.io/aggregate-to-view: \"true\"." },
      { id: 'C', text: "Add the CRD to the default ServiceAccount RoleBinding." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "IPVS removes the need for a CNI plugin." },
      { id: 'B', text: "IPVS eliminates the need for EndpointSlices." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "kubectl apply --prune against a single manifest." },
      { id: 'B', text: "kubectl replace with environment variables expanded by the shell." },
      { id: 'C', text: "kubectl apply -k with a base plus per-environment Kustomize overlays that patch replicas and images." },
