export const K8S_CKA_QUESTIONS_4 = [
  {
    ...
    options: [
      ...
      { id: 'B', text: "Apply the snapshot with kubectl apply -f etcd-backup.db." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "node-role.kubernetes.io/control-plane:NoSchedule, removed with kubectl taint nodes NODE node-role.kubernetes.io/control-plane-" },
      ...
      { id: 'D', text: "node.kubernetes.io/unreachable:NoExecute, removed by restarting the kubelet" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "An egress rule allowing UDP and TCP port 53 to the kube-system CoreDNS pods." },
      { id: 'D', text: "An ingress rule allowing port 53 from CoreDNS." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "maxUnavailable: 0 and maxSurge: 2" },
      { id: 'D', text: "maxUnavailable: 2 and maxSurge: 0" }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "maxUnavailable: 0 forbids the deployment from going below the desired count, and a non-zero maxSurge gives the controller room to create new pods before old ones are removed. Option B guarantees two pods are unavailable, Recreate deletes every pod before starting new ones, and the percentage pair allows five pods to be unavailable at once.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Secret data is base64-encoded in the API and is only encrypted at rest if encryption at rest is configured on the API server." },
      { id: 'B', text: "Secret values are hashed, so the original password cannot be recovered." },
      { id: 'C', text: "Secrets are stored outside etcd in a dedicated vault." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Provide an EncryptionConfiguration file and pass --encryption-provider-config to kube-apiserver, then rewrite existing Secrets." },
      { id: 'B', text: "Enable TLS between the API server and etcd." },
      { id: 'C', text: "Set encrypted: true in each Secret metadata." },
      { id: 'D', text: "Turn on the PodSecurity admission controller." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Increase the pod memory limits." },
      { id: 'B', text: "Add a toleration for node.kubernetes.io/disk-pressure so the pods stay." },
      { id: 'C', text: "Prune unused images and stopped containers on the node." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Expose a NodePort per application and document the port numbers." },
      ...
      { id: 'D', text: "Use an ExternalName Service pointing at the pod IP." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    correctAnswers: ['A'],
    type: "single",
    explanation: "Cron fields are minute, hour, day of month, month, day of week, so 02:30 daily is \"30 2 * * *\". Option B runs at 30 minutes past hour 2 reversed into an invalid hour, option C runs every minute of hour 2 on the 30th, and option D runs at 02:00 on the 30th of each month.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "kube-scheduler preempted lower priority pods on a node where eviction would make the pending pod schedulable." },
      { id: 'B', text: "The kubelet on node02 evicted the pod because of memory pressure." },
      { id: 'C', text: "The ReplicaSet controller scaled down to free capacity." },
      { id: 'D', text: "The API server rejected the lower priority pod at admission time." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Create a PodSecurityPolicy object with privileged: false." },
      { id: 'C', text: "Label the namespace with pod-security.kubernetes.io/enforce: restricted or baseline." },
      { id: 'D', text: "Set securityContext.privileged: false on the namespace object." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "kubectl certificate approve --all" },
      { id: 'B', text: "systemctl restart kubelet" },
      { id: 'C', text: "kubeadm reset and rejoin the node" },
      { id: 'D', text: "kubeadm certs renew all, followed by restarting the control plane static pods and refreshing the admin kubeconfig" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "kubectl expose pod api --type=LoadBalancer" },
      { id: 'B', text: "kubectl proxy --port=8080" },
      { id: 'C', text: "kubectl attach pod/api" },
      { id: 'D', text: "kubectl port-forward pod/api 8080:8080" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The pod is evicted and rescheduled." },
      ...
      { id: 'D', text: "It keeps the old contents, because subPath mounts are not updated when the ConfigMap changes." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Only external etcd supports TLS between the API server and etcd." },
      { id: 'B', text: "A stacked topology requires an odd number of etcd members while an external topology does not." },
      { id: 'C', text: "A stacked topology couples an etcd member to each control plane node so losing one node loses both roles, while an external etcd topology decouples them at the cost of more machines." },
      { id: 'D', text: "A stacked topology cannot be created with kubeadm." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "terminationGracePeriodSeconds: 0" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "kubectl logs POD --previous" },
      { id: 'C', text: "kubectl logs POD -c proxy" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Set the kubernetes.io/ingress.class annotation." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Both values are advisory and are not enforced." },
      { id: 'D', text: "The request applies to the pod and the limit applies to the namespace." }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Requests drive scheduling and are subtracted from node allocatable capacity, while limits are enforced by cgroups at runtime - CPU is throttled and memory over the limit is OOM killed. Option B reverses the roles, limits are genuinely enforced, and namespace-wide caps come from ResourceQuota rather than container limits.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "No provisioner matches, so the claim stays Pending until the StorageClass exists or the claim references an existing class." },
      { id: 'C', text: "A PersistentVolume of 100Gi is created without a class." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Service ClusterIPs are assigned from the pod CIDR." },
      { id: 'B', text: "The node network must be inside the pod CIDR for pods to reach nodes." },
      { id: 'C', text: "The pod CIDR, the Service CIDR, and the node network must not overlap, because each is routed differently." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "kubectl config unset current-context, then kubectl get pods -n apps" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The Service selector matches only the old pods, which blocks the rollout." },
      { id: 'C', text: "The new pods fail their readiness probe, so maxUnavailable prevents the controller from removing more old pods." },
      { id: 'D', text: "The deployment strategy is Recreate, which keeps both versions running." }
    ],
    correctAnswers: ['C'],
    type: "single",
