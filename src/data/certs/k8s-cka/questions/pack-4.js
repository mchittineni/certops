export const K8S_CKA_QUESTIONS_4 = [
  {
    ...
    options: [
      { id: 'A', text: "Run etcdctl snapshot restore with --data-dir=/var/lib/etcd-restore, then point the etcd static pod hostPath at that directory so the kubelet restarts etcd against it." },
      { id: 'B', text: "Copy the snapshot file into /var/lib/etcd and restart the kubelet." },
      { id: 'C', text: "Run etcdctl snapshot restore and the running etcd picks up the new data automatically." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A hostAliases entry for the database hostname." },
      { id: 'B', text: "An ingress rule allowing port 53 from CoreDNS." },
      { id: 'C', text: "An egress rule allowing UDP and TCP port 53 to the kube-system CoreDNS pods." },
      { id: 'D', text: "A dnsPolicy of ClusterFirstWithHostNet on each pod." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "maxUnavailable: 2 and maxSurge: 0" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Secrets are stored outside etcd in a dedicated vault." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set encrypted: true in each Secret metadata." },
      { id: 'B', text: "Turn on the PodSecurity admission controller." },
      { id: 'C', text: "Provide an EncryptionConfiguration file and pass --encryption-provider-config to kube-apiserver, then rewrite existing Secrets." },
      { id: 'D', text: "Enable TLS between the API server and etcd." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set ephemeral-storage requests and limits on the workloads writing large files." },
      { id: 'B', text: "Add a toleration for node.kubernetes.io/disk-pressure so the pods stay." },
      { id: 'C', text: "Increase the pod memory limits." },
      { id: 'D', text: "Prune unused images and stopped containers on the node." }
    ],
    correctAnswers: ['A', 'D'],
    type: "multiple",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Create a LoadBalancer Service and wait for an external IP." },
      { id: 'B', text: "Expose a NodePort per application and document the port numbers." },
      { id: 'C', text: "Use an ExternalName Service pointing at the pod IP." },
      { id: 'D', text: "Deploy an ingress controller reachable from outside, and route applications through Ingress objects." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The kubelet on node02 evicted the pod because of memory pressure." },
      { id: 'B', text: "kube-scheduler preempted lower priority pods on a node where eviction would make the pending pod schedulable." },
      { id: 'C', text: "The API server rejected the lower priority pod at admission time." },
      { id: 'D', text: "The ReplicaSet controller scaled down to free capacity." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "systemctl restart kubelet" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "kubectl port-forward pod/api 8080:8080" },
      { id: 'B', text: "kubectl attach pod/api" },
      { id: 'C', text: "kubectl proxy --port=8080" },
      { id: 'D', text: "kubectl expose pod api --type=LoadBalancer" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The pod is evicted and rescheduled." },
      { id: 'C', text: "The container is restarted automatically to pick up the change." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Add a label controller=nginx to the Ingress." },
      { id: 'B', text: "Set spec.ingressClassName: nginx on the Ingress." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Disable swap with swapoff -a and comment the swap entry out of /etc/fstab so it stays off across reboots." },
      { id: 'C', text: "Delete the node object and rejoin the cluster." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An APIService registering an aggregated API server." },
      { id: 'B', text: "A kubectl plugin that shells out to a database." },
      { id: 'C', text: "A CustomResourceDefinition with an OpenAPI v3 structural schema." },
      { id: 'D', text: "A ValidatingAdmissionWebhook on ConfigMaps." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The request applies to the pod and the limit applies to the namespace." },
      ...
      { id: 'C', text: "The request is the maximum the container may use; the limit is what the scheduler reserves." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A PersistentVolume of 100Gi is created without a class." },
      { id: 'B', text: "The API server rejects the claim at creation time." },
      { id: 'C', text: "The default StorageClass is used automatically after a timeout." },
      { id: 'D', text: "No provisioner matches, so the claim stays Pending until the StorageClass exists or the claim references an existing class." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "kubectl config set-cluster staging, then kubectl config view" },
      { id: 'D', text: "kubectl config use-context staging, then kubectl config set-context --current --namespace=apps" }
    ],
    correctAnswers: ['D'],
    type: "single",
