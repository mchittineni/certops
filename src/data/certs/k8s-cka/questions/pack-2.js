export const K8S_CKA_QUESTIONS_2 = [
  {
    ...
    options: [
      { id: 'A', text: "/etc/kubernetes/manifests" },
      { id: 'B', text: "/etc/systemd/system/kubelet.service.d" },
      { id: 'C', text: "/var/lib/kubelet/pods" },
      { id: 'D', text: "/etc/kubernetes/pki" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "podSelector: {} with policyTypes: [Ingress, Egress] and no rules." },
      { id: 'C', text: "namespaceSelector: {} with policyTypes: [Ingress] and no ingress rules." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "kubectl delete node node01" },
      { id: 'B', text: "kubectl cordon node01" },
      { id: 'C', text: "kubectl taint nodes node01 maintenance=true:PreferNoSchedule" },
      { id: 'D', text: "kubectl drain node01 --ignore-daemonsets" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "cp -r /var/lib/etcd /opt/snap.db while etcd is running" },
      { id: 'B', text: "ETCDCTL_API=3 etcdctl snapshot save /opt/snap.db --endpoints=https://127.0.0.1:2379 --cacert=/etc/kubernetes/pki/etcd/ca.crt --cert=/etc/kubernetes/pki/etcd/server.crt --key=/etc/kubernetes/pki/etcd/server.key" },
      { id: 'C', text: "ETCDCTL_API=3 etcdctl snapshot restore /opt/snap.db" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Create a docker-registry Secret and reference it in the pod spec imagePullSecrets." },
      { id: 'B', text: "Set imagePullPolicy to IfNotPresent on the container." },
      { id: 'C', text: "Increase the kubelet image pull timeout." },
      { id: 'D', text: "Add a ServiceAccount token volume mount to the container." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An ExternalName Service pointing at the pod IP addresses." },
      { id: 'B', text: "A NodePort Service with externalTrafficPolicy: Local." },
      { id: 'C', text: "A ClusterIP Service with sessionAffinity: ClientIP." },
      { id: 'D', text: "A Service with clusterIP: None referenced as the StatefulSet serviceName." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "kubectl delete deployment/api and reapply the manifest" },
      { id: 'B', text: "kubectl rollout restart deployment/api" },
      { id: 'C', text: "kubectl rollout pause deployment/api" },
      { id: 'D', text: "kubectl rollout undo deployment/api" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A Role with pods/list plus a ClusterRoleBinding to the ServiceAccount." },
      ...
      { id: 'D', text: "A Role with pods/list in the build namespace plus a RoleBinding to the ServiceAccount." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "kube-proxy is running in iptables mode instead of IPVS." },
      { id: 'B', text: "CoreDNS has not created a record for the Service." },
      { id: 'C', text: "The Service is missing a NodePort assignment." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Pod B, because Burstable pods exceed their requests." },
      { id: 'B', text: "The kubelet evicts them in pod creation order regardless of class." },
      { id: 'C', text: "Pod A, because Guaranteed pods hold the most reserved memory." },
      { id: 'D', text: "Pod C, because it is BestEffort." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "data.cache.pod.cluster.local" },
      { id: 'B', text: "cache.data.svc.cluster.local" },
      { id: 'C', text: "svc.cache.data.cluster.local" },
      { id: 'D', text: "cache.web.svc.cluster.local" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Upgrade the kubeadm binary, run kubeadm upgrade plan and apply, then drain the node and upgrade kubelet and kubectl, then uncordon." },
      { id: 'B', text: "Upgrade the kubelet, then run kubeadm upgrade apply, then upgrade the kubeadm binary." },
      { id: 'C', text: "Drain every worker first, then upgrade the control plane binaries in any order." },
      { id: 'D', text: "Run kubeadm upgrade node on the control plane, then upgrade the kubeadm binary." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A DaemonSet." },
      { id: 'B', text: "A Deployment with replicas equal to the current node count." },
      { id: 'C', text: "A StatefulSet with one replica per node." },
      { id: 'D', text: "A CronJob scheduled on every node." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Restarting containers that exit non-zero." },
      { id: 'B', text: "Taints with the NoSchedule effect." },
      { id: 'C', text: "Node affinity rules on the pod spec." },
      { id: 'D', text: "ResourceQuota limits on a namespace." }
    ],
    correctAnswers: ['B', 'C'],
    type: "multiple",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "kubectl top pods -n api" },
      { id: 'B', text: "kubectl logs -n api --all-containers" },
      { id: 'C', text: "kubectl describe namespace api" },
      { id: 'D', text: "kubectl get pods -n api -o wide" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A priorityClassName with a high value." },
      { id: 'B', text: "A ResourceQuota limiting pods to five." },
      { id: 'C', text: "A PodDisruptionBudget with minAvailable: 3." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The image entrypoint exited normally after finishing its work." },
      { id: 'B', text: "The node ran out of disk and the kubelet evicted the pod." },
      { id: 'C', text: "The container exceeded its memory limit and the kernel killed it." },
