export const K8S_CKA_QUESTIONS_2 = [
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Inspect the kubelet service with systemctl status kubelet and journalctl -u kubelet." },
      { id: 'B', text: "Delete the node object so the scheduler stops considering it." },
      { id: 'C', text: "Restart kube-apiserver on the control plane node." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "podSelector: {} with policyTypes: [Ingress, Egress] and no rules." },
      { id: 'B', text: "namespaceSelector: {} with policyTypes: [Ingress] and no ingress rules." },
      { id: 'C', text: "podSelector: {} with policyTypes: [Ingress] and no ingress rules." },
      { id: 'D', text: "podSelector matching every app label, with an ingress rule allowing 0.0.0.0/0." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "An empty podSelector selects every pod in the policy namespace, and listing Ingress in policyTypes with no ingress rules denies all incoming traffic while leaving egress untouched. Option B would also cut off all egress, option C explicitly allows everything, and namespaceSelector is only valid inside a rule - it is not a substitute for the top-level podSelector.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set imagePullPolicy to IfNotPresent on the container." },
      { id: 'B', text: "Add a ServiceAccount token volume mount to the container." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A ClusterRole with pods/list plus a RoleBinding in the build namespace." },
      { id: 'B', text: "A Role with pods/list plus a ClusterRoleBinding to the ServiceAccount." },
      { id: 'C', text: "A Role with pods/list in the build namespace plus a RoleBinding to the ServiceAccount." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "The Service selector does not match the pod labels." },
  ...
  },
  {
    ...
    correctAnswers: ['C'],
    type: "single",
    explanation: "Service records follow the pattern service.namespace.svc.cluster-domain, so the cache Service in the data namespace is cache.data.svc.cluster.local. Option B points at the wrong namespace, the pod subdomain is used for pod IP records rather than Services, and option D scrambles the label order.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Restart etcd and wait for the API server to re-register." },
    ...
    correctAnswers: ['D'],
    type: "single",
    explanation: "With the API server down, every kubectl path is unavailable, so diagnosis has to happen on the node: crictl lists the exited API server container and its logs show the manifest error, and journalctl -u kubelet shows the kubelet failing to start the static pod. Options A and D both require a working API server, and restarting etcd does not fix an invalid API server manifest.",
    referenceUrl: "https://kubernetes.io/docs/tasks/debug/debug-cluster/",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "With a single env entry using valueFrom.configMapKeyRef." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "kubectl top pods -n api" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A HorizontalPodAutoscaler with minReplicas: 3." },
      { id: 'C', text: "A ResourceQuota limiting pods to five." },
      { id: 'D', text: "A priorityClassName with a high value." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Install or repair the cluster CNI plugin so a config appears in /etc/cni/net.d." },
      { id: 'D', text: "Restart kube-proxy on the control plane node." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The container exceeded its memory limit and the kernel killed it." },
      { id: 'B', text: "The image entrypoint exited normally after finishing its work." },
      { id: 'C', text: "The node ran out of disk and the kubelet evicted the pod." },
