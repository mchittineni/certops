export const K8S_CKA_QUESTIONS_11 = [
  {
    ...
    options: [
      { id: 'A', text: "The Secret must be marked shared: true." },
      { id: 'B', text: "RBAC must grant the pod ServiceAccount read access across namespaces." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A Deployment adds a controller that maintains the desired replica count, replaces failed pods, and manages rolling updates and rollbacks; a bare pod is never recreated once it dies." },
      { id: 'C', text: "A Deployment is required for a pod to receive an IP address." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Delete the PVs so they are recreated with the data intact." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Set automountServiceAccountToken: false on the pod spec or the ServiceAccount." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "kube-proxy only programs rules in one direction." },
      ...
      { id: 'D', text: "NetworkPolicies always apply symmetrically, so the CNI plugin must be broken." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An emptyDir populated by an init container." },
      { id: 'B', text: "Three separate volumeMounts at the same mountPath." },
      { id: 'C', text: "A hostPath volume aggregating the files." },
      { id: 'D', text: "A projected volume with configMap, secret, and serviceAccountToken sources." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "kubectl apply -f DIR --recursive" },
      { id: 'B', text: "kubectl apply --directory=DIR" },
      { id: 'C', text: "kubectl apply -f DIR/*.yaml --all" },
      { id: 'D', text: "kubectl create -f DIR --deep" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A sidecar declared as an init container with restartPolicy Always is excluded from Job completion, whereas an ordinary long-running container in the pod would keep the Job from finishing." },
      { id: 'B', text: "Jobs never complete when they contain more than one container." },
      { id: 'C', text: "backoffLimit must be set to 0 for a Job to complete." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An ExternalName Service pointing at the ClusterIP." },
      { id: 'B', text: "A ClusterIP Service, optionally with internalTrafficPolicy and a NetworkPolicy restricting which pods may connect." },
