export const K8S_CKA_QUESTIONS_11 = [
  {
    ...
    options: [
      { id: 'A', text: "The kubelet needs a cluster-wide Secret cache enabled." },
      { id: 'B', text: "The Secret must be marked shared: true." },
      { id: 'C', text: "Secrets are namespaced and a pod can only mount Secrets from its own namespace, so the Secret must be created in web." },
      { id: 'D', text: "RBAC must grant the pod ServiceAccount read access across namespaces." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "All workers first, then the control plane nodes together." },
      { id: 'B', text: "Control plane nodes last, after every worker reports the new version." },
      { id: 'C', text: "First control plane node with kubeadm upgrade apply, remaining control plane nodes with kubeadm upgrade node, then workers one at a time with drain, kubeadm upgrade node, kubelet upgrade, and uncordon." },
      { id: 'D', text: "All nodes simultaneously to minimise the maintenance window." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A Deployment is required for a pod to receive an IP address." },
      { id: 'B', text: "A bare pod cannot mount volumes." },
      ...
      { id: 'D', text: "A Deployment runs faster because it bypasses the scheduler." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Egress policies require a matching ingress rule for return traffic." },
      { id: 'B', text: "Egress policies always block health checks." },
      { id: 'C', text: "Health checks bypass NetworkPolicy entirely, so the policy is irrelevant." },
      { id: 'D', text: "It is not the egress policy blocking the checks directly - probe traffic is inbound - so look for an ingress policy on the same pods, or an egress rule the application needs for the endpoint it reports on." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "kubectl top node NODE, which shows requests." },
      ...
      { id: 'D', text: "kubectl get node NODE -o wide." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Set the pod QoS class to BestEffort." },
      { id: 'C', text: "Set prevent_preemption: true on the pod." },
      { id: 'D', text: "Add a PodDisruptionBudget with minAvailable 1." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Policies are directional and additive per pod, so B pods likely lack an ingress rule allowing A - or A pods lack one allowing B - since return traffic on an established connection is permitted but a new connection in the other direction is not." },
      { id: 'B', text: "Cross-namespace traffic requires a Service in both namespaces." },
      { id: 'C', text: "NetworkPolicies always apply symmetrically, so the CNI plugin must be broken." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Three separate volumeMounts at the same mountPath." },
      { id: 'B', text: "An emptyDir populated by an init container." },
      { id: 'C', text: "A projected volume with configMap, secret, and serviceAccountToken sources." },
      { id: 'D', text: "A hostPath volume aggregating the files." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "kubectl apply --directory=DIR" },
      { id: 'B', text: "kubectl apply -f DIR --recursive" },
      { id: 'C', text: "kubectl create -f DIR --deep" },
      { id: 'D', text: "kubectl apply -f DIR/*.yaml --all" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Omitting CPU limits makes the pod BestEffort." },
      { id: 'B', text: "CPU limits are mandatory for scheduling to work." },
      { id: 'C', text: "CPU limits cause throttling once the ceiling is reached, which can add latency even on an idle node, so some teams set CPU requests without limits while always setting memory limits because memory is incompressible." },
      { id: 'D', text: "Memory limits cause throttling in the same way CPU limits do." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "API objects revert to the snapshot, so anything created in those two hours is gone from the cluster view even though the real cloud resources and running containers may still exist and must be reconciled." }
    ],
    correctAnswers: ['D'],
    type: "single",
