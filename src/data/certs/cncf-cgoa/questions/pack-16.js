export const CNCF_CGOA_QUESTIONS_16 = [
  {
    ...
    options: [
      { id: 'A', text: "The Kubernetes kube-apiserver renders HCL and Helm charts." },
      { id: 'B', text: "Manifest rendering is performed inside the user's browser client." },
      { id: 'C', text: "The `argocd-server` API server handles all Git cloning and manifest rendering directly." },
      { id: 'D', text: "The `argo-cd-repo-server` clones the repository, renders manifests (via Kustomize, Helm, or plugins), and returns them to the controller." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    scenario: "A high-throughput enterprise Kubernetes platform experiences rapid scaling across hundreds of applications and requires automated, consistent GitOps reconciliation. The GitOps engineer needs to understand which Argo CD internal component is responsible for cloning the Git repository and generating Kubernetes manifests.",
    ...
    options: [
      { id: 'A', text: "Manifest rendering is performed inside the user's browser client." },
      { id: 'B', text: "The Kubernetes kube-apiserver renders HCL and Helm charts." },
      { id: 'C', text: "The `argo-cd-repo-server` clones the repository, renders manifests (via Kustomize, Helm, or plugins), and returns them to the controller." },
      { id: 'D', text: "The `argocd-server` API server handles all Git cloning and manifest rendering directly." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "The `argo-cd-repo-server` clones the repository, renders manifests (via Kustomize, Helm, or plugins), and returns them to the controller. Argo CD decomposes duties across microservices: `argocd-server` exposes the gRPC/REST API and Web UI; `argocd-repo-server` clones Git repositories and generates raw Kubernetes manifests; `argocd-application-controller` continuously compares live state against desired state and executes synchronization.",
  ...
  },
  {
    ...
    scenario: "A security compliance auditor requires cryptographic commit verification, secret protection, and automated drift prevention across all GitOps-managed clusters. The GitOps engineer needs to understand which Argo CD internal component is responsible for cloning the Git repository and generating Kubernetes manifests.",
  ...
  },
  {
    ...
    scenario: "An enterprise is modernizing traditional imperative deployment scripts and adopting declarative GitOps continuous delivery across hybrid cloud Kubernetes clusters. The GitOps engineer needs to understand which Argo CD internal component is responsible for cloning the Git repository and generating Kubernetes manifests.",
  ...
  },
  {
    ...
    scenario: "A platform reliability team is optimizing system resilience to eliminate single points of failure, streamline rollback capabilities, and automate self-healing. The GitOps engineer needs to understand which Argo CD internal component is responsible for cloning the Git repository and generating Kubernetes manifests.",
    question: "Which design pattern or configuration eliminates operational bottlenecks and guarantees cluster availability?",
    options: [
      { id: 'A', text: "The `argocd-server` API server handles all Git cloning and manifest rendering directly." },
      { id: 'B', text: "Manifest rendering is performed inside the user's browser client." },
      { id: 'C', text: "The `argo-cd-repo-server` clones the repository, renders manifests (via Kustomize, Helm, or plugins), and returns them to the controller." },
      { id: 'D', text: "The Kubernetes kube-apiserver renders HCL and Helm charts." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "The `argo-cd-repo-server` clones the repository, renders manifests (via Kustomize, Helm, or plugins), and returns them to the controller. Argo CD decomposes duties across microservices: `argocd-server` exposes the gRPC/REST API and Web UI; `argocd-repo-server` clones Git repositories and generates raw Kubernetes manifests; `argocd-application-controller` continuously compares live state against desired state and executes synchronization.",
  ...
  },
  {
    ...
    scenario: "A cloud-native platform engineering team is establishing high-availability deployment patterns, automated disaster recovery procedures, and operational resilience baselines using GitOps. The GitOps engineer needs to configure an Argo CD application to automatically deploy new Git commits, delete orphaned resources, and revert out-of-band manual changes.",
    question: "Which architectural approach or configuration satisfies these GitOps disaster recovery and operational continuity objectives?",
    options: [
      { id: 'A', text: "Set `prune: false` so deleted resources remain running in the cluster indefinitely." },
      { id: 'B', text: "Rely on a nightly cron job running `kubectl apply`." },
      { id: 'C', text: "Enable `automated: { prune: true, selfHeal: true }` inside the application sync policy specification." },
      { id: 'D', text: "Disable auto-sync and require engineers to click the 'Sync' button in the UI for every commit." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    scenario: "A high-throughput enterprise Kubernetes platform experiences rapid scaling across hundreds of applications and requires automated, consistent GitOps reconciliation. The GitOps engineer needs to configure an Argo CD application to automatically deploy new Git commits, delete orphaned resources, and revert out-of-band manual changes.",
    question: "Which architectural pattern or GitOps engine configuration manages this delivery scale effectively while preventing controller bottlenecks and sync delays?",
    options: [
      { id: 'A', text: "Set `prune: false` so deleted resources remain running in the cluster indefinitely." },
  ...
  },
  {
    ...
    scenario: "A security compliance auditor requires cryptographic commit verification, secret protection, and automated drift prevention across all GitOps-managed clusters. The GitOps engineer needs to configure an Argo CD application to automatically deploy new Git commits, delete orphaned resources, and revert out-of-band manual changes.",
    question: "Which solution implements these mandatory GitOps security and governance controls?",
    options: [
      { id: 'A', text: "Set `prune: false` so deleted resources remain running in the cluster indefinitely." },
      { id: 'B', text: "Disable auto-sync and require engineers to click the 'Sync' button in the UI for every commit." },
  ...
  },
  {
    ...
    scenario: "An enterprise is modernizing traditional imperative deployment scripts and adopting declarative GitOps continuous delivery across hybrid cloud Kubernetes clusters. The GitOps engineer needs to configure an Argo CD application to automatically deploy new Git commits, delete orphaned resources, and revert out-of-band manual changes.",
    question: "Which practice or platform feature enables the engineering team to modernize delivery workflows with minimal operational friction?",
    options: [
      { id: 'A', text: "Rely on a nightly cron job running `kubectl apply`." },
      { id: 'B', text: "Disable auto-sync and require engineers to click the 'Sync' button in the UI for every commit." },
      { id: 'C', text: "Set `prune: false` so deleted resources remain running in the cluster indefinitely." },
      { id: 'D', text: "Enable `automated: { prune: true, selfHeal: true }` inside the application sync policy specification." }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Enable `automated: { prune: true, selfHeal: true }` inside the application sync policy specification. Argo CD automated sync policies provide complete convergence. Setting `automated: {}` automatically syncs when Git commits land. `prune: true` deletes resources removed from Git, and `selfHeal: true` automatically reverts manual modifications made directly to the cluster.",
  ...
  },
  {
    ...
    scenario: "A platform reliability team is optimizing system resilience to eliminate single points of failure, streamline rollback capabilities, and automate self-healing. The GitOps engineer needs to configure an Argo CD application to automatically deploy new Git commits, delete orphaned resources, and revert out-of-band manual changes.",
    question: "Which design pattern or configuration eliminates operational bottlenecks and guarantees cluster availability?",
    options: [
      { id: 'A', text: "Set `prune: false` so deleted resources remain running in the cluster indefinitely." },
      { id: 'B', text: "Enable `automated: { prune: true, selfHeal: true }` inside the application sync policy specification." },
      { id: 'C', text: "Rely on a nightly cron job running `kubectl apply`." },
      { id: 'D', text: "Disable auto-sync and require engineers to click the 'Sync' button in the UI for every commit." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Put both resources in the same YAML file with no annotations." },
      { id: 'D', text: "Deploy the migration and application simultaneously and hope the database finishes first." }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Annotate the migration Job with `sync-wave: \"0\"` and the application Deployment with `sync-wave: \"1\"`. Argo CD Sync Waves order resource synchronization. Resources in wave `0` are applied first. Argo CD waits for all resources in wave `0` to reach a healthy status before applying resources in wave `1`, allowing clean orchestration of migrations, dependencies, and application rollouts.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Annotate the migration Job with `sync-wave: \"0\"` and the application Deployment with `sync-wave: \"1\"`." },
      { id: 'C', text: "Deploy the migration and application simultaneously and hope the database finishes first." },
      { id: 'D', text: "Sync waves are not supported in Argo CD." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    scenario: "A cloud-native platform engineering team is establishing high-availability deployment patterns, automated disaster recovery procedures, and operational resilience baselines using GitOps. The GitOps engineer needs to execute a canary release of a containerized service with automated metric analysis (e.g., Prometheus 5xx error rate) determining promotion or rollback.",
    question: "Which architectural approach or configuration satisfies these GitOps disaster recovery and operational continuity objectives?",
    options: [
      ...
      { id: 'B', text: "Switch 100% of user traffic to the new version immediately without analysis." },
      { id: 'C', text: "Use a standard Kubernetes Deployment and manually change replica counts by hand." },
  ...
  },
  {
    ...
    scenario: "A high-throughput enterprise Kubernetes platform experiences rapid scaling across hundreds of applications and requires automated, consistent GitOps reconciliation. The GitOps engineer needs to execute a canary release of a containerized service with automated metric analysis (e.g., Prometheus 5xx error rate) determining promotion or rollback.",
    question: "Which architectural pattern or GitOps engine configuration manages this delivery scale effectively while preventing controller bottlenecks and sync delays?",
    options: [
      { id: 'A', text: "Switch 100% of user traffic to the new version immediately without analysis." },
      { id: 'B', text: "Use a standard Kubernetes Deployment and manually change replica counts by hand." },
      { id: 'C', text: "Deploy the new version to a separate unmonitored test cluster." },
      { id: 'D', text: "Replace the Kubernetes Deployment with an Argo `Rollout` resource configured with canary steps and an `AnalysisTemplate`." }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Replace the Kubernetes Deployment with an Argo `Rollout` resource configured with canary steps and an `AnalysisTemplate`. Argo Rollouts replaces the standard Kubernetes Deployment with a `Rollout` controller supporting advanced canary and blue-green strategies. Linking an `AnalysisTemplate` queries metrics (Prometheus, Datadog) at each canary step, automatically aborting and rolling back if error thresholds are exceeded.",
  ...
  },
  {
    ...
    scenario: "A security compliance auditor requires cryptographic commit verification, secret protection, and automated drift prevention across all GitOps-managed clusters. The GitOps engineer needs to execute a canary release of a containerized service with automated metric analysis (e.g., Prometheus 5xx error rate) determining promotion or rollback.",
    question: "Which solution implements these mandatory GitOps security and governance controls?",
    options: [
      { id: 'A', text: "Deploy the new version to a separate unmonitored test cluster." },
      { id: 'B', text: "Use a standard Kubernetes Deployment and manually change replica counts by hand." },
  ...
  },
  {
    ...
    scenario: "An enterprise is modernizing traditional imperative deployment scripts and adopting declarative GitOps continuous delivery across hybrid cloud Kubernetes clusters. The GitOps engineer needs to execute a canary release of a containerized service with automated metric analysis (e.g., Prometheus 5xx error rate) determining promotion or rollback.",
    question: "Which practice or platform feature enables the engineering team to modernize delivery workflows with minimal operational friction?",
    options: [
      { id: 'A', text: "Replace the Kubernetes Deployment with an Argo `Rollout` resource configured with canary steps and an `AnalysisTemplate`." },
      { id: 'B', text: "Switch 100% of user traffic to the new version immediately without analysis." },
      { id: 'C', text: "Deploy the new version to a separate unmonitored test cluster." },
      { id: 'D', text: "Use a standard Kubernetes Deployment and manually change replica counts by hand." }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Replace the Kubernetes Deployment with an Argo `Rollout` resource configured with canary steps and an `AnalysisTemplate`. Argo Rollouts replaces the standard Kubernetes Deployment with a `Rollout` controller supporting advanced canary and blue-green strategies. Linking an `AnalysisTemplate` queries metrics (Prometheus, Datadog) at each canary step, automatically aborting and rolling back if error thresholds are exceeded.",
  ...
  },
  {
    ...
    scenario: "A platform reliability team is optimizing system resilience to eliminate single points of failure, streamline rollback capabilities, and automate self-healing. The GitOps engineer needs to execute a canary release of a containerized service with automated metric analysis (e.g., Prometheus 5xx error rate) determining promotion or rollback.",
    question: "Which design pattern or configuration eliminates operational bottlenecks and guarantees cluster availability?",
    options: [
      { id: 'A', text: "Replace the Kubernetes Deployment with an Argo `Rollout` resource configured with canary steps and an `AnalysisTemplate`." },
      { id: 'B', text: "Deploy the new version to a separate unmonitored test cluster." },
      { id: 'C', text: "Switch 100% of user traffic to the new version immediately without analysis." },
      { id: 'D', text: "Use a standard Kubernetes Deployment and manually change replica counts by hand." }
    ],
    correctAnswers: ['A'],
  ...
  },
  {
    ...
    scenario: "A cloud-native platform engineering team is establishing high-availability deployment patterns, automated disaster recovery procedures, and operational resilience baselines using GitOps. The GitOps engineer needs to understand how Flux v2 organizes GitOps reconciliation through distinct Kubernetes Custom Resource Definitions.",
    question: "Which architectural approach or configuration satisfies these GitOps disaster recovery and operational continuity objectives?",
    options: [
      { id: 'A', text: "Flux requires an external SQL database to track Git commits." },
      { id: 'B', text: "Source acquisition and manifest application are locked into a single indivisible CRD." },
      { id: 'C', text: "Flux uses a `GitRepository` (or OCIRepository) managed by source-controller, and a `Kustomization` managed by kustomize-controller." },
      { id: 'D', text: "Flux runs as a single monolithic process that bypasses the Kubernetes API." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    scenario: "A high-throughput enterprise Kubernetes platform experiences rapid scaling across hundreds of applications and requires automated, consistent GitOps reconciliation. The GitOps engineer needs to understand how Flux v2 organizes GitOps reconciliation through distinct Kubernetes Custom Resource Definitions.",
    question: "Which architectural pattern or GitOps engine configuration manages this delivery scale effectively while preventing controller bottlenecks and sync delays?",
    options: [
      { id: 'A', text: "Flux requires an external SQL database to track Git commits." },
      { id: 'B', text: "Flux runs as a single monolithic process that bypasses the Kubernetes API." },
      { id: 'C', text: "Source acquisition and manifest application are locked into a single indivisible CRD." },
      { id: 'D', text: "Flux uses a `GitRepository` (or OCIRepository) managed by source-controller, and a `Kustomization` managed by kustomize-controller." }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Flux uses a `GitRepository` (or OCIRepository) managed by source-controller, and a `Kustomization` managed by kustomize-controller. Flux v2 is composed of specialized Kubernetes controllers. The `source-controller` watches and pulls Git repos, Helm charts, or OCI artifacts (`GitRepository`). The `kustomize-controller` evaluates Kustomize overlays and applies the generated manifests to the cluster.",
  ...
  },
  {
    ...
    scenario: "A security compliance auditor requires cryptographic commit verification, secret protection, and automated drift prevention across all GitOps-managed clusters. The GitOps engineer needs to understand how Flux v2 organizes GitOps reconciliation through distinct Kubernetes Custom Resource Definitions.",
    question: "Which solution implements these mandatory GitOps security and governance controls?",
    options: [
      { id: 'A', text: "Flux runs as a single monolithic process that bypasses the Kubernetes API." },
      { id: 'B', text: "Source acquisition and manifest application are locked into a single indivisible CRD." },
      { id: 'C', text: "Flux uses a `GitRepository` (or OCIRepository) managed by source-controller, and a `Kustomization` managed by kustomize-controller." },
      { id: 'D', text: "Flux requires an external SQL database to track Git commits." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "Flux uses a `GitRepository` (or OCIRepository) managed by source-controller, and a `Kustomization` managed by kustomize-controller. Flux v2 is composed of specialized Kubernetes controllers. The `source-controller` watches and pulls Git repos, Helm charts, or OCI artifacts (`GitRepository`). The `kustomize-controller` evaluates Kustomize overlays and applies the generated manifests to the cluster.",
  ...
  },
  {
    ...
    scenario: "An enterprise is modernizing traditional imperative deployment scripts and adopting declarative GitOps continuous delivery across hybrid cloud Kubernetes clusters. The GitOps engineer needs to understand how Flux v2 organizes GitOps reconciliation through distinct Kubernetes Custom Resource Definitions.",
    question: "Which practice or platform feature enables the engineering team to modernize delivery workflows with minimal operational friction?",
    options: [
      { id: 'A', text: "Flux requires an external SQL database to track Git commits." },
      { id: 'B', text: "Source acquisition and manifest application are locked into a single indivisible CRD." },
      { id: 'C', text: "Flux runs as a single monolithic process that bypasses the Kubernetes API." },
      { id: 'D', text: "Flux uses a `GitRepository` (or OCIRepository) managed by source-controller, and a `Kustomization` managed by kustomize-controller." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    scenario: "A platform reliability team is optimizing system resilience to eliminate single points of failure, streamline rollback capabilities, and automate self-healing. The GitOps engineer needs to understand how Flux v2 organizes GitOps reconciliation through distinct Kubernetes Custom Resource Definitions.",
    question: "Which design pattern or configuration eliminates operational bottlenecks and guarantees cluster availability?",
    options: [
      { id: 'A', text: "Flux requires an external SQL database to track Git commits." },
      { id: 'B', text: "Flux uses a `GitRepository` (or OCIRepository) managed by source-controller, and a `Kustomization` managed by kustomize-controller." },
      { id: 'C', text: "Flux runs as a single monolithic process that bypasses the Kubernetes API." },
