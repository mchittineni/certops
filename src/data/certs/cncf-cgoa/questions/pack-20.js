export const CNCF_CGOA_QUESTIONS_20 = [
  {
    ...
    options: [
      ...
      { id: 'B', text: "Make the private company repository public to avoid authentication." },
      { id: 'C', text: "Configure a repository-specific, read-only SSH Deploy Key (or fine-grained Personal Access Token) with access restricted strictly to that repository." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Share the corporate GitHub organization owner account credentials with the controller." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Bind the GitOps controller ServiceAccount to namespace-scoped Roles or restricted ClusterRoles that prohibit modifying cluster-admin bindings or core namespaces." }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Bind the GitOps controller ServiceAccount to namespace-scoped Roles or restricted ClusterRoles that prohibit modifying cluster-admin bindings or core namespaces. By default, GitOps controllers often request broad `cluster-admin` privileges. Hardening the platform requires scoping controller permissions: restricting management to designated application namespaces and denying permissions to mutate sensitive security infrastructure (e.g., PSPs, admission webhooks, kube-system).",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Disable CI validation checks to speed up developer pull requests." },
  ...
  },
  {
    ...
    question: "Which design pattern or configuration eliminates operational bottlenecks and guarantees cluster availability?",
    options: [
      { id: 'A', text: "Rely on the in-cluster GitOps controller to fail after changes are merged to production." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Configure Dex or native OIDC integration in Argo CD with Microsoft Entra ID or Okta, mapping OIDC groups to Argo CD RBAC roles." },
      { id: 'D', text: "Disable authentication on the Argo CD web server." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "Configure Dex or native OIDC integration in Argo CD with Microsoft Entra ID or Okta, mapping OIDC groups to Argo CD RBAC roles. Argo CD supports OpenID Connect (OIDC) and OAuth2 (via bundled Dex or native OIDC). Integrating with enterprise identity providers (Okta, Entra ID, GitHub) enables multi-factor authentication and maps corporate group memberships directly to granular Argo CD RBAC permissions.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Allow anonymous developers to push unsigned commits to deployment branches." },
      { id: 'C', text: "Trust all commits and image tags blindly without cryptographic verification." },
      { id: 'D', text: "Enforce GPG commit signature verification on the Git repository and verify container image signatures using Cosign before reconciliation." }
    ],
    correctAnswers: ['D'],
    type: "single",
