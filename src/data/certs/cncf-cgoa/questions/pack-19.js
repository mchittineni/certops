export const CNCF_CGOA_QUESTIONS_19 = [
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Commit base64-encoded Kubernetes Secret YAML directly to the public Git repository." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Commit base64-encoded Kubernetes Secret YAML directly to the public Git repository." },
      { id: 'D', text: "Use encrypted secret repositories (e.g., Sealed Secrets, SOPS) or external secret operators that resolve credentials from KMS/Vault at runtime." }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Use encrypted secret repositories (e.g., Sealed Secrets, SOPS) or external secret operators that resolve credentials from KMS/Vault at runtime. Kubernetes Secrets are merely base64-encoded, not encrypted. Committing raw Secret YAML to Git exposes credentials to anyone with repository read access. GitOps secret solutions either encrypt secrets before committing to Git (SOPS/Sealed Secrets) or fetch them dynamically from external vaults.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Encrypt secrets using a symmetric password committed to `.gitignore`." },
      { id: 'C', text: "Share the cluster's private decryption key with all developers." },
      { id: 'D', text: "Encrypt secrets locally using `kubeseal` with the cluster's public key, generating a `SealedSecret` custom resource committed to Git." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Use Mozilla SOPS with cloud KMS (AWS KMS, GCP KMS, Azure Key Vault, or age) and integrate with Flux decryption or Argo CD plugins." },
  ...
  },
  {
    ...
    question: "Which practice or platform feature enables the engineering team to modernize delivery workflows with minimal operational friction?",
    options: [
      { id: 'A', text: "Store encryption keys in plaintext inside the repository root." },
      { id: 'B', text: "Use Mozilla SOPS with cloud KMS (AWS KMS, GCP KMS, Azure Key Vault, or age) and integrate with Flux decryption or Argo CD plugins." },
      { id: 'C', text: "Encrypt the entire YAML file into an unreadable binary blob that cannot be diffed in pull requests." },
      { id: 'D', text: "Use standard GPG encryption on the entire repository." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Write a custom bash script running on an EC2 instance that runs `kubectl create secret` every 5 minutes." },
      { id: 'D', text: "Copy passwords manually from the AWS Console and paste them into terminal commands." }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Deploy the External Secrets Operator (ESO) and configure `SecretStore` and `ExternalSecret` CRDs in Git. The External Secrets Operator (ESO) integrates external secret management systems (AWS Secrets Manager, Azure Key Vault, HashiCorp Vault, GCP Secret Manager) with Kubernetes. An `ExternalSecret` resource declared in Git instructs the operator to fetch the secret and create a native Kubernetes Secret.",
