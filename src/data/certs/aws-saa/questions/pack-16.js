export const AWS_SAA_QUESTIONS_16 = [
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Export the KMS key material and import it into an external HSM in the secondary region." },
      { id: 'B', text: "Use AWS Certificate Manager to generate a shared regional public key certificate." },
      { id: 'C', text: "Configure an S3 Bucket Policy allowing cross-region principal access to the single-region KMS key." },
      { id: 'D', text: "A multi-region primary key replicated to the destination region." }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Create a multi-Region primary key in the source region and replicate it as a multi-Region replica key to the destination region. KMS multi-Region keys are AWS KMS keys in different AWS Regions that have the same key material and key ID. They allow client applications to encrypt data in one region and decrypt it in another region locally without making cross-region network calls to KMS.",
  ...
  },
  {
    ...
    question: "Which solution properly implements these mandatory security and governance controls?",
    options: [
      { id: 'A', text: "Store the database password in an unencrypted environment variable in the EC2 instance user data." },
      { id: 'B', text: "Secrets Manager automatic rotation with the provided Lambda, read via SDK." },
  ...
  },
  {
    ...
    question: "Which solution enables a robust, highly available architecture while minimizing ongoing operational overhead?",
    options: [
      { id: 'A', text: "Configure an AWS Systems Manager Parameter Store String parameter and update it manually every month." },
      { id: 'B', text: "Store the database password in an unencrypted environment variable in the EC2 instance user data." },
      { id: 'C', text: "Deploy an AWS Config rule to terminate instances that have credentials older than 30 days." },
  ...
  },
  {
    ...
    question: "Which architecture or service configuration satisfies these disaster recovery and regional resilience requirements?",
    options: [
      { id: 'A', text: "Remove the default VPC in all unapproved regions across all member accounts." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "An SCP at the root denying any `aws:RequestedRegion` outside the two." },
      { id: 'C', text: "Configure IAM user policies in each member account with regional Deny statements individually." },
  ...
  },
  {
    ...
    question: "Which solution enables a robust, highly available architecture while minimizing ongoing operational overhead?",
    options: [
      { id: 'A', text: "Configure an AWS Organizations Service Control Policy that lists individual developer names." },
      { id: 'B', text: "Attach an IAM Deny policy to the AWS account root user." },
  ...
  },
  {
    ...
    question: "Which solution properly implements these mandatory security and governance controls?",
    options: [
      { id: 'A', text: "Grant full AdministratorAccess to developers and audit actions after the fact using AWS CloudTrail." },
      { id: 'B', text: "Implement IAM Attribute-Based Access Control (ABAC) policies using the `aws:PrincipalTag` and `aws:ResourceTag` condition keys." },
      { id: 'C', text: "Create a separate IAM group and policy for every individual project and update policies on each team change." },
      { id: 'D', text: "Configure AWS Systems Manager Session Manager with hardcoded user IP addresses." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Grant full AdministratorAccess to developers and audit actions after the fact using AWS CloudTrail." },
      { id: 'B', text: "Create a separate IAM group and policy for every individual project and update policies on each team change." },
      { id: 'C', text: "Configure AWS Systems Manager Session Manager with hardcoded user IP addresses." },
      { id: 'D', text: "Implement IAM Attribute-Based Access Control (ABAC) policies using the `aws:PrincipalTag` and `aws:ResourceTag` condition keys." }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Implement IAM Attribute-Based Access Control (ABAC) policies using the `aws:PrincipalTag` and `aws:ResourceTag` condition keys. Attribute-Based Access Control (ABAC) uses tags attached to IAM principals (users/roles) and AWS resources to make authorization decisions dynamically. A single policy that checks `aws:PrincipalTag/project == aws:ResourceTag/project` scales automatically as new projects and resources are added without updating IAM policies.",
