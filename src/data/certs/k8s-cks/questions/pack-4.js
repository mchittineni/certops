export const K8S_CKS_QUESTIONS_4 = [
  {
    ...
    options: [
      { id: 'A', text: "Set <code>pod-security.kubernetes.io/enforce: restricted</code> and <code>pod-security.kubernetes.io/warn: restricted</code> on the namespace metadata labels" },
      { id: 'B', text: "Set <code>pod-security.kubernetes.io/enforce: privileged</code> and <code>pod-security.kubernetes.io/audit: baseline</code>" },
      { id: 'C', text: "Set <code>security.openshift.io/scc: restricted</code> on the pod template" },
      { id: 'D', text: "Annotate the namespace with <code>admission.k8s.io/pod-security: 'strict'</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>k8s.io/pss-version: v1.30</code>" },
      { id: 'B', text: "<code>security.kubernetes.io/target-version: 1.30</code>" },
      { id: 'C', text: "<code>pod-security.kubernetes.io/version-pin: '1.30'</code>" },
      { id: 'D', text: "<code>pod-security.kubernetes.io/enforce-version: v1.30</code>" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Mounted volumes automatically notify external certificate authorities when secrets expire" },
      { id: 'B', text: "Environment variables can only contain 64 bytes of plaintext data" },
      { id: 'C', text: "Secrets mounted as volumes are automatically encrypted using asymmetric RSA keys inside the container" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Mount <code>/etc/vault</code> as a hostPath volume on all worker nodes" },
      { id: 'B', text: "Create a CronJob that runs <code>kubectl create secret</code> every 10 minutes from external vault API queries" },
      { id: 'C', text: "Configure <code>--encryption-provider-config</code> with a plaintext token pointing to the vault address" },
      { id: 'D', text: "Deploy the Kubernetes Secrets Store CSI Driver and configure a <code>SecretProviderClass</code> referencing the external secret vault provider" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Create a <code>PeerAuthentication</code> with <code>spec.mtls.mode: PERMISSIVE</code>" },
      { id: 'B', text: "Create a <code>PeerAuthentication</code> in namespace <code>banking</code> with <code>spec.mtls.mode: STRICT</code>" },
      { id: 'C', text: "Annotate the namespace with <code>istio.io/tls: 'enabled'</code>" },
      { id: 'D', text: "Configure a <code>DestinationRule</code> with <code>tls.mode: DISABLE</code>" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Create an ingress NetworkPolicy requiring HTTPS on port 443" },
      { id: 'C', text: "Deploy an Envoy sidecar proxy into every application pod manually" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Run <code>kubectl patch pod &lt;pod-name&gt; -p '{\"spec\":{\"containers\":[{\"name\":\"debug\",\"image\":\"ubuntu\"}]}}'</code>" },
      { id: 'B', text: "Run <code>kubectl cp &lt;pod-name&gt;:/bin/sh ./local-sh</code>" },
      { id: 'C', text: "Run <code>kubectl debug -it &lt;pod-name&gt; --image=busybox:latest --target=&lt;container-name&gt;</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A <code>ResourceQuota</code> resource specifying <code>requests.cpu</code> and <code>limits.memory</code>" },
      { id: 'B', text: "A <code>LimitRange</code> resource defining <code>default</code>, <code>defaultRequest</code>, <code>max</code>, and <code>min</code> resource constraints" },
      { id: 'C', text: "A PodDisruptionBudget specifying <code>minAvailable: 1</code>" },
      { id: 'D', text: "A NetworkPolicy specifying bandwidth rate limits" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A NodeAffinity rule targeting nodes labeled with <code>quota=true</code>" },
      { id: 'B', text: "A <code>LimitRange</code> specifying <code>max.cpu: '16'</code> and <code>max.memory: 64Gi</code>" },
      { id: 'C', text: "A <code>ResourceQuota</code> specifying <code>limits.cpu: '16'</code>, <code>limits.memory: 64Gi</code>, and <code>pods: '50'</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Containers must run as a non-root user (<code>runAsNonRoot: true</code>), drop all Linux capabilities (except possibly <code>NET_BIND_SERVICE</code>), and set <code>allowPrivilegeEscalation: false</code>" },
      { id: 'B', text: "Containers are prohibited from mounting persistent volumes" },
      { id: 'C', text: "Pods must have at least three replicas in every deployment" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>gitRepo</code>, <code>iscsi</code>, <code>cephfs</code>, and <code>awsElasticBlockStore</code>" },
      { id: 'B', text: "<code>configMap</code>, <code>emptyDir</code>, <code>projected</code>, <code>secret</code>, <code>downwardAPI</code>, <code>persistentVolumeClaim</code>, and <code>csi</code>" },
      { id: 'C', text: "<code>hostPath</code>, <code>nfs</code>, <code>glusterfs</code>, and <code>local</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy a Vault Agent sidecar container using Vault Agent Injector annotations to authenticate to Vault via the Pod's ServiceAccount token, fetch dynamic lease credentials, and render them to a shared in-memory volume" },
      { id: 'B', text: "Store dynamic credentials in a Kubernetes ConfigMap updated via an external bash script" },
      { id: 'C', text: "Embed Vault root administrative tokens inside application container environment variables" },
      { id: 'D', text: "Grant the application pod the <code>cluster-admin</code> ClusterRole to generate secrets directly" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Deploy an admission controller that encrypts host swap sectors" },
      ...
      { id: 'D', text: "Enable swap on an encrypted NFS mount with <code>failSwapOn: false</code>" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The container can force worker nodes into kernel panic using raw sockets" },
      ...
      { id: 'D', text: "The compromised container can craft raw IP and ICMP packets, forge ARP responses, and execute ARP spoofing or DNS spoofing attacks against other pods sharing the same virtual bridge network" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>pod-security.kubernetes.io/mode: test</code>" },
      { id: 'B', text: "<code>pod-security.kubernetes.io/audit: restricted</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The Downward API requires the container to execute with <code>privileged: true</code>" },
      { id: 'B', text: "Sensitive credentials or configuration tokens placed in annotations could be accidentally exposed to unprivileged container processes" },
      { id: 'C', text: "Downward API files are permanently stored on worker node disks" },
      { id: 'D', text: "Downward API volumes disable container memory limits" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "<code>CAP_NET_RAW</code>" },
      { id: 'C', text: "<code>CAP_SYS_TIME</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Annotate the Secret with <code>kubernetes.io/file-mode: '0400'</code>" },
      { id: 'B', text: "Set <code>defaultMode: 0400</code> under the Secret volume specification in <code>spec.volumes</code>" },
      { id: 'C', text: "Set <code>readOnlyRootFilesystem: true</code> in the container securityContext" },
      { id: 'D', text: "Run <code>chmod 400</code> inside the application container entrypoint script" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Volume mounts communicate directly with etcd via WebSockets" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Linkerd compares the source and destination container IP addresses against an internal DNS table" },
      { id: 'C', text: "Linkerd checks shared static passwords stored in the <code>linkerd-config</code> ConfigMap" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure a <code>LimitRange</code> on the signing namespace" },
      { id: 'B', text: "Set <code>automountServiceAccountToken: false</code> on the web tier pods" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Mount <code>/var/log</code> as a hostPath volume with <code>readOnly: false</code>" },
      { id: 'B', text: "Configure <code>maxReplicas: 1</code> in the HorizontalPodAutoscaler" },
      { id: 'C', text: "Configure <code>resources.limits.ephemeral-storage</code> and <code>resources.requests.ephemeral-storage</code> in the container specification" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Pod Security Admission requires deploying an external OPA Gatekeeper cluster" },
      { id: 'B', text: "Pod Security Admission only inspects pods running in the <code>kube-system</code> namespace" },
      { id: 'C', text: "Pod Security Admission must be configured directly inside <code>/etc/kubernetes/admin.conf</code>" },
      { id: 'D', text: "Pod Security Admission is applied declaratively using labels on <strong>namespaces</strong> rather than complex RBAC bindings and PSP resources" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The Pod starts successfully with the environment variable set to an empty string" },
      { id: 'B', text: "The worker node crashes and reboots to clear container runtime cache" },
      { id: 'C', text: "The API server automatically creates an empty Secret with a random 32-byte value" },
      { id: 'D', text: "The Pod creation succeeds at the API level, but the pod fails to start and enters <code>CreateContainerConfigError</code> state until the secret and key exist" }
    ],
    correctAnswers: ['D'],
    type: "single",
