export const K8S_CKS_QUESTIONS_10 = [
  {
    ...
    options: [
      { id: 'A', text: "<code>ValidatingAdmissionPolicy</code> and <code>ValidatingAdmissionPolicyBinding</code>" },
      { id: 'B', text: "<code>MutatingWebhookConfiguration</code>" },
      { id: 'C', text: "<code>CustomResourceDefinition</code> validation schemas" },
      { id: 'D', text: "<code>LimitRange</code> admission controller" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>object.spec.replicas &gt;= 2</code>" },
      { id: 'B', text: "<code>input.review.object.spec.replicas &gt;= 2</code>" },
      { id: 'C', text: "<code>spec.replicas &gt; 2 or die()</code>" },
      { id: 'D', text: "<code>regex(object.spec.replicas, '^[2-9]$')</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The binding executes an external shell script to read local environment variables" },
      { id: 'B', text: "The binding passes secrets via HTTP GET parameters to the API server" },
      { id: 'C', text: "The policy must be duplicated with hardcoded strings into every namespace" },
      { id: 'D', text: "The binding specifies a <code>paramRef</code> that references a custom configuration object or ConfigMap containing the environment-specific registry list via <code>params</code> in CEL" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The webhook does not log incoming requests to disk" },
      { id: 'C', text: "The webhook ignores non-root containers" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Disable TLS verification by setting <code>insecureSkipVerify: true</code> on the API server static pod" },
      ...
      { id: 'D', text: "Embed the base64-encoded root CA certificate that signed the webhook server's certificate into the <code>clientConfig.caBundle</code> field of the webhook configuration" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>kube-bench</code>" },
      { id: 'B', text: "<code>kube-hunter</code>" },
      { id: 'C', text: "<code>trivy</code>" },
      { id: 'D', text: "<code>sonobuoy</code>" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A <code>ConstraintTemplate</code> (defining the Rego logic and CRD schema) and a corresponding <code>Constraint</code> CRD (instantiating the template with scope and parameters)" },
      { id: 'B', text: "A <code>ValidatingWebhookConfiguration</code> and a <code>ClusterRole</code>" },
      { id: 'C', text: "A <code>PolicyRule</code> and a <code>PolicyBinding</code>" },
      { id: 'D', text: "A <code>ConfigMap</code> and an <code>AdmissionPlugin</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A <code>mutate</code> rule with <code>patchStrategicMerge</code>" },
      { id: 'B', text: "A <code>generate</code> rule that clones a label ConfigMap" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Requests are allowed to proceed without sidecar injection" },
      { id: 'B', text: "All new pod creation requests fail immediately with an API error, ensuring uninspected pods never run at the expense of cluster deployment availability" },
      { id: 'C', text: "The API server automatically restarts the webhook pod" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure <code>reinvocationPolicy: IfNeeded</code> and ensure the webhook logic is idempotent (producing no changes if the desired state is already present)" },
      { id: 'B', text: "Set <code>timeoutSeconds: 0</code> in the webhook configuration" },
      { id: 'C', text: "Disable admission control on the API server" },
      { id: 'D', text: "Change the webhook to a ValidatingAdmissionWebhook" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Set <code>spec.enforcementAction: deny</code>" },
      { id: 'C', text: "Set <code>spec.mode: passive</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set <code>rules: [{ apiGroups: ['*'], resources: ['nodes'] }]</code>" },
      { id: 'B', text: "Set <code>failurePolicy: Ignore</code> across all namespaces" },
      { id: 'C', text: "Delete the <code>kube-system</code> namespace" },
      { id: 'D', text: "Configure <code>namespaceSelector.matchExpressions</code> to exclude namespaces with label <code>kubernetes.io/metadata.name: kube-system</code>" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Add <code>--anonymous-auth=true</code> to <code>/etc/kubernetes/manifests/kube-apiserver.yaml</code>" },
      { id: 'B', text: "Delete <code>/etc/kubernetes/kubelet.conf</code>" },
      { id: 'C', text: "Add an ingress rule blocking port 6443" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "It allows the kubelet to issue TLS certificates to pods" },
      { id: 'C', text: "It encrypts the host root filesystem using client certificates" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A <code>validate</code> rule with <code>enforce</code> mode" },
      { id: 'B', text: "A <code>mutate</code> rule using a <code>patchStrategicMerge</code> overlay" },
      { id: 'C', text: "A <code>generate</code> rule triggered by ServiceAccount creation" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Add an emptyDir volume to the frontend pod" },
      { id: 'B', text: "Deploy an egress NetworkPolicy targeting port 80" },
      { id: 'C', text: "Add a <code>matchCondition</code> with expression <code>object.metadata.labels['tier'] == 'frontend'</code>" },
      { id: 'D', text: "Create a separate webhook server for frontend pods" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "In the etcd CoreDNS configuration file" },
      { id: 'B', text: "In <code>/var/lib/kubelet/config.yaml</code>, set <code>etcd.auth: client</code>" },
      { id: 'C', text: "In <code>/etc/kubernetes/manifests/etcd.yaml</code>, ensure <code>--client-cert-auth=true</code> is present in the etcd command flags" },
      { id: 'D', text: "In <code>/etc/kubernetes/manifests/kube-apiserver.yaml</code>, add <code>--etcd-client-cert-auth=true</code>" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>alertTemplate</code>" },
      { id: 'B', text: "<code>rejectionText</code>" },
      { id: 'C', text: "<code>errorMessage</code>" },
      { id: 'D', text: "<code>messageExpression</code> using string concatenation or formatting in CEL (e.g., <code>'CPU request ' + object.spec.containers[0].resources.requests.cpu + ' exceeds maximum limit 4'</code>)" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Mount <code>/etc/kubernetes/admin.conf</code> as read-only" },
      { id: 'B', text: "Annotate the webhook with <code>immutable: true</code>" },
      { id: 'C', text: "Ensure that permissions to <code>create, update, patch, delete</code> on resources <code>validatingwebhookconfigurations</code> and <code>mutatingwebhookconfigurations</code> in API group <code>admissionregistration.k8s.io</code> are strictly restricted to <code>cluster-admin</code>" },
      { id: 'D', text: "Set <code>readOnlyRootFilesystem: true</code> on the API server pod" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set <code>authorization.mode: ABAC</code>" },
      { id: 'B', text: "Set <code>authorization.mode: AlwaysAllow</code>" },
      { id: 'C', text: "Set <code>authorization.mode: Webhook</code> in <code>/var/lib/kubelet/config.yaml</code>" },
      { id: 'D', text: "Set <code>authorization.mode: Node</code>" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Pass the Cosign public key to the Linux kernel via <code>sysctl</code>" },
      { id: 'B', text: "Mount the vulnerability scan report as a ConfigMap in the target namespace" },
      { id: 'C', text: "In the <code>verifyImages</code> rule, declare an <code>attestations</code> block specifying the predicate type (e.g., <code>https://cosign.sigstore.dev/attestation/vuln/v1</code>) and conditions inspecting CVE counts" },
      { id: 'D', text: "Run <code>trivy image</code> inside a Kubernetes init container" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Execute <code>chown kubelet:kubelet /var/lib/kubelet/config.yaml</code>" },
      { id: 'B', text: "Execute <code>chown root:root /etc/systemd/system/kubelet.service.d/10-kubeadm.conf && chmod 600 /etc/systemd/system/kubelet.service.d/10-kubeadm.conf</code>" },
      { id: 'C', text: "Execute <code>chmod 777 /etc/systemd/system/kubelet.service.d/10-kubeadm.conf</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>!object.spec.containers.exists(c, has(c.securityContext) && has(c.securityContext.privileged) && c.securityContext.privileged == true)</code>" },
      { id: 'B', text: "<code>object.spec.privileged == false</code>" },
