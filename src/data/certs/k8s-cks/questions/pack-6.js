export const K8S_CKS_QUESTIONS_6 = [
  {
    ...
    options: [
      { id: 'A', text: "Setting <code>readOnlyRootFilesystem: true</code> and declaring resource requests and limits" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Execute <code>trivy k8s cluster --all-namespaces</code>" },
      ...
      { id: 'D', text: "Execute <code>trivy image --severity HIGH,CRITICAL --exit-code 1 --ignore-unfixed &lt;image-name&gt;</code>" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Build the application binary in a temporary builder stage (<code>FROM golang:1.22 AS builder</code>) and copy only the compiled executable into a minimal final stage (<code>FROM gcr.io/distroless/static-debian12</code>)" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Run <code>RUN chown -R nobody:nogroup /usr/bin</code>" },
      { id: 'C', text: "Run <code>RUN chmod -R 777 /bin</code>" },
      { id: 'D', text: "Run <code>RUN rm -rf /etc/passwd</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Alpine Linux uses glibc while Distroless exclusively uses musl" },
      { id: 'B', text: "Alpine includes the <code>apk</code> package manager and BusyBox shell (<code>/bin/sh</code>), providing tools an attacker can use post-exploitation; Distroless completely omits shells and package managers" },
      { id: 'C', text: "Alpine images cannot be scanned by Trivy or Grype" },
      { id: 'D', text: "Distroless images run only in privileged mode" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Hadolint Dockerfile linter" },
      { id: 'C', text: "Kubeadm cluster bootstrapper" },
      { id: 'D', text: "Etcdctl snapshot utility" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A Validating Admission Policy (or OPA Gatekeeper / Kyverno policy) checking that <code>spec.containers[*].image</code> starts with the approved domain prefix" },
      ...
      { id: 'D', text: "Configuring <code>--enable-bootstrap-token-auth=false</code> on kube-apiserver" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "A standard TLS server certificate issued by Let's Encrypt" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set <code>ENV NODE_ENV=production</code>, install dependencies using <code>npm ci --only=production</code>, and switch execution using <code>USER node</code>" },
      { id: 'B', text: "Set <code>USER root</code> and install all development dependencies using <code>npm install</code>" },
      { id: 'C', text: "Execute the container using <code>npm start</code> without an explicit USER directive" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Standard OS package scanners only inspect distribution packages (deb, rpm, apk), missing language-level application dependencies (Maven, npm, pip, Go modules) unless language-specific dependency scanning is enabled" },
      { id: 'B', text: "Log4j operates exclusively in kernel space where scanners cannot reach" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Set <code>defaultAllow: true</code> to avoid disrupting developers during webhook outages" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "If the remote server or network transit is compromised (MITM), arbitrary malicious code is executed with root build privileges without checksum or signature verification" },
      { id: 'B', text: "The container image size increases by 4GB" },
      { id: 'C', text: "The pipe command causes Docker layer caching to run out of memory" },
