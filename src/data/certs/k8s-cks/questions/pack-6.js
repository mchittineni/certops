export const K8S_CKS_QUESTIONS_6 = [
  {
    ...
    options: [
      { id: 'A', text: "Specifying an explicit namespace and configuring a liveness probe" },
      ...
      { id: 'C', text: "Configuring an egress NetworkPolicy with DNS port 53 allowed" },
      { id: 'D', text: "Setting <code>readOnlyRootFilesystem: true</code> and declaring resource requests and limits" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Execute <code>trivy fs --format table /var/run/docker.sock</code>" },
      { id: 'B', text: "Execute <code>trivy image --severity LOW,MEDIUM --exit-code 0 &lt;image-name&gt;</code>" },
      { id: 'C', text: "Execute <code>trivy image --severity HIGH,CRITICAL --exit-code 1 --ignore-unfixed &lt;image-name&gt;</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Execute <code>cosign verify --key cosign.pub &lt;registry-url&gt;/&lt;image&gt;:&lt;tag&gt;</code>" },
      { id: 'B', text: "Execute <code>cosign sign --key cosign.key &lt;image&gt;</code>" },
      { id: 'C', text: "Execute <code>cosign generate-key-pair &lt;image&gt;</code>" },
      { id: 'D', text: "Execute <code>cosign triangulate --verify &lt;image&gt;</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Build the application binary in a temporary builder stage (<code>FROM golang:1.22 AS builder</code>) and copy only the compiled executable into a minimal final stage (<code>FROM gcr.io/distroless/static-debian12</code>)" },
      { id: 'C', text: "Add <code>--no-cache</code> to every RUN command inside a single build stage" },
      { id: 'D', text: "Run <code>RUN apt-get update && apt-get install golang</code> inside an Ubuntu base image" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Pinning package versions automatically encrypts the installed binaries with TLS" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Execute <code>syft &lt;image-name&gt; -o spdx-json &gt; sbom.json</code>" },
      { id: 'B', text: "Execute <code>kubectl get pods -o spdx &gt; sbom.json</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Execute <code>trivy fs /</code> inside every running pod every midnight" },
      { id: 'B', text: "Rebuild the container images from scratch and run Docker scout" },
      { id: 'C', text: "Run <code>grype sbom:sbom.json</code> against stored SBOM files to instantly match package inventories against the latest vulnerability databases" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Cosign stores plaintext image hashes in a shared Kubernetes ConfigMap" },
      { id: 'B', text: "Cosign uses an OIDC identity token from the CI/CD pipeline (e.g., GitHub Actions), obtains an ephemeral short-lived X.509 certificate from Fulcio, and records the signature in Rekor's tamper-evident transparency log" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Specify <code>imagePullPolicy: Always</code> with the <code>:latest</code> tag" },
      { id: 'B', text: "Pin the container image by its immutable SHA-256 digest: <code>image: my-app@sha256:7f83b1657ff1fc53b92dc18148a1d65dfc2d4b1fa3d677284addd200126d9069</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Distroless images encrypt all container memory using the node's TPM chip" },
      { id: 'B', text: "Distroless images automatically kill any process that receives incoming HTTP traffic" },
      { id: 'C', text: "Distroless images contain strictly the application and its minimal runtime dependencies; they contain no shells (<code>sh</code>, <code>bash</code>), no package managers (<code>apt</code>, <code>dpkg</code>), and no standard Unix utilities (<code>curl</code>, <code>wget</code>)" },
      { id: 'D', text: "Distroless images run all applications inside a hardware hypervisor sandbox" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Checkov scans running worker node memory for active malware rootkits" },
      { id: 'B', text: "Checkov intercepts network packets at the eBPF layer to block DDoS attacks" },
      { id: 'C', text: "Checkov performs static policy analysis on IaC templates (Terraform, CloudFormation, Helm, Kubernetes YAML) to detect misconfigurations, excessive permissions, and compliance violations prior to infrastructure deployment" },
      { id: 'D', text: "Checkov replaces the Linux kernel seccomp filter on worker nodes" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Run <code>RUN chown -R nobody:nogroup /usr/bin</code>" },
      { id: 'B', text: "Run <code>RUN chmod -R 777 /bin</code>" },
      { id: 'C', text: "Run <code>RUN rm -rf /etc/passwd</code>" },
      { id: 'D', text: "Run <code>RUN find / -type f \\( -perm -4000 -o -perm -2000 \\) -exec chmod a-s {} + 2&gt;/dev/null || true</code>" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A <code>verifyImages</code> rule specifying the target image repository pattern, <code>attestors</code> referencing the Cosign public key, and <code>mutateDigest: true</code>" },
      { id: 'C', text: "A <code>validate</code> rule matching <code>spec.containers[*].image</code> with a regex pattern" },
      { id: 'D', text: "A <code>generate</code> rule that creates a Secret containing the Cosign private key" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "It replaces private registry credentials with temporary AWS IAM roles" },
      { id: 'B', text: "It pulls the container image onto the control plane node to run antivirus scans" },
      { id: 'C', text: "It intercepts pod creation requests, sends the container image references to an external webhook service for policy evaluation (e.g., verifying vulnerability scan status and signatures), and admits or rejects the pod based on the response" },
      { id: 'D', text: "It automatically rebuilds vulnerable Docker images using cached layers" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Alpine includes the <code>apk</code> package manager and BusyBox shell (<code>/bin/sh</code>), providing tools an attacker can use post-exploitation; Distroless completely omits shells and package managers" },
      { id: 'B', text: "Alpine images cannot be scanned by Trivy or Grype" },
      { id: 'C', text: "Distroless images run only in privileged mode" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Hadolint Dockerfile linter" },
      { id: 'B', text: "Etcdctl snapshot utility" },
      { id: 'C', text: "Kubeadm cluster bootstrapper" },
      { id: 'D', text: "Conftest (leveraging Open Policy Agent's Rego engine)" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Docker reuses the cached layer from the previous <code>apt-get update</code>, meaning subsequent builds may install stale, vulnerable packages rather than latest security patches" },
      { id: 'B', text: "The build fails immediately with a syntax error" },
      { id: 'C', text: "The apt cache is permanently encrypted on disk" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Setting <code>readOnlyPort: 0</code> on worker node kubelets" },
      { id: 'B', text: "An egress NetworkPolicy blocking external HTTPS traffic to port 443" },
      { id: 'C', text: "Configuring <code>--enable-bootstrap-token-auth=false</code> on kube-apiserver" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A standard TLS server certificate issued by Let's Encrypt" },
      { id: 'B', text: "An MD5 checksum file uploaded to Amazon S3" },
      { id: 'C', text: "An in-toto attestation signed with Cosign and recorded in the Rekor ledger containing the build environment metadata, commit SHA, and builder identity" },
      { id: 'D', text: "A base64-encoded Kubernetes ConfigMap containing git logs" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Execute the container using <code>npm start</code> without an explicit USER directive" },
      { id: 'B', text: "Set <code>ENV NODE_ENV=production</code>, install dependencies using <code>npm ci --only=production</code>, and switch execution using <code>USER node</code>" },
      { id: 'C', text: "Set <code>USER root</code> and install all development dependencies using <code>npm install</code>" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Trivy cannot scan files residing outside <code>/usr/bin</code>" },
      { id: 'C', text: "Java applications are immune to container scanning tools" },
      { id: 'D', text: "Standard OS package scanners only inspect distribution packages (deb, rpm, apk), missing language-level application dependencies (Maven, npm, pip, Go modules) unless language-specific dependency scanning is enabled" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set <code>defaultAllow: true</code> to avoid disrupting developers during webhook outages" },
      { id: 'B', text: "Configure <code>defaultAllow: audit</code> to write violations to syslog" },
      { id: 'C', text: "Omit <code>defaultAllow</code> to allow the cluster administrator to approve pods manually" },
      { id: 'D', text: "Set <code>defaultAllow: false</code> so that if the backend webhook server fails or is unreachable, the API server rejects pod creation" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The container image size increases by 4GB" },
      { id: 'B', text: "Bash scripts cannot execute inside Docker containers" },
      { id: 'C', text: "If the remote server or network transit is compromised (MITM), arbitrary malicious code is executed with root build privileges without checksum or signature verification" },
      { id: 'D', text: "The pipe command causes Docker layer caching to run out of memory" }
    ],
    correctAnswers: ['C'],
    type: "single",
