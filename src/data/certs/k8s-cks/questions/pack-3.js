export const K8S_CKS_QUESTIONS_3 = [
  {
    ...
    options: [
      { id: 'A', text: "The container forces the node to reboot when its process finishes execution" },
      { id: 'B', text: "The container can snoop on host loopback traffic, sniff network traffic of all other pods on the node, view and signal all host processes, and interact with host shared memory" },
      { id: 'C', text: "The container is automatically granted the <code>cluster-admin</code> RBAC role by the API server" },
      { id: 'D', text: "The container encrypts all node file systems using root certificates" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure an egress NetworkPolicy blocking UDP port 53" },
      { id: 'B', text: "Enforce the Pod Security Standard <code>baseline</code> or <code>restricted</code> profile, or deploy admission policies (e.g., OPA Gatekeeper, Kyverno) that forbid <code>hostPath</code> volume types" },
      { id: 'C', text: "Enable the <code>NodeRestriction</code> admission plugin on <code>kube-apiserver</code>" },
      { id: 'D', text: "Set <code>readOnlyRootFilesystem: true</code> on the container specification" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Annotate the pod with <code>container.apparmor.security.beta.kubernetes.io/mode: 'audit-only'</code>" },
      ...
      { id: 'D', text: "Mount <code>/var/log/audit/audit.log</code> as an emptyDir volume inside the container" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Check <code>/etc/kubernetes/manifests/kube-apiserver.yaml</code> for audit denial entries" },
      { id: 'B', text: "Query <code>kubectl logs</code> for the container to view the seccomp trace table" },
      { id: 'C', text: "Inspect the host audit logs via <code>/var/log/audit/audit.log</code> (or <code>journalctl -k</code> / <code>dmesg</code>) for <code>type=SECCOMP</code> audit messages containing the system call number (<code>syscall=...</code>)" },
      { id: 'D', text: "Run <code>crictl stats</code> to view blocked system call counts" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "<code>CAP_SYS_ADMIN</code> automatically overrides Kubernetes RBAC bindings and grants cluster-admin tokens" },
      { id: 'C', text: "<code>CAP_SYS_ADMIN</code> forces kubelet to mount host <code>/etc/shadow</code> into the container" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Execute <code>crictl inspectp 9099</code>" },
      { id: 'B', text: "Execute <code>ip link show port 9099</code>" },
      { id: 'C', text: "Execute <code>find / -name 9099</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Add <code>securityContext.privileged: false</code> to <code>kube-proxy</code>" },
      { id: 'B', text: "Set <code>fs.suid_dumpable = 0</code> in <code>/etc/sysctl.d/99-security.conf</code> and configure <code>* hard core 0</code> in <code>/etc/security/limits.conf</code>" },
      { id: 'C', text: "Mount <code>/var/log</code> with <code>readOnly: true</code> in all pod specifications" },
      { id: 'D', text: "Configure <code>--enable-core-dumps=false</code> on <code>kube-apiserver</code>" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Cilium CNI with eBPF host routing enabled" },
      { id: 'B', text: "Standard containerd with AppArmor enforce profiles" },
      { id: 'C', text: "Kata Containers (using QEMU, Cloud-Hypervisor, or Firecracker via <code>kata-runtime</code>)" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set <code>PermitRootLogin yes</code> and configure <code>Port 2222</code> with password authentication" },
      { id: 'B', text: "Set <code>AllowUsers *</code> and disable <code>PubkeyAuthentication</code>" },
      { id: 'C', text: "Set <code>StrictModes no</code> and configure <code>X11Forwarding yes</code>" },
      { id: 'D', text: "Set <code>PermitRootLogin no</code>, <code>PasswordAuthentication no</code>, <code>MaxAuthTries 3</code>, and <code>ClientAliveInterval 300</code>" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The container's logs are routed directly to the control plane etcd database" },
      { id: 'B', text: "The container is granted access to all host devices in <code>/dev</code>, disables seccomp and AppArmor profiles, and inherits nearly all Linux kernel capabilities, effectively negating container isolation" },
      { id: 'C', text: "The container is assigned a public IP address directly on the cloud provider's network" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "AIDE (Advanced Intrusion Detection Environment) or Tripwire" },
      { id: 'B', text: "CRI-O log parser" },
      { id: 'C', text: "CoreDNS metrics scraper" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure <code>pidsLimit</code> in the container runtime / kubelet or declare <code>spec.containers[].resources.limits</code> with PID limits where supported" },
      { id: 'B', text: "Set <code>securityContext.runAsNonRoot: true</code>" },
      { id: 'C', text: "Mount <code>/proc</code> with <code>readOnly: true</code>" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Set <code>readOnlyRootFilesystem: true</code> in the pod's <code>securityContext</code>" },
      { id: 'C', text: "Set <code>readOnly: true</code> in the container's <code>volumeMounts</code> entry referencing the <code>hostPath</code> volume" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set <code>umask 027</code> or <code>umask 077</code> in <code>/etc/profile</code> and systemd service unit configurations" },
      { id: 'B', text: "Set <code>chmod 600 /</code> across the entire filesystem" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set <code>net.ipv4.tcp_rfc1337 = 0</code>" },
      { id: 'B', text: "Set <code>net.ipv4.ip_forward = 1</code> and disable bridge netfilter" },
      { id: 'C', text: "Set <code>net.ipv4.conf.all.accept_source_route = 1</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>readOnlyRootFilesystem: true</code>" },
      { id: 'B', text: "<code>runAsNonRoot: true</code>" },
      { id: 'C', text: "<code>allowPrivilegeEscalation: false</code>" },
      { id: 'D', text: "<code>privileged: false</code>" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Kubernetes assigns each Pod its own dedicated IPC namespace by default; ensuring <code>hostIPC: false</code> (the default) prevents pods from accessing host POSIX shared memory" },
      { id: 'B', text: "Kubernetes shares the host IPC namespace by default and requires <code>isolateIPC: true</code>" },
      { id: 'C', text: "Pods share an IPC namespace across the entire namespace unless an AppArmor profile is loaded" },
      { id: 'D', text: "IPC namespace isolation is only supported when running under gVisor" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Create configuration files in <code>/etc/modprobe.d/</code> setting <code>install &lt;module-name&gt; /bin/true</code> (or <code>blacklist &lt;module-name&gt;</code>)" },
      { id: 'B', text: "Remove the <code>/lib/modules</code> directory entirely from the worker node filesystem" },
      { id: 'C', text: "Set <code>allowPrivilegeEscalation: false</code> in the kubelet configuration" },
      { id: 'D', text: "Add an iptables drop rule for protocol 47 on the external firewall" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Add the <code>CAP_DAC_OVERRIDE</code> capability to the container" },
      { id: 'B', text: "Set <code>privileged: true</code> on the volume mount specification" },
      { id: 'C', text: "Configure <code>securityContext.supplementalGroups: [2000]</code> (or <code>fsGroup: 2000</code>) in the Pod specification" },
      { id: 'D', text: "Set <code>securityContext.runAsUser: 0</code> and change file permissions via an entrypoint script" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "It maps UID 0 (root) inside the container to an unprivileged high-numbered UID (e.g., UID 100000) on the host, ensuring that even if a process breaks out, it has zero privileges on the host" },
      { id: 'B', text: "It mounts the host root filesystem as an immutable tmpfs volume" },
      { id: 'C', text: "It forces all container processes to execute inside a virtual machine hardware sandbox" },
      { id: 'D', text: "It automatically disables all networking when a container attempts to execute a shell" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Inspect <code>Seccomp:</code> in <code>/proc/&lt;PID&gt;/status</code> (where <code>2</code> indicates seccomp filtering mode is enabled)" },
      { id: 'B', text: "Inspect <code>Uid:</code> to confirm it is not 0" },
      { id: 'C', text: "Inspect <code>CapEff:</code> to ensure it is set to all zeros" },
      { id: 'D', text: "Inspect <code>Cpus_allowed:</code> to confirm CPU core pin state" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Modify <code>/etc/security/limits.conf</code> to set <code>* soft nproc 5</code>" },
      { id: 'B', text: "Configure an admission webhook that denies pods requesting SSH ports" },
      { id: 'C', text: "Configure <code>pam_faillock.so</code> (or legacy <code>pam_tally2.so</code>) in <code>/etc/pam.d/common-auth</code> with <code>deny=5 unlock_time=900</code>" },
      { id: 'D', text: "Add <code>--max-auth-failures=5</code> to the kubelet configuration file" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set permissions to <code>0777</code> to allow log shippers running as unprivileged users to read logs" },
      { id: 'B', text: "Delete <code>/var/log/pods</code> and stream all logs directly over raw UDP to external syslog" },
      { id: 'C', text: "Mount <code>/var/log/pods</code> inside every pod with <code>readOnly: false</code>" },
      { id: 'D', text: "Ensure <code>/var/log/pods</code> directories are owned by <code>root:root</code> with permissions <code>0700</code> or <code>0750</code>, and configure kubelet log rotation parameters (<code>containerLogMaxSize</code> and <code>containerLogMaxFiles</code>)" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Execute <code>systemd-cgls</code>" },
      { id: 'B', text: "Execute <code>ipcs</code> (or <code>ipcs -a</code>)" },
      { id: 'C', text: "Execute <code>crictl stats</code>" },
      { id: 'D', text: "Execute <code>iptables -L -n -v</code>" }
    ],
    correctAnswers: ['B'],
    type: "single",
