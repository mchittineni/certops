export const K8S_CKS_QUESTIONS_7 = [
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Change the password of the root account to a complex 64-character string" },
      { id: 'B', text: "Uninstall and completely remove the <code>sudo</code> package from the container image during the build process" },
      { id: 'C', text: "Set <code>securityContext.privileged: false</code> in the pod spec" },
      { id: 'D', text: "Configure <code>sudoers</code> with <code>NOPASSWD: ALL</code> for the unprivileged user" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Check that <code>contains(image, '@sha256:')</code> evaluates to true for every container image in <code>input.review.object.spec.containers</code>" },
      { id: 'C', text: "Check that <code>endswith(image, ':latest')</code> evaluates to true" },
      { id: 'D', text: "Verify that the image string starts with <code>https://</code>" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Encrypt all environment variables using base64 before passing them to the pod" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Run <code>helm install --dry-run</code> on the production cluster control plane" },
      { id: 'C', text: "Upload charts directly to Docker Hub without validation" },
      { id: 'D', text: "Run <code>etcdctl check perf</code> on the Helm repository host" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "It runs as a kernel module on all worker nodes" },
      { id: 'B', text: "It operates as a <code>ValidatingWebhookConfiguration</code> that intercepts Pod creation requests, validates image signatures against configured public keys, and admits or rejects the pod accordingly" },
      { id: 'C', text: "It runs as an external cron job that deletes running pods every hour" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "<code>RUN apt-get update && apt-get install -y --no-install-recommends nginx && rm -rf /var/lib/apt/lists/*</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Clair executes the container in a virtual sandbox and observes runtime system calls" },
      { id: 'B', text: "Clair decrypts container binaries using private hardware keys" },
      { id: 'C', text: "Clair inspects the filesystem layers of the container image, extracts package metadata, and matches installed package versions against known vulnerability databases (such as CVE databases and vendor security advisories)" },
      { id: 'D', text: "Clair intercepts network traffic between the container registry and worker nodes" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy an admission controller that rejects pods older than 24 hours" },
      { id: 'B', text: "Hardcode container images to <code>imagePullPolicy: Never</code>" },
      { id: 'C', text: "Disable automated scanning to avoid alert fatigue" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Leave <code>USER root</code> active and set <code>chmod 777 /</code>" },
      { id: 'C', text: "Run <code>microdnf install</code> as user 1001 without granting root access" },
      { id: 'D', text: "Run <code>microdnf install -y ... && microdnf clean all</code> as root, and then add <code>USER 1001</code> before the final command" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Hardware failures on physical worker nodes" },
      { id: 'C', text: "Security misconfigurations such as missing securityContexts, root execution, unconfined seccomp profiles, and permissive capabilities" },
      { id: 'D', text: "CVEs in operating system packages" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>rekor-cli get --log-index &lt;index&gt;</code> or <code>rekor-cli search --sha &lt;hash&gt;</code>" },
      { id: 'B', text: "<code>kubectl get rekor &lt;hash&gt;</code>" },
      { id: 'C', text: "<code>crictl inspect &lt;hash&gt;</code>" },
      { id: 'D', text: "<code>openssl verify -rekor &lt;hash&gt;</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
