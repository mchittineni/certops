export const K8S_CKS_QUESTIONS_7 = [
  {
    ...
    options: [
      { id: 'A', text: "Run <code>cosign verify --key local-rsa.pub &lt;image&gt;</code>" },
      { id: 'B', text: "Run <code>cosign check-oidc --issuer github.com &lt;image&gt;</code>" },
      { id: 'C', text: "Run <code>cosign verify --certificate-identity-regexp &lt;regex&gt; --certificate-oidc-issuer https://token.actions.githubusercontent.com &lt;image&gt;</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set <code>ENV USER=appuser</code> in the builder stage" },
      { id: 'B', text: "Create an unprivileged user using <code>RUN addgroup -S appgroup && adduser -S appuser -G appgroup</code> and set <code>USER appuser</code> before the <code>ENTRYPOINT</code>" },
      { id: 'C', text: "Add <code>RUN chmod 755 /app</code>" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "A <code>.gitignore</code> file containing CVE numbers" },
      { id: 'D', text: "A Kubernetes ConfigMap named <code>trivy-exemptions</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Verify that the image string starts with <code>https://</code>" },
      { id: 'B', text: "Check that <code>input.review.object.metadata.namespace == 'production'</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Signatures and attestations are stored as standard OCI artifacts directly in the container registry alongside the image manifest, enabling cross-registry replication without external signature databases" },
      { id: 'B', text: "Signatures replace the container root filesystem with an encrypted archive" },
      { id: 'C', text: "Notation executes within the Linux kernel to sign system calls" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Migrate workloads to Distroless base images that eliminate Bash and shell interpreters entirely" },
      { id: 'B', text: "Encrypt all environment variables using base64 before passing them to the pod" },
      { id: 'C', text: "Configure <code>hostIPC: true</code> in the pod specification" },
      { id: 'D', text: "Run containers with <code>securityContext.privileged: true</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Run <code>etcdctl check perf</code> on the Helm repository host" },
      { id: 'B', text: "Run <code>helm lint</code> followed by <code>helm template</code> piped into <code>kubesec scan</code> or <code>checkov</code>" },
      { id: 'C', text: "Upload charts directly to Docker Hub without validation" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A <code>verifyImages</code> rule specifying <code>mutateDigest: false</code>" },
      { id: 'C', text: "A <code>mutate</code> rule that automatically renames <code>:latest</code> to <code>:v1.0.0</code>" },
      { id: 'D', text: "A <code>generate</code> rule that creates a default NetworkPolicy for tagged images" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Share the registry administrator password with all senior developers" },
      { id: 'B', text: "Enforce granular registry RBAC with dedicated robot accounts for CI/CD pipelines granting write access only to verified build runners, with human developers restricted to read-only permissions on production repos" },
      { id: 'C', text: "Store registry passwords in a public GitHub repository" },
      { id: 'D', text: "Disable authentication on the container registry and use IP whitelisting" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The Docker build daemon refuses to produce an image artifact" },
      { id: 'B', text: "The container runtime will execute the application process with root UID 0 by default, increasing the blast radius and potential for container escape if the application is compromised" },
      { id: 'C', text: "Root containers cannot communicate across Kubernetes NetworkPolicies" },
      { id: 'D', text: "The image size increases by 500MB" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "It modifies the <code>kube-scheduler</code> algorithm to ignore unsigned pods" },
      { id: 'D', text: "It runs as an external cron job that deletes running pods every hour" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>RUN apt-get update && apt-get install -y --no-install-recommends nginx && rm -rf /var/lib/apt/lists/*</code>" },
      { id: 'B', text: "<code>RUN apt-get update && apt-get install nginx</code>" },
      { id: 'C', text: "<code>RUN apt-get install nginx && apt-get clean</code>" },
      { id: 'D', text: "<code>RUN apt-get update</code> followed by a separate <code>RUN rm -rf /var/lib/apt/lists/*</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Clair intercepts network traffic between the container registry and worker nodes" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "<code>export DOCKER_CONTENT_TRUST=1</code>" },
      { id: 'C', text: "<code>export DOCKER_SIGNING_ENABLED=1</code>" },
      { id: 'D', text: "<code>export DOCKER_VERIFY_IMAGES=true</code>" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Schedule automated daily or weekly CI/CD pipeline builds that pull latest patched base images, re-run vulnerability scans, and trigger rolling cluster deployments" },
      ...
      { id: 'D', text: "Deploy an admission controller that rejects pods older than 24 hours" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Run <code>microdnf install -y ... && microdnf clean all</code> as root, and then add <code>USER 1001</code> before the final command" },
      ...
      { id: 'D', text: "Delete <code>/etc/passwd</code> after installing packages" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "CVEs in operating system packages" },
      ...
      { id: 'C', text: "DNS latency between cluster pods" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Specify <code>COPY --from=builder --chown=appuser:appgroup /app/build /app</code>" },
      { id: 'B', text: "Run <code>RUN chown -R appuser:appgroup /app</code> in a subsequent RUN layer" },
      { id: 'C', text: "Set <code>USER root</code> before running the application" },
      { id: 'D', text: "Mount <code>/app</code> as an emptyDir volume" }
    ],
    correctAnswers: ['A'],
    type: "single",
