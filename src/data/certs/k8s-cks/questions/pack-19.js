export const K8S_CKS_QUESTIONS_19 = [
  {
    ...
    options: [
      { id: 'A', text: "Read the container's overlayfs upper (`diff/`) directory on the node to list the changed files." },
    ...
    correctAnswers: ['A'],
    type: "single",
    explanation: "Read the container's writable overlayfs upper directory on the node (the `diff/` or snapshot `fs` directory named as `upperdir` in the container's overlay mount). Every file the container created or modified since it started is copied up into that directory, and deletions are recorded there as whiteouts, so it shows exactly how the running filesystem has drifted from the image. crictl has no `diff` subcommand; that command exists in Docker and Podman, not in the CRI tooling. `crictl inspect` and `crictl images --digests` only confirm which image the container started from, not what changed inside it afterwards, and running `find` through `kubectl exec` relies on binaries inside the compromised container, misses deleted files, and trusts timestamps an attacker can reset.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Read the container's overlayfs upper (`diff/`) directory on the node to list the changed files." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Run `kubectl exec` with `find / -newer /etc/hostname` to list files written since the start." },
      { id: 'B', text: "Read the container's overlayfs upper (`diff/`) directory on the node to list the changed files." },
