export const K8S_CKS_QUESTIONS_8 = [
  ...
  },
  {
    ...
    domainId: "d6",
    domainName: "Monitoring, Logging and Runtime Security",
    title: "Investigating Container File Drift in the Overlay Upper Layer",
  ...
  },
  {
    ...
    domainId: "d6",
    domainName: "Monitoring, Logging and Runtime Security",
    title: "Selecting Falco's Modern eBPF Driver",
    ...
    options: [
      ...
      { id: 'D', text: "Set <code>engine: { kind: modern_ebpf }</code>, which uses the probe bundled in Falco" }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "The modern eBPF probe is selected with <code>engine.kind: modern_ebpf</code>. It is a CO-RE probe embedded in Falco itself, so nothing has to be downloaded or compiled and no kernel module is loaded, and it is the default driver in current Falco. The legacy eBPF probe (<code>engine.kind: ebpf</code> pointing at a compiled <code>probe</code> object) was dropped in Falco 0.44.0. <code>kmod</code> and forcing the kernel module both need the module that this node forbids, and <code>nodriver</code> runs Falco without any syscall driver at all.",
    referenceUrl: "https://kubernetes.io/docs/concepts/security/",
