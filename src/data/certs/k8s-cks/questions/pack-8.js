export const K8S_CKS_QUESTIONS_8 = [
  {
    ...
    options: [
      ...
      { id: 'C', text: "Set <code>level: None</code> for resources <code>secrets</code>" },
      { id: 'D', text: "Set <code>level: Request</code> with an omitStages filter" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Add <code>--audit-policy-file=/etc/kubernetes/audit-policy.yaml</code>, <code>--audit-log-path=/var/log/kubernetes/audit.log</code>, <code>--audit-log-maxage=30</code>, <code>--audit-log-maxbackup=10</code>, and mount the host directories into the static pod" },
      { id: 'B', text: "Add <code>--enable-audit-logs=true</code> and set <code>--audit-destination=syslog</code> without volume mounts" },
      { id: 'C', text: "Configure <code>audit: true</code> in the kube-apiserver ConfigMap in <code>kube-system</code>" },
      { id: 'D', text: "Set <code>--log-dir=/var/log/audit</code> on the kubelet daemon on each worker node" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "<code>Panic</code>" },
      { id: 'C', text: "<code>ResponseStarted</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>evt.type = execve and container.id != host and proc.name in (bash, sh, zsh, ksh) and proc.tty != 0</code>" },
      { id: 'B', text: "<code>container.image = 'ubuntu' and syscall = 'fork'</code>" },
      { id: 'C', text: "<code>evt.type = connect and fd.port = 22</code>" },
      { id: 'D', text: "<code>evt.type = open and file.name = '/bin/bash'</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Falco rules must be compiled into C code before loading" },
      { id: 'B', text: "Falco uses Rego syntax identical to Open Policy Agent" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>syscall = chdir and dir = /etc</code>" },
      { id: 'B', text: "<code>evt.type = read and fd.name = /etc/passwd</code>" },
      { id: 'C', text: "<code>evt.type = clone and proc.name = 'vipw'</code>" },
      { id: 'D', text: "<code>evt.type in (open, openat, openat2) and evt.is_open_write=true and fd.name startswith /etc/ and container.id != host</code>" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Execute <code>chroot /proc/&lt;container-pid&gt;/root tcpdump</code>" },
      { id: 'B', text: "Execute <code>crictl exec &lt;container-id&gt; tcpdump</code>" },
      { id: 'C', text: "Execute <code>kill -SIGSTOP &lt;container-pid&gt;</code>" },
      { id: 'D', text: "Execute <code>nsenter -t &lt;container-pid&gt; -n tcpdump -i eth0</code>" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "These devices can only be mounted by CoreDNS" },
      { id: 'B', text: "Access to <code>/dev/mem</code> causes the kubelet to restart immediately" },
      { id: 'C', text: "Access to these devices automatically resets the node's network interface" },
      { id: 'D', text: "Direct access to <code>/dev/mem</code> or <code>/proc/kcore</code> exposes physical host memory, allowing attackers to extract cryptographic keys, read other processes' memory, or overwrite kernel code" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure <code>falco_sink: all</code>" },
      { id: 'B', text: "Set <code>log_level: debug</code> in kubelet.conf" },
      { id: 'C', text: "Set <code>output_format: json</code> with <code>destination: /var/log/audit.log</code>" },
      { id: 'D', text: "Set <code>stdout_output: { enabled: true }</code> and <code>syslog_output: { enabled: true }</code>" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Run <code>etcdctl get /containers/diff</code>" },
      { id: 'C', text: "Run <code>crictl stats --format json</code>" },
      { id: 'D', text: "Run <code>kubectl logs &lt;pod&gt; --all-files</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Query <code>kubectl get events -n kube-system</code>" },
      ...
      { id: 'D', text: "Filter the audit log for HTTP status code 404" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Monitor <code>evt.type = read</code> on <code>/tmp</code>" },
      { id: 'B', text: "Monitor <code>syscall = mprotect</code> with PROT_READ only" },
      { id: 'C', text: "Monitor <code>evt.type = socket</code> for TCP connections" },
      { id: 'D', text: "Monitor <code>evt.type = memfd_create</code> or <code>proc.cmdline startswith '/memfd:'</code>" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set <code>--audit-log-path=/dev/null</code> on the API server" },
      { id: 'B', text: "Add early audit rules matching users <code>system:kube-proxy</code> and <code>system:nodes</code> with verbs <code>get, watch, list</code> and set <code>level: None</code>" },
      { id: 'C', text: "Set <code>level: RequestResponse</code> for all resources across all namespaces" },
      { id: 'D', text: "Delete the <code>kube-proxy</code> DaemonSet from the cluster" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Multiple <code>read</code> calls on <code>/etc/hosts</code>" },
      ...
      { id: 'C', text: "Continuous <code>gettimeofday</code> invocations" },
      { id: 'D', text: "Binding to localhost port 8080" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Execute <code>strace -p 14205 -f -e trace=all</code>" },
      { id: 'B', text: "Execute <code>gdb --batch --pid 14205</code>" },
      { id: 'C', text: "Execute <code>crictl inspect 14205</code>" },
      { id: 'D', text: "Execute <code>top -p 14205</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "<code>--log-retention-period=1y</code>" },
      { id: 'C', text: "<code>--audit-retention-days=365</code>" },
      { id: 'D', text: "<code>--audit-log-maxbackup=365</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Run <code>crictl stop &lt;container-id&gt;</code>" },
      { id: 'B', text: "Copy <code>/var/log/pods</code> to an external USB stick" },
      { id: 'C', text: "Dump the memory pages of the container process using <code>gcore &lt;PID&gt;</code> (or reading <code>/proc/&lt;PID&gt;/mem</code> using LiME / AVML)" },
      { id: 'D', text: "Run <code>kubectl delete pod &lt;pod-name&gt; --now</code>" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>evt.type = recvfrom and fd.port = 53</code>" },
      { id: 'B', text: "<code>evt.type = connect and evt.dir = &lt; and (fd.typechar = 4 or fd.typechar = 6) and container.id != host and not inbound</code>" },
      { id: 'C', text: "<code>evt.type = accept and container.id = host</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set <code>driver: none</code>" },
      { id: 'B', text: "Set <code>runtime: containerd</code>" },
      { id: 'C', text: "Set <code>kernel_module: force</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An audit event with <code>verb: get</code> on resource <code>nodes</code>" },
      ...
      { id: 'D', text: "An audit event with <code>verb: list</code>, <code>objectRef.resource: secrets</code>, and an empty (all namespaces) <code>objectRef.namespace</code> field" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>evt.type = open and file.name = '/var/lib/kubelet/config.yaml'</code>" },
      { id: 'B', text: "<code>evt.type = bind and fd.port = 6443</code>" },
      { id: 'C', text: "<code>evt.type = accept and fd.port = 80</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>--audit-stream-url=https://siem.enterprise.com/logs</code>" },
      { id: 'B', text: "<code>--audit-webhook-config-file=/etc/kubernetes/audit-webhook.yaml</code>" },
      { id: 'C', text: "<code>--enable-audit-streaming=true</code>" },
      { id: 'D', text: "<code>--audit-remote-syslog=splunk:514</code>" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Detect known miner process names (e.g., <code>xmrig</code>, <code>minerd</code>) and monitor network connections on common mining pool ports (3333, 4444, 5555) or Stratum protocol negotiation strings" },
      { id: 'C', text: "Block all UDP packets on port 53" },
      { id: 'D', text: "Check for HTTP 200 responses from Google" }
    ],
    correctAnswers: ['B'],
    type: "single",
