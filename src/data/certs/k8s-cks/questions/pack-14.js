export const K8S_CKS_QUESTIONS_14 = [
  {
    ...
    options: [
      ...
      { id: 'D', text: "Run untrusted code inside standard runc containers with root privileges." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Rely on Linux namespace isolation as a sufficient sandbox for hostile code." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "Deploy a sandboxed container runtime (like gVisor with `runsc` or Kata Containers) and specify the corresponding `RuntimeClass` in the pod spec. Standard containers share the host Linux kernel, meaning a kernel vulnerability (e.g., Dirty COW, Dirty Pipe) can allow complete host compromise. Sandboxed runtimes like gVisor (`runsc`) intercept system calls in userspace, providing an impermeable virtualization barrier between untrusted workloads and the host kernel.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Ensure developer Roles omit `get`, `list`, and `watch` permissions on `secrets` resources, granting access only to `configmaps` and `pods`." },
      { id: 'D', text: "Store database passwords in ConfigMaps so Secrets are not needed." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Hardcode passwords in application container environment variables." },
      { id: 'D', text: "Store database passwords in ConfigMaps so Secrets are not needed." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Deploy the application as an uncompiled source code repository." },
      { id: 'C', text: "Use a full Debian or Ubuntu image and install developer tools in production." },
      { id: 'D', text: "Install netcat, curl, and nmap in the container image for debugging." }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Build the application using Google Distroless Java base images, removing package managers, shells, and utilities. Minimal and distroless images contain only the application and its runtime dependencies. By eliminating package managers (`apt`, `yum`) and command shells (`sh`, `bash`), attackers cannot download rootkits or execute shell scripts even if an application remote code execution flaw exists.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Leave the container root filesystem writable with chmod 777 permissions." },
      { id: 'D', text: "Set `readOnlyRootFilesystem: true` in the container `securityContext` and mount an `emptyDir` volume explicitly at `/tmp` if scratch space is required." }
    ],
    correctAnswers: ['D'],
    type: "single",
