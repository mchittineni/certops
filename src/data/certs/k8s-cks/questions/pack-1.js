export const K8S_CKS_QUESTIONS_1 = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Configure a NetworkPolicy with <code>podSelector: { role: 'deny-all' }</code> and omit the <code>policyTypes</code> field" },
      { id: 'D', text: "Create a NetworkPolicy with <code>policyTypes: ['Ingress']</code> and specify <code>ingress: [{ from: [] }]</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Define an egress rule specifying <code>ports: [{ port: 80, protocol: 'TCP' }]</code> without an <code>ipBlock</code> definition" },
      { id: 'B', text: "Configure a NetworkPolicy that restricts pod DNS queries to internal cluster service names only" },
      { id: 'C', text: "Add an egress rule with <code>to: [{ ipBlock: { cidr: '0.0.0.0/0', except: ['169.254.169.254/32'] } }]</code> under <code>policyTypes: ['Egress']</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy a sidecar container to all control plane pods that encrypts secret payloads using mutual TLS before writing to etcd" },
      { id: 'B', text: "Create an <code>EncryptionConfiguration</code> file specifying an encryption provider such as <code>aescbc</code> or <code>kms</code>, mount the configuration into the <code>kube-apiserver</code> pod, set the <code>--encryption-provider-config</code> flag, and run <code>kubectl get secrets -A -o json | kubectl replace -f -</code> to rewrite existing secrets" },
      { id: 'C', text: "Modify <code>etcd.conf</code> to enable disk volume encryption on <code>/var/lib/etcd</code> and restart the <code>etcd</code> systemd service unit" },
      { id: 'D', text: "Annotate all secret resources with <code>kubernetes.io/encrypt: 'true'</code> and trigger a rolling restart of all application worker nodes" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure <code>--authorization-mode=AlwaysDeny</code> and remove the client CA file flag" },
      { id: 'B', text: "Set <code>--allow-privileged=false</code> and configure <code>--bind-address=0.0.0.0</code>" },
      { id: 'C', text: "Set <code>--enable-bootstrap-token-auth=false</code> and configure <code>--secure-port=8080</code>" },
      { id: 'D', text: "Set <code>--anonymous-auth=false</code> and ensure <code>--insecure-port=0</code> is configured (or entirely omitted in modern versions where the insecure port has been deprecated and removed)" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set <code>automountServiceAccountToken: false</code> in the Pod specification or on the associated ServiceAccount resource" },
      { id: 'B', text: "Delete the <code>default</code> ServiceAccount in the target application namespace" },
      { id: 'C', text: "Set <code>serviceAccountName: none</code> inside the Pod template specification" },
      { id: 'D', text: "Define an empty volume named <code>service-token</code> in the pod and set <code>readOnly: true</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Enable the <code>TokenRequestValidation</code> feature gate on all kubelet daemons across worker nodes" },
      { id: 'B', text: "Set <code>--service-account-lookup=false</code> and configure a custom etcd certificate bundle" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set <code>--client-cert-auth=true</code>, <code>--trusted-ca-file</code>, <code>--cert-file</code>, and <code>--key-file</code>" },
      { id: 'B', text: "Set <code>--insecure-transport-security=false</code> and configure <code>--enable-v2=false</code>" },
      { id: 'C', text: "Set <code>--peer-client-cert-auth=true</code> and configure <code>--auto-tls=true</code>" },
      { id: 'D', text: "Set <code>--listen-client-urls=http://127.0.0.1:2379</code> and disable SSL encryption" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Specify <code>apiGroups: ['']</code>, <code>resources: ['*']</code>, and <code>verbs: ['inspect', 'read']</code>" },
      { id: 'B', text: "Specify <code>apiGroups: ['core']</code>, <code>resources: ['pods', 'secrets']</code>, and <code>verbs: ['get']</code>" },
      { id: 'C', text: "Specify <code>apiGroups: ['*']</code>, <code>resources: ['pods']</code>, and <code>verbs: ['*']</code>" },
      { id: 'D', text: "Specify <code>apiGroups: ['']</code>, <code>resources: ['pods', 'pods/log', 'pods/status']</code>, and <code>verbs: ['get', 'list', 'watch']</code>" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The API server enforces that a user cannot create or update a Role or RoleBinding with permissions that exceed the user's own current permissions, unless the user has the explicit <code>escalate</code> verb on roles or <code>bind</code> verb on the target role" },
      ...
      { id: 'C', text: "The API server automatically drops any RoleBinding referencing a ClusterRole unless executed by the <code>system:masters</code> group" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set <code>readOnlyPort: 0</code>, <code>authentication.anonymous.enabled: false</code>, and <code>authorization.mode: Webhook</code>" },
      { id: 'B', text: "Set <code>readOnlyPort: 10255</code>, <code>authentication.webhook.enabled: false</code>, and <code>authorization.mode: AlwaysAllow</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Download the corresponding <code>.sha256</code> or <code>.sha512</code> checksum file from the official Kubernetes release bucket and verify using <code>sha256sum --check &lt;binary&gt;.sha256</code>" },
      { id: 'B', text: "Execute the binary with <code>--version</code> and verify that the output string matches the release branch tag" },
      { id: 'C', text: "Run <code>strings &lt;binary&gt; | grep 'kubernetes.io'</code> to verify the embedded source repository string" },
      { id: 'D', text: "Rely exclusively on TLS certificate validation during the HTTPS file download from the public CDN" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Specify an ingress rule with <code>from: []</code> and set <code>policyTypes: ['Ingress']</code>" },
      ...
      { id: 'D', text: "Specify an ingress rule with <code>from: [{ namespaceSelector: { matchLabels: { 'kubernetes.io/metadata.name': 'ingress-nginx' } }, podSelector: { matchLabels: { 'app.kubernetes.io/name': 'ingress-nginx' } } }]</code>" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Set <code>--tls-cipher-suites=TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256,TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384,TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384,TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305,TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Change the <code>cluster-admin</code> ClusterRole definition to remove the <code>delete</code> verb globally across the cluster" },
      ...
      { id: 'C', text: "Delete the <code>ClusterRoleBinding</code>, create a scoped <code>Role</code> with only the required deployment verbs (e.g., <code>create</code>, <code>patch</code>, <code>get</code> on <code>deployments</code> and <code>services</code>), and bind it via a <code>RoleBinding</code> inside the target deployment namespace" },
      { id: 'D', text: "Downgrade the ServiceAccount token by adding <code>automountServiceAccountToken: false</code> while maintaining the <code>cluster-admin</code> binding" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Add an egress rule with <code>to: [{ ipBlock: { cidr: '0.0.0.0/0' } }]</code> on port <code>8080</code>" },
      { id: 'C', text: "Add an ingress rule allowing CoreDNS to initiate connections back to the database pod on port 53" },
      { id: 'D', text: "Set <code>policyTypes: ['Ingress']</code> so egress traffic is no longer subject to NetworkPolicy rules" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Ensure permissions are set to <code>0644</code> and owned by <code>kubelet:root</code>" },
      { id: 'B', text: "Set permissions to <code>0777</code> to allow containerized backup tools to dump snapshot files" },
      { id: 'C', text: "Ensure permissions are set to <code>0755</code> and owned by <code>nobody:nogroup</code>" },
      { id: 'D', text: "Ensure ownership is <code>etcd:etcd</code> (or <code>root:root</code> depending on setup) and permissions are strictly set to <code>0700</code> using <code>chmod 700 /var/lib/etcd</code>" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The insecure port causes etcd to disable encryption at rest for all newly created secrets" },
      { id: 'B', text: "The insecure port transmits unencrypted HTTP traffic and completely bypasses all authentication, authorization, and admission control checks" },
      { id: 'C', text: "The insecure port forces all pods to communicate without NetworkPolicies" },
      { id: 'D', text: "The insecure port only accepts requests from the <code>system:masters</code> group over plain HTTP" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Ownership <code>root:admin</code> and permissions <code>0660</code>" },
      { id: 'D', text: "Ownership <code>kubelet:root</code> and permissions <code>0640</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Define an egress <code>ipBlock</code> with <code>cidr: 0.0.0.0/0</code> and add the control plane subnet CIDR (e.g., <code>10.0.1.0/24</code>) to the <code>except</code> list" },
      { id: 'B', text: "Configure an ingress rule with <code>from: [{ ipBlock: { cidr: '10.0.1.0/24' } }]</code>" },
      { id: 'C', text: "Create a NetworkPolicy targeting the <code>kube-system</code> namespace with an empty podSelector" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Run <code>crictl inspect --output json &lt;etcd-container-id&gt; | grep 'encrypted'</code>" },
      { id: 'B', text: "Run <code>openssl verify -CAfile /etc/kubernetes/pki/ca.crt /var/lib/etcd/member/snap/db</code>" },
      { id: 'C', text: "Run <code>kubectl get secret &lt;secret-name&gt; -o yaml</code> and inspect the <code>metadata.encrypted</code> annotation" },
      { id: 'D', text: "Run <code>ETCDCTL_API=3 etcdctl --cacert=&lt;ca&gt; --cert=&lt;cert&gt; --key=&lt;key&gt; get /registry/secrets/&lt;namespace&gt;/&lt;secret-name&gt;</code> and verify that the output begins with <code>k8s:enc:aescbc:v1:</code> or similar provider prefix" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Delete all existing secrets from etcd, restart <code>kube-apiserver</code> with the new key, and redeploy applications from Helm charts" },
      { id: 'C', text: "Replace the old key directly with the new key in <code>enc.yaml</code> and restart all control plane and worker nodes simultaneously" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Configure <code>--insecure-kubelet-tls=true</code> and set <code>--kubelet-preferred-address-types=InternalIP</code>" },
      { id: 'C', text: "Configure <code>--kubelet-port=10255</code> and disable <code>--kubelet-https</code>" },
      { id: 'D', text: "Set <code>--anonymous-auth=true</code> and remove the <code>--client-ca-file</code> parameter" }
    ],
    correctAnswers: ['A'],
    type: "single",
