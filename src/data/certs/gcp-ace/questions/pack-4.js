export const GCP_ACE_QUESTIONS_4 = [
  {
    ...
    options: [
      { id: 'A', text: "Convert the VPC network from Custom Mode to Auto Mode" },
      { id: 'B', text: "Restrict VM deployment to sole-tenant hardware" },
      { id: 'C', text: "Target firewall rules to the specific Service Account under which the database instances run, rather than using network tags" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "External Passthrough Network Load Balancer" },
      { id: 'B', text: "Internal Application Load Balancer" },
      { id: 'C', text: "Cloud DNS with latency-based routing records" },
      { id: 'D', text: "External Application Load Balancer (Global HTTPS)" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Cloud Armor rate limiting proxy" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An HA VPN tunnel connected to on-premises" },
      { id: 'B', text: "An internet gateway route pointing to 0.0.0.0/0" },
      { id: 'C', text: "A Cloud NAT gateway with manual IP assignment" },
      { id: 'D', text: "A proxy-only subnet in the region with purpose set to REGIONAL_MANAGED_PROXY" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Create a Cloud DNS Public Zone with DNSSEC enabled" },
      { id: 'B', text: "Create a Cloud DNS Private Zone authorized for the specific VPC network" },
      { id: 'C', text: "Deploy BIND9 DNS servers on two Compute Engine VMs" },
      { id: 'D', text: "Maintain /etc/hosts files across all VM instances using Ansible" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure a VPC firewall rule allowing outbound port 53 to 0.0.0.0/0" },
      { id: 'B', text: "Deploy Cloud NAT with DNS proxy enabled" },
      { id: 'C', text: "Create a Cloud DNS Forwarding Zone for the domain '.corp.local' with forwarding targets set to the on-premises DNS server IP addresses" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Cloud Storage FUSE mount" },
      { id: 'B', text: "Local SSD attached in raid-1" },
      { id: 'C', text: "Regional Persistent Disk (Regional PD)" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Write a Python script on each VM that executes 'dd' and uploads disk images to Cloud Storage via cron" },
      { id: 'B', text: "Deploy Cloud Backup and DR standalone virtual appliances" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Firebase Cloud Functions legacy runtime" },
      { id: 'B', text: "Cloud Functions (2nd gen)" },
      { id: 'C', text: "Cloud Functions (1st gen) with memory scaled to 8 GB" },
      { id: 'D', text: "App Engine Standard Environment" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Export database tables to Cloud Bigtable every 10 minutes" },
      { id: 'B', text: "Migrate reporting queries to execute against the binary log files directly" },
      { id: 'C', text: "Enable High Availability on the primary instance and direct analysts to the standby instance" },
      { id: 'D', text: "Create one or more Cloud SQL Read Replicas and configure reporting tools to connect to the read replica endpoints" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Storage auto-increase and Cloud Monitoring alerts" },
      { id: 'C', text: "Automated daily backups and Binary Logging" },
      { id: 'D', text: "Customer-Managed Encryption Keys and SSL/TLS certificates" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Create a Dual-Region Cloud Storage bucket and enable Turbo Replication" },
      { id: 'B', text: "Create a Multi-Region bucket with default replication settings" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy a Custom Machine Type configured with 2 vCPUs and 32 GB of RAM (extended memory)" },
      { id: 'B', text: "Run the caching daemon inside Google Cloud Shell" },
      { id: 'C', text: "Deploy the application on Cloud Run with concurrency set to 1" },
      { id: 'D', text: "Deploy an e2-micro instance and attach a 32 GB persistent disk formatted as swap space" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Cloud Bigtable in SSD mode" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "General-Purpose (E2) machine series" },
      { id: 'B', text: "Accelerator-Optimized (A2) machine series" },
      { id: 'C', text: "Memory-Optimized (M2) machine series" },
      { id: 'D', text: "Compute-Optimized (C2 or C3) machine series" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Subnet IP ranges will automatically be truncated from /20 to /24" },
      { id: 'C', text: "All running Compute Engine instances in the VPC will be terminated during conversion" },
      { id: 'D', text: "Converting to Custom Mode is a permanent, irreversible operation; existing subnets retain their IP ranges, but future subnets must be created manually" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Grant the primitive Owner role on shared-net-prod to developers in billing-dev" },
      { id: 'B', text: "Attach 'billing-dev' as a Service Project to 'shared-net-prod' in Shared VPC settings, and grant 'roles/compute.networkUser' on the subnet to the developers or service accounts in 'billing-dev'" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "BigQuery handles high-throughput streaming appends natively via the BigQuery Storage Write API without key-range hotspotting, using distributed columnar storage (Capacitor)" },
      { id: 'B', text: "BigQuery tables must be rebuilt daily to re-balance timestamp splits" },
      { id: 'C', text: "BigQuery requires hashing timestamps before streaming data" },
      { id: 'D', text: "BigQuery limits streaming writes to 100 rows per second to prevent hotspots" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Create a Multi-Region bucket and configure Cloud Storage Transfer Service to execute every 15 minutes" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Cloud Run for Anthos on-premises" },
      { id: 'C', text: "Compute Engine Managed Instance Groups" },
      { id: 'D', text: "GKE Standard with custom node pools" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Provision a 3-node GKE Standard cluster and configure an internal ClusterIP service" },
      { id: 'B', text: "Deploy the container to Cloud Run with concurrency settings configured and allow unauthenticated invocations" },
      { id: 'C', text: "Deploy the application to Google App Engine flexible environment with min_instances set to 5" },
      { id: 'D', text: "Launch an E2-standard-4 Compute Engine instance and install Docker manually over SSH" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy unmanaged instance groups in three different regions behind an internal TCP proxy" },
      { id: 'B', text: "Deploy a single Zonal MIG with autohealing enabled" },
      { id: 'C', text: "Manually create standalone VMs in three zones and configure DNS round-robin routing" },
      { id: 'D', text: "Create an instance template and deploy a Regional Managed Instance Group (MIG) distributed across multiple zones with an HTTP health check and autoscaling enabled" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure an Object Lifecycle Management policy on the bucket with two rules: transition to Nearline after 30 days, and delete objects after 365 days" },
      { id: 'B', text: "Change the default storage class of the bucket to Archive Storage immediately upon creation" },
      { id: 'C', text: "Deploy an EC2 instance that mounts the bucket via FUSE and runs a nightly cron script" },
      { id: 'D', text: "Write a Cloud Function triggered by Cloud Scheduler that runs gsutil mv and gsutil rm daily" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Enable High Availability (regional configuration) on the primary Cloud SQL instance and provision one or more read replicas in the same or separate zones" },
      { id: 'B', text: "Configure an automated export to Cloud Storage every 10 minutes and read from CSV files" },
      { id: 'C', text: "Deploy two standalone single-zone Cloud SQL instances and write custom replication scripts" },
      { id: 'D', text: "Enable point-in-time recovery on a single-zone instance without secondary standby" }
    ],
    correctAnswers: ['A'],
    type: "single",
