export const GCP_ACE_QUESTIONS_7 = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Set --concurrency=1 so each container handles one task" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Enable Object Versioning on the Cloud Storage bucket" },
      { id: 'B', text: "Switch the storage class to Coldline Storage" },
      { id: 'C', text: "Apply a VPC firewall rule blocking HTTP DELETE requests" },
      { id: 'D', text: "Enable Bucket Lock with compliance mode" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Require authentication and have Scheduler call it with an OIDC token." },
      { id: 'B', text: "Protect the Cloud Run service with a basic HTTP username and password in metadata" },
      { id: 'C', text: "Deploy Cloud Run with '--allow-unauthenticated' and check IP headers in application code" },
      { id: 'D', text: "Deploy Cloud Run inside an unmanaged Compute Engine instance" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "bq query --create_partitioned_table [DATASET].[TABLE]" },
      { id: 'B', text: "gcloud bigquery tables create [TABLE] --partition=daily" },
      { id: 'C', text: "bq mk --table --time_partitioning_type=DAY [DATASET].[TABLE]" },
      { id: 'D', text: "bq mk --table --partition_by=date [DATASET].[TABLE]" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "requests guarantee the baseline, limits cap the maximum" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Cloud Scheduler invoking Compute Engine startup scripts" },
      { id: 'B', text: "Google Cloud Deploy" },
      { id: 'C', text: "Artifact Registry automated deployment hooks" },
      { id: 'D', text: "Cloud Build with custom inline shell scripts" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Write a bash script that curls the website from an internal VM and writes to a text file" },
      { id: 'C', text: "A multi-region uptime check with an alerting policy and notification channels." },
      { id: 'D', text: "Configure a Compute Engine health check and set the autohealing threshold to 3 minutes" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An aggregated sink for all audit logs into a locked Archive-class bucket." },
      { id: 'B', text: "Increase the retention period of the _Default and _Required log buckets in Cloud Logging to 2,555 days" },
      { id: 'C', text: "Stream all logs to a Cloud Pub/Sub topic with message retention set to 7 years" },
      { id: 'D', text: "Export logs daily to a BigQuery table with a 30-day partition expiration policy" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Run 'gcloud compute disks restore' to overwrite the running boot disk in place" },
      { id: 'C', text: "Create a disk from the snapshot and swap it in as the boot disk." },
      { id: 'D', text: "Delete the entire VPC network and redeploy from Cloud Shell" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Perform an instance Reset operation" },
      { id: 'C', text: "Perform a Stop operation followed by a Start operation on the instance" },
      { id: 'D', text: "Execute 'sudo reboot' from inside the guest Linux operating system" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "When new firewall rules are applied to the VPC network" },
      { id: 'B', text: "When the guest operating system is rebooted using 'sudo reboot'" },
      { id: 'C', text: "During standard Google Cloud live migration events" },
      { id: 'D', text: "When the instance is stopped or terminated by the user" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Disable the GKE cluster autoscaler" },
      { id: 'B', text: "Convert the cluster from Regional to Zonal" },
      { id: 'C', text: "Remove all IAM permissions from Google Cloud Support" },
      { id: 'D', text: "A maintenance exclusion for those ten days" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Upgrade the instances to Memory-Optimized machine types" },
      { id: 'C', text: "Enable VPC Flow Logs on the subnet" },
      { id: 'D', text: "Assign the 'roles/monitoring.admin' role to the VM's service account" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The container's liveness probe failed 3 consecutive times" },
      { id: 'B', text: "The container was evicted due to an invalid CPU request" },
      { id: 'C', text: "The node running the pod lost network connectivity to the master control plane" },
      { id: 'D', text: "The process exceeded its memory limit and the kernel OOM-killed it" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Stream logs to a Compute Engine instance running an ELK stack" },
      { id: 'B', text: "An aggregated log sink at the organization level, into BigQuery" },
      { id: 'C', text: "Configure Cloud Monitoring uptime checks with webhook notifications" },
      { id: 'D', text: "Write a script that exports logs to Cloud Storage and runs daily bq load jobs" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Delete the Cloud Logging API from the project" },
      { id: 'B', text: "A Cloud Logging exclusion for severity<=INFO" },
      { id: 'C', text: "Set the retention period on the '_Default' log bucket to 1 day" },
      { id: 'D', text: "Configure an alert policy with notification channels disabled" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Install a custom cron script on the VMs that scrapes logs every 5 minutes" },
      { id: 'C', text: "A log-based counter metric with an alerting policy on it" },
      { id: 'D', text: "Deploy Cloud Trace with distributed context propagation" }
    ],
    correctAnswers: ['C'],
    type: "single",
