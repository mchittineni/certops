export const GCP_ACE_QUESTIONS_7 = [
  {
    ...
    options: [
      { id: 'A', text: "Enable Horizontal Pod Autoscaler" },
      ...
      { id: 'D', text: "Configure Node Auto-Upgrade with a 24-hour maintenance window" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Deploy Cloud Run behind an Internal Network Load Balancer" },
      { id: 'D', text: "Reduce concurrency using '--concurrency=1' (or a low value like 2-5) so each container processes only one transcoding task at a time" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Enable Bucket Lock with compliance mode" },
      { id: 'B', text: "Switch the storage class to Coldline Storage" },
      { id: 'C', text: "Enable Object Versioning on the Cloud Storage bucket" },
      { id: 'D', text: "Apply a VPC firewall rule blocking HTTP DELETE requests" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy Cloud Run with '--allow-unauthenticated' and check IP headers in application code" },
      { id: 'B', text: "Protect the Cloud Run service with a basic HTTP username and password in metadata" },
      { id: 'C', text: "Deploy Cloud Run inside an unmanaged Compute Engine instance" },
      { id: 'D', text: "Deploy Cloud Run with '--no-allow-unauthenticated', and configure Cloud Scheduler to invoke the service using an OIDC token signed by a dedicated service account granted 'roles/run.invoker'" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "gcloud bigquery tables create [TABLE] --partition=daily" },
      { id: 'B', text: "bq mk --table --time_partitioning_type=DAY [DATASET].[TABLE]" },
      { id: 'C', text: "bq mk --table --partition_by=date [DATASET].[TABLE]" },
      { id: 'D', text: "bq query --create_partitioned_table [DATASET].[TABLE]" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Enable Vertical Pod Autoscaler in auto mode" },
      { id: 'D', text: "Define 'resources.requests' to guarantee baseline capacity, and define 'resources.limits' to hard-cap maximum CPU and memory consumption for each container" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "gcloud compute instances create [REPLICA_NAME] --sql-master=[PRIMARY_NAME]" },
      { id: 'B', text: "gcloud sql instances create [REPLICA_NAME] --master-instance-name=[PRIMARY_NAME] --region=europe-west1" },
      { id: 'C', text: "gcloud sql instances replicate [PRIMARY_NAME] --to-region=europe-west1" },
      { id: 'D', text: "gcloud sql instances clone [PRIMARY_NAME] [REPLICA_NAME] --region=europe-west1" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Cloud Build with custom inline shell scripts" },
      { id: 'B', text: "Artifact Registry automated deployment hooks" },
      { id: 'C', text: "Cloud Scheduler invoking Compute Engine startup scripts" },
      { id: 'D', text: "Google Cloud Deploy" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Configure a Compute Engine health check and set the autohealing threshold to 3 minutes" },
      { id: 'C', text: "Write a bash script that curls the website from an internal VM and writes to a text file" },
      { id: 'D', text: "Create a Cloud Monitoring Uptime Check targeting the public URL from multiple geographic regions, and attach an Alerting Policy configured with email and PagerDuty notification channels" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Stream all logs to a Cloud Pub/Sub topic with message retention set to 7 years" },
      { id: 'B', text: "Increase the retention period of the _Default and _Required log buckets in Cloud Logging to 2,555 days" },
      { id: 'C', text: "Export logs daily to a BigQuery table with a 30-day partition expiration policy" },
      { id: 'D', text: "Create an aggregated log sink in Cloud Logging with a filter matching all audit logs, routing to a Cloud Storage bucket configured with Archive storage class and an object retention lock" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "gcloud compute instances get-serial-port-output [INSTANCE_NAME] --zone=[ZONE]" },
      { id: 'B', text: "gcloud logging read 'resource.type=gce_instance AND logName=console'" },
      { id: 'C', text: "gcloud compute ssh [INSTANCE_NAME] --troubleshoot" },
      { id: 'D', text: "gcloud compute instances tail-logs [INSTANCE_NAME]" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Create a new persistent disk from snapshot 'boot-snap-safe', stop the VM, detach the corrupted boot disk, attach the new disk as the boot disk, and start the VM" },
      { id: 'C', text: "Delete the entire VPC network and redeploy from Cloud Shell" },
      { id: 'D', text: "Run 'gcloud compute disks restore' to overwrite the running boot disk in place" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Execute 'sudo reboot' from inside the guest Linux operating system" },
      { id: 'C', text: "Perform an instance Reset operation" },
      { id: 'D', text: "Perform a Stop operation followed by a Start operation on the instance" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "When the instance is stopped or terminated by the user" },
      { id: 'B', text: "When new firewall rules are applied to the VPC network" },
      { id: 'C', text: "During standard Google Cloud live migration events" },
      { id: 'D', text: "When the guest operating system is rebooted using 'sudo reboot'" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Manual node-by-node kubelet package upgrade via SSH" },
      { id: 'B', text: "Blue/Green node pool upgrade" },
      { id: 'C', text: "Immediate in-place node drain without surge" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Remove all IAM permissions from Google Cloud Support" },
      { id: 'B', text: "Disable the GKE cluster autoscaler" },
      { id: 'C', text: "Configure a Maintenance Exclusion for the cluster covering the 10-day date range" },
      { id: 'D', text: "Convert the cluster from Regional to Zonal" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "kubectl delete deployment frontend-app" },
      { id: 'B', text: "kubectl rollout restart deployment/frontend-app" },
      { id: 'C', text: "kubectl rollout undo deployment/frontend-app" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Enable VPC Flow Logs on the subnet" },
      { id: 'C', text: "Assign the 'roles/monitoring.admin' role to the VM's service account" },
      { id: 'D', text: "Upgrade the instances to Memory-Optimized machine types" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The container was evicted due to an invalid CPU request" },
      { id: 'B', text: "The container process exceeded its configured memory limit in the pod specification and was terminated by the Linux kernel Out-Of-Memory killer" },
      { id: 'C', text: "The container's liveness probe failed 3 consecutive times" },
      { id: 'D', text: "The node running the pod lost network connectivity to the master control plane" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "SELECT * FROM logs WHERE type='vm' AND error='timeout'" },
      { id: 'C', text: "resource.type=\"gce_instance\" AND resource.labels.zone=~\"^us-east1\" AND severity>=ERROR AND textPayload=~\"timeout\"" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Write a script that exports logs to Cloud Storage and runs daily bq load jobs" },
      { id: 'B', text: "Stream logs to a Compute Engine instance running an ELK stack" },
      { id: 'C', text: "Create an aggregated Log Sink at the Organization level configured with a destination of BigQuery" },
      { id: 'D', text: "Configure Cloud Monitoring uptime checks with webhook notifications" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set the retention period on the '_Default' log bucket to 1 day" },
      { id: 'B', text: "Configure an alert policy with notification channels disabled" },
      { id: 'C', text: "Create a Log Exclusion in Cloud Logging with a filter matching 'severity<=INFO'" },
      { id: 'D', text: "Delete the Cloud Logging API from the project" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Create a Log-Based Counter Metric in Cloud Logging matching the failure log filter, and build an Alerting Policy based on that metric" },
      { id: 'C', text: "Deploy Cloud Trace with distributed context propagation" },
      { id: 'D', text: "Install a custom cron script on the VMs that scrapes logs every 5 minutes" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The backup will fail because Cloud SQL does not allow restoring into the same region" },
      { id: 'B', text: "The instance will be converted to an unmanaged Compute Engine VM" },
