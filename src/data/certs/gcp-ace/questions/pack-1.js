export const GCP_ACE_QUESTIONS_1 = [
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "The user has Compute Viewer permissions only on projects created prior to the role assignment"
      },
      {
        "id": "B",
        "text": "IAM permissions cannot be applied at the folder level and must be defined on the Organization node"
      },
      {
        "id": "C",
        "text": "The user has Compute Viewer permissions on all current and future projects located inside the Engineering folder due to downward policy inheritance"
      },
      {
        "id": "D",
        "text": "The role must be manually re-applied to each project within the folder to take effect"
      }
    ],
    "correctAnswers": [
      "C"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Enable the default billing checkbox that terminates all running VMs immediately at 100% spend"
      },
      {
        "id": "B",
        "text": "Configure Cloud Monitoring uptime checks to ping the billing console every minute"
      },
      {
        "id": "C",
        "text": "Set a hard spending cap in the gcloud CLI using the command gcloud billing cap set --limit"
      },
      {
        "id": "D",
        "text": "Configure a Cloud Billing budget to publish notifications to a Cloud Pub/Sub topic, and trigger a Cloud Run service or Cloud Function that programmatically disables billing or stops workloads"
      }
    ],
    "correctAnswers": [
      "D"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Uninstall and reinstall the Google Cloud SDK inside separate operating system directories"
      },
      {
        "id": "B",
        "text": "Run gcloud auth login and re-authenticate in the browser every time the project changes"
      },
      {
        "id": "C",
        "text": "Create named gcloud configurations using gcloud config configurations create and switch between them using gcloud config configurations activate"
      },
      {
        "id": "D",
        "text": "Edit the local credentials.json file manually in a text editor before running commands"
      }
    ],
    "correctAnswers": [
      "C"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Download monthly PDF invoices from the Cloud Console and run OCR extraction scripts"
      },
      {
        "id": "B",
        "text": "Stream Cloud Audit Logs to a Cloud Storage bucket and write custom Python map-reduce scripts"
      },
      {
        "id": "C",
        "text": "Configure a cron job that executes gcloud billing accounts describe every hour"
      },
      {
        "id": "D",
        "text": "Enable Cloud Billing export to BigQuery (Standard and Detailed usage cost data) and query the resulting tables using SQL"
      }
    ],
    "correctAnswers": [
      "D"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Configure the Cloud Billing budget to publish threshold notifications to a Cloud Pub/Sub topic, and deploy a Cloud Function subscribed to the topic that calls the Cloud Billing API to unlink the project from the billing account"
      },
      {
        "id": "B",
        "text": "Set the budget threshold to 100% and enable the 'Auto-terminate on breach' checkbox in the Cloud Billing console"
      },
      {
        "id": "C",
        "text": "Write an hourly cron job on a Compute Engine instance that queries the Cloud Billing API and stops instances when cost exceeds $500"
      },
      {
        "id": "D",
        "text": "Apply an Organization Policy constraint that limits Compute Engine vCPU hours to $500 equivalent per month"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Enforce the Organization Policy constraint 'constraints/compute.vmExternalIpAccess' on the production folder or organization root"
      },
      {
        "id": "B",
        "text": "Revoke the Compute Admin role from all developers and assign them Compute Viewer"
      },
      {
        "id": "C",
        "text": "Create a VPC firewall rule that blocks ingress traffic on 0.0.0.0/0 with priority 1000"
      },
      {
        "id": "D",
        "text": "Delete the default internet gateway from all production VPC networks"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Create named configurations using 'gcloud config configurations create' for each environment, set the respective account, project, and compute properties in each, and switch between them using 'gcloud config configurations activate'"
      },
      {
        "id": "B",
        "text": "Maintain separate bash shell scripts that execute 'gcloud auth login' with stored plaintext credentials upon terminal launch"
      },
      {
        "id": "C",
        "text": "Install multiple instances of the Google Cloud SDK in different local filesystem directories"
      },
      {
        "id": "D",
        "text": "Pass the --project, --account, and --region flags manually on every gcloud CLI invocation"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "roles/iam.serviceAccountUser on the target service account resource"
      },
      {
        "id": "B",
        "text": "roles/iam.serviceAccountTokenCreator at the project level"
      },
      {
        "id": "C",
        "text": "roles/iam.serviceAccountAdmin on the project"
      },
      {
        "id": "D",
        "text": "roles/iam.serviceAccountKeyAdmin on the target service account"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Grant the runner's base identity the 'roles/iam.serviceAccountTokenCreator' role on the target service account, and execute commands using the '--impersonate-service-account' flag in gcloud"
      },
      {
        "id": "B",
        "text": "Generate a P12 certificate key in the Cloud Console and store it in an environment variable on the runner"
      },
      {
        "id": "C",
        "text": "Embed the service account credentials in the git repository using git-crypt"
      },
      {
        "id": "D",
        "text": "Assign the primitive Owner role to the base identity of the CI/CD runner"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Detailed usage cost export to BigQuery"
      },
      {
        "id": "B",
        "text": "Standard usage cost export to BigQuery"
      },
      {
        "id": "C",
        "text": "Pricing data export to BigQuery"
      },
      {
        "id": "D",
        "text": "Cloud Audit Logs Data Access export to Cloud Storage"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Bind the Kubernetes Service Account (KSA) to the Google Service Account (GSA) using the 'roles/iam.workloadIdentityUser' role, and annotate the KSA with the GSA email address"
      },
      {
        "id": "B",
        "text": "Mount the Compute Engine default service account token into the pod filesystem using a hostPath volume"
      },
      {
        "id": "C",
        "text": "Run 'gcloud auth login' inside the container startup script"
      },
      {
        "id": "D",
        "text": "Grant the 'roles/storage.objectAdmin' role directly to the GKE worker node service account"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Define the Organization Policy constraint 'constraints/gcp.resourceLocations' with an allowed list of 'in:eu-locations' or specific European region codes"
      },
      {
        "id": "B",
        "text": "Configure a VPC firewall rule blocking all egress traffic destined for non-European IP ranges"
      },
      {
        "id": "C",
        "text": "Instruct project owners to set their local gcloud CLI compute/region configuration to europe-west1"
      },
      {
        "id": "D",
        "text": "Create an IAM condition on the Owner role requiring location headers matching Europe"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "id": "B",
        "text": "gcloud compute instances enable-api"
      },
      {
        "id": "C",
        "text": "gcloud components install compute"
      },
      {
        "id": "D",
        "text": "gcloud config set api/compute enabled"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "id": "D",
        "text": "roles/resourcemanager.projectBillingManager on the organization root"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Execute 'gcloud projects undelete analytics-prod' within the 30-day soft-delete grace period"
      },
      {
        "id": "B",
        "text": "Open an urgent P1 support ticket with Google Cloud Support to restore from tape backup"
      },
      {
        "id": "C",
        "text": "Create a new project with the identical project ID and re-import data"
      },
      {
        "id": "D",
        "text": "Restore the project from Cloud Storage using gsutil restore"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Create Google Groups in Cloud Identity or Google Workspace corresponding to job functions (e.g., backend-devs@company.com), bind IAM roles to the groups, and manage permissions by adding or removing users from the groups"
      },
      {
        "id": "B",
        "text": "Assign the primitive Editor role to all engineers at the organization root level"
      },
      {
        "id": "C",
        "text": "Write a Python script that scrapes IAM policies daily and emails reports to managers"
      },
      {
        "id": "D",
        "text": "Create a single shared user-managed service account and share its JSON key among all engineers"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "IAM Policy Analyzer in the Google Cloud Console"
      },
      {
        "id": "B",
        "text": "gcloud projects get-iam-policy"
      },
      {
        "id": "C",
        "text": "Cloud Asset Inventory export to CSV"
      },
      {
        "id": "D",
        "text": "Cloud Logging filter for IAM changes"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Enforce the constraint 'constraints/iam.disableServiceAccountKeyCreation' at the organization root"
      },
      {
        "id": "B",
        "text": "Revoke the Service Account Admin role from all developers"
      },
      {
        "id": "C",
        "text": "Deploy an automated Cloud Function that deletes keys immediately after creation"
      },
      {
        "id": "D",
        "text": "Configure a Cloud Storage lifecycle policy to delete JSON files"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "id": "B",
        "text": "gcloud auth login --enable-adc"
      },
      {
        "id": "C",
        "text": "gcloud config set auth/application_default true"
      },
      {
        "id": "D",
        "text": "export GOOGLE_APPLICATION_CREDENTIALS=/dev/null"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "IAM permissions inherit strictly downwards in the resource hierarchy; inherited permissions cannot be revoked or overridden at a child level"
      },
      {
        "id": "B",
        "text": "The administrator lacks the Security Admin role in payroll-app"
      },
      {
        "id": "C",
        "text": "The project must be placed into maintenance mode before editing inherited roles"
      },
      {
        "id": "D",
        "text": "The engineer holds the primitive Owner role at the project level"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "gcloud compute instances list --filter=\"zone:us-central1-a AND status:RUNNING AND name:worker*\" --format=\"value(networkInterfaces[0].networkIP)\""
      },
      {
        "id": "B",
        "text": "gcloud compute instances list | grep worker | awk '{print $4}'"
      },
      {
        "id": "C",
        "text": "gcloud compute instances describe worker-* --zone=us-central1-a --format=text"
      },
      {
        "id": "D",
        "text": "gcloud compute instances list --query=\"select networkIP where name like 'worker%'\""
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "id": "B",
        "text": "roles/billing.user on the organization root"
      },
      {
        "id": "C",
      ...
      {
        "id": "D",
        "text": "roles/owner on all projects"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "id": "B",
        "text": "Google Cloud Identity-Aware Proxy (IAP)"
      },
      {
        "id": "C",
        "text": "Active Directory Migration Tool (ADMT)"
      },
      {
        "id": "D",
        "text": "Cloud Identity Connector for Azure"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Add an IAM policy binding with an IAM Condition evaluating request.time < timestamp('2026-09-05T16:00:00Z')"
      },
      {
        "id": "B",
        "text": "Create a Cloud Scheduler job that executes a gcloud IAM revocation command at 16:00"
      },
      {
        "id": "C",
        "text": "Set the user password expiration to 4 hours in Cloud Identity"
      },
      {
        "id": "D",
        "text": "Grant the primitive Editor role and enable two-factor authentication"
      }
    ],
    "correctAnswers": [
      "A"
