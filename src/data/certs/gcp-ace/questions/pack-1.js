export const GCP_ACE_QUESTIONS_1 = [
  {
    ...
    options: [
      { id: 'A', text: "The role must be manually re-applied to each project within the folder to take effect" },
      { id: 'B', text: "The user has Compute Viewer on all current and future projects in the folder" },
      { id: 'C', text: "IAM permissions cannot be applied at the folder level and must be defined on the Organization node" },
      { id: 'D', text: "The user has Compute Viewer permissions only on projects created prior to the role assignment" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set a hard spending cap in the gcloud CLI using the command gcloud billing cap set --limit" },
      { id: 'B', text: "Enable the default billing checkbox that terminates all running VMs immediately at 100% spend" },
      { id: 'C', text: "Configure Cloud Monitoring uptime checks to ping the billing console every minute" },
      { id: 'D', text: "A budget notification to Pub/Sub triggering a function that disables billing" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Create named gcloud configurations and switch with configurations activate" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Download monthly PDF invoices from the Cloud Console and run OCR extraction scripts" },
      { id: 'B', text: "Enable Cloud Billing export to BigQuery and query the tables with SQL" },
      { id: 'C', text: "Stream Cloud Audit Logs to a Cloud Storage bucket and write custom Python map-reduce scripts" },
      { id: 'D', text: "Configure a cron job that executes gcloud billing accounts describe every hour" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A budget threshold to Pub/Sub, and a function that unlinks the billing account." },
      { id: 'B', text: "Set the budget threshold to 100% and enable the 'Auto-terminate on breach' checkbox in the Cloud Billing console" },
      { id: 'C', text: "Write an hourly cron job on a Compute Engine instance that queries the Cloud Billing API and stops instances when cost exceeds $500" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Delete the default internet gateway from all production VPC networks" },
      { id: 'B', text: "Create a VPC firewall rule that blocks ingress traffic on 0.0.0.0/0 with priority 1000" },
      { id: 'C', text: "Revoke the Compute Admin role from all developers and assign them Compute Viewer" },
      { id: 'D', text: "Enforce constraints/compute.vmExternalIpAccess on the production folder" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Maintain separate bash shell scripts that execute 'gcloud auth login' with stored plaintext credentials upon terminal launch" },
      { id: 'B', text: "Install multiple instances of the Google Cloud SDK in different local filesystem directories" },
      { id: 'C', text: "Pass the --project, --account, and --region flags manually on every gcloud CLI invocation" },
      { id: 'D', text: "A named gcloud configuration per environment, each with its own account and project." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "roles/iam.serviceAccountAdmin on the project" },
      { id: 'C', text: "roles/iam.serviceAccountTokenCreator at the project level" },
      { id: 'D', text: "roles/iam.serviceAccountKeyAdmin on the target service account" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Embed the service account credentials in the git repository using git-crypt" },
      { id: 'B', text: "Generate a P12 certificate key in the Cloud Console and store it in an environment variable on the runner" },
      { id: 'C', text: "Assign the primitive Owner role to the base identity of the CI/CD runner" },
      { id: 'D', text: "Grant serviceAccountTokenCreator and use --impersonate-service-account." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Standard usage cost export to BigQuery" },
      { id: 'B', text: "Cloud Audit Logs Data Access export to Cloud Storage" },
      { id: 'C', text: "Detailed usage cost export to BigQuery" },
      { id: 'D', text: "Pricing data export to BigQuery" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Grant the 'roles/storage.objectAdmin' role directly to the GKE worker node service account" },
      { id: 'B', text: "Bind the KSA with workloadIdentityUser and annotate it with the GSA email." },
      { id: 'C', text: "Run 'gcloud auth login' inside the container startup script" },
      { id: 'D', text: "Mount the Compute Engine default service account token into the pod filesystem using a hostPath volume" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure a VPC firewall rule blocking all egress traffic destined for non-European IP ranges" },
      { id: 'B', text: "Set constraints/gcp.resourceLocations to allow only EU locations." },
      { id: 'C', text: "Instruct project owners to set their local gcloud CLI compute/region configuration to europe-west1" },
      { id: 'D', text: "Create an IAM condition on the Owner role requiring location headers matching Europe" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "roles/billing.user on the billing account" },
      { id: 'B', text: "roles/resourcemanager.projectBillingManager on the organization root" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Bind IAM roles to job-function Google Groups and manage membership." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Cloud Logging filter for IAM changes" },
      { id: 'B', text: "IAM Policy Analyzer in the Google Cloud Console" },
      { id: 'C', text: "Cloud Asset Inventory export to CSV" },
      { id: 'D', text: "gcloud projects get-iam-policy" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Revoke the Service Account Admin role from all developers" },
      { id: 'B', text: "Enforce iam.disableServiceAccountKeyCreation at the org root" },
      { id: 'C', text: "Configure a Cloud Storage lifecycle policy to delete JSON files" },
      { id: 'D', text: "Deploy an automated Cloud Function that deletes keys immediately after creation" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Permissions inherit downwards and cannot be revoked at a child." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An IAM policy binding with a request.time expiry condition" },
