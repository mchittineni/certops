export const GCP_ACE_QUESTIONS = [
  {
    ...
    options: [
      { id: 'A', text: "gcloud compute instances create-with-container app --container-image=IMAGE, repeated three times" },
      { id: 'B', text: "gcloud container clusters resize CLUSTER --num-nodes=3, then kubectl apply -f service.yaml" },
      { id: 'C', text: "kubectl run app --image=IMAGE, then kubectl expose pod app --type=ClusterIP --port=80" },
      { id: 'D', text: "kubectl create deployment app --image=IMAGE --replicas=3, then kubectl expose deployment app --type=LoadBalancer --port=80 --target-port=8080" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "roles/editor at the project level" },
      { id: 'C', text: "roles/storage.admin" },
      { id: 'D', text: "roles/storage.objectAdmin" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "A snapshot schedule (resource policy) attached to the persistent disks, with a 14-day retention policy." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Schedule a Cloud Function to page through the Logging API and write CSV files to a Compute Engine persistent disk." },
      ...
      { id: 'D', text: "Increase the retention of the _Required log bucket to 730 days." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Enable Workload Identity on the cluster and bind each Kubernetes service account to a dedicated Google service account via an IAM policy on the workloadIdentityUser role." },
      { id: 'B', text: "Grant the Compute Engine default service account the Editor basic role at the project level." },
      ...
      { id: 'D', text: "Grant the node pool service account the union of all permissions the three microservices need." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Cloud Spanner with a multi-region instance configuration." },
      { id: 'B', text: "Bigtable with a multi-cluster routing app profile." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy the revision with --no-traffic so it starts at 0% of traffic." },
      { id: 'B', text: "Put an external HTTP(S) Load Balancer in front of two separate Cloud Run services and split traffic with a URL map weight." },
      { id: 'C', text: "Scale the previous revision to zero instances so the new revision absorbs all requests gradually." },
      { id: 'D', text: "Deploy the revision normally and rely on Cloud Run sending traffic to the newest revision only after health checks pass." },
      { id: 'E', text: "Use gcloud run services update-traffic --to-revisions REVISION=5 and later --to-latest to finish the rollout." }
    ],
    correctAnswers: ['A', 'E'],
    type: "multiple",
