export const CNCF_OTCA_QUESTIONS_15 = [
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Run two separate collector daemons on every worker node." },
      { id: 'B', text: "Configure applications to make duplicate network calls to both backends." },
  ...
  },
  {
    ...
    question: "Which OpenTelemetry component design or collector configuration handles this volume efficiently while protecting downstream storage backends?",
    options: [
      { id: 'A', text: "Hardcode pod names inside application environment files." },
      { id: 'B', text: "Pass Kubernetes metadata in every HTTP query parameter from the client." },
      { id: 'C', text: "The collector cannot interact with the Kubernetes API." },
      { id: 'D', text: "Add the `k8sattributes` processor to the collector pipeline and configure RBAC permissions for the collector to read Kubernetes pod metadata." }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Add the `k8sattributes` processor to the collector pipeline and configure RBAC permissions for the collector to read Kubernetes pod metadata. The `k8sattributes` processor queries the local kubelet or Kubernetes API to match the IP address of incoming telemetry connections against running pods, automatically attaching metadata like `k8s.pod.name`, `k8s.namespace.name`, and `k8s.deployment.name` to telemetry records.",
  ...
  },
  {
    ...
    question: "Which OpenTelemetry configuration or processor implementation satisfies these security and governance controls?",
    options: [
      { id: 'A', text: "Pass Kubernetes metadata in every HTTP query parameter from the client." },
      { id: 'B', text: "Add the `k8sattributes` processor to the collector pipeline and configure RBAC permissions for the collector to read Kubernetes pod metadata." },
      { id: 'C', text: "The collector cannot interact with the Kubernetes API." },
  ...
  },
  {
    ...
    question: "Which architectural approach or OpenTelemetry configuration satisfies these enterprise observability objectives?",
    options: [
      { id: 'A', text: "Manually instrument every function call with both a trace span and a metric counter." },
      { id: 'B', text: "Configure the `spanmetrics` connector to consume spans from a traces pipeline and output request count and duration metrics into a metrics pipeline." },
      { id: 'C', text: "Metrics cannot be derived from distributed traces." },
      { id: 'D', text: "Run an external cron job that queries the tracing backend and calculates metrics." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    question: "Which architectural approach or OpenTelemetry configuration satisfies these enterprise observability objectives?",
    options: [
      { id: 'A', text: "SSH into the collector pod and run curl against localhost:80." },
      { id: 'B', text: "Check if the collector log file size is increasing." },
      { id: 'C', text: "Enable the `health_check` extension on port 13133 and optionally the `zpages` extension for live diagnostic debugging." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Enable the `health_check` extension on port 13133 and optionally the `zpages` extension for live diagnostic debugging." }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Enable the `health_check` extension on port 13133 and optionally the `zpages` extension for live diagnostic debugging. Extensions in the OpenTelemetry Collector provide capabilities that do not process telemetry directly. The `health_check` extension exposes an HTTP `/` endpoint on port 13133 for Kubernetes probes, and `zpages` provides in-process diagnostic pages for active traces and RPCs.",
