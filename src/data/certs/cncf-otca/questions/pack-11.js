export const CNCF_OTCA_QUESTIONS_11 = [
  {
    ...
    options: [
      ...
      { id: 'D', text: "Rely on unstructured plaintext log files printed to console output." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Disable distributed tracing to reduce network serialization overhead." },
      { id: 'B', text: "Adopt OpenTelemetry as the unified standard for collecting distributed traces, metrics, logs, and contextual baggage." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Write custom proprietary telemetry interfaces from scratch." },
      { id: 'D', text: "Force library users to install a local Redis database for telemetry buffering." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Bundle the complete OpenTelemetry SDK and specific vendor exporter binaries directly into the library package." }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Instrument library code using the lightweight `OpenTelemetry API` exclusively, allowing consuming applications to register the `OpenTelemetry SDK` at runtime. OpenTelemetry strictly separates the API (the interface for instrumentation, containing no implementation) from the SDK (the implementation of the API, managing processors, samplers, and exporters). Libraries depend only on the API, enabling zero-dependency overhead if telemetry is disabled.",
  ...
  },
  {
    ...
    question: "Which architectural approach or OpenTelemetry configuration satisfies these enterprise observability objectives?",
    options: [
      { id: 'A', text: "Serialize all telemetry as unstructured CSV text files transmitted over FTP." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Translate attribute keys manually using external SQL conversion scripts." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    question: "Which OpenTelemetry component design or collector configuration handles this volume efficiently while protecting downstream storage backends?",
    options: [
      { id: 'A', text: "Allow every development team to invent their own custom attribute names." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Rely on wall-clock synchronized timestamps across servers." },
      { id: 'D', text: "Pass user email addresses in URL query strings as trace identifiers." }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Inject and extract W3C TraceContext headers (`traceparent`, `tracestate`) across HTTP request headers and message metadata boundaries. Distributed tracing requires context propagation across network boundaries. The OpenTelemetry propagators inject and extract W3C TraceContext headers (`traceparent: version-trace_id-parent_id-trace_flags`), linking asynchronous RPC calls into a coherent end-to-end distributed trace graph.",
