export const AWS_SCS_QUESTIONS_5 = [
  {
    ...
    options: [
      { id: 'A', text: "Establish an SSH tunnel between the instances" },
      { id: 'B', text: "Reference the security group ID of the application servers in VPC A (e.g. <code>sg-app12345</code>) as the source in the database security group ingress rule" },
      { id: 'C', text: "Attach a Network ACL rule permitting port 5432 to 0.0.0.0/0" },
      { id: 'D', text: "Specify the entire subnet CIDR block of VPC A (10.1.0.0/16)" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure all rules as stateless rules" },
      ...
      { id: 'D', text: "Disable stateless inspection on the firewall policy" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "<code>\"Condition\": { \"StringEquals\": { \"aws:PrincipalOrgID\": \"o-exampleorg123\" } }</code>" },
      { id: 'C', text: "<code>\"Condition\": { \"StringLike\": { \"s3:prefix\": \"*\" } }</code>" },
      { id: 'D', text: "<code>\"Condition\": { \"IpAddress\": { \"aws:SourceIp\": \"0.0.0.0/0\" } }</code>" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set the action to <code>Block</code> with HTTP 500 error code" },
      { id: 'B', text: "Configure the WAF rule action to <code>Challenge</code> (silent browser token verification) or <code>CAPTCHA</code> (interactive visual puzzle)" },
      { id: 'C', text: "Set the action to <code>Count</code> and ignore the logs" },
      { id: 'D', text: "Attach an Amazon Cognito User Pool to the load balancer" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "<code>stickiness.lb_cookie.duration_seconds</code>" },
      { id: 'C', text: "<code>health_check_interval_seconds</code>" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Create a TLS listener on port 443 with an ACM certificate and attach a security group directly to the Network Load Balancer" },
      { id: 'B', text: "Attach an AWS WAF Web ACL directly to the NLB TCP listener" },
      { id: 'C', text: "Convert the NLB into a Classic Load Balancer" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "In the Network ACL inbound rule configuration" },
      { id: 'C', text: "In the IAM role trust policy attached to the instance" },
      { id: 'D', text: "In the EC2 User Data script by writing parameters to <code>/etc/sysctl.d/99-security.conf</code> and running <code>sysctl -p /etc/sysctl.d/99-security.conf</code>" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Enable S3 Versioning and configure MFA Delete using the AWS CLI with the AWS account root credentials and a physical MFA token" },
      ...
      { id: 'D', text: "Enable S3 Server Access Logging and S3 Event Notifications" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>reject ip any -&gt; any (payload: \"malware_c2_beacon\")</code>" },
      { id: 'B', text: "<code>deny tcp any any -&gt; any any (action: block; string: \"malware_c2_beacon\";)</code>" },
      { id: 'C', text: "<code>drop http $HOME_NET any -&gt; $EXTERNAL_NET any (msg:\"Malware C2 Beacon Detected\"; content:\"malware_c2_beacon\"; http_uri; sid:1000001; rev:1;)</code>" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Spot Instances with placement groups" },
      { id: 'C', text: "Amazon EC2 Dedicated Instances" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Configure a WAF SQL injection match statement that inspects the <code>Single header</code> or <code>Cookie</code> with the specific cookie name <code>session_token</code>" },
      { id: 'C', text: "Inspect the query string for SQL injection patterns" },
      { id: 'D', text: "Attach a rate-based rule on the IP address" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "<code>restricted-common-ports</code>" },
      { id: 'B', text: "<code>s3-bucket-public-read-prohibited</code>" },
      { id: 'C', text: "<code>ec2-instance-managed-by-ssm</code>" },
      { id: 'D', text: "<code>cloudtrail-enabled</code>" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure CloudFront Origin Access Identity (OAI) with an open S3 bucket policy" },
      { id: 'B', text: "Configure Route 53 CNAME records pointing directly to S3" },
      { id: 'C', text: "Store S3 objects with public-read ACLs" },
      { id: 'D', text: "Configure CloudFront Origin Access Control (OAC), and update the S3 bucket policy to allow s3:GetObject only when <code>aws:SourceArn</code> matches the CloudFront distribution ARN" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The certificate must be renewed manually every 13 months via the AWS CLI" },
      { id: 'B', text: "The certificate private key must be exported to an S3 bucket" },
      { id: 'C', text: "The domain registrar must send a signed fax to Amazon Trust Services" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "AWS Systems Manager Run Command using the <code>AWS-RunShellScript</code> document, logging standard output to an encrypted S3 bucket" },
      { id: 'B', text: "AWS CodeDeploy deploying a full application update" },
      { id: 'C', text: "AWS Lambda running inside the default VPC" },
      { id: 'D', text: "SSH executed via bash loops over a public bastion host" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Transitive routing is enabled by default in all VPC peering connections" },
      { id: 'B', text: "Transitive routing occurs only if Network ACLs allow port 80" },
      { id: 'C', text: "VPC B automatically acts as a NAT gateway for VPC A" },
      { id: 'D', text: "AWS VPC Peering strictly forbids transitive routing; traffic cannot hop through an intermediate VPC, so VPC A cannot reach VPC C unless a direct peering connection is established" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "CloudFront Response Headers Policies attached to the distribution's cache behaviors" },
      { id: 'B', text: "Route 53 latency routing policies" },
      { id: 'C', text: "S3 metadata tags on each uploaded static file" },
      { id: 'D', text: "AWS WAF Web ACL request transformation rules" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Attach an AWS WAF Web ACL to the NAT Gateway" },
      { id: 'B', text: "Enable TLS Inspection configuration on the Network Firewall policy, providing an internal CA certificate imported into AWS Certificate Manager (ACM) to generate re-signed certificates for inspected sessions" },
      { id: 'C', text: "Deploy CloudFront in reverse proxy mode inside the private subnet" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Amazon EC2 instances directly and Network Load Balancers" },
      { id: 'B', text: "Amazon S3 buckets and Amazon EFS file systems" },
      { id: 'C', text: "Amazon RDS databases and Amazon ElastiCache clusters" },
      { id: 'D', text: "Application Load Balancers (ALB), Amazon API Gateway REST APIs, AWS AppSync GraphQL APIs, Amazon Cognito User Pools, and AWS App Runner services" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Relying solely on node-level security groups on the EC2 worker instances" },
      { id: 'B', text: "Amazon EKS Security Groups for Pods using the Amazon VPC CNI plugin to attach dedicated Elastic Network Interfaces (ENIs) with distinct security groups directly to individual Kubernetes pods" },
      { id: 'C', text: "Standard Kubernetes ClusterIP services without VPC integration" },
      { id: 'D', text: "Deploying an external squid proxy in a separate VPC" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Change the instance tenancy from Default to Dedicated" },
      { id: 'C', text: "Store the EC2 instance ID in a private S3 bucket" },
      { id: 'D', text: "Remove all security groups from the instance" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "CloudFront caches all POST requests for 24 hours" },
      { id: 'B', text: "The application embeds the AWS WAF JavaScript SDK to obtain an encrypted WAF token (cookie) verified by Challenge or CAPTCHA rules on sensitive mutating endpoints" },
      { id: 'C', text: "The ALB drops all HTTP requests containing cookies" },
      { id: 'D', text: "AWS WAF replaces all user passwords with one-time SMS codes" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Enable S3 bucket default encryption" },
      { id: 'C', text: "Attach an IAM policy denying ec2:RunInstances" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Configure a 0.0.0.0/0 route pointing to blackhole" },
      { id: 'C', text: "Delete the subnet from the VPC" },
      { id: 'D', text: "Add a route 0.0.0.0/0 pointing to an Internet Gateway with a disabled state" }
    ],
    correctAnswers: ['A'],
    type: "single",
