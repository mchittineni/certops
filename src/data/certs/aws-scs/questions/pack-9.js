export const AWS_SCS_QUESTIONS_9 = [
  {
    ...
    options: [
      ...
      { id: 'B', text: "The destination account must copy the shared snapshot into its own account, re-encrypting it with a Customer Managed Key owned by the destination account" },
      { id: 'C', text: "Aurora cluster snapshots cannot be shared across AWS accounts" },
      { id: 'D', text: "The destination account can restore the cluster directly using the source KMS key without copying" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Grant <code>kms:*</code> to both administrators and applications" },
      ...
      { id: 'C', text: "Convert the key to an AWS managed key" },
      { id: 'D', text: "Use IAM permissions boundaries alone without modifying the key policy" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Parameters must be stored in plaintext to be accessed cross-account" },
      { id: 'B', text: "Parameter Store does not support cross-account resource policies directly; member accounts must assume an IAM role in the central configuration account that has permissions to <code>ssm:GetParameter</code> and <code>kms:Decrypt</code>" },
      { id: 'C', text: "Parameter Store supports resource policies natively like Secrets Manager" },
      { id: 'D', text: "Parameters are automatically replicated to all member accounts" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Use an AWS STS temporary session on every mobile phone" },
      { id: 'B', text: "Call the <code>GetPublicKey</code> API once (or distribute the public key PEM file with the app), and use standard crypto libraries to verify the signature locally" },
      { id: 'C', text: "Store the KMS private key in the mobile application binary" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Grant the service principal <code>logs.&lt;region&gt;.amazonaws.com</code> permissions to <code>kms:Encrypt*</code>, <code>kms:Decrypt*</code>, <code>kms:ReEncrypt*</code>, <code>kms:GenerateDataKey*</code>, and <code>kms:Describe*</code> with an <code>arn:aws:logs</code> encryption context condition" },
      { id: 'C', text: "Grant <code>kms:*</code> to the IAM user who created the log group" },
      { id: 'D', text: "Attach an S3 bucket policy to the CloudWatch log group" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "AWS IAM Multi-Factor Authentication (MFA)" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The object is encrypted twice (double encryption)" },
      { id: 'B', text: "The upload uses **SSE-KMS** as specified in the request header; default encryption only applies when an incoming upload does not specify an encryption header" },
      { id: 'C', text: "The upload is rejected with a 400 Bad Request error" },
      { id: 'D', text: "The upload defaults to SSE-S3 and ignores the request header" }
    ],
    correctAnswers: ['B'],
    type: "single",
