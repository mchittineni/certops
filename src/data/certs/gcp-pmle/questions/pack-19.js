export const GCP_PMLE_QUESTIONS_19 = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Deploy the model on an unmanaged Compute Engine instance without load balancing." },
      { id: 'C', text: "Deploy the model specifying minReplicaCount=2, maxReplicaCount=20, and target CPU/GPU utilization." },
      { id: 'D', text: "Configure a static single-node endpoint with minReplicaCount=1 and maxReplicaCount=1." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "Deploy the model specifying minReplicaCount=2, maxReplicaCount=20, and target CPU/GPU utilization. Vertex AI Online Prediction endpoints provide automated horizontal autoscaling. Administrators define `minReplicaCount` to maintain baseline capacity and prevent cold starts, and `maxReplicaCount` to cap operational spending, while the platform scales nodes dynamically based on target utilization.",
  ...
  },
  {
    ...
    question: "Which concept or service configuration satisfies these operational resilience objectives?",
    options: [
      { id: 'A', text: "Deploy 500 Compute Engine instances running an unmanaged prediction loop." },
      { id: 'B', text: "Export the 50 million records to CSV files on a local laptop and run predictions locally." },
      { id: 'C', text: "Write a Python script that iterates over 50 million rows and makes individual HTTP calls to an Online Prediction endpoint." },
  ...
  },
  {
    ...
    question: "Which solution properly implements these mandatory security and governance controls?",
    options: [
      { id: 'A', text: "Schedule an aggressive retraining pipeline that retrains the model every 10 minutes regardless of data drift." },
      { id: 'B', text: "Disable model retraining permanently once a model achieves 90% validation accuracy." },
      { id: 'C', text: "Configure a Cloud Monitoring alert on model drift metrics to invoke a Cloud Function that triggers the Vertex AI Pipeline." },
  ...
  },
  {
    ...
    question: "Which principle or solution enables the enterprise to achieve these cloud migration goals?",
    options: [
      { id: 'A', text: "Deploy every newly retrained model directly to production without comparative evaluation." },
      { id: 'B', text: "Require a full committee vote and manual administrative approval for every automated pipeline execution." },
      { id: 'C', text: "Evaluate the model by manually inspecting the first 5 rows of test predictions." },
  ...
  },
  {
    ...
    question: "Which solution properly implements these mandatory security and governance controls?",
    options: [
      { id: 'A', text: "Open port 80 to the public internet on the prediction endpoint." },
