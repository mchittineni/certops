export const AWS_DOP_QUESTIONS_6 = [
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure an EC2 user data script to grep local log files and reboot the instance upon errors" },
      { id: 'B', text: "Write a Lambda function that downloads all logs from CloudWatch every minute and writes an email" },
      { id: 'C', text: "Stream the entire log group to an on-premises Syslog server over the public internet" },
      { id: 'D', text: "Create a CloudWatch Logs metric filter with a filter pattern of { $.status = 500 } that publishes a custom metric, and attach a CloudWatch metric alarm to that metric" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "AWS Trusted Advisor security check reports emailed weekly to engineering leads" },
      { id: 'B', text: "AWS X-Ray with active tracing enabled on API Gateway and Lambda, using the AWS X-Ray SDK to propagate trace headers and record subsegments" },
      { id: 'C', text: "Amazon CloudWatch Basic Monitoring enabled on the underlying EC2 host instances" },
      { id: 'D', text: "VPC Flow Logs enabled on the primary NAT Gateway network interfaces" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure an SQS queue in front of Amazon SNS to deduplicate incoming alarm messages based on message body hash" },
      { id: 'B', text: "Use CloudWatch Anomaly Detection on all metrics with a threshold band of 5 standard deviations" },
      { id: 'C', text: "Create Amazon CloudWatch Composite Alarms combining underlying metric alarms using boolean logic (AND, OR, NOT) and route only the composite alarm actions to the PagerDuty SNS topic" },
      { id: 'D', text: "Increase the evaluation period of all CloudWatch alarms from 1 minute to 60 minutes" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Create a CloudWatch Logs Subscription Filter targeting Amazon Kinesis Data Firehose, configure a Lambda function for data transformation/masking inside Firehose, and set OpenSearch as the destination with an S3 backup bucket for failed records" },
      { id: 'B', text: "Export the CloudWatch Logs log group to Amazon S3 daily using the CreateExportTask API, and ingest from S3 using Logstash" },
      { id: 'C', text: "Write a scheduled cron job running every minute on an EC2 instance that runs CloudWatch Logs Insights queries and pushes results to OpenSearch" },
      { id: 'D', text: "Configure an EventBridge rule matching CloudWatch Logs events and invoke a synchronous Lambda function to insert documents directly into OpenSearch" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "AWS X-Ray service map filtering by client response time" },
      { id: 'B', text: "CloudWatch Metric Math applying the RATE and SEARCH functions across access logs" },
      { id: 'C', text: "CloudWatch Anomaly Detection configured on the API Gateway 4XXError metric" },
      { id: 'D', text: "CloudWatch Contributor Insights with a rule configured on the API Gateway access logs evaluating top contributors by client IP or API key" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Write metrics to an Amazon SQS queue and deploy a dedicated consumer fleet of EC2 instances to call PutMetricData" },
      { id: 'B', text: "Print structured JSON log events conforming to the CloudWatch Embedded Metric Format (EMF) to stdout; CloudWatch Logs extracts the custom metrics automatically at no extra API cost" },
      { id: 'C', text: "Send metrics over TCP to an Amazon EC2 instance running a StatsD daemon that batches PutMetricData calls" },
      { id: 'D', text: "Call the PutMetricData API synchronously inside the Lambda function for every transaction using the AWS SDK" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Record customer account IDs as X-Ray Annotations (key-value pairs indexed for search), and record debug payloads as X-Ray Metadata (non-indexed key-value pairs)" },
      { id: 'B', text: "Record customer account IDs as X-Ray Metadata and debug payloads as X-Ray Annotations" },
      { id: 'C', text: "Pass both values as HTTP custom headers in the trace header string (X-Amzn-Trace-Id)" },
      { id: 'D', text: "Write both customer account IDs and debug payloads to CloudWatch Logs with custom metric filters" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Write a custom script using Boto3 that assumes IAM roles in all 200 accounts every 5 minutes and pushes metrics into a central account" },
      { id: 'B', text: "Configure Amazon CloudWatch Cross-Account Observability using AWS Observability Access Manager (OAM), defining the central monitoring account as a sink and sharing links from member source accounts across the organization" },
      { id: 'C', text: "Deploy an Amazon OpenSearch Service cluster in each account and establish cross-cluster VPC peering connections" },
      { id: 'D', text: "Deploy an Amazon S3 replication rule that copies log archives to a central S3 bucket and run Athena queries" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy an EC2 cron job that checks top output every 5 seconds and sends an email via Amazon SES" },
      { id: 'B', text: "Set the CloudWatch metric period to 60 seconds with an evaluation count of 1 datapoint" },
      { id: 'C', text: "Enable CloudWatch Detailed Monitoring on the EC2 instances" },
      { id: 'D', text: "Publish custom metrics with StorageResolution set to 1 (high-resolution 1-second metrics) and configure a high-resolution CloudWatch alarm with an evaluation period of 10 or 30 seconds" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy the AWS Distro for OpenTelemetry (ADOT) Collector as a sidecar or DaemonSet in the EKS cluster, configuring the OTLP receiver with X-Ray and Prometheus Remote Write exporters" },
      { id: 'B', text: "Use Amazon CloudWatch Synthetics canaries to probe pod endpoints over localhost" },
      { id: 'C', text: "Configure application pods to write trace files to an Amazon EFS volume shared with a central monitoring server" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set the alarm evaluation period to 7 days (10,080 minutes) to average out weekly cyclicality" },
      { id: 'B', text: "Enable CloudWatch Anomaly Detection on the RequestCount metric and create an alarm that triggers when the metric breaches the predicted threshold band (e.g., lower band breach with threshold 3)" },
      { id: 'C', text: "Configure two static alarms: one evaluated during weekdays and one evaluated during weekends" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy an AWS WAF rule that blocks HTTP requests containing numbers with 9 digits" },
      { id: 'B', text: "Configure a CloudWatch Logs Data Protection Policy on the log group specifying managed data identifiers for SSN and passport numbers with the Deidentify/Mask operation" },
      { id: 'C', text: "Write a custom bash script running on EC2 to grep and redact strings before logs reach stdout" },
      { id: 'D', text: "Export all logs to S3 Glacier Deep Archive and restrict S3 console access" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Deploy a custom Lambda function that inspects incoming HTTP headers and drops traces before sending to the daemon" },
      ...
      { id: 'D', text: "Configure an API Gateway throttle limit of 50 RPS on the checkout resource" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Deploy a Lambda function in the cluster VPC that runs kubectl top nodes every 10 seconds" },
      { id: 'D', text: "Configure worker nodes to send syslog events to an external Nagios server" }
    ],
    correctAnswers: ['B'],
    type: "single",
