export const AWS_DOP_QUESTIONS_4 = [
  {
    ...
    options: [
      { id: 'A', text: "An AWS Lambda function triggered by a weekly EventBridge rule that takes an EBS snapshot of a running bastion host" },
      { id: 'B', text: "AWS EC2 Image Builder configured with an image recipe, CIS benchmark build component, automated test components, and a distribution configuration targeting multiple regions" },
      { id: 'C', text: "An AWS Systems Manager State Manager association executing a bash script that writes an AMI directly to Amazon S3" },
      { id: 'D', text: "An AWS CodeBuild job running a Packer script on a local Docker container that launches an EC2 instance via SSH" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Write an AWS Lambda function that queries the AWS Security Bulletin RSS feed and reboots instances using the EC2 StopInstances API" },
      { id: 'B', text: "Configure AWS Config rules with automated remediation documents that run yum update when instance uptime exceeds 7 days" },
      { id: 'C', text: "Create separate Patch Baselines with auto-approval delays (2 days vs 7 days), create SSM Maintenance Windows scheduled accordingly, and assign instance targets using tag-based filtering (Environment=Production)" },
      { id: 'D', text: "Run AWS Systems Manager Inventory across all instances and execute yum update manually via Session Manager during the weekend" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An Amazon EventBridge rule that catches resource creation events and runs an SSM Automation runbook to add tags" },
      { id: 'B', text: "A CloudFormation dynamic reference pointing to an Amazon DynamoDB table holding required tag definitions" },
      { id: 'C', text: "AWS CloudFormation StackSets with override parameters configured in the account administration settings" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure an AWS::CloudFormation::WaitCondition resource that receives signals from the ALB target group" },
      { id: 'B', text: "Configure a CreationPolicy attribute with ResourceSignal set to count 4 and a timeout of 10 minutes" },
      { id: 'C', text: "Configure an UpdatePolicy attribute with AutoScalingRollingUpdate specifying MaxBatchSize, MinInstancesInService, and WaitOnResourceSignals" },
      { id: 'D', text: "Configure an UpdateReplacePolicy attribute set to Retain on the LaunchTemplate resource" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure DeletionPolicy: Retain on all individual resources in the template" },
      { id: 'B', text: "Attach an IAM permission boundary to all users that denies cloudformation:DescribeStacks" },
      { id: 'C', text: "Set the stack status to ROLLBACK_COMPLETE and lock the template in Amazon S3" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy an SSH bastion script that connects to each instance, runs rpm -qa, and appends output to a local CSV file" },
      { id: 'B', text: "Configure an AWS Lambda function triggered by instance reboot events to inspect the /opt directory via SSM Run Command" },
      { id: 'C', text: "Use Amazon CloudWatch Logs Insights to search for package manager stdout logs across all EC2 system logs" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Execute a CloudFormation Change Set with Replacement set to Conditional on the monolithic template" },
      { id: 'B', text: "Take an RDS snapshot, delete the monolithic stack, and launch the new stack from the snapshot using CloudFormation CLI" },
      ...
      { id: 'D', text: "Use AWS Database Migration Service (AWS DMS) to replicate the database to a newly created RDS instance in the new stack" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Set TimeoutSeconds to 50 and ExecutionLimit to 10 in the SSM Document parameters" },
      { id: 'C', text: "Configure an Amazon SQS FIFO queue with a maximum receive count of 10" },
      { id: 'D', text: "Set MaxConcurrency to 50 and MaxErrors to 10 in the SendCommand API request" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Use '{{resolve:s3:MyBucket/config.json}}' for both parameters using an S3 pre-signed URL" },
      { id: 'B', text: "Use Fn::ImportValue for both values exported from a shared infrastructure stack" },
      { id: 'C', text: "Use '{{resolve:ssm:ApiBaseUrl}}' for the base URL, and '{{resolve:secretsmanager:ApiToken:SecretString:token}}' for the API token" },
      { id: 'D', text: "Use CloudFormation Parameters with Default values hardcoded in a parameter file committed to Git" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Create an Amazon CloudWatch billing alarm that triggers an Amazon SNS topic when EBS storage costs increase" },
      { id: 'B', text: "Schedule a cron job on a management EC2 instance that runs an AWS CLI command to delete all volumes without tags" },
      { id: 'C', text: "Deploy the AWS Config managed rule 'ec2-volume-inuse-check', configure an automatic remediation action pointing to an SSM Automation document that snapshots and deletes the detached volume, and set the remediation parameter to execute when non-compliant" },
      { id: 'D', text: "Configure an Amazon S3 Lifecycle rule to transition EBS volume snapshots to S3 Glacier Deep Archive" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set Rollback on failure to 'Disabled' (or use the --disable-rollback CLI flag) during stack creation" },
      { id: 'B', text: "Add DeletionPolicy: Retain to all resources in the template" },
      { id: 'C', text: "Configure an Amazon SQS Dead-Letter Queue on the CloudFormation service role" },
      { id: 'D', text: "Set TerminationProtection to 'Enabled' on the stack" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy an AWS Config rule that terminates EC2 instances not using the default AWS-managed patch baseline" },
      { id: 'B', text: "Share the custom Patch Baseline from the management or delegated administrator account across the organization using AWS Resource Access Manager (AWS RAM)" },
      { id: 'C', text: "Manually copy-paste the Patch Baseline JSON configuration into each member account's Systems Manager console" },
      { id: 'D', text: "Write a Python script that runs daily in each account to download the patch list from an Amazon S3 bucket" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Execute curl against the instance metadata service (IMDS) and send an SNS notification upon failure" },
      { id: 'B', text: "Run /opt/aws/bin/cfn-init to process the AWS::CloudFormation::Init metadata, followed immediately by /opt/aws/bin/cfn-signal with --exit-code $? passed to the stack WaitCondition or CreationPolicy" },
      { id: 'C', text: "Run /opt/aws/bin/cfn-hup in daemon mode and reboot the instance using shutdown -r now" },
      { id: 'D', text: "Run cfn-get-metadata and parse the JSON output using a custom python script in the user data" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Include the 'Transform: AWS::CodeDeployBlueGreen' declaration in the template and define an AWS::CodeDeploy::BlueGreen hook specifying the ECS service, test listener, and target groups" },
      { id: 'B', text: "Use CloudFormation Fn::Join to merge two different ECS task definitions into a single container definition" },
      { id: 'C', text: "Deploy two distinct CloudFormation stacks and toggle an AWS Route 53 Weighted CNAME record manually" },
      { id: 'D', text: "Configure an EC2 Auto Scaling group with a target tracking scaling policy set to CPUUtilization 50%" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An AWS Systems Manager State Manager association executing a shell script that verifies CIS controls" },
      { id: 'B', text: "AWS Config Conformance Packs deployed at the AWS Organizations level as an Organization Conformance Pack" },
      { id: 'C', text: "Individual AWS Config rules deployed via manual CloudFormation templates in each account" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set the DeletionPolicy attribute on the S3 bucket resource to Delete in the CloudFormation template" },
      { id: 'B', text: "Configure an S3 lifecycle rule to expire current versions after 1 day" },
      { id: 'C', text: "Add a CloudFormation custom resource backed by Lambda that empties all object versions and delete markers from the S3 bucket during the Delete request before the bucket is deleted" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "AWS Systems Manager Quick Setup Host Management" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Attach an IAM permission boundary to developer roles denying rds:ModifyDBCluster" },
      { id: 'B', text: "Apply a CloudFormation Stack Policy to the stack that has an 'Effect: Deny' on 'Update:*' actions targeting the Aurora cluster resource logical ID" },
      { id: 'C', text: "Set UpdateReplacePolicy: Retain on the Aurora DBCluster resource in the template" },
      { id: 'D', text: "Enable Termination Protection on the CloudFormation stack in the console" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Attach an Elastic Load Balancer connection draining timeout set to 60 seconds" },
      { id: 'B', text: "Increase the Auto Scaling scale-in cooldown period to 24 hours" },
      ...
      { id: 'D', text: "Set the EC2 instance shutdown behavior to Stop instead of Terminate" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "EC2 instances running self-managed PostgreSQL with custom rsync scripts over VPN" },
      { id: 'B', text: "Amazon RDS for MySQL with cross-region read replicas using asynchronous binlog replication" },
      { id: 'C', text: "Amazon Aurora Global Database with managed storage-level replication across target regions" },
      { id: 'D', text: "Amazon DynamoDB single-region table with periodic nightly S3 export backups" }
    ],
    correctAnswers: ['C'],
    type: "single",
