export const AWS_DOP_QUESTIONS_4 = [
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "AWS EC2 Image Builder configured with an image recipe, CIS benchmark build component, automated test components, and a distribution configuration targeting multiple regions"
      },
      {
        "id": "B",
        "text": "An AWS CodeBuild job running a Packer script on a local Docker container that launches an EC2 instance via SSH"
      },
      {
        "id": "C",
        "text": "An AWS Lambda function triggered by a weekly EventBridge rule that takes an EBS snapshot of a running bastion host"
      },
      {
        "id": "D",
        "text": "An AWS Systems Manager State Manager association executing a bash script that writes an AMI directly to Amazon S3"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Create separate Patch Baselines with auto-approval delays (2 days vs 7 days), create SSM Maintenance Windows scheduled accordingly, and assign instance targets using tag-based filtering (Environment=Production)"
      },
      {
        "id": "B",
        "text": "Run AWS Systems Manager Inventory across all instances and execute yum update manually via Session Manager during the weekend"
      },
      {
        "id": "C",
        "text": "Write an AWS Lambda function that queries the AWS Security Bulletin RSS feed and reboots instances using the EC2 StopInstances API"
      },
      {
        "id": "D",
        "text": "Configure AWS Config rules with automated remediation documents that run yum update when instance uptime exceeds 7 days"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "AWS CloudFormation Macros backed by an AWS Lambda function that parses the template JSON/YAML and injects the required tags before CloudFormation creates the change set"
      },
      {
        "id": "B",
        "text": "AWS CloudFormation StackSets with override parameters configured in the account administration settings"
      },
      {
        "id": "C",
        "text": "An Amazon EventBridge rule that catches resource creation events and runs an SSM Automation runbook to add tags"
      },
      {
        "id": "D",
        "text": "A CloudFormation dynamic reference pointing to an Amazon DynamoDB table holding required tag definitions"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Configure an UpdatePolicy attribute with AutoScalingRollingUpdate specifying MaxBatchSize, MinInstancesInService, and WaitOnResourceSignals"
      },
      {
        "id": "B",
        "text": "Configure a CreationPolicy attribute with ResourceSignal set to count 4 and a timeout of 10 minutes"
      },
      {
        "id": "C",
        "text": "Configure an UpdateReplacePolicy attribute set to Retain on the LaunchTemplate resource"
      },
      {
        "id": "D",
        "text": "Configure an AWS::CloudFormation::WaitCondition resource that receives signals from the ALB target group"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Enable Termination Protection on the CloudFormation stack"
      },
      {
        "id": "B",
        "text": "Attach an IAM permission boundary to all users that denies cloudformation:DescribeStacks"
      },
      {
        "id": "C",
        "text": "Configure DeletionPolicy: Retain on all individual resources in the template"
      },
      {
        "id": "D",
        "text": "Set the stack status to ROLLBACK_COMPLETE and lock the template in Amazon S3"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Configure AWS Systems Manager Inventory on all managed instances, enable Resource Data Sync to an Amazon S3 bucket in a central account, and query the synchronized inventory data using Amazon Athena"
      },
      {
        "id": "B",
        "text": "Deploy an SSH bastion script that connects to each instance, runs rpm -qa, and appends output to a local CSV file"
      },
      {
        "id": "C",
        "text": "Use Amazon CloudWatch Logs Insights to search for package manager stdout logs across all EC2 system logs"
      },
      {
        "id": "D",
        "text": "Configure an AWS Lambda function triggered by instance reboot events to inspect the /opt directory via SSM Run Command"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Set DeletionPolicy to Retain on the RDS instance, remove it from the monolithic stack template, update the stack so the resource is retained, and then use the CloudFormation 'Import Existing Resources' feature in the new stack"
      },
      {
        "id": "B",
        "text": "Take an RDS snapshot, delete the monolithic stack, and launch the new stack from the snapshot using CloudFormation CLI"
      },
      {
        "id": "C",
        "text": "Use AWS Database Migration Service (AWS DMS) to replicate the database to a newly created RDS instance in the new stack"
      },
      {
        "id": "D",
        "text": "Execute a CloudFormation Change Set with Replacement set to Conditional on the monolithic template"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Set MaxConcurrency to 50 and MaxErrors to 10 in the SendCommand API request"
      },
      {
        "id": "B",
        "text": "Set TimeoutSeconds to 50 and ExecutionLimit to 10 in the SSM Document parameters"
      },
      {
        "id": "C",
        "text": "Configure an Auto Scaling lifecycle hook with a heartbeat timeout of 50 seconds"
      },
      {
        "id": "D",
        "text": "Configure an Amazon SQS FIFO queue with a maximum receive count of 10"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Use '{{resolve:ssm:ApiBaseUrl}}' for the base URL, and '{{resolve:secretsmanager:ApiToken:SecretString:token}}' for the API token"
      },
      {
        "id": "B",
        "text": "Use CloudFormation Parameters with Default values hardcoded in a parameter file committed to Git"
      },
      {
        "id": "C",
        "text": "Use Fn::ImportValue for both values exported from a shared infrastructure stack"
      },
      {
        "id": "D",
        "text": "Use '{{resolve:s3:MyBucket/config.json}}' for both parameters using an S3 pre-signed URL"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Deploy the AWS Config managed rule 'ec2-volume-inuse-check', configure an automatic remediation action pointing to an SSM Automation document that snapshots and deletes the detached volume, and set the remediation parameter to execute when non-compliant"
      },
      {
        "id": "B",
        "text": "Schedule a cron job on a management EC2 instance that runs an AWS CLI command to delete all volumes without tags"
      },
      {
        "id": "C",
        "text": "Create an Amazon CloudWatch billing alarm that triggers an Amazon SNS topic when EBS storage costs increase"
      },
      {
        "id": "D",
        "text": "Configure an Amazon S3 Lifecycle rule to transition EBS volume snapshots to S3 Glacier Deep Archive"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Set Rollback on failure to 'Disabled' (or use the --disable-rollback CLI flag) during stack creation"
      },
      {
        "id": "B",
        "text": "Set TerminationProtection to 'Enabled' on the stack"
      },
      {
        "id": "C",
        "text": "Add DeletionPolicy: Retain to all resources in the template"
      },
      {
        "id": "D",
        "text": "Configure an Amazon SQS Dead-Letter Queue on the CloudFormation service role"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Update the trust relationship of AWSCloudFormationStackSetExecutionRole in account 123456789012 to allow the sts:AssumeRole action from the administrator account's AWSCloudFormationStackSetAdministrationRole ARN"
      },
      {
        "id": "B",
        "text": "Attach an administrator access policy to the target account's root user credentials"
      },
      {
        "id": "C",
        "text": "Add an S3 bucket policy granting s3:GetObject to all IAM roles across the organization"
      },
      {
        "id": "D",
        "text": "Configure the administrator role to use AWS Organizations SCP bypass permissions"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Share the custom Patch Baseline from the management or delegated administrator account across the organization using AWS Resource Access Manager (AWS RAM)"
      },
      {
        "id": "B",
        "text": "Manually copy-paste the Patch Baseline JSON configuration into each member account's Systems Manager console"
      },
      {
        "id": "C",
        "text": "Write a Python script that runs daily in each account to download the patch list from an Amazon S3 bucket"
      },
      {
        "id": "D",
        "text": "Deploy an AWS Config rule that terminates EC2 instances not using the default AWS-managed patch baseline"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Run /opt/aws/bin/cfn-init to process the AWS::CloudFormation::Init metadata, followed immediately by /opt/aws/bin/cfn-signal with --exit-code $? passed to the stack WaitCondition or CreationPolicy"
      },
      {
        "id": "B",
        "text": "Run /opt/aws/bin/cfn-hup in daemon mode and reboot the instance using shutdown -r now"
      },
      {
        "id": "C",
        "text": "Execute curl against the instance metadata service (IMDS) and send an SNS notification upon failure"
      },
      {
        "id": "D",
        "text": "Run cfn-get-metadata and parse the JSON output using a custom python script in the user data"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Include the 'Transform: AWS::CodeDeployBlueGreen' declaration in the template and define an AWS::CodeDeploy::BlueGreen hook specifying the ECS service, test listener, and target groups"
      },
      {
        "id": "B",
        "text": "Use CloudFormation Fn::Join to merge two different ECS task definitions into a single container definition"
      },
      {
        "id": "C",
        "text": "Configure an EC2 Auto Scaling group with a target tracking scaling policy set to CPUUtilization 50%"
      },
      {
        "id": "D",
        "text": "Deploy two distinct CloudFormation stacks and toggle an AWS Route 53 Weighted CNAME record manually"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "AWS Config Conformance Packs deployed at the AWS Organizations level as an Organization Conformance Pack"
      },
      {
        "id": "B",
        "text": "Individual AWS Config rules deployed via manual CloudFormation templates in each account"
      },
      {
        "id": "C",
        "text": "An AWS Systems Manager State Manager association executing a shell script that verifies CIS controls"
      },
      {
        "id": "D",
        "text": "An AWS Organizations Service Control Policy (SCP) blocking all console actions except read-only access"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Upgrade the parameter tier from Standard to Advanced to allow parameter sizes up to 8 KB and support Parameter Policies (such as expiration)"
      },
      {
        "id": "B",
        "text": "Compress the configuration payload using gzip and encode it in Base64 before saving as a Standard parameter"
      },
      {
        "id": "C",
        "text": "Split the configuration into two separate AWS Secrets Manager secrets"
      },
      {
        "id": "D",
        "text": "Store the configuration file in an Amazon S3 bucket and save the S3 pre-signed URL in Parameter Store"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Grant ssm:GetParameters*, ssm:PutParameter on Resource 'arn:aws:ssm:region:account:parameter/app/dev/*' and 'arn:aws:ssm:region:account:parameter/app/test/*'"
      },
      {
        "id": "B",
        "text": "Grant ssm:* on Resource '*' with a Condition checking the principal's IP address"
      },
      {
        "id": "C",
        "text": "Grant ssm:DescribeParameters on all resources and deny ssm:GetParameter in the account SCP"
      },
      {
        "id": "D",
        "text": "Store production parameters in an encrypted S3 bucket and grant developers read-only permissions to S3"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Add a CloudFormation custom resource backed by Lambda that empties all object versions and delete markers from the S3 bucket during the Delete request before the bucket is deleted"
      },
      {
        "id": "B",
        "text": "Set the DeletionPolicy attribute on the S3 bucket resource to Delete in the CloudFormation template"
      },
      {
        "id": "C",
        "text": "Configure an S3 lifecycle rule to expire current versions after 1 day"
      },
      {
        "id": "D",
        "text": "Attach an S3 bucket policy denying s3:PutObject during stack deletion"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "AWS Systems Manager Quick Setup Host Management"
      },
      {
        "id": "B",
      ...
      {
        "id": "D",
        "text": "AWS Systems Manager AppConfig"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Apply a CloudFormation Stack Policy to the stack that has an 'Effect: Deny' on 'Update:*' actions targeting the Aurora cluster resource logical ID"
      },
      {
        "id": "B",
        "text": "Enable Termination Protection on the CloudFormation stack in the console"
      },
      {
        "id": "C",
        "text": "Attach an IAM permission boundary to developer roles denying rds:ModifyDBCluster"
      },
      {
        "id": "D",
        "text": "Set UpdateReplacePolicy: Retain on the Aurora DBCluster resource in the template"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Enable Canary Settings on the API Gateway deployment stage, configure percentage weight to 5.0%, and promote or drop the canary based on CloudWatch metrics"
      },
      {
        "id": "B",
        "text": "Deploy two separate API Gateway APIs and use Route 53 Weighted routing to split traffic"
      },
      {
        "id": "C",
        "text": "Deploy an AWS Application Load Balancer in front of API Gateway with weighted target groups"
      },
      {
        "id": "D",
        "text": "Use an AWS Lambda function URL with alias weighted routing configured"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Increase the Auto Scaling scale-in cooldown period to 24 hours"
      },
      {
        "id": "B",
        "text": "Set the EC2 instance shutdown behavior to Stop instead of Terminate"
      },
      {
        "id": "C",
        "text": "Attach an Elastic Load Balancer connection draining timeout set to 60 seconds"
      },
      {
        "id": "D",
        "text": "Configure an Auto Scaling lifecycle hook for autoscaling:EC2_INSTANCE_TERMINATING, send a heartbeat signal while the job finishes, and complete the lifecycle action when processing completes"
      }
    ],
    "correctAnswers": [
      "D"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Amazon DynamoDB single-region table with periodic nightly S3 export backups"
      },
      {
        "id": "B",
        "text": "Amazon Aurora Global Database with managed storage-level replication across target regions"
      },
      {
        "id": "C",
        "text": "EC2 instances running self-managed PostgreSQL with custom rsync scripts over VPN"
      },
      {
        "id": "D",
        "text": "Amazon RDS for MySQL with cross-region read replicas using asynchronous binlog replication"
      }
    ],
    "correctAnswers": [
      "B"
