export const AWS_DOP_QUESTIONS_10 = [
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Attach an AWS Organizations Tag Policy with enforcement enabled ('enforced_for') to the organization root or target OUs"
      },
      {
        "id": "B",
        "text": "Deploy an AWS Lambda function in each account that runs a daily tagging audit"
      },
      {
        "id": "C",
        "text": "Create an Amazon CloudWatch billing alarm that alerts on untagged resources"
      },
      {
        "id": "D",
        "text": "Instruct engineers to follow a wiki guide when creating resources"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Enable Amazon S3 Block Public Access at the AWS Account level, enabling all four settings (BlockPublicAcls, IgnorePublicAcls, BlockPublicPolicy, RestrictPublicBuckets)"
      },
      {
        "id": "B",
        "text": "Remove all IAM users from the AWS account"
      },
      {
        "id": "C",
        "text": "Configure a VPC Endpoint for S3 and delete the VPC Internet Gateway"
      },
      {
        "id": "D",
        "text": "Deploy an AWS WAF Web ACL on all S3 buckets"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "IAM Access Analyzer Unused Access Analyzer"
      },
      {
        "id": "B",
        "text": "IAM Access Analyzer External Access Analyzer"
      },
      {
        "id": "C",
      ...
      {
        "id": "D",
        "text": "Amazon GuardDuty IAM findings"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Create an AWS Organizations Organization Trail in the management (or delegated administrator) account that logs to an S3 bucket in the Log Archive account encrypted with a Customer Managed Key (CMK)"
      },
      {
        "id": "B",
        "text": "Create separate regional trails manually in each member account console"
      },
      {
        "id": "C",
        "text": "Configure an EC2 syslog daemon to pull CloudTrail logs via the AWS API"
      },
      {
        "id": "D",
        "text": "Export CloudWatch Logs to an S3 bucket in each account weekly"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Deploy the AWS Config managed rule 'restricted-ssh' with an automatic remediation action that executes the SSM Automation document 'AWS-DisablePublicAccessForSecurityGroup'"
      },
      {
        "id": "B",
        "text": "Configure an IAM policy that denies the ec2:CreateSecurityGroup action"
      },
      {
        "id": "C",
        "text": "Deploy an Amazon Inspector assessment targeting EC2 instances"
      },
      {
        "id": "D",
        "text": "Create an AWS WAF rule that filters port 22 traffic"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Create a custom domain name in API Gateway, enable Mutual TLS, and upload the truststore PEM file (containing CA root and intermediate certificates) to an Amazon S3 bucket referenced by API Gateway"
      },
      {
        "id": "B",
        "text": "Configure an API Gateway Lambda authorizer to inspect the client IP address"
      },
      {
        "id": "C",
        "text": "Deploy an Application Load Balancer in front of API Gateway with basic authentication headers"
      },
      {
        "id": "D",
        "text": "Use Amazon Cognito user pools with SMS multi-factor authentication"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Ensure the ECR repository is encrypted with a KMS CMK, configure the KMS key policy to grant 'kms:Decrypt' to the AWS Lambda service principal (or execution role), and configure an ECR repository policy granting 'ecr:BatchGetImage' and 'ecr:GetDownloadUrlForLayer' to Lambda"
      },
      {
        "id": "B",
        "text": "Store the container image in a public Docker Hub repository"
      },
      {
        "id": "C",
        "text": "Disable KMS encryption on the Amazon ECR repository"
      },
      {
        "id": "D",
        "text": "Grant the Lambda function full AdministratorAccess IAM permissions"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Attach a VPC Endpoint Policy to the S3 Gateway Endpoint that specifies an 'Allow' on 's3:*' only for resources matching 'arn:aws:s3:::company-*' or with a Condition on 'aws:ResourceAccount' equal to '111122223333'"
      },
      {
        "id": "B",
        "text": "Modify the EC2 instance security group outbound rules"
      },
      {
        "id": "C",
        "text": "Disable VPC Endpoint and route traffic through a NAT Gateway"
      },
      {
        "id": "D",
        "text": "Enable S3 Transfer Acceleration on internal buckets"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "AWS KMS Multi-Region Keys (MRKs)"
      },
      {
        "id": "B",
        "text": "AWS CloudHSM classic clusters"
      },
      {
        "id": "C",
        "text": "Exporting private keys to an S3 bucket with Cross-Region Replication"
      },
      {
        "id": "D",
        "text": "Using default AWS-managed keys in each region"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "A policy statement with 'Effect: Deny', 'Principal: *', 'Action: s3:*', and a Condition 'Bool: { \"aws:SecureTransport\": \"false\" }'"
      },
      {
        "id": "B",
        "text": "A policy statement with 'Effect: Allow' and a Condition 'StringEquals: { \"aws:Protocol\": \"https\" }'"
      },
      {
        "id": "C",
        "text": "Enabling default SSE-KMS encryption on the bucket properties"
      },
      {
        "id": "D",
        "text": "Configuring an Amazon CloudFront distribution with Origin Access Control"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Secrets Manager uses staging labels: AWSPENDING (new secret being tested), AWSCURRENT (active secret), and AWSPREVIOUS (previous secret retained for rollback), allowing the rotation Lambda function to manage both tokens during rotation"
      },
      {
        "id": "B",
        "text": "Secrets Manager creates two separate secrets with different ARNs"
      },
      {
        "id": "C",
        "text": "Secrets Manager pauses all application microservices for 1 hour during rotation"
      },
      {
        "id": "D",
        "text": "Developers must manually update application config files in Git"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Amazon Inspector (v2) using the AWS Systems Manager Agent (SSM Agent)"
      },
      {
        "id": "B",
        "text": "Amazon GuardDuty VPC Flow Log analysis"
      },
      {
        "id": "C",
        "text": "AWS Config managed rules evaluated periodically"
      },
      {
        "id": "D",
        "text": "AWS Security Hub CIS benchmark checks"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "AWS KMS enforces a mandatory waiting period of 7 to 30 days for any key deletion request (ScheduleKeyDeletion), during which the key enters the PendingDeletion state and can be cancelled"
      },
      {
        "id": "B",
        "text": "KMS keys can be deleted immediately if the administrator passes the --force flag in the AWS CLI"
      },
      {
        "id": "C",
        "text": "KMS keys are automatically recreated by CloudFormation if deleted"
      },
      {
        "id": "D",
        "text": "KMS requires physical authorization via an on-premises HSM smart card"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Apply an IAM Service Control Policy (SCP) denying 'ec2:RunInstances' unless 'ec2:MetadataHttpTokens' is set to 'required', and update existing instances using the modify-instance-metadata-options CLI command with HttpTokens set to required"
      },
      {
        "id": "B",
        "text": "Block port 80 outbound in all VPC security groups"
      },
      {
        "id": "C",
        "text": "Disable the EC2 Instance Metadata Service entirely on all instances"
      },
      {
        "id": "D",
        "text": "Deploy an AWS WAF rule that blocks HTTP requests to 169.254.169.254"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "AWS CloudTrail Lake with an event data store configured to retain events for up to 7 years"
      },
      {
        "id": "B",
        "text": "CloudTrail standard Event History in the console"
      },
      {
        "id": "C",
        "text": "CloudWatch Logs Insights with default retention"
      },
      {
        "id": "D",
        "text": "Amazon DynamoDB with automated TTL"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Pass an inline IAM Session Policy when calling STS AssumeRole (or AssumeRoleWithWebIdentity) that scopes down the session to only Tenant A's S3 path"
      },
      {
        "id": "B",
        "text": "Create thousands of distinct IAM roles, one for each tenant"
      },
      {
        "id": "C",
        "text": "Modify the S3 bucket policy synchronously before each job execution"
      },
      {
        "id": "D",
        "text": "Encrypt each tenant's folder with a different client-side password"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Configure AWS WAF to send logs to an Amazon Kinesis Data Firehose delivery stream named with the required 'aws-waf-logs-' prefix, pointing to the centralized Amazon S3 bucket with dynamic partitioning enabled"
      },
      {
        "id": "B",
        "text": "Write a Python script on an EC2 instance that queries the AWS WAF API every 5 minutes"
      },
      {
        "id": "C",
        "text": "Stream logs directly to Amazon DynamoDB using an EventBridge rule"
      },
      {
        "id": "D",
        "text": "Download WAF logs manually from the CloudWatch Metrics console"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "An EC2 Image Builder Distribution Configuration specifying the target regions, target AWS account IDs (or AWS Organizations ARN), and AMI launch permissions"
      },
      {
        "id": "B",
        "text": "An AWS Lambda function that copies snapshots manually using the ec2:CopySnapshot API"
      },
      {
        "id": "C",
        "text": "An S3 bucket policy with public read access to raw VMDK files"
      },
      {
        "id": "D",
        "text": "An AWS CloudFormation StackSet deployed to every account"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "AWS IAM Access Analyzer (External Access Analyzer)"
      },
      {
        "id": "B",
        "text": "AWS Trusted Advisor Performance check"
      },
      {
        "id": "C",
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Associate the KMS Key ARN with the log group using the 'aws logs associate-kms-key' CLI command, ensuring the KMS key policy grants the regional CloudWatch Logs service principal permissions to generate data keys and decrypt"
      },
      {
        "id": "B",
        "text": "CloudWatch Logs only supports AWS-managed encryption keys"
      },
      {
        "id": "C",
        "text": "Re-create all log groups with SSE-S3 encryption enabled"
      },
      {
        "id": "D",
        "text": "Deploy an EC2 instance to encrypt log files locally before sending to CloudWatch"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Deploy the AWS Config managed rule 'iam-password-policy' with an automated remediation action executing the SSM Automation document 'AWS-UpdateAccountPasswordPolicy'"
      },
      {
        "id": "B",
        "text": "Attach an IAM permission boundary to all IAM users"
      },
      {
        "id": "C",
        "text": "Deploy an AWS Lambda function that deletes users without passwords"
      },
      {
        "id": "D",
        "text": "Enable MFA on the AWS root user account"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Use the Secrets Manager 'Multi-User Rotation' strategy with two database users: one user rotates and updates its password in Secrets Manager while the alternate user services active client connections, swapping roles alternately on each rotation"
      },
      {
        "id": "B",
        "text": "Use the Single-User Rotation strategy with a 10-minute database reboot"
      },
      {
        "id": "C",
        "text": "Hardcode credentials in application memory and disable rotation"
      },
      {
        "id": "D",
        "text": "Deploy an AWS WAF rule that buffers SQL requests during rotation"
      }
    ],
    "correctAnswers": [
      "A"
