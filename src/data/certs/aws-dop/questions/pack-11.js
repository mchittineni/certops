export const AWS_DOP_QUESTIONS_11 = [
  {
    ...
    options: [
      ...
      { id: 'D', text: "Encrypt the S3 artifact bucket with an AWS KMS Customer Managed Key (CMK) granting decrypt permissions to workload account roles, and configure CodePipeline stages to assume deployment roles in target accounts." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Use default aws/s3 KMS encryption and grant public read access to the artifact bucket." },
      { id: 'D', text: "Copy artifacts manually via SCP across VPC peering connections." }
    ],
    correctAnswers: ['B'],
    type: "single",
    explanation: "Encrypt the S3 artifact bucket with an AWS KMS Customer Managed Key (CMK) granting decrypt permissions to workload account roles, and configure CodePipeline stages to assume deployment roles in target accounts. Cross-account CodePipeline deployments require a KMS Customer Managed Key (CMK) because the default AWS-managed `aws/s3` key cannot be shared across AWS accounts. The pipeline assumes a role in each target account with permissions to read from S3 and decrypt artifacts using the CMK.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy independent standalone pipelines in each target account pulling from unauthenticated Git repositories." },
      { id: 'B', text: "Copy artifacts manually via SCP across VPC peering connections." },
      { id: 'C', text: "Use default aws/s3 KMS encryption and grant public read access to the artifact bucket." },
      { id: 'D', text: "Encrypt the S3 artifact bucket with an AWS KMS Customer Managed Key (CMK) granting decrypt permissions to workload account roles, and configure CodePipeline stages to assume deployment roles in target accounts." }
    ],
    correctAnswers: ['D'],
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Perform an in-place rolling update replacing all container tasks simultaneously." },
      { id: 'D', text: "Configure an ECS Blue/Green deployment using AWS CodeDeploy with a canary traffic-shifting routing preference (e.g., CodeDeployDefault.ECSCanary10Percent5Minutes) and CloudWatch alarm rollbacks." }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Configure an ECS Blue/Green deployment using AWS CodeDeploy with a canary traffic-shifting routing preference (e.g., CodeDeployDefault.ECSCanary10Percent5Minutes) and CloudWatch alarm rollbacks. AWS CodeDeploy Blue/Green deployments for Amazon ECS utilize Application Load Balancer target groups to route traffic. Canary (e.g., 10% for 5 minutes) or linear shifting gradually shifts traffic, monitoring CloudWatch alarms to trigger instantaneous automated rollbacks if errors spike.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Deploy a secondary duplicate ECS cluster and update public Route 53 DNS records manually." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Terminate 15 instances manually and launch 15 new instances." },
      { id: 'C', text: "Create a custom CodeDeploy deployment configuration specifying `MinimumHealthyHosts` with type `FLEET_PERCENT` set to `85`." },
      { id: 'D', text: "Use `CodeDeployDefault.AllAtOnce` deployment configuration." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Create a custom CodeDeploy deployment configuration specifying `MinimumHealthyHosts` with type `FLEET_PERCENT` set to `85`." },
      { id: 'D', text: "Use `CodeDeployDefault.OneAtATime` which guarantees only 50% capacity." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "Create a custom CodeDeploy deployment configuration specifying `MinimumHealthyHosts` with type `FLEET_PERCENT` set to `85`. AWS CodeDeploy allows authoring custom deployment configurations where `MinimumHealthyHosts` can be specified as a percentage or absolute count. Setting `FLEET_PERCENT: 85` guarantees CodeDeploy upgrades instances in batches that never drop active serving capacity below 85%.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Use `CodeDeployDefault.AllAtOnce` deployment configuration." },
      { id: 'B', text: "Use `CodeDeployDefault.OneAtATime` which guarantees only 50% capacity." },
      { id: 'C', text: "Create a custom CodeDeploy deployment configuration specifying `MinimumHealthyHosts` with type `FLEET_PERCENT` set to `85`." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Download all dependencies over public Git repositories sequentially on every build." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Enable CodeBuild local caching for Docker layers and source dependencies, or configure an Amazon S3 build cache in `buildspec.yml`." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Store dependencies on ephemeral EC2 instance store disks without cache configuration." }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Enable CodeBuild local caching for Docker layers and source dependencies, or configure an Amazon S3 build cache in `buildspec.yml`. AWS CodeBuild supports local caching (modes: `LOCAL_DOCKER_LAYER_CACHE`, `LOCAL_SOURCE_CACHE`, `LOCAL_CUSTOM_CACHE`) on the build host and remote Amazon S3 caching. Enabling Docker layer caching avoids rebuilding unchanged intermediate image layers, drastically cutting build duration.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Insert a Manual Approval stage in CodePipeline configured with an Amazon SNS notification topic, restricting approval permissions via IAM policies." }
    ],
    correctAnswers: ['D'],
    type: "single",
