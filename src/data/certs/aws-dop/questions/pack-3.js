export const AWS_DOP_QUESTIONS_3 = [
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "AWS CloudFormation StackSets with service-managed permissions and auto-deployment enabled for target Organizational Units (OUs)" },
      { id: 'B', text: "Nested stacks configured with cross-region export and import values" },
      { id: 'C', text: "A single global CloudFormation template deployed only in the management account" },
      { id: 'D', text: "A shell script executing aws cloudformation create-stack sequentially over an SSH loop" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "DeletionPolicy: Retain" },
      { id: 'B', text: "Metadata: DoNotDelete" },
      { id: 'C', text: "UpdatePolicy: Ignore" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Amazon Inspector scheduled network reachability scans" },
      { id: 'B', text: "AWS Systems Manager Run Command executed manually after an outage" },
      { id: 'C', text: "AWS Systems Manager State Manager associations applied on a defined schedule using instance tags" },
      { id: 'D', text: "AWS CloudTrail alerts triggering an SNS notification to administrators" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Amazon S3 bucket with versioning enabled" },
      { id: 'B', text: "AWS KMS Customer Managed Keys" },
      { id: 'C', text: "AWS Systems Manager Parameter Store Standard tier" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy a custom Lambda function triggered by AWS Organizations account creation events to invoke CreateStackInstances API calls manually" },
      ...
      { id: 'C', text: "Configure an EventBridge rule matching CreateAccount that triggers an AWS Systems Manager Automation document to run CloudFormation CLI commands" },
      { id: 'D', text: "Use self-managed permissions with manually provisioned AWSCloudFormationStackSetAdministrationRole and execution roles in each member account" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy an Amazon EventBridge rule that detects CloudFormation CreateStack API calls in AWS CloudTrail and invokes a Lambda function to cancel the stack" },
      { id: 'B', text: "Create a pre-commit Git hook that executes a local bash script to parse CloudFormation JSON/YAML files on developer laptops" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Increase the Lambda function timeout from 3 minutes to 15 minutes to allow sufficient time for CloudFormation to time out the stack" },
      { id: 'B', text: "Wrap the entire Lambda handler logic in a comprehensive try/finally block that sends a FAILED response with the original RequestId, StackId, and PhysicalResourceId to the pre-signed S3 response URL" },
      { id: 'C', text: "Configure an Amazon SQS dead-letter queue (DLQ) on the Lambda function to store failed events and trigger an automated stack rollback" },
      { id: 'D', text: "Configure the custom resource in the CloudFormation template with an explicit DeletionPolicy set to Retain" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure an ASG Lifecycle Hook for instance launch and terminate, use SSM Run Command to patch instances in batches using an SSM Maintenance Window with MaxConcurrency set to 20% and MaxErrors set to 0" },
      { id: 'B', text: "Reboot all EC2 instances using an Amazon EventBridge scheduled rule that invokes an AWS Systems Manager Run Command targeting the instances' AWS tags" },
      ...
      { id: 'D', text: "Create an SSM Maintenance Window that runs the AWS-RunPatchBaseline SSM document targeting all 50 instances simultaneously with MaxConcurrency set to 100%" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Pass the credentials as sensitive CloudFormation input parameters with the NoEcho property set to true in the template parameters section" },
      ...
      { id: 'C', text: "Store the database credentials in an Amazon S3 bucket encrypted with SSE-S3 and fetch them using an AWS Lambda backed custom resource" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Attach an IAM permission boundary to all CloudFormation execution roles denying the rds:DeleteDBInstance action" },
      ...
      { id: 'D', text: "Add a DependsOn attribute linking the RDS instance to an Amazon S3 bucket holding daily backups" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy an AWS Lambda function with concurrency set to 20 that triggers SSH commands on each target EC2 instance" },
      ...
      { id: 'C', text: "Configure an AWS OpsWorks for Puppet Enterprise master server to run an ad-hoc puppet agent task across all nodes" },
      { id: 'D', text: "Write a multi-threaded Python script using Boto3 on a local bastion host to iterate through each account sequentially" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A CloudFormation action with ActionMode set to CHANGE_SET_REPLACE, followed by a Manual Approval action, followed by a CloudFormation action with ActionMode set to CHANGE_SET_EXECUTE" },
      { id: 'B', text: "A CloudFormation action with ActionMode set to CREATE_UPDATE, followed immediately by an S3 put action" },
      { id: 'C', text: "A CodeBuild action running cfn-lint, followed by a CloudFormation action with ActionMode set to DELETE_ONLY" },
      { id: 'D', text: "A Step Functions action running a custom Python script that invokes the AWS CLI cloudformation create-stack command" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "AWS Config Conformance Packs deployed with automated SSM remediation documents targeting S3 PutBucketEncryption" },
      { id: 'C', text: "AWS Config Proactive Rules evaluated using the CloudFormation Guard or AWS Config SDK in the CI/CD pipeline before stack execution" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Enable VPC Flow Logs on the instance subnet and configure a CloudWatch metric filter for port 22 traffic" },
      { id: 'B', text: "Configure Session Manager preferences in the AWS Systems Manager console to enable CloudWatch logging and KMS encryption specifying the CMK ARN, and ensure the EC2 instance profile has kms:GenerateDataKey and kms:Decrypt permissions" },
      { id: 'C', text: "Attach an IAM policy allowing the ssm:StartSession action with a Condition key requiring an active VPN tunnel" },
      { id: 'D', text: "Install an OpenSSH server on EC2 instances and write a cron job that uploads /var/log/secure to an S3 bucket every 5 minutes" }
    ],
    correctAnswers: ['B'],
    type: "single",
