export const AWS_DOP_QUESTIONS_3 = [
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Hardcode the certificate values in the template parameters and update them manually before deletion"
      },
      {
        "id": "B",
        "text": "Write an EC2 user data script that calls the third-party API and stores the certificate in a local text file"
      },
      {
        "id": "C",
        "text": "Create an AWS::CloudFormation::CustomResource backed by an AWS Lambda function that handles Create, Update, and Delete event types and responds with status and physical resource IDs to the pre-signed S3 response URL"
      },
      {
        "id": "D",
        "text": "Use CloudFormation Fn::ImportValue to directly query the external third-party REST API"
      }
    ],
    "correctAnswers": [
      "C"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "AWS CloudFormation StackSets with service-managed permissions and auto-deployment enabled for target Organizational Units (OUs)"
      },
      {
        "id": "B",
        "text": "A shell script executing aws cloudformation create-stack sequentially over an SSH loop"
      },
      {
        "id": "C",
        "text": "Nested stacks configured with cross-region export and import values"
      },
      {
        "id": "D",
        "text": "A single global CloudFormation template deployed only in the management account"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "UpdatePolicy: Ignore"
      },
      {
        "id": "B",
        "text": "Condition: IsProduction"
      },
      {
        "id": "C",
        "text": "Metadata: DoNotDelete"
      },
      {
        "id": "D",
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "AWS Systems Manager State Manager associations applied on a defined schedule using instance tags"
      },
      {
        "id": "B",
        "text": "AWS CloudTrail alerts triggering an SNS notification to administrators"
      },
      {
        "id": "C",
        "text": "Amazon Inspector scheduled network reachability scans"
      },
      {
        "id": "D",
        "text": "AWS Systems Manager Run Command executed manually after an outage"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "id": "B",
        "text": "Amazon S3 bucket with versioning enabled"
      },
      {
        "id": "C",
        "text": "AWS Secrets Manager with built-in Lambda rotation functions"
      },
      {
        "id": "D",
        "text": "AWS KMS Customer Managed Keys"
      }
    ],
    "correctAnswers": [
      "C"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Use self-managed permissions with manually provisioned AWSCloudFormationStackSetAdministrationRole and execution roles in each member account"
      },
      {
        "id": "B",
        "text": "Use service-managed permissions with trusted access enabled between AWS Organizations and CloudFormation, and enable automatic deployments with retain stacks set to false"
      },
      {
        "id": "C",
        "text": "Deploy a custom Lambda function triggered by AWS Organizations account creation events to invoke CreateStackInstances API calls manually"
      },
      {
        "id": "D",
        "text": "Configure an EventBridge rule matching CreateAccount that triggers an AWS Systems Manager Automation document to run CloudFormation CLI commands"
      }
    ],
    "correctAnswers": [
      "B"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Implement an AWS Config rule with an automated remediation action that terminates non-compliant resources within 5 minutes of creation"
      },
      {
        "id": "B",
        "text": "Develop and publish an AWS CloudFormation Hook with a target filter for S3 buckets and EBS volumes configured with failure mode set to FAIL"
      },
      {
        "id": "C",
        "text": "Create a pre-commit Git hook that executes a local bash script to parse CloudFormation JSON/YAML files on developer laptops"
      },
      {
        "id": "D",
        "text": "Deploy an Amazon EventBridge rule that detects CloudFormation CreateStack API calls in AWS CloudTrail and invokes a Lambda function to cancel the stack"
      }
    ],
    "correctAnswers": [
      "B"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Wrap the entire Lambda handler logic in a comprehensive try/finally block that sends a FAILED response with the original RequestId, StackId, and PhysicalResourceId to the pre-signed S3 response URL"
      },
      {
        "id": "B",
        "text": "Increase the Lambda function timeout from 3 minutes to 15 minutes to allow sufficient time for CloudFormation to time out the stack"
      },
      {
        "id": "C",
        "text": "Configure an Amazon SQS dead-letter queue (DLQ) on the Lambda function to store failed events and trigger an automated stack rollback"
      },
      {
        "id": "D",
        "text": "Configure the custom resource in the CloudFormation template with an explicit DeletionPolicy set to Retain"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Move the security group ingress/egress rules into standalone AWS::EC2::SecurityGroupIngress and AWS::EC2::SecurityGroupEgress resources defined in a separate dedicated stack or Stack B"
      },
      {
        "id": "B",
        "text": "Combine both stacks into a single nested stack that imports values using Fn::GetAtt across parent and child templates"
      },
      {
        "id": "C",
        "text": "Configure AWS Systems Manager Parameter Store dynamic references to retrieve the security group IDs at deploy time instead of using CloudFormation exports"
      },
      {
        "id": "D",
        "text": "Disable rollback on both CloudFormation stacks and run the stack updates concurrently using AWS CLI"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Execute CloudFormation Drift Detection on the stack, inspect the Drift Details in the console or CLI, update the CloudFormation template to match the drifted live state, and execute a Change Set"
      },
      {
        "id": "B",
        "text": "Delete the drifted resources manually in the console and run a CloudFormation UpdateStack operation with rollback disabled"
      },
      {
        "id": "C",
        "text": "Run CloudFormation CancelUpdateStack and execute an Import Existing Resources operation on all modified resources"
      },
      {
        "id": "D",
        "text": "Terminate the CloudFormation stack with Retain on stateful resources and recreate the entire stack from scratch"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Create an SSM Maintenance Window that runs the AWS-RunPatchBaseline SSM document targeting all 50 instances simultaneously with MaxConcurrency set to 100%"
      },
      {
        "id": "B",
        "text": "Configure an ASG Lifecycle Hook for instance launch and terminate, use SSM Run Command to patch instances in batches using an SSM Maintenance Window with MaxConcurrency set to 20% and MaxErrors set to 0"
      },
      {
        "id": "C",
        "text": "Reboot all EC2 instances using an Amazon EventBridge scheduled rule that invokes an AWS Systems Manager Run Command targeting the instances' AWS tags"
      },
      {
        "id": "D",
        "text": "Deploy an AWS Lambda function that disables the ALB target group, runs yum update on all instances via SSH, and re-enables the target group"
      }
    ],
    "correctAnswers": [
      "B"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Use CloudFormation dynamic references with the 'resolve:secretsmanager' and 'resolve:ssm-secure' pattern in the resource properties"
      },
      {
        "id": "B",
        "text": "Store the database credentials in an Amazon S3 bucket encrypted with SSE-S3 and fetch them using an AWS Lambda backed custom resource"
      },
      {
        "id": "C",
        "text": "Pass the credentials as sensitive CloudFormation input parameters with the NoEcho property set to true in the template parameters section"
      },
      {
        "id": "D",
        "text": "Hardcode an encrypted KMS ciphertext blob in the template and decrypt it inside the user data script using the AWS CLI"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "AWS Systems Manager State Manager association configured with an SSM document targeting instances by tag on a 30-minute schedule"
      },
      {
        "id": "B",
        "text": "AWS Systems Manager Run Command triggered manually by security operators during scheduled weekly audits"
      },
      {
        "id": "C",
        "text": "AWS Systems Manager Session Manager with port forwarding configured to inspect daemon status over SSH"
      },
      {
        "id": "D",
        "text": "AWS Systems Manager Parameter Store configured with parameter change notification EventBridge rules"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Configure DeletionPolicy: Snapshot and UpdateReplacePolicy: Snapshot on the stateful resources in the CloudFormation template"
      },
      {
        "id": "B",
        "text": "Attach an IAM permission boundary to all CloudFormation execution roles denying the rds:DeleteDBInstance action"
      },
      {
        "id": "C",
        "text": "Enable TerminationProtection: true on the CloudFormation stack settings in the AWS Management Console"
      },
      {
        "id": "D",
        "text": "Add a DependsOn attribute linking the RDS instance to an Amazon S3 bucket holding daily backups"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Publish shared configuration values and ARNs to AWS Systems Manager Parameter Store (or AWS AppConfig), and have consuming stacks reference them using CloudFormation dynamic references"
      },
      {
        "id": "B",
        "text": "Consolidate all microservices and networking components into a monolithic single-template CloudFormation stack"
      },
      {
        "id": "C",
        "text": "Create a nested stack hierarchy with a single root template that imports all child outputs via Fn::GetAtt"
      },
      {
        "id": "D",
        "text": "Delete the importing stacks temporarily, update the core networking exports, and redeploy the importing stacks"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Use AWS Systems Manager Automation with multi-account and multi-region execution, specifying an administration role, MaxConcurrency set to 20, and MaxErrors set to 5"
      },
      {
        "id": "B",
        "text": "Write a multi-threaded Python script using Boto3 on a local bastion host to iterate through each account sequentially"
      },
      {
        "id": "C",
        "text": "Deploy an AWS Lambda function with concurrency set to 20 that triggers SSH commands on each target EC2 instance"
      },
      {
        "id": "D",
        "text": "Configure an AWS OpsWorks for Puppet Enterprise master server to run an ad-hoc puppet agent task across all nodes"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "A CloudFormation action with ActionMode set to CREATE_UPDATE, followed immediately by an S3 put action"
      },
      {
        "id": "B",
        "text": "A CloudFormation action with ActionMode set to CHANGE_SET_REPLACE, followed by a Manual Approval action, followed by a CloudFormation action with ActionMode set to CHANGE_SET_EXECUTE"
      },
      {
        "id": "C",
        "text": "A CodeBuild action running cfn-lint, followed by a CloudFormation action with ActionMode set to DELETE_ONLY"
      },
      {
        "id": "D",
        "text": "A Step Functions action running a custom Python script that invokes the AWS CLI cloudformation create-stack command"
      }
    ],
    "correctAnswers": [
      "B"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "AWS Config Proactive Rules evaluated using the CloudFormation Guard or AWS Config SDK in the CI/CD pipeline before stack execution"
      },
      {
        "id": "B",
        "text": "AWS Config Detective Rules running on a periodic 24-hour evaluation schedule across member accounts"
      },
      {
        "id": "C",
        "text": "AWS Config Conformance Packs deployed with automated SSM remediation documents targeting S3 PutBucketEncryption"
      },
      {
        "id": "D",
        "text": "An AWS Organizations Service Control Policy (SCP) denying the s3:CreateBucket API call unless an MFA token is present"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Configure Session Manager preferences in the AWS Systems Manager console to enable CloudWatch logging and KMS encryption specifying the CMK ARN, and ensure the EC2 instance profile has kms:GenerateDataKey and kms:Decrypt permissions"
      },
      {
        "id": "B",
        "text": "Install an OpenSSH server on EC2 instances and write a cron job that uploads /var/log/secure to an S3 bucket every 5 minutes"
      },
      {
        "id": "C",
        "text": "Enable VPC Flow Logs on the instance subnet and configure a CloudWatch metric filter for port 22 traffic"
      },
      {
        "id": "D",
        "text": "Attach an IAM policy allowing the ssm:StartSession action with a Condition key requiring an active VPN tunnel"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Store root and child templates in an Amazon S3 bucket, and reference the child template URLs using the AWS::CloudFormation::Stack resource in the root template"
      },
      {
        "id": "B",
        "text": "Use the AWS::CloudFormation::WaitCondition resource to poll individual stacks deployed sequentially from developer workstations"
      },
      {
        "id": "C",
        "text": "Write a monolithic CloudFormation template exceeding 1 MB and compress it using GZIP before uploading to the AWS CLI"
      },
      {
        "id": "D",
        "text": "Use CloudFormation Exports exclusively and trigger cross-account CloudFormation updates via an Amazon SNS topic"
      }
    ],
    "correctAnswers": [
      "A"
