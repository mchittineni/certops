export const AWS_DOP_QUESTIONS_9 = [
  {
    ...
    options: [
      { id: 'A', text: "Enable S3 Replication Time Control (S3 RTC) and configure CloudWatch alarms on the ReplicationLatency and OperationsFailedReplication metrics" },
      { id: 'B', text: "Write a Python script that compares S3 bucket inventory files daily using Athena" },
      { id: 'C', text: "Enable S3 Transfer Acceleration on both buckets" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An AWS CloudFormation template applied manually in each member account console" },
      { id: 'B', text: "An AWS Organizations SCP denying instance launches without tags" },
      { id: 'C', text: "AWS Systems Manager Quick Setup deployed at the AWS Organizations management level targeting all member accounts" },
      { id: 'D', text: "A manual bash script executed via SSH on each instance" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Amazon EventBridge rules with cron expressions" },
      { id: 'B', text: "AWS Lambda with a 15-minute execution limit" },
      { id: 'C', text: "AWS Step Functions Standard Workflows" },
      { id: 'D', text: "Amazon SQS with message delay timers" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "AWS Systems Manager State Manager" },
      { id: 'B', text: "AWS Systems Manager Maintenance Windows" },
      { id: 'C', text: "AWS Systems Manager OpsCenter" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A scheduled cron job on an EC2 instance that executes the ec2:CreateSnapshot API" },
      ...
      { id: 'D', text: "AWS CloudFormation custom resources that recreate volumes daily" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "AWS Trusted Advisor Service Limits check" },
      { id: 'B', text: "Amazon Macie data discovery" },
      { id: 'C', text: "AWS CloudTrail Event History with standard filters" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "KMS Customer Managed Keys only support symmetrical encryption via AWS CLI root credentials" },
      { id: 'B', text: "KMS keys cannot be accessed by IAM roles within the same AWS account" },
      { id: 'C', text: "The IAM role must be assigned an Elastic IP address to authenticate with KMS" },
      { id: 'D', text: "The KMS key policy does not contain a statement delegating access to the account root or specifically authorizing the IAM role" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Rotate passwords manually during quarterly maintenance downtime windows" },
      { id: 'B', text: "Write an on-premises batch script that changes passwords and emails credentials to developers" },
      ...
      { id: 'D', text: "Store passwords in clear text in an S3 bucket with versioning and object lock enabled" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An SCP with an explicit 'Allow' statement that lists only 'eu-west-1' and 'eu-central-1' in the Action element" },
      { id: 'B', text: "An IAM permission boundary applied to individual IAM users in member accounts" },
      ...
      { id: 'D', text: "An AWS Config rule that automatically terminates instances launched outside of the approved European regions" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Write a Python script on an on-premises server that connects over public internet to change the password" },
      { id: 'B', text: "Store database credentials in plaintext in an Amazon S3 bucket with versioning enabled" },
      { id: 'C', text: "Deploy the rotation Lambda function in the same VPC and private subnets as the Aurora database, configure a Secrets Manager VPC Interface Endpoint (PrivateLink) with appropriate security group rules, and configure Secrets Manager automatic rotation" },
      { id: 'D', text: "Attach an Internet Gateway to the database private subnets to allow the rotation Lambda to reach AWS endpoints" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure an AWS Config rule that triggers an alert when a role with AdministratorAccess is created" },
      { id: 'B', text: "Use Service Control Policies (SCPs) to deny IAM role creation across all member accounts" },
      { id: 'C', text: "Attach an IAM Permission Boundary to developer identities requiring that any 'iam:CreateRole' or 'iam:PutRolePolicy' call must specify the same permission boundary ARN" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure the CloudFront cache behavior with 'Viewer Protocol Policy: Redirect HTTP to HTTPS', set the CloudFront Security Policy to TLSv1.2_2021, and configure the ALB HTTPS listener with a modern predefined SSL Security Policy (such as ELBSecurityPolicy-TLS13-1-2-2021-06)" },
      { id: 'B', text: "Deploy an AWS WAF rule that blocks HTTP requests with a 403 status code" },
      { id: 'C', text: "Enable Route 53 DNSSEC on the domain hosted zone" },
      { id: 'D', text: "Disable port 80 in the VPC security group and configure a self-signed certificate on the EC2 instances" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "AWS Systems Manager Run Command executing OpenSCAP scripts" },
      { id: 'B', text: "AWS Trusted Advisor Cost Optimization dashboard" },
      { id: 'C', text: "Amazon CloudWatch Billing Alarms" },
      { id: 'D', text: "AWS Security Hub with automated security standards enabled across all member accounts" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Generate temporary AWS credentials using AWS CloudShell and paste them into GitHub before each build" },
      { id: 'B', text: "Deploy an EC2 instance in a public subnet to act as a self-hosted GitHub Actions runner" },
      { id: 'C', text: "Rotate the static IAM access keys manually every 7 days in the GitHub secrets settings" },
      { id: 'D', text: "Configure an IAM OpenID Connect (OIDC) identity provider for GitHub in AWS, and create an IAM role with a trust policy allowing GitHub Actions to assume the role using the sts:AssumeRoleWithWebIdentity action with repository-specific Subject (sub) conditions" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Use Email Validation and configure an automated email parsing Lambda function" },
      { id: 'C', text: "Write a script that calls the acm:RequestCertificate API every 90 days" },
      { id: 'D', text: "Purchase certificates from a third-party CA and import them into ACM annually" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Deploy the policies to a production account and check CloudWatch alarms" },
      { id: 'C', text: "Execute 'aws cloudtrail lookup-events' to verify IAM API calls" },
      { id: 'D', text: "Run 'aws accessanalyzer validate-policy' in the CI build step to check policies for syntax errors, security warnings, and general recommendations" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Enable S3 Object Lock in Governance Mode with a retention period of 7 years" },
      { id: 'B', text: "Enable S3 Versioning and S3 Object Lock in Compliance Mode with a retention period of 7 years on the bucket" },
      { id: 'C', text: "Enable S3 Server-Side Encryption with Customer-Provided Keys (SSE-C)" },
      { id: 'D', text: "Configure an S3 Bucket Policy with an explicit Deny on s3:DeleteObject" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Enable AWS Config managed rules for EC2 instances" },
      { id: 'B', text: "Configure Amazon Inspector on EC2 worker nodes" },
      { id: 'C', text: "Enable Amazon GuardDuty with EKS Audit Log Monitoring and GuardDuty EKS Runtime Monitoring" },
      { id: 'D', text: "Deploy an open-source Falco DaemonSet that writes logs to a local file" }
    ],
    correctAnswers: ['C'],
    type: "single",
