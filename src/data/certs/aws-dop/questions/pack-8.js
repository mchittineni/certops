export const AWS_DOP_QUESTIONS_8 = [
  {
    ...
    options: [
      ...
      { id: 'B', text: "An AWS Lambda function in the degraded region modifying security group rules to deny traffic" },
      { id: 'C', text: "An AWS CloudFormation StackSets update that decreases desired capacity in the failing region to 0" },
      { id: 'D', text: "Route 53 standard DNS records updated via a Python script calling the ChangeResourceRecordSets API during an incident" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "AWS Systems Manager Incident Manager with configured Response Plans, Engagement Plans, Escalation Plans, and AWS Chatbot integration" },
      { id: 'B', text: "Amazon EventBridge rules triggering an AWS Step Functions state machine that sends text messages via Amazon Pinpoint" },
      { id: 'C', text: "AWS Trusted Advisor automated notifications sent via Amazon SES" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "AWS Fault Injection Simulator (FIS) configured with experiment templates, target filters, and CloudWatch alarms defined as Stop Conditions" },
      { id: 'B', text: "An AWS Lambda function that deletes Auto Scaling instances using the AWS CLI" },
      { id: 'C', text: "AWS Systems Manager Run Command executing stress-ng across all nodes simultaneously" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure a daily AWS Config rule that scans IAM credential reports and flags exposed keys" },
      { id: 'B', text: "Write a script on an administrative workstation that polls GuardDuty findings every 15 minutes" },
      { id: 'C', text: "Instruct the security engineer to manually log into the IAM console and click 'Revoke active sessions' upon receiving an email" },
      { id: 'D', text: "Create an Amazon EventBridge rule matching the specific GuardDuty finding type, targeting an AWS Systems Manager Automation document (or AWS Lambda function) that revokes active IAM sessions and attaches an inline Deny policy, while publishing an alert to an Amazon SNS topic" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy an Amazon SQS queue in each member account and configure cross-account VPC peering to the central account" },
      { id: 'B', text: "Grant the member accounts permission to put events onto the central account's event bus using an EventBridge resource policy in the central account, and create EventBridge rules in member accounts targeting the central bus ARN" },
      { id: 'C', text: "Configure AWS CloudTrail in each member account to publish to an Amazon SNS topic in the central account" },
      { id: 'D', text: "Write an AWS Lambda function in each member account that writes events to an Amazon S3 bucket in the central account" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "A Systems Manager Run Command that executes 'rm -rf /' inside the running guest OS" },
      { id: 'D', text: "An AWS Config rule that restarts the instance in single-user mode" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Use SQS Dead-Letter Queue Redrive (StartMessageMoveTask API or console 'Start DLQ redrive') to move messages back to the source queue" },
      { id: 'D', text: "Delete the DLQ and let the primary queue regenerate the lost messages" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "An AWS Systems Manager State Manager association running a bash script every 6 hours" },
      { id: 'C', text: "An Amazon Inspector assessment running weekly against EC2 instances" },
      { id: 'D', text: "An AWS Config periodic rule scheduled to run once every 24 hours" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Create an Amazon EventBridge rule matching AWS Health events for 'AWS_EC2_INSTANCE_RETIREMENT_SCHEDULED', targeting an AWS Step Functions state machine or SSM Automation runbook that waits until the scheduled maintenance window and executes ec2:StopInstances followed by ec2:StartInstances" },
      { id: 'C', text: "Create a CloudWatch billing alarm that alerts the billing department when retirement occurs" },
      { id: 'D', text: "Configure an Auto Scaling lifecycle hook with a 48-hour timeout" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Increase the Lambda function timeout from 1 minute to 15 minutes" },
      ...
      { id: 'D', text: "Enable CloudTrail data events on the EventBridge rule" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Use AWS Fault Injection Simulator (FIS) with the 'aws:rds:failover-db-cluster' action targeting the Aurora cluster, monitoring CloudWatch metrics for downtime and connection recovery" },
      { id: 'B', text: "Delete the primary DB instance using the AWS CLI and wait for Auto Scaling to launch a replacement" },
      { id: 'C', text: "Block port 3306 on the database security group for 10 minutes" },
      { id: 'D', text: "Manually reboot the primary database instance via the console using the 'Reboot with failover' option while timing with a stopwatch" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Write an intermediate AWS Lambda function between EventBridge and Amazon SNS solely to format strings" },
      { id: 'B', text: "Modify the EC2 instance operating system syslog format" },
      { id: 'C', text: "Configure Amazon SES email templates with custom Handlebars logic" },
      { id: 'D', text: "Use an EventBridge Input Transformer with Input Path to extract event variables and Input Template to format the human-readable message string" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Reboot the instance repeatedly to clear OS buffer caches" },
      { id: 'B', text: "Delete the target group from the Application Load Balancer" },
      { id: 'C', text: "Stop the instance using the EC2 console; Auto Scaling will pause until the instance is started" },
      { id: 'D', text: "Put the instance into the 'Standby' state using the EnterStandby API, inspect the instance, and return it using ExitStandby when complete" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Use AWS CloudTrail Insights to detect the execution and pause the runbook" },
      { id: 'B', text: "Add a sleep step for 30 minutes in the bash script to give managers time to inspect the database" },
      { id: 'C', text: "Configure an Amazon SNS topic subscribed to the manager's phone number" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Write a regex string directly in the EventBridge event bus properties" },
      { id: 'D', text: "Configure an S3 lifecycle policy that invokes an AWS Lambda function for files over 1 GB" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy the AWS Config managed rule 's3-bucket-level-public-access-prohibited' with an automatic remediation pointing to the SSM Automation document 'AWS-DisableS3BucketPublicReadWrite' (or 'AWS-ConfigureS3BucketPublicAccessBlock')" },
      { id: 'B', text: "Write a Python script running in an EC2 cron job every 24 hours" },
      { id: 'C', text: "Create an IAM policy denying the s3:ListBucket action to all developers" },
      { id: 'D', text: "Deploy an Amazon CloudWatch billing alarm for S3 transfer costs" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "AWS Chatbot configured with Slack workspace integration and an IAM role defining permissions for channel members" },
      { id: 'B', text: "An open-source webhook running on a public EC2 instance listening for Slack slash commands" },
      { id: 'C', text: "Amazon Pinpoint two-way SMS integration" },
      { id: 'D', text: "Amazon SES email forwarding to a Slack email integration address" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Use the FIS action 'aws:network:disrupt-connectivity' (or container-level network latency injection actions)" },
      { id: 'B', text: "Change the MTU size on the VPC Internet Gateway to 500 bytes" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Rely on CloudWatch Logs default retention without configuring lifecycle hooks" },
      { id: 'D', text: "Use the EC2 StopInstances API to pause the instance indefinitely" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "AWS Trusted Advisor Cost Optimization recommendations" },
      { id: 'B', text: "AWS Cost Explorer anomaly detection reports" },
      { id: 'C', text: "Incident Manager Post-Incident Analysis (PIR) with automated timeline generation, root cause documentation, and AWS Systems Manager OpsCenter action item integration" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Deploy an AWS Global Accelerator with endpoint weights set to 50/50" },
      { id: 'C', text: "Configure a Route 53 Simple Routing Policy with two IP addresses" },
      { id: 'D', text: "Write a Python script that modifies DNS records when an on-call engineer receives an email" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy an Amazon DynamoDB table to store event definitions" },
      { id: 'B', text: "Use AWS Glue Data Catalog to crawl Amazon SQS queues" },
      { id: 'C', text: "Manually write JSON Schema documents and upload them to an Amazon S3 bucket" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Create a CloudWatch alarm on CPUUtilization and attach the EC2 action 'Reboot this instance'" },
      { id: 'D', text: "Deploy an AWS Lambda function that calls ec2:TerminateInstances and ec2:RunInstances" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Increase the Kinesis shard retention period from 24 hours to 7 days" },
      { id: 'B', text: "Add an Amazon SQS queue in front of every consumer" },
      { id: 'C', text: "Enable Enhanced Fan-Out (EFO) on the consumers to provide dedicated 2 MB/sec throughput per consumer per shard using HTTP/2" },
      { id: 'D', text: "Replace Kinesis Data Streams with Amazon DynamoDB Streams" }
    ],
    correctAnswers: ['C'],
    type: "single",
