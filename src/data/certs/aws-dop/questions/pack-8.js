export const AWS_DOP_QUESTIONS_8 = [
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Amazon Route 53 Application Recovery Controller (ARC) Routing Controls backed by Readiness Checks and regional health endpoints"
      },
      {
        "id": "B",
        "text": "Route 53 standard DNS records updated via a Python script calling the ChangeResourceRecordSets API during an incident"
      },
      {
        "id": "C",
        "text": "An AWS Lambda function in the degraded region modifying security group rules to deny traffic"
      },
      {
        "id": "D",
        "text": "An AWS CloudFormation StackSets update that decreases desired capacity in the failing region to 0"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "AWS Systems Manager Incident Manager with configured Response Plans, Engagement Plans, Escalation Plans, and AWS Chatbot integration"
      },
      {
        "id": "B",
        "text": "Amazon SNS topics subscribed to a distribution email list"
      },
      {
        "id": "C",
        "text": "AWS Trusted Advisor automated notifications sent via Amazon SES"
      },
      {
        "id": "D",
        "text": "Amazon EventBridge rules triggering an AWS Step Functions state machine that sends text messages via Amazon Pinpoint"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "AWS Fault Injection Simulator (FIS) configured with experiment templates, target filters, and CloudWatch alarms defined as Stop Conditions"
      },
      {
        "id": "B",
        "text": "A bash script running in an EC2 cron job that executes 'kill -9' on random processes"
      },
      {
        "id": "C",
        "text": "AWS Systems Manager Run Command executing stress-ng across all nodes simultaneously"
      },
      {
        "id": "D",
        "text": "An AWS Lambda function that deletes Auto Scaling instances using the AWS CLI"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Create an Amazon EventBridge rule matching the specific GuardDuty finding type, targeting an AWS Systems Manager Automation document (or AWS Lambda function) that revokes active IAM sessions and attaches an inline Deny policy, while publishing an alert to an Amazon SNS topic"
      },
      {
        "id": "B",
        "text": "Configure a daily AWS Config rule that scans IAM credential reports and flags exposed keys"
      },
      {
        "id": "C",
        "text": "Write a script on an administrative workstation that polls GuardDuty findings every 15 minutes"
      },
      {
        "id": "D",
        "text": "Instruct the security engineer to manually log into the IAM console and click 'Revoke active sessions' upon receiving an email"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Grant the member accounts permission to put events onto the central account's event bus using an EventBridge resource policy in the central account, and create EventBridge rules in member accounts targeting the central bus ARN"
      },
      {
        "id": "B",
        "text": "Deploy an Amazon SQS queue in each member account and configure cross-account VPC peering to the central account"
      },
      {
        "id": "C",
        "text": "Write an AWS Lambda function in each member account that writes events to an Amazon S3 bucket in the central account"
      },
      {
        "id": "D",
        "text": "Configure AWS CloudTrail in each member account to publish to an Amazon SNS topic in the central account"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "An EventBridge rule triggers an SSM Automation runbook that replaces the instance's security groups with an isolation security group having no ingress or egress rules, takes EBS snapshots of all attached volumes, and tags the instance as 'Quarantined'"
      },
      {
        "id": "B",
        "text": "An EventBridge rule triggers an AWS Lambda function that immediately calls ec2:TerminateInstances"
      },
      {
        "id": "C",
        "text": "An AWS Config rule that restarts the instance in single-user mode"
      },
      {
        "id": "D",
        "text": "A Systems Manager Run Command that executes 'rm -rf /' inside the running guest OS"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Use SQS Dead-Letter Queue Redrive (StartMessageMoveTask API or console 'Start DLQ redrive') to move messages back to the source queue"
      },
      {
        "id": "B",
        "text": "Write a Python script on an EC2 instance that reads messages from the DLQ and puts them into an S3 bucket"
      },
      {
        "id": "C",
        "text": "Delete the DLQ and let the primary queue regenerate the lost messages"
      },
      {
        "id": "D",
        "text": "Configure an AWS Lambda function with concurrency set to 1,000 to read from the DLQ and send HTTP POST requests"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "An Amazon EventBridge rule matching AWS CloudTrail API calls for AuthorizeSecurityGroupIngress, triggering an AWS Lambda function that evaluates the CIDR block and calls RevokeSecurityGroupIngress if 0.0.0.0/0 is present"
      },
      {
        "id": "B",
        "text": "An AWS Config periodic rule scheduled to run once every 24 hours"
      },
      {
        "id": "C",
        "text": "An Amazon Inspector assessment running weekly against EC2 instances"
      },
      {
        "id": "D",
        "text": "An AWS Systems Manager State Manager association running a bash script every 6 hours"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Create an Amazon EventBridge rule matching AWS Health events for 'AWS_EC2_INSTANCE_RETIREMENT_SCHEDULED', targeting an AWS Step Functions state machine or SSM Automation runbook that waits until the scheduled maintenance window and executes ec2:StopInstances followed by ec2:StartInstances"
      },
      {
        "id": "B",
        "text": "Wait for the scheduled retirement date and let AWS terminate the instance automatically"
      },
      {
        "id": "C",
        "text": "Configure an Auto Scaling lifecycle hook with a 48-hour timeout"
      },
      {
        "id": "D",
        "text": "Create a CloudWatch billing alarm that alerts the billing department when retirement occurs"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Configure a Dead-Letter Queue (Amazon SQS) on the EventBridge rule target and set an appropriate retry policy with maximum event age and retry attempts"
      },
      {
        "id": "B",
        "text": "Increase the Lambda function timeout from 1 minute to 15 minutes"
      },
      {
        "id": "C",
        "text": "Enable CloudTrail data events on the EventBridge rule"
      },
      {
        "id": "D",
        "text": "Deploy a secondary EventBridge rule with an identical pattern pointing to an Amazon SNS topic"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Use AWS Fault Injection Simulator (FIS) with the 'aws:rds:failover-db-cluster' action targeting the Aurora cluster, monitoring CloudWatch metrics for downtime and connection recovery"
      },
      {
        "id": "B",
        "text": "Manually reboot the primary database instance via the console using the 'Reboot with failover' option while timing with a stopwatch"
      },
      {
        "id": "C",
        "text": "Delete the primary DB instance using the AWS CLI and wait for Auto Scaling to launch a replacement"
      },
      {
        "id": "D",
        "text": "Block port 3306 on the database security group for 10 minutes"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Use an EventBridge Input Transformer with Input Path to extract event variables and Input Template to format the human-readable message string"
      },
      {
        "id": "B",
        "text": "Write an intermediate AWS Lambda function between EventBridge and Amazon SNS solely to format strings"
      },
      {
        "id": "C",
        "text": "Configure Amazon SES email templates with custom Handlebars logic"
      },
      {
        "id": "D",
        "text": "Modify the EC2 instance operating system syslog format"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Put the instance into the 'Standby' state using the EnterStandby API, inspect the instance, and return it using ExitStandby when complete"
      },
      {
        "id": "B",
        "text": "Stop the instance using the EC2 console; Auto Scaling will pause until the instance is started"
      },
      {
        "id": "C",
        "text": "Delete the target group from the Application Load Balancer"
      },
      {
        "id": "D",
        "text": "Reboot the instance repeatedly to clear OS buffer caches"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Add an 'aws:approve' step in the Automation document specifying the Approvers (IAM users or roles) and MinRequiredApprovals"
      },
      {
        "id": "B",
        "text": "Add a sleep step for 30 minutes in the bash script to give managers time to inspect the database"
      },
      {
        "id": "C",
        "text": "Configure an Amazon SNS topic subscribed to the manager's phone number"
      },
      {
        "id": "D",
        "text": "Use AWS CloudTrail Insights to detect the execution and pause the runbook"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Use content-based filtering with 'suffix' matching on object key and numeric range matching 'numeric': ['>', 1073741824] on object size in the event pattern"
      },
      {
        "id": "B",
        "text": "Write a regex string directly in the EventBridge event bus properties"
      },
      {
        "id": "C",
        "text": "Configure an S3 lifecycle policy that invokes an AWS Lambda function for files over 1 GB"
      },
      {
        "id": "D",
        "text": "Use an Amazon SQS message filter policy matching S3 metadata headers"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Deploy the AWS Config managed rule 's3-bucket-level-public-access-prohibited' with an automatic remediation pointing to the SSM Automation document 'AWS-DisableS3BucketPublicReadWrite' (or 'AWS-ConfigureS3BucketPublicAccessBlock')"
      },
      {
        "id": "B",
        "text": "Create an IAM policy denying the s3:ListBucket action to all developers"
      },
      {
        "id": "C",
        "text": "Deploy an Amazon CloudWatch billing alarm for S3 transfer costs"
      },
      {
        "id": "D",
        "text": "Write a Python script running in an EC2 cron job every 24 hours"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "AWS Chatbot configured with Slack workspace integration and an IAM role defining permissions for channel members"
      },
      {
        "id": "B",
        "text": "An open-source webhook running on a public EC2 instance listening for Slack slash commands"
      },
      {
        "id": "C",
        "text": "Amazon SES email forwarding to a Slack email integration address"
      },
      {
        "id": "D",
        "text": "Amazon Pinpoint two-way SMS integration"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "RDS Multi-AZ updates the standby instance first, performs an automatic failover to the updated standby, and then updates the original primary; to roll back if issues arise, initiate a reboot with failover back to the previous primary if not yet upgraded or restore from the pre-maintenance snapshot"
      },
      {
        "id": "B",
        "text": "RDS instances cannot be rolled back; the database must be completely re-imported from CSV files"
      },
      {
        "id": "C",
        "text": "Disable Multi-AZ before running any maintenance updates"
      },
      {
        "id": "D",
        "text": "CloudFormation automatically rolls back RDS database updates without snapshot restoration"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Use the FIS action 'aws:network:disrupt-connectivity' (or container-level network latency injection actions)"
      },
      {
        "id": "B",
        "text": "Modify the VPC route table to route traffic through an unconfigured NAT instance"
      },
      {
        "id": "C",
        "text": "Change the MTU size on the VPC Internet Gateway to 500 bytes"
      },
      {
        "id": "D",
        "text": "Deploy an AWS WAF rule that drops 10% of incoming requests"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Configure an Auto Scaling Lifecycle Hook for EC2_INSTANCE_TERMINATING; an EventBridge rule catches the lifecycle action event and triggers an SSM Automation runbook that runs a script to collect logs, upload to S3, and complete the lifecycle action"
      },
      {
        "id": "B",
        "text": "Run a cron job that uploads logs to S3 every 24 hours"
      },
      {
        "id": "C",
        "text": "Use the EC2 StopInstances API to pause the instance indefinitely"
      },
      {
        "id": "D",
        "text": "Rely on CloudWatch Logs default retention without configuring lifecycle hooks"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Incident Manager Post-Incident Analysis (PIR) with automated timeline generation, root cause documentation, and AWS Systems Manager OpsCenter action item integration"
      },
      {
        "id": "B",
        "text": "AWS Trusted Advisor Cost Optimization recommendations"
      },
      {
        "id": "C",
        "text": "Amazon CloudWatch ServiceLens dependency maps"
      },
      {
        "id": "D",
        "text": "AWS Cost Explorer anomaly detection reports"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Configure a Route 53 Failover Routing Policy with a Primary record pointing to the primary region's ALB associated with a Route 53 Health Check, and a Secondary record pointing to the disaster recovery region's ALB"
      },
      {
        "id": "B",
        "text": "Configure a Route 53 Simple Routing Policy with two IP addresses"
      },
      {
        "id": "C",
        "text": "Write a Python script that modifies DNS records when an on-call engineer receives an email"
      },
      {
        "id": "D",
        "text": "Deploy an AWS Global Accelerator with endpoint weights set to 50/50"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Enable EventBridge Schema Discovery on the event bus, and download code bindings directly from the EventBridge Schema Registry"
      },
      {
        "id": "B",
        "text": "Manually write JSON Schema documents and upload them to an Amazon S3 bucket"
      },
      {
        "id": "C",
        "text": "Use AWS Glue Data Catalog to crawl Amazon SQS queues"
      },
      {
        "id": "D",
        "text": "Deploy an Amazon DynamoDB table to store event definitions"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Create a CloudWatch alarm on the 'StatusCheckFailed_System' metric and attach the EC2 action 'Recover this instance'"
      },
      {
        "id": "B",
        "text": "Create a CloudWatch alarm on CPUUtilization and attach the EC2 action 'Reboot this instance'"
      },
      {
        "id": "C",
        "text": "Deploy an AWS Lambda function that calls ec2:TerminateInstances and ec2:RunInstances"
      },
      {
        "id": "D",
        "text": "Configure an Auto Scaling group with Minimum=1, Maximum=1, and Desired=1"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Enable Enhanced Fan-Out (EFO) on the consumers to provide dedicated 2 MB/sec throughput per consumer per shard using HTTP/2"
      },
      {
        "id": "B",
        "text": "Increase the Kinesis shard retention period from 24 hours to 7 days"
      },
      {
        "id": "C",
        "text": "Add an Amazon SQS queue in front of every consumer"
      },
      {
        "id": "D",
        "text": "Replace Kinesis Data Streams with Amazon DynamoDB Streams"
      }
    ],
    "correctAnswers": [
      "A"
