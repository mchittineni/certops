export const AWS_DOP_QUESTIONS_7 = [
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "CloudWatch Logs Live Tail"
      },
      {
        "id": "B",
        "text": "CloudWatch Logs Insights query in batch mode"
      },
      {
        "id": "C",
        "text": "CloudWatch Metric Filters"
      },
      {
        "id": "D",
        "text": "Amazon S3 Select on exported log files"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Use CloudWatch Metric Math to calculate '(m1 / m2) * 100' (where m1 is HTTP 5xx count and m2 is total request count) and set the alarm threshold to 1.0"
      },
      {
        "id": "B",
        "text": "Create an SQS dead-letter queue and monitor the ApproximateNumberOfMessagesVisible metric"
      },
      {
        "id": "C",
        "text": "Configure an ALB listener rule to redirect 5xx responses to an error tracking Lambda function"
      },
      {
        "id": "D",
        "text": "Deploy an AWS WAF rate-based rule set to 100 requests per 5 minutes"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Use the AWS X-Ray SDK to create custom subsegments around each downstream HTTP and database call, passing the trace context into the worker threads"
      },
      {
        "id": "B",
        "text": "Increase the Lambda memory size to 10 GB to allow X-Ray to sample faster"
      },
      {
        "id": "C",
        "text": "Print the start and end timestamp of each call to stdout using standard logging"
      },
      {
        "id": "D",
        "text": "Attach an IAM policy with xray:PutTraceSegments to the Lambda execution role"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Use CloudWatch Logs Insights and execute a query parsing the query duration, aggregating with avg(duration) as avg_dur by query, sorting by avg_dur desc, and limiting to 10"
      },
      {
        "id": "B",
        "text": "Download all log files to an EC2 instance and run a Python script with pandas"
      },
      {
        "id": "C",
        "text": "Stream logs to Amazon Kinesis and write an Apache Flink application in Kinesis Data Analytics"
      },
      {
        "id": "D",
        "text": "Create a CloudWatch Metric Filter that extracts duration into a custom metric"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Enable CloudWatch Lambda Insights on the Lambda functions via the Lambda Insights extension layer"
      },
      {
        "id": "B",
        "text": "Configure AWS CloudTrail data events on Lambda invoke calls"
      },
      {
        "id": "C",
        "text": "Deploy an EC2 instance running Prometheus to ping Lambda functions continuously"
      },
      {
        "id": "D",
        "text": "Write custom Python logging code to measure time elapsed between function entry and exit"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Create a metric filter counting occurrences of 'AGENT_STATUS: OK', and configure the CloudWatch alarm with TreatMissingData set to 'breaching' and a threshold < 1 over two 5-minute evaluation periods"
      },
      {
        "id": "B",
        "text": "Configure the alarm with TreatMissingData set to 'ignore' and threshold >= 1"
      },
      {
        "id": "C",
        "text": "Configure an Amazon EventBridge scheduled rule that checks the S3 bucket size every 5 minutes"
      },
      {
        "id": "D",
        "text": "Create a metric filter that assigns a default value of 1 and set TreatMissingData to 'notBreaching'"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "CloudWatch Synthetics GUI Workflow Canaries running Node.js Puppeteer or Python Selenium scripts on scheduled intervals"
      },
      {
        "id": "B",
        "text": "Route 53 DNS health checks querying the homepage HTTP status code"
      },
      {
        "id": "C",
        "text": "AWS WAF Bot Control rules configured in count mode"
      },
      {
        "id": "D",
        "text": "CloudWatch Metric Filters searching for HTTP 200 in access logs"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Create a CloudWatch Logs Subscription Filter on the /var/log/messages log group matching the exploit pattern, targeting an AWS Lambda function that modifies the instance security group to an isolation security group"
      },
      {
        "id": "B",
        "text": "Configure Amazon Athena to run an hourly query on CloudWatch log exports in S3 and invoke Lambda via SNS"
      },
      {
        "id": "C",
        "text": "Use an AWS Systems Manager State Manager association scheduled to run every 2 hours"
      },
      {
        "id": "D",
        "text": "Configure an AWS Config rule evaluated on 24-hour configuration changes"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Execute a query in CloudWatch Logs Insights on the VPC Flow Log group: 'filter dstPort = 22 and action = \"REJECT\" | stats count(*) as rejections by srcAddr | sort rejections desc'"
      },
      {
        "id": "B",
        "text": "Download all VPC Flow Log gzip archives from S3 and write a grep script on a local workstation"
      },
      {
        "id": "C",
        "text": "Enable AWS GuardDuty and wait for an SSH brute-force finding to appear in Security Hub"
      },
      {
        "id": "D",
        "text": "Configure an Amazon EventBridge rule matching VPC Flow Log creation events"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Store the CloudWatch Agent JSON configuration file in AWS Systems Manager Parameter Store, and use the AmazonCloudWatch-ManageAgent SSM document to apply the configuration across instances"
      },
      {
        "id": "B",
        "text": "Hardcode the configuration file inside the EC2 user data script and reboot all instances"
      },
      {
        "id": "C",
        "text": "Upload the configuration to an S3 bucket and run a cron job on instances that downloads it daily"
      },
      {
        "id": "D",
        "text": "Embed the configuration in an Amazon Machine Image (AMI) and deploy an ASG instance refresh"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Amazon CloudWatch ServiceLens"
      },
      {
        "id": "B",
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Set the CloudWatch Logs retention period to 30 days on log groups, and stream logs via a Subscription Filter to Kinesis Data Firehose which archives them to an Amazon S3 bucket with S3 Lifecycle rules transitioning objects to S3 Glacier Deep Archive after 30 days"
      },
      {
        "id": "B",
        "text": "Set CloudWatch Logs retention to Never Expire and request an AWS enterprise discount"
      },
      {
        "id": "C",
        "text": "Write a monthly cron job that uses the AWS CLI to export logs to EBS volumes attached to a management instance"
      },
      {
        "id": "D",
        "text": "Delete log groups every 30 days and instruct developers to write logs to local /tmp directories"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Amazon CloudWatch Application Signals"
      },
      {
        "id": "B",
      ...
      {
        "id": "D",
        "text": "AWS Systems Manager Explorer"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Batch multiple MetricDatum objects into a single PutMetricData request (up to 1,000 metrics or 1 MB per call) and implement exponential backoff with jitter"
      },
      {
        "id": "B",
        "text": "Open an AWS support ticket to increase the PutMetricData API limit to 10,000,000 calls per second"
      },
      {
        "id": "C",
        "text": "Store metrics in an Amazon DynamoDB table and query it using CloudWatch Synthetics"
      },
      {
        "id": "D",
        "text": "Send metric values over UDP to an Amazon SNS topic"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Enable active tracing on Lambda and SQS, and ensure the producer embeds the X-Ray trace header (AWSTraceHeader) in the SQS message system attributes; Lambda extracts and continues the trace segment"
      },
      {
        "id": "B",
        "text": "Pass the trace ID in the SQS message body JSON and manually construct a new segment with a random trace ID"
      },
      {
        "id": "C",
        "text": "Replace Amazon SQS with an Amazon Kinesis Data Stream"
      },
      {
        "id": "D",
        "text": "Configure an API Gateway WebSocket API to maintain persistent connection state"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Share the CloudWatch dashboard publicly with a passphrase or via Single Sign-On (SSO) using an external OpenID Connect (OIDC) identity provider"
      },
      {
        "id": "B",
        "text": "Take daily screenshots of the dashboard and email them via Amazon SES"
      },
      {
        "id": "C",
        "text": "Create a public Amazon S3 website bucket and mirror the dashboard HTML"
      },
      {
        "id": "D",
        "text": "Grant the external stakeholders IAM AdministratorAccess credentials in a sandbox account"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Enable CloudTrail Log File Integrity Validation, enable S3 Object Lock in compliance mode on the target bucket, and monitor CloudTrail delivery metrics in CloudWatch"
      },
      {
        "id": "B",
        "text": "Configure Amazon S3 server-side encryption with SSE-S3 and enable S3 versioning"
      },
      {
        "id": "C",
        "text": "Run an hourly Lambda function that compares MD5 hashes of local files against S3 objects"
      },
      {
        "id": "D",
        "text": "Deploy an AWS Config rule that restarts CloudTrail every 24 hours"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Create a CloudWatch Logs Metric Filter with metric dimensions, setting the dimension name to TenantId and dimension value to $tenantId extracted from the log pattern"
      },
      {
        "id": "B",
        "text": "Write a separate Metric Filter for every individual tenant with hardcoded filter strings"
      },
      {
        "id": "C",
        "text": "Stream all logs to Amazon Redshift and generate daily billing reports"
      },
      {
        "id": "D",
        "text": "Configure Amazon Athena to export CSV files to an S3 bucket every minute"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Amazon CloudWatch Network Flow Monitor (part of CloudWatch Network Monitor)"
      },
      {
        "id": "B",
      ...
      {
        "id": "C",
        "text": "Amazon Inspector network reachability package"
      },
      {
        "id": "D",
        "text": "Amazon GuardDuty DNS log monitoring"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Configure the alarm with an 'M out of N' evaluation rule, such as 3 out of 5 datapoints breaching the threshold within a 5-minute evaluation window"
      },
      {
        "id": "B",
        "text": "Change the alarm metric statistic from Average to Maximum"
      },
      {
        "id": "C",
        "text": "Increase the alarm threshold to 10,000 milliseconds"
      },
      {
        "id": "D",
        "text": "Disable alarm actions between 00:00 and 08:00 UTC"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "In each secondary region, configure CloudWatch Logs Subscription Filters to stream to a local Kinesis Data Stream, replicated to the primary region's Kinesis Data Firehose via an AWS Lambda function or cross-region event bridge, delivering to OpenSearch"
      },
      {
        "id": "B",
        "text": "Configure CloudWatch Logs in all secondary regions to stream directly to an OpenSearch endpoint in the primary region over public internet"
      },
      {
        "id": "C",
        "text": "Run an hourly script on an EC2 instance that copies /var/log directories across regions using rsync over SSH"
      },
      {
        "id": "D",
        "text": "Export CloudWatch Logs to regional S3 buckets and configure S3 Cross-Region Replication (CRR) with Athena"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "RAM and disk space are operating system-level metrics that the hypervisor cannot observe; install the unified Amazon CloudWatch agent on the EC2 instances to publish these metrics"
      },
      {
        "id": "B",
        "text": "Detailed Monitoring was disabled on the instance; enabling Detailed Monitoring automatically adds memory metrics"
      },
      {
        "id": "C",
        "text": "The instance security group blocked outbound port 443 to the CloudWatch API"
      },
      {
        "id": "D",
        "text": "CloudWatch does not support memory metrics on Linux operating systems"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Configure CodeDeploy deployment rollback alarms targeting CloudWatch metric alarms, stopping traffic shifting and reverting automatically if an alarm triggers"
      },
      {
        "id": "B",
        "text": "Configure a manual approval gate in CodePipeline requiring an operator to visually inspect Grafana dashboards"
      },
      {
        "id": "C",
        "text": "Schedule an AWS Lambda function to poll CloudWatch metrics once after 24 hours"
      },
      {
        "id": "D",
        "text": "Deploy an AWS Config rule that triggers an EC2 reboot when CPU exceeds 80%"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "Create an Amazon EventBridge rule that matches AWS Config compliance change events or CloudTrail PutBucketAcl API calls, triggering an AWS Systems Manager Automation runbook (or Lambda function) that applies S3 Block Public Access and sends an SNS notification"
      },
      {
        "id": "B",
        "text": "Send an email to the bucket creator requesting that they manually update bucket permissions"
      },
      {
        "id": "C",
        "text": "Configure an S3 bucket lifecycle policy that deletes all objects in public buckets"
      },
      {
        "id": "D",
        "text": "Write a cron job on an EC2 instance that checks S3 permissions once every 24 hours"
      }
    ],
    "correctAnswers": [
      "A"
  ...
  },
  {
    ...
    "options": [
      {
        "id": "A",
        "text": "EventBridge Archive and Replay"
      },
      {
        "id": "B",
        "text": "EventBridge Schema Discovery"
      },
      {
        "id": "C",
        "text": "Amazon SQS Dead-Letter Queue redrive"
      },
      {
        "id": "D",
        "text": "CloudWatch Logs Insights export"
      }
    ],
    "correctAnswers": [
      "A"
