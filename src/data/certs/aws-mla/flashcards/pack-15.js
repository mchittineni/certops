export const AWS_MLA_FLASHCARDS_15 = [
  {
    id: "aws-mla-fc-351",
    difficulty: "easy",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: Blue/Green Deployment Guardrails in High-Frequency FinTech Trading",
    hint: "Focus on deployment-guardrails and blue-green best practices.",
    back: "<strong>Blue/Green Deployment Guardrails</strong>: Configure deployment guardrails using Canary or Linear traffic shifting with automated rollback triggered by CloudWatch 5xx error or latency alarms. Core MLOps rationale: Deployment guardrails perform the shift and, crucially, the rollback automatically against CloudWatch alarms, so a bad model is withdrawn even at three in the m...",
    tags: ["deployment-guardrails", "blue-green", "canary", "High-Frequency FinTech Trading"]
  },
  {
    id: "aws-mla-fc-352",
    difficulty: "easy",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Pipelines Orchestration in Healthcare Patient Records & HIPAA",
    hint: "Focus on pipelines and mlops best practices.",
    back: "<strong>SageMaker Pipelines Orchestration</strong>: Define a SageMaker Pipeline combining ProcessingStep, TrainingStep, ModelStep, and ConditionStep to automate model retraining and registry promotion. Core MLOps rationale: SageMaker Pipelines is native to the platform, so every step records lineage, repeated steps are cached, and a ConditionStep gates registry promotion on the eva...",
    tags: ["pipelines", "mlops", "orchestration", "Healthcare Patient Records & HIPAA"]
  },
  {
    id: "aws-mla-fc-353",
    difficulty: "easy",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Real-Time vs Asynchronous Inference in Global E-Commerce Black Friday Scale",
    hint: "Focus on inference and asynchronous-inference best practices.",
    back: "<strong>SageMaker Real-Time vs Asynchronous Inference</strong>: Use Asynchronous Inference for large payloads up to 1GB and processing times up to 1 hour, or Real-Time Endpoints for sub-second interactive latency. Core MLOps rationale: Asynchronous Inference is built for this shape: the request is queued, the payload can reach a gigabyte, the job may run for an hour, and the caller is notified...",
    tags: ["inference", "asynchronous-inference", "real-time", "Global E-Commerce Black Friday Scale"]
  },
  {
    id: "aws-mla-fc-354",
    difficulty: "easy",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Multi-Model Endpoints (MME) in Autonomous Vehicle Telemetry",
    hint: "Focus on mme and multi-model best practices.",
    back: "<strong>SageMaker Multi-Model Endpoints (MME)</strong>: Deploy a SageMaker Multi-Model Endpoint (MME) to host thousands of distinct customer models on a shared compute instance pool, loading models dynamically from S3. Core MLOps rationale: A multi-model endpoint keeps the models in S3 and pages them into a shared instance pool on demand, so cost tracks the working set rather than the catalogue, wh...",
    tags: ["mme", "multi-model", "cost-efficiency", "Autonomous Vehicle Telemetry"]
  },
  {
    id: "aws-mla-fc-355",
    difficulty: "easy",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: Blue/Green Deployment Guardrails in Multi-Tenant B2B SaaS Platform",
    hint: "Focus on deployment-guardrails and blue-green best practices.",
    back: "<strong>Blue/Green Deployment Guardrails</strong>: Configure deployment guardrails using Canary or Linear traffic shifting with automated rollback triggered by CloudWatch 5xx error or latency alarms. Core MLOps rationale: Deployment guardrails perform the shift and, crucially, the rollback automatically against CloudWatch alarms, so a bad model is withdrawn even at three in the m...",
    tags: ["deployment-guardrails", "blue-green", "canary", "Multi-Tenant B2B SaaS Platform"]
  },
  {
    id: "aws-mla-fc-356",
    difficulty: "medium",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Pipelines Orchestration in Media Streaming & Global CDN",
    hint: "Focus on pipelines and mlops best practices.",
    back: "<strong>SageMaker Pipelines Orchestration</strong>: Define a SageMaker Pipeline combining ProcessingStep, TrainingStep, ModelStep, and ConditionStep to automate model retraining and registry promotion. Core MLOps rationale: SageMaker Pipelines is native to the platform, so every step records lineage, repeated steps are cached, and a ConditionStep gates registry promotion on the eva...",
    tags: ["pipelines", "mlops", "orchestration", "Media Streaming & Global CDN"]
  },
  {
    id: "aws-mla-fc-357",
    difficulty: "medium",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Real-Time vs Asynchronous Inference in Aerospace Satellite Ground Systems",
    hint: "Focus on inference and asynchronous-inference best practices.",
    back: "<strong>SageMaker Real-Time vs Asynchronous Inference</strong>: Use Asynchronous Inference for large payloads up to 1GB and processing times up to 1 hour, or Real-Time Endpoints for sub-second interactive latency. Core MLOps rationale: Asynchronous Inference is built for this shape: the request is queued, the payload can reach a gigabyte, the job may run for an hour, and the caller is notified...",
    tags: ["inference", "asynchronous-inference", "real-time", "Aerospace Satellite Ground Systems"]
  },
  {
    id: "aws-mla-fc-358",
    difficulty: "medium",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Multi-Model Endpoints (MME) in Telecommunications 5G Core Network",
    hint: "Focus on mme and multi-model best practices.",
    back: "<strong>SageMaker Multi-Model Endpoints (MME)</strong>: Deploy a SageMaker Multi-Model Endpoint (MME) to host thousands of distinct customer models on a shared compute instance pool, loading models dynamically from S3. Core MLOps rationale: A multi-model endpoint keeps the models in S3 and pages them into a shared instance pool on demand, so cost tracks the working set rather than the catalogue, wh...",
    tags: ["mme", "multi-model", "cost-efficiency", "Telecommunications 5G Core Network"]
  },
  {
    id: "aws-mla-fc-359",
    difficulty: "medium",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: Blue/Green Deployment Guardrails in Renewable Energy Smart Grid IoT",
    hint: "Focus on deployment-guardrails and blue-green best practices.",
    back: "<strong>Blue/Green Deployment Guardrails</strong>: Configure deployment guardrails using Canary or Linear traffic shifting with automated rollback triggered by CloudWatch 5xx error or latency alarms. Core MLOps rationale: Deployment guardrails perform the shift and, crucially, the rollback automatically against CloudWatch alarms, so a bad model is withdrawn even at three in the m...",
    tags: ["deployment-guardrails", "blue-green", "canary", "Renewable Energy Smart Grid IoT"]
  },
  {
    id: "aws-mla-fc-360",
    difficulty: "medium",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Pipelines Orchestration in Supply Chain Cold-Chain Logistics",
    hint: "Focus on pipelines and mlops best practices.",
    back: "<strong>SageMaker Pipelines Orchestration</strong>: Define a SageMaker Pipeline combining ProcessingStep, TrainingStep, ModelStep, and ConditionStep to automate model retraining and registry promotion. Core MLOps rationale: SageMaker Pipelines is native to the platform, so every step records lineage, repeated steps are cached, and a ConditionStep gates registry promotion on the eva...",
    tags: ["pipelines", "mlops", "orchestration", "Supply Chain Cold-Chain Logistics"]
  },
  {
    id: "aws-mla-fc-361",
    difficulty: "medium",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Real-Time vs Asynchronous Inference in Banking Core Ledger & Payments",
    hint: "Focus on inference and asynchronous-inference best practices.",
    back: "<strong>SageMaker Real-Time vs Asynchronous Inference</strong>: Use Asynchronous Inference for large payloads up to 1GB and processing times up to 1 hour, or Real-Time Endpoints for sub-second interactive latency. Core MLOps rationale: Asynchronous Inference is built for this shape: the request is queued, the payload can reach a gigabyte, the job may run for an hour, and the caller is notified...",
    tags: ["inference", "asynchronous-inference", "real-time", "Banking Core Ledger & Payments"]
  },
  {
    id: "aws-mla-fc-362",
    difficulty: "medium",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Multi-Model Endpoints (MME) in Genomic Sequencing & Biotech Pipeline",
    hint: "Focus on mme and multi-model best practices.",
    back: "<strong>SageMaker Multi-Model Endpoints (MME)</strong>: Deploy a SageMaker Multi-Model Endpoint (MME) to host thousands of distinct customer models on a shared compute instance pool, loading models dynamically from S3. Core MLOps rationale: A multi-model endpoint keeps the models in S3 and pages them into a shared instance pool on demand, so cost tracks the working set rather than the catalogue, wh...",
    tags: ["mme", "multi-model", "cost-efficiency", "Genomic Sequencing & Biotech Pipeline"]
  },
  {
    id: "aws-mla-fc-363",
    difficulty: "medium",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: Blue/Green Deployment Guardrails in Defense-Grade Zero-Trust Network",
    hint: "Focus on deployment-guardrails and blue-green best practices.",
    back: "<strong>Blue/Green Deployment Guardrails</strong>: Configure deployment guardrails using Canary or Linear traffic shifting with automated rollback triggered by CloudWatch 5xx error or latency alarms. Core MLOps rationale: Deployment guardrails perform the shift and, crucially, the rollback automatically against CloudWatch alarms, so a bad model is withdrawn even at three in the m...",
    tags: ["deployment-guardrails", "blue-green", "canary", "Defense-Grade Zero-Trust Network"]
  },
  {
    id: "aws-mla-fc-364",
    difficulty: "medium",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Pipelines Orchestration in Online Multiplayer Gaming Engine",
    hint: "Focus on pipelines and mlops best practices.",
    back: "<strong>SageMaker Pipelines Orchestration</strong>: Define a SageMaker Pipeline combining ProcessingStep, TrainingStep, ModelStep, and ConditionStep to automate model retraining and registry promotion. Core MLOps rationale: SageMaker Pipelines is native to the platform, so every step records lineage, repeated steps are cached, and a ConditionStep gates registry promotion on the eva...",
    tags: ["pipelines", "mlops", "orchestration", "Online Multiplayer Gaming Engine"]
  },
  {
    id: "aws-mla-fc-365",
    difficulty: "medium",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Real-Time vs Asynchronous Inference in Insurance Risk & Actuarial Modeling",
    hint: "Focus on inference and asynchronous-inference best practices.",
    back: "<strong>SageMaker Real-Time vs Asynchronous Inference</strong>: Use Asynchronous Inference for large payloads up to 1GB and processing times up to 1 hour, or Real-Time Endpoints for sub-second interactive latency. Core MLOps rationale: Asynchronous Inference is built for this shape: the request is queued, the payload can reach a gigabyte, the job may run for an hour, and the caller is notified...",
    tags: ["inference", "asynchronous-inference", "real-time", "Insurance Risk & Actuarial Modeling"]
  },
  {
    id: "aws-mla-fc-366",
    difficulty: "medium",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Multi-Model Endpoints (MME) in Pharmaceutical Clinical Trial Platform",
    hint: "Focus on mme and multi-model best practices.",
    back: "<strong>SageMaker Multi-Model Endpoints (MME)</strong>: Deploy a SageMaker Multi-Model Endpoint (MME) to host thousands of distinct customer models on a shared compute instance pool, loading models dynamically from S3. Core MLOps rationale: A multi-model endpoint keeps the models in S3 and pages them into a shared instance pool on demand, so cost tracks the working set rather than the catalogue, wh...",
    tags: ["mme", "multi-model", "cost-efficiency", "Pharmaceutical Clinical Trial Platform"]
  },
  {
    id: "aws-mla-fc-367",
    difficulty: "medium",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: Blue/Green Deployment Guardrails in Smart City Traffic & Mobility Sensor Hub",
    hint: "Focus on deployment-guardrails and blue-green best practices.",
    back: "<strong>Blue/Green Deployment Guardrails</strong>: Configure deployment guardrails using Canary or Linear traffic shifting with automated rollback triggered by CloudWatch 5xx error or latency alarms. Core MLOps rationale: Deployment guardrails perform the shift and, crucially, the rollback automatically against CloudWatch alarms, so a bad model is withdrawn even at three in the m...",
    tags: ["deployment-guardrails", "blue-green", "canary", "Smart City Traffic & Mobility Sensor Hub"]
  },
  {
    id: "aws-mla-fc-368",
    difficulty: "medium",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Pipelines Orchestration in Digital Identity & Biometric Verification",
    hint: "Focus on pipelines and mlops best practices.",
    back: "<strong>SageMaker Pipelines Orchestration</strong>: Define a SageMaker Pipeline combining ProcessingStep, TrainingStep, ModelStep, and ConditionStep to automate model retraining and registry promotion. Core MLOps rationale: SageMaker Pipelines is native to the platform, so every step records lineage, repeated steps are cached, and a ConditionStep gates registry promotion on the eva...",
    tags: ["pipelines", "mlops", "orchestration", "Digital Identity & Biometric Verification"]
  },
  {
    id: "aws-mla-fc-369",
    difficulty: "medium",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Real-Time vs Asynchronous Inference in Legal Discovery & Semantic Document Search",
    hint: "Focus on inference and asynchronous-inference best practices.",
    back: "<strong>SageMaker Real-Time vs Asynchronous Inference</strong>: Use Asynchronous Inference for large payloads up to 1GB and processing times up to 1 hour, or Real-Time Endpoints for sub-second interactive latency. Core MLOps rationale: Asynchronous Inference is built for this shape: the request is queued, the payload can reach a gigabyte, the job may run for an hour, and the caller is notified...",
    tags: ["inference", "asynchronous-inference", "real-time", "Legal Discovery & Semantic Document Search"]
  },
  {
    id: "aws-mla-fc-370",
    difficulty: "medium",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Multi-Model Endpoints (MME) in AdTech Real-Time Bidding Exchange",
    hint: "Focus on mme and multi-model best practices.",
    back: "<strong>SageMaker Multi-Model Endpoints (MME)</strong>: Deploy a SageMaker Multi-Model Endpoint (MME) to host thousands of distinct customer models on a shared compute instance pool, loading models dynamically from S3. Core MLOps rationale: A multi-model endpoint keeps the models in S3 and pages them into a shared instance pool on demand, so cost tracks the working set rather than the catalogue, wh...",
    tags: ["mme", "multi-model", "cost-efficiency", "AdTech Real-Time Bidding Exchange"]
  },
  {
    id: "aws-mla-fc-371",
    difficulty: "hard",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: Blue/Green Deployment Guardrails in Precision Agriculture & Drone Scouting",
    hint: "Focus on deployment-guardrails and blue-green best practices.",
    back: "<strong>Blue/Green Deployment Guardrails</strong>: Configure deployment guardrails using Canary or Linear traffic shifting with automated rollback triggered by CloudWatch 5xx error or latency alarms. Core MLOps rationale: Deployment guardrails perform the shift and, crucially, the rollback automatically against CloudWatch alarms, so a bad model is withdrawn even at three in the m...",
    tags: ["deployment-guardrails", "blue-green", "canary", "Precision Agriculture & Drone Scouting"]
  },
  {
    id: "aws-mla-fc-372",
    difficulty: "hard",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Pipelines Orchestration in Industrial Robotics Predictive Maintenance",
    hint: "Focus on pipelines and mlops best practices.",
    back: "<strong>SageMaker Pipelines Orchestration</strong>: Define a SageMaker Pipeline combining ProcessingStep, TrainingStep, ModelStep, and ConditionStep to automate model retraining and registry promotion. Core MLOps rationale: SageMaker Pipelines is native to the platform, so every step records lineage, repeated steps are cached, and a ConditionStep gates registry promotion on the eva...",
    tags: ["pipelines", "mlops", "orchestration", "Industrial Robotics Predictive Maintenance"]
  },
  {
    id: "aws-mla-fc-373",
    difficulty: "hard",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Real-Time vs Asynchronous Inference in Educational Remote Proctoring Platform",
    hint: "Focus on inference and asynchronous-inference best practices.",
    back: "<strong>SageMaker Real-Time vs Asynchronous Inference</strong>: Use Asynchronous Inference for large payloads up to 1GB and processing times up to 1 hour, or Real-Time Endpoints for sub-second interactive latency. Core MLOps rationale: Asynchronous Inference is built for this shape: the request is queued, the payload can reach a gigabyte, the job may run for an hour, and the caller is notified...",
    tags: ["inference", "asynchronous-inference", "real-time", "Educational Remote Proctoring Platform"]
  },
  {
    id: "aws-mla-fc-374",
    difficulty: "hard",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: SageMaker Multi-Model Endpoints (MME) in Real Estate Valuation & Geo-Spatial Analytics",
    hint: "Focus on mme and multi-model best practices.",
    back: "<strong>SageMaker Multi-Model Endpoints (MME)</strong>: Deploy a SageMaker Multi-Model Endpoint (MME) to host thousands of distinct customer models on a shared compute instance pool, loading models dynamically from S3. Core MLOps rationale: A multi-model endpoint keeps the models in S3 and pages them into a shared instance pool on demand, so cost tracks the working set rather than the catalogue, wh...",
    tags: ["mme", "multi-model", "cost-efficiency", "Real Estate Valuation & Geo-Spatial Analytics"]
  },
  {
    id: "aws-mla-fc-375",
    difficulty: "hard",
    certId: "aws-mla",
    domainId: "d3",
    front: "AWS ML Engineer: Blue/Green Deployment Guardrails in Disaster Emergency Dispatch & Operations",
    hint: "Focus on deployment-guardrails and blue-green best practices.",
    back: "<strong>Blue/Green Deployment Guardrails</strong>: Configure deployment guardrails using Canary or Linear traffic shifting with automated rollback triggered by CloudWatch 5xx error or latency alarms. Core MLOps rationale: Deployment guardrails perform the shift and, crucially, the rollback automatically against CloudWatch alarms, so a bad model is withdrawn even at three in the m...",
    tags: ["deployment-guardrails", "blue-green", "canary", "Disaster Emergency Dispatch & Operations"]
  }
];

export default AWS_MLA_FLASHCARDS_15;
