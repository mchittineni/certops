export const AWS_DVA_QUESTIONS_4 = [
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A Retry block with BackoffRate plus a Catch to a fallback" },
      { id: 'B', text: "Increase the state machine timeout to 1 year" },
      { id: 'C', text: "Wrap the Lambda function in an infinite while loop" },
      { id: 'D', text: "Configure an SQS queue between every Step Functions state" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "AWS AppSync GraphQL Subscriptions" },
      { id: 'B', text: "Amazon S3 Static Hosting" },
      { id: 'C', text: "Amazon Simple Email Service (SES)" },
      { id: 'D', text: "AWS CloudFormation Change Sets" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Enhanced Fan-Out with HTTP/2 SubscribeToShard API" },
      { id: 'B', text: "Standard SQS polling with maximum batch size" },
      { id: 'C', text: "CloudWatch Logs Insights export" },
      { id: 'D', text: "Kinesis Data Firehose buffer interval tuning" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An execution role granting GetItem and Query on that table ARN" },
      { id: 'B', text: "Attach the AmazonDynamoDBFullAccess policy to the default account root user" },
      { id: 'C', text: "Attach the AWSLambdaBasicExecutionRole and add dynamodb:* inline" },
      { id: 'D', text: "Enable DynamoDB Streams without IAM policies" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "sts:AssumeRole" },
      { id: 'B', text: "sts:GetFederationToken" },
      { id: 'C', text: "iam:CreateAccessKey" },
      { id: 'D', text: "sts:DecodeAuthorizationMessage" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "GenerateDataKey, encrypt locally, erase the plaintext key" },
      { id: 'B', text: "Transmit the 50 GB payload directly to the kms:Encrypt API endpoint" },
      { id: 'C', text: "Store the plaintext encryption key in the S3 object metadata headers" },
      { id: 'D', text: "Disable encryption at rest to avoid KMS API limits" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The key policy does not delegate to the account root principal" },
      { id: 'B', text: "KMS keys do not support IAM policies under any circumstances" },
      { id: 'C', text: "The developer must be logged in as the AWS root account user" },
      { id: 'D', text: "KMS keys take 7 days to become active after creation" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Write a cron job that calls kms:UpdateKeyDescription annually" },
      { id: 'B', text: "Manually create a new CMK each year, re-encrypt all existing database records, and delete the old key" },
      { id: 'C', text: "Rotate the IAM access keys of the developer every 90 days" },
      { id: 'D', text: "Enable Automatic Key Rotation on the Customer Managed Key (CMK) in KMS" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The Secrets Manager client-side caching library" },
      { id: 'B', text: "Fetch the secret in the handler and cache it in a module-level variable" },
      { id: 'C', text: "Store the password in an unencrypted S3 bucket" },
      { id: 'D', text: "Disable SSL/TLS encryption between Lambda and the database" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Built-in rotation with a Lambda function" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A user pool for sign-in, an identity pool for AWS credentials" },
      { id: 'B', text: "Cognito User Pool alone for both authentication and STS credential generation" },
      { id: 'C', text: "Cognito Identity Pool alone without a user directory" },
      { id: 'D', text: "IAM users created programmatically for every mobile user" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Enable unauthenticated identities with a read-only guest role" },
      { id: 'B', text: "Create an IAM root user access key and embed it in the mobile app binary" },
      { id: 'C', text: "Disable all authentication on the backend DynamoDB table" },
      { id: 'D', text: "Create a dummy user in Cognito User Pools with password 'guest'" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Embed company AWS IAM access keys in the client-side JavaScript bundle" },
      { id: 'B', text: "Make the S3 bucket completely public with anonymous write access" },
      { id: 'C', text: "Require users to install the AWS CLI and configure local profiles" },
      { id: 'D', text: "Generate a presigned S3 PUT URL in the backend for the browser" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Condition: { IpAddress: { aws:SourceIp: 10.0.0.0/8 } }" },
      { id: 'B', text: "Condition: { StringEquals: { s3:x-amz-server-side-encryption: AES256 } }" },
      { id: 'C', text: "Condition: { Null: { aws:PrincipalArn: true } }" },
      { id: 'D', text: "Condition: { Bool: { aws:SecureTransport: false } }" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Embed an IAM secret key in every client browser request" },
      { id: 'C', text: "Deploy a custom Lambda authorizer that manually verifies the JWT signature" },
      { id: 'D', text: "Pass the user password in an HTTP Basic Authorization header" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Access is denied because an explicit Deny statement in any policy always overrides all Allow statements" },
      { id: 'B', text: "Access is allowed because the user-level policy takes precedence over the group policy" },
      { id: 'C', text: "Access is granted on weekdays but denied on weekends" },
      { id: 'D', text: "Access is permitted if the request uses multi-factor authentication" }
    ],
    correctAnswers: ['A'],
    type: "single",
