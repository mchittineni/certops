export const AWS_DVA_QUESTIONS_3 = [
  {
    ...
    options: [
      { id: 'A', text: "Use the Import API with the x-amazon-apigateway extensions" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Send Cache-Control: max-age=0 with the InvalidateCache right" },
      { id: 'B', text: "Delete and recreate the API Gateway stage" },
      { id: 'C', text: "Restart the API Gateway service from the management console" },
      { id: 'D', text: "Change the REST API deployment ID" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Enable X-Ray Tracing checkbox in the API Gateway Stage settings" },
      { id: 'B', text: "Install the X-Ray daemon inside the API Gateway deployment package" },
      { id: 'C', text: "Add an X-Ray tracking header manually in client browser cookies" },
      { id: 'D', text: "Configure Amazon CloudWatch Logs Insights subscription filters" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Create an IAM user in Account A and embed access keys in the API request headers" },
      { id: 'B', text: "Attach a DynamoDB stream between the two accounts" },
      { id: 'C', text: "Configure an S3 bucket policy in Account B with public read access" },
      { id: 'D', text: "Grant lambda:InvokeFunction to the API execution ARN in Account B" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Decrease the provisioned write capacity units (WCU)" },
      { id: 'B', text: "Scan the entire table instead of querying" },
      { id: 'C', text: "A high-cardinality composite key, or write sharding" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Create a GSI with category as partition key and query it" },
      { id: 'B', text: "Double the provisioned read capacity units (RCU) on the base table" },
      { id: 'C', text: "Execute the Scan operation using multiple threads in parallel" },
      { id: 'D', text: "Export the table to S3 on every read operation" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Transactional Scan Read" },
      { id: 'B', text: "Strongly Consistent Read" },
      { id: 'C', text: "Eventually Consistent Read" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Global Secondary Index (GSI) with Status as the partition key" },
      { id: 'B', text: "Local Secondary Index (LSI) with Status as the partition key" },
      { id: 'C', text: "Composite primary key on the base table" },
      { id: 'D', text: "DynamoDB Accelerator (DAX) secondary key" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "PutItem with ReturnValues" },
      { id: 'C', text: "TransactWriteItems" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Enable DynamoDB Streams on the table and configure an AWS Lambda trigger" },
      { id: 'B', text: "Run a cron job that executes a full table Scan every minute" },
      { id: 'C', text: "Configure S3 lifecycle rules on the DynamoDB backup bucket" },
      { id: 'D', text: "Enable CloudTrail data events and write a custom Bash log parser" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Decrease the write capacity units (WCU) to zero at midnight" },
      { id: 'B', text: "Schedule a nightly Lambda function that runs a Scan and calls DeleteItem on every expired item" },
      { id: 'C', text: "Enable Time to Live (TTL) on a timestamp attribute in epoch seconds" },
      { id: 'D', text: "Configure an S3 Glacier archival lifecycle rule" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Pass LastEvaluatedKey back as ExclusiveStartKey" },
      { id: 'B', text: "Repeat the exact same query with double the RCU" },
      { id: 'C', text: "Increase the DynamoDB single response limit to 100 MB" },
      { id: 'D', text: "Use a Scan operation with a random offset" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A FIFO queue with a message group ID" },
      { id: 'B', text: "Amazon SQS standard queue with long polling" },
      { id: 'C', text: "Amazon SQS dead letter queue" },
      { id: 'D', text: "Amazon SNS standard topic" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Increase the SQS delay seconds on the queue" },
      { id: 'C', text: "Configure a Dead Letter Queue with maximum receive count set to 1" },
      { id: 'D', text: "Delete the message immediately when received and recreate it if processing fails" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy an EC2 instance that sleeps for 24 hours" },
      { id: 'B', text: "Enable long polling with ReceiveMessageWaitTimeSeconds" },
      { id: 'C', text: "Increase the polling frequency to 1 millisecond intervals" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Send the order event to a single SQS queue and let all three services compete for messages" },
      { id: 'B', text: "Publish the event to an SNS topic and subscribe three SQS queues" },
      { id: 'C', text: "Have each service poll the database directly using cron jobs" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Create separate SNS topics for every customer in the database" },
      { id: 'B', text: "Filter the messages inside the downstream service after receiving them" },
      { id: 'C', text: "Configure a Subscription Filter Policy on the Fraud Alert SNS subscription" },
