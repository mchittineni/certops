export const AWS_DVA_QUESTIONS_1 = [
  {
    ...
    options: [
      { id: 'A', text: "Switch the Lambda function runtime from Node.js to Java without ahead-of-time compilation" },
      { id: 'B', text: "Configure Provisioned Concurrency for the Lambda function alias or version" },
      { id: 'C', text: "Increase ephemeral disk storage in /tmp to 10 GB" },
      { id: 'D', text: "Increase the function timeout setting from 3 seconds to 15 minutes" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deploy Amazon RDS Proxy between Lambda and the PostgreSQL database" },
      { id: 'B', text: "Initialize a new database connection inside the Lambda handler function on every request" },
      { id: 'C', text: "Increase the memory of the Lambda function to 10,240 MB" },
      { id: 'D', text: "Disable multi-AZ deployment on the RDS instance" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Increase the Lambda ephemeral storage (/tmp) setting up to 10,240 MB" },
      { id: 'B', text: "Mount an Amazon S3 bucket directly as a local POSIX block device in the handler code" },
      { id: 'C', text: "Write the extracted files to the /var/task deployment package directory" },
      { id: 'D', text: "Attach an EBS General Purpose SSD volume to the Lambda function directly" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Increase the MaximumRecordAgeInSeconds to 7 days" },
      { id: 'B', text: "Set the BatchSize to 10,000 records" },
      { id: 'C', text: "BisectBatchOnFunctionError and a failure destination" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Split the code across 32 individual Lambda layers" },
      { id: 'C', text: "Compress the file using gzip and stream it into memory during cold start" },
      { id: 'D', text: "Upload the 8 GB file directly as a standard zip file to the Lambda console" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Increase the allocated memory setting in Lambda configuration" },
      { id: 'B', text: "Configure multi-threading flags in the operating system environment" },
      { id: 'C', text: "Add an additional network interface (ENI) to the VPC configuration" },
      { id: 'D', text: "Enable provisioned concurrency on the default $LATEST version" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "AWS_IAM" },
      { id: 'B', text: "NONE" },
      { id: 'C', text: "COGNITO_USER_POOLS" },
      { id: 'D', text: "BASIC_AUTH" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Lambda requires all private databases to be exposed via an internet gateway" },
      { id: 'B', text: "Lambda creates shared Hyperplane Elastic Network Interfaces (ENIs) per subnet during configuration" },
      { id: 'C', text: "Lambda establishes an IPSec VPN tunnel between the AWS Lambda service and the VPC" },
      { id: 'D', text: "Lambda assigns a public IPv4 address to every concurrent execution container" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Maximum Concurrency (ScalingConfig)" },
      { id: 'B', text: "BatchSize" },
      { id: 'C', text: "ReceiveMessageWaitTimeSeconds" },
      { id: 'D', text: "VisibilityTimeout" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Lambda Aliases with routing configuration weights" },
      { id: 'B', text: "Route 53 latency-based routing records" },
      { id: 'C', text: "Application Load Balancer path routing rules" },
      { id: 'D', text: "CloudWatch Synthetics canaries" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Three retry attempts and a failure destination" },
      { id: 'B', text: "Delete the DynamoDB stream and recreate it" },
      { id: 'C', text: "Set the Lambda function timeout to 1 second" },
      { id: 'D', text: "Increase the DynamoDB write capacity units (WCU)" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The execution role lacks the CloudWatch Logs write permissions" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The function fails because 1024 MB exceeds AWS limits" },
      { id: 'B', text: "Time drops by 87.5% and cost stays about the same" },
      { id: 'C', text: "Cost increases by a factor of 800% because memory is higher" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Save the API keys in plain text inside the S3 deployment bucket metadata" },
      { id: 'C', text: "Pass credentials in HTTP query strings during function invocation" },
      { id: 'D', text: "Store the credentials in SSM Parameter String parameters read at cold start" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Update the runtime parameter in the function configuration or SAM template to nodejs20.x" },
      { id: 'B', text: "Provision a new Amazon EC2 instance and install Node.js manually" },
      { id: 'C', text: "Compile Node.js 20 binaries into a Lambda Layer and invoke them via shell script" },
      { id: 'D', text: "Reboot the AWS data center hosting the serverless fleet" }
    ],
    correctAnswers: ['A'],
    type: "single",
