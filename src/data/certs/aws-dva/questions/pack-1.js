export const AWS_DVA_QUESTIONS_1 = [
  {
    ...
    options: [
      { id: 'A', text: "Configure Provisioned Concurrency for the Lambda function alias or version" },
      { id: 'B', text: "Increase the function timeout setting from 3 seconds to 15 minutes" },
      { id: 'C', text: "Increase ephemeral disk storage in /tmp to 10 GB" },
      { id: 'D', text: "Switch the Lambda function runtime from Node.js to Java without ahead-of-time compilation" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Disable multi-AZ deployment on the RDS instance" },
      { id: 'B', text: "Deploy Amazon RDS Proxy between Lambda and the PostgreSQL database" },
      { id: 'C', text: "Initialize a new database connection inside the Lambda handler function on every request" },
      { id: 'D', text: "Increase the memory of the Lambda function to 10,240 MB" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Write the extracted files to the /var/task deployment package directory" },
      { id: 'B', text: "Attach an EBS General Purpose SSD volume to the Lambda function directly" },
      { id: 'C', text: "Mount an Amazon S3 bucket directly as a local POSIX block device in the handler code" },
      { id: 'D', text: "Increase the Lambda ephemeral storage (/tmp) setting up to 10,240 MB" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Bundle the dependencies into every individual function deployment zip package" },
      { id: 'B', text: "Upload the libraries to an S3 bucket and download them inside the handler on every invocation" },
      { id: 'C', text: "Package the shared libraries into a Lambda Layer and reference the layer ARN in each function" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Inspect CloudWatch Logs manually and rerun failed jobs from the console" },
      { id: 'C', text: "Configure a Dead Letter Queue (DLQ) in the Amazon S3 lifecycle rules" },
      { id: 'D', text: "Write custom try/catch blocks that invoke an Amazon SNS topic synchronously" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Write the configuration data to Amazon SQS and read it on each execution" },
      { id: 'C', text: "Store the configuration in the event object passed into the handler parameter" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Upload the 8 GB file directly as a standard zip file to the Lambda console" },
      { id: 'C', text: "Split the code across 32 individual Lambda layers" },
      { id: 'D', text: "Compress the file using gzip and stream it into memory during cold start" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Migrate the billing service to Amazon EC2 Spot Instances" },
      { id: 'B', text: "Increase the AWS Lambda execution timeout to 900 seconds" },
      { id: 'C', text: "Configure Provisioned Concurrency on the non-critical functions" },
      { id: 'D', text: "Allocate Reserved Concurrency specifically to the critical Lambda function" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Mount an Amazon EFS volume to store pre-compiled class bytecode" },
      { id: 'B', text: "Replace the JVM with Node.js runtime code" },
      { id: 'C', text: "Enable AWS Lambda SnapStart for the Java function and publish a new version" },
      { id: 'D', text: "Increase function RAM from 1 GB to 2 GB" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Add an additional network interface (ENI) to the VPC configuration" },
      { id: 'B', text: "Enable provisioned concurrency on the default $LATEST version" },
      { id: 'C', text: "Configure multi-threading flags in the operating system environment" },
      { id: 'D', text: "Increase the allocated memory setting in Lambda configuration" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "BASIC_AUTH" },
      { id: 'B', text: "COGNITO_USER_POOLS" },
      { id: 'C', text: "AWS_IAM" },
      { id: 'D', text: "NONE" }
    ],
    correctAnswers: ['C'],
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Lambda creates shared Hyperplane Elastic Network Interfaces (ENIs) per subnet during configuration" },
      { id: 'B', text: "Lambda assigns a public IPv4 address to every concurrent execution container" },
      { id: 'C', text: "Lambda establishes an IPSec VPN tunnel between the AWS Lambda service and the VPC" },
      { id: 'D', text: "Lambda requires all private databases to be exposed via an internet gateway" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "ReceiveMessageWaitTimeSeconds" },
      { id: 'B', text: "VisibilityTimeout" },
      { id: 'C', text: "BatchSize" },
      { id: 'D', text: "Maximum Concurrency (ScalingConfig)" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Route 53 latency-based routing records" },
      { id: 'B', text: "Application Load Balancer path routing rules" },
      { id: 'C', text: "CloudWatch Synthetics canaries" },
      { id: 'D', text: "Lambda Aliases with routing configuration weights" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Reboot the AWS data center hosting the serverless fleet" },
      { id: 'B', text: "Update the runtime parameter in the function configuration or SAM template to nodejs20.x" },
      { id: 'C', text: "Provision a new Amazon EC2 instance and install Node.js manually" },
      { id: 'D', text: "Compile Node.js 20 binaries into a Lambda Layer and invoke them via shell script" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The physical server rack ID hosting the hypervisor" },
      { id: 'C', text: "The caller IAM user credentials and access key secret" },
