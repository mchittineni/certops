export const HASHICORP_TFA_QUESTIONS_3 = [
  {
    ...
    title: "HCP Terraform Cost Estimation Requirements and Capabilities",
    scenario: "A FinOps director activates Cost Estimation in HCP Terraform.",
    question: "Which cloud providers are natively supported by HCP Terraform Cost Estimation for computing hourly and monthly cost deltas?",
    options: [
    ...
    correctAnswers: ['A'],
    type: "single",
    explanation: "HCP Terraform provides built-in cost estimation for major public cloud providers: AWS, Microsoft Azure, and Google Cloud Platform. It analyzes resource changes against cloud pricing APIs before apply.",
    referenceUrl: "https://developer.hashicorp.com/terraform/cloud-docs/cost-estimation",
    tags: ["HCP Terraform","Cost Estimation","FinOps"]
  },
  {
    id: "hashicorp-tfa-52",
    difficulty: "medium",
    ...
    domainId: "d8",
    domainName: "HCP Terraform",
    title: "HCP Terraform Audit Logging and SOC 2 Compliance",
    scenario: "An enterprise security audit requires logging every user interaction in HCP Terraform, including who viewed state files, who triggered runs, and who modified environment variables.",
    question: "Which HCP Terraform feature exports immutable event streams to enterprise SIEM platforms via Splunk or Datadog integrations?",
    options: [
    ...
    correctAnswers: ['A'],
    type: "single",
    explanation: "HCP Terraform provides an `Audit Logging API` (Audit Trail) that records comprehensive, tamper-proof logs of all user actions, run initiations, secret modifications, and state accesses, exportable to external SIEM systems for compliance.",
    referenceUrl: "https://developer.hashicorp.com/terraform/cloud-docs/users-teams-organizations/audit-trails",
    tags: ["HCP Terraform","Audit Logging","Compliance"]
  },
  {
    id: "hashicorp-tfa-53",
    difficulty: "hard",
    ...
    domainId: "d8",
    domainName: "HCP Terraform",
    title: "HCP Terraform Run Triggers for Cross-Workspace Coordination",
    ...
    question: "Which HCP Terraform feature coordinates automated execution across workspaces?",
    options: [
    ...
    tags: ["HCP Terraform","Run Triggers","Workspace Orchestration"]
  },
  {
    id: "hashicorp-tfa-54",
    difficulty: "medium",
    ...
    options: [
      { id: 'A', text: "Soft-Mandatory (can be overridden by authorized users)" },
      { id: 'B', text: "Optional" },
      { id: 'C', text: "Hard-Mandatory" },
      { id: 'D', text: "Advisory (prints warnings only)" }
    ],
    correctAnswers: ['C'],
    type: "single",
    ...
    tags: ["HCP Terraform","Sentinel","Enforcement Levels"]
  },
  {
    id: "hashicorp-tfa-55",
    difficulty: "hard",
    ...
    scenario: "A government agency with strict air-gapped security mandates requires all HCP Terraform capabilities (VCS integration, private registry, remote execution, Sentinel) hosted inside their private, self-contained data center.",
    question: "Which HashiCorp product provides self-hosted private deployments of HCP Terraform?",
    options: [
      { id: 'A', text: "Terraform Enterprise (TFE)" },
      { id: 'B', text: "Terraform Community Edition" },
      { id: 'C', text: "Terraform CLI local" },
      { id: 'D', text: "HashiCorp Consul alone" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "`Terraform Enterprise` (TFE) is the self-hosted, air-gapped distribution of HCP Terraform. It installs on customer-owned infrastructure (VMs, Kubernetes) and provides complete organizational governance, private registries, and remote execution isolated from the public internet.",
  ...
  },
  {
    ...
    domainId: "d8",
    domainName: "HCP Terraform",
    title: "HCP Terraform agents for Private Infrastructure Provisioning",
    scenario: "An enterprise uses hosted HCP Terraform (SaaS) but needs to provision resources inside an isolated on-premises datacenter that has no inbound public internet connectivity.",
    question: "Which HCP Terraform component runs inside the private network to execute remote runs locally?",
    options: [
      { id: 'A', text: "HCP Terraform agents (TFC Agents)" },
      { id: 'B', text: "Opening inbound SSH port 22 on the corporate firewall" },
      { id: 'C', text: "Public NAT gateways on the internet" },
      { id: 'D', text: "HCP Terraform cannot manage private infrastructure" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "`HCP Terraform agents` allow hosted HCP Terraform to communicate with private, isolated infrastructure. The agent runs inside the private network, polls HCP Terraform via outbound-only HTTPS, executes the run locally, and returns state updates.",
    referenceUrl: "https://developer.hashicorp.com/terraform/cloud-docs/run/agents",
    tags: ["HCP Terraform","Agents","Hybrid Cloud"]
  },
  {
    id: "hashicorp-tfa-57",
    difficulty: "hard",
    ...
    domainId: "d8",
    domainName: "HCP Terraform",
    title: "Terraform Variable Sets in HCP Terraform",
    scenario: "An organization manages 100 workspaces in HCP Terraform. All 100 workspaces require the exact same AWS credentials (`AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`).",
    question: "Which HCP Terraform feature shares a unified set of environment or Terraform variables across multiple workspaces?",
    options: [
      { id: 'A', text: "Global State Files" },
      { id: 'B', text: "Sentinel Policies" },
      { id: 'C', text: "Copying and pasting credentials 100 times manually" },
      { id: 'D', text: "Variable Sets" }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "`Variable Sets` in HCP Terraform allow administrators to define a group of variables (sensitive credentials or common configuration values) once and apply them globally across all workspaces or to selected workspace subsets.",
    referenceUrl: "https://developer.hashicorp.com/terraform/cloud-docs/workspaces/variables/variable-sets",
    tags: ["HCP Terraform","Variable Sets","Credential Management"]
  },
  {
    id: "hashicorp-tfa-58",
    difficulty: "medium",
    ...
    domainId: "d8",
    domainName: "HCP Terraform",
    title: "HCP Terraform Secure Environment Variables (HCL vs Sensitive)",
    scenario: "A developer adds a database master password as a variable in an HCP Terraform workspace. Other developers with workspace read access must not be able to view the password in the UI or API.",
    question: "Which setting on the HCP Terraform variable hides its value after creation?",
    options: [
    ...
    correctAnswers: ['A'],
    type: "single",
    explanation: "Marking a variable as `Sensitive` in HCP Terraform encrypts the value with Vault and makes it write-only. The value is permanently masked in the UI and API, visible only during run execution.",
    referenceUrl: "https://developer.hashicorp.com/terraform/cloud-docs/workspaces/variables#sensitive-values",
    tags: ["HCP Terraform","Sensitive Variables","Security"]
  },
  {
    id: "hashicorp-tfa-59",
    difficulty: "hard",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "builtin_data" },
      { id: 'B', text: "local_resource" },
      { id: 'C', text: "terraform_data" },
      { id: 'D', text: "null_resource v2" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    scenario: "A platform team configures access for 50 junior engineers in HCP Terraform. The junior engineers must be able to view workspaces and run speculative plans on pull requests, but must be blocked from executing `terraform apply` directly to production.",
    question: "Which workspace permission level should be granted to the junior engineers team?",
    options: [
      { id: 'A', text: "Write permission (Apply)" },
      { id: 'B', text: "Plan permission (Read + Plan)" },
      { id: 'C', text: "Read-only permission alone" },
      { id: 'D', text: "Admin permission" }
    ],
    correctAnswers: ['B'],
    type: "single",
    explanation: "HCP Terraform provides granular workspace permissions: `Read` (view runs/state), `Plan` (trigger and view speculative plans), `Write` (execute applies), and `Admin` (full workspace configuration and secret management).",
    ...
    tags: ["HCP Terraform","RBAC","Workspace Permissions"]
  },
  {
    id: "hashicorp-tfa-64",
    difficulty: "medium",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "terraform plan" },
      { id: 'C', text: "terraform login" },
      { id: 'D', text: "terraform refresh" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    question: "Which flag saves an execution plan to a file for deterministic application?",
    options: [
    ...
    correctAnswers: ['A'],
    type: "single",
    explanation: "Running `terraform plan -out=tfplan` writes the calculated execution plan to a file. Applying that file (`terraform apply tfplan`) guarantees that Terraform executes the exact actions planned, preventing drift or concurrent changes from slipping in. The plan file is not encrypted and can contain sensitive values, so protect it as carefully as state.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "-only=aws_security_group.sg_debug" },
      { id: 'C', text: "-resource=aws_security_group.sg_debug" },
      { id: 'D', text: "-filter=aws_security_group.sg_debug" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "terraform apply -replace=\"aws_instance.web\"" },
      { id: 'B', text: "terraform taint aws_instance.web (deprecated legacy command)" },
      { id: 'C', text: "terraform recreate aws_instance.web" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "terraform apply -refresh-only, which asks before saving state" }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "`terraform apply -refresh-only` queries remote cloud APIs, compares live reality against the state file, displays drift, and prompts to update the state file to match reality WITHOUT making any changes to cloud infrastructure. `terraform plan -refresh-only` shows the same drift but never writes state, and the standalone `terraform refresh` command is deprecated because it updates state without review.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "terraform state show aws_instance.db" },
      { id: 'B', text: "terraform describe aws_instance.db" },
      { id: 'C', text: "terraform inspect aws_instance.db" },
      { id: 'D', text: "terraform state get aws_instance.db" }
    ],
    correctAnswers: ['A'],
    type: "single",
