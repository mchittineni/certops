export const HASHICORP_TFA_QUESTIONS_7 = [
  {
    id: "hashicorp-tfa-151",
    difficulty: "easy",
    certId: "hashicorp-tfa",
    domainId: "d4",
    domainName: "Terraform configuration",
    title: "Sensitive Data Exposure in the State File",
    scenario: "A developer uses a database resource block that creates an RDS instance with a master password generated by the random_password resource.",
    question: "Where is the master password stored in plain text by Terraform?",
    options: [
      { id: 'A', text: "In the state file, local or remote" },
      { id: 'B', text: "In the dependency lock file, beside the versions" },
      { id: 'C', text: "It is never stored anywhere by Terraform" },
      { id: 'D', text: "In the Linux kernel keyring" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Terraform state files record the exact real-world attributes of every managed resource, including passwords, private keys, and API tokens in cleartext JSON. Securing the backend storage with encryption-at-rest and strict IAM permissions is mandatory.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/state/sensitive-data",
    tags: ["Terraform State", "Sensitive Data", "Security"]
  },
  {
    id: "hashicorp-tfa-152",
    difficulty: "medium",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "State File Lineage and Serial Numbers",
    scenario: "A team investigates how Terraform tracks state versions and prevents accidental overwriting of newer state files with stale backups.",
    question: "What two state metadata fields ensure state integrity across updates?",
    options: [
      { id: 'A', text: "The committing user's git_commit and author_name" },
      { id: 'B', text: "A content checksum and the cloud account id" },
      { id: 'C', text: "The format version number and the write timestamp" },
      { id: 'D', text: "The lineage identifier and the incrementing serial" }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "In `terraform.tfstate`, `lineage` is a unique UUID generated when state is first created. `serial` is an integer that increments by 1 on every write. If a client attempts to push a state with a lower serial or mismatched lineage, Terraform rejects the push to prevent state regressions.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/state",
    tags: ["Terraform State", "Lineage and Serial", "State Metadata"]
  },
  {
    id: "hashicorp-tfa-153",
    difficulty: "easy",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "Terraform Remote State Data Source (terraform_remote_state)",
    scenario: "A microservices deployment team needs to read the VPC ID and private subnet IDs created by a separate infrastructure repository.",
    question: "Which data source reads output values directly from another project's remote state file?",
    options: [
      { id: 'A', text: "data \"terraform_remote_state\"" },
      { id: 'B', text: "data \"terraform_state_reader\"" },
      { id: 'C', text: "data \"remote_output\"" },
      { id: 'D', text: "data \"cross_project_state\"" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "`terraform_remote_state` retrieves the root module output values from another Terraform configuration's state file via its remote backend (e.g. S3, GCS, or HCP Terraform), allowing architectural decoupling across teams.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/state/remote-state-data",
    tags: ["Terraform State", "terraform_remote_state", "Cross-Project Architecture"]
  },
  {
    id: "hashicorp-tfa-154",
    difficulty: "medium",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "State Locking: Concurrency Protection Mechanics",
    scenario: "Two engineers simultaneously run `terraform apply` on the same infrastructure code using an AWS S3 backend with DynamoDB locking.",
    question: "What happens to the second engineer's execution?",
    options: [
      { id: 'A', text: "The second apply overwrites the first apply's state silently" },
      { id: 'B', text: "It halts with a state lock error naming the lock's id and owner" },
      { id: 'C', text: "The cloud provider pauses all running virtual machines" },
      { id: 'D', text: "Both applies run in parallel, merging their changes into state" }
    ],
    correctAnswers: ['B'],
    type: "single",
    explanation: "`State locking` prevents multiple concurrent executions from corrupting the state file. When an operation starts, Terraform acquires an exclusive lock. Any concurrent process attempting to acquire the lock is rejected with an error message detailing the active lock owner.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/state/locking",
    tags: ["Terraform State", "State Locking", "Concurrency"]
  },
  {
    id: "hashicorp-tfa-155",
    difficulty: "easy",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "Backends Supporting Native State Locking",
    scenario: "A platform architect evaluates remote backends that natively support state locking without requiring third-party plugins.",
    question: "Which group of backends natively supports state locking?",
    options: [
      { id: 'A', text: "Local backend on NFS mounts" },
      { id: 'B', text: "S3, GCS, Azure Blob, and HCP Terraform" },
      { id: 'C', text: "Local backend with the state committed to Git" },
      { id: 'D', text: "Raw HTTP backend without locking endpoints" }
    ],
    correctAnswers: ['B'],
    type: "single",
    explanation: "Native state locking is supported by `gcs`, `azurerm` (blob leases), `s3` (`use_lockfile = true` since Terraform 1.10; the older `dynamodb_table` option is deprecated), `consul`, and HCP Terraform. The local backend only locks on a local filesystem, and committing state to Git gives no locking at all.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/settings/backends/configuration#available-backends",
    tags: ["Terraform Backends", "Supported Locking", "Architecture"]
  },
  {
    id: "hashicorp-tfa-156",
    difficulty: "medium",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "State Migration Workflow: Local to Remote Backend",
    scenario: "A developer begins a project with local state. When collaborating with colleagues, the developer adds an `s3` backend block to `main.tf`.",
    question: "Which command migrates the existing local state into the remote S3 bucket?",
    options: [
      { id: 'A', text: "terraform init -migrate-state" },
      { id: 'B', text: "terraform state push -force" },
      { id: 'C', text: "terraform state upload" },
      { id: 'D', text: "terraform apply -remote" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "When changing backend configurations, running `terraform init` detects the change. Passing `-migrate-state` (or confirming 'yes' interactively) automatically uploads the existing state to the new remote backend while preserving history and serial numbers.",
    referenceUrl: "https://developer.hashicorp.com/terraform/cli/commands/init#backend-initialization",
    tags: ["Terraform State", "State Migration", "-migrate-state"]
  },
  {
    id: "hashicorp-tfa-157",
    difficulty: "easy",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "Risks of Manual State File Editing",
    scenario: "A developer attempts to resolve a naming conflict by opening `terraform.tfstate` in a text editor (VS Code) and editing resource IDs manually.",
    question: "Why is manual modification of JSON state files strongly discouraged?",
    options: [
      { id: 'A', text: "Cloud providers ban accounts that edit text files" },
      { id: 'B', text: "A manual edit can corrupt the document and desynchronise the serial" },
      { id: 'C', text: "Terraform state files are encrypted with proprietary algorithms that cannot be opened" },
      { id: 'D', text: "Text editors automatically delete .tf files" }
    ],
    correctAnswers: ['B'],
    type: "single",
    explanation: "HashiCorp strongly advises against editing state files by hand. Even small errors in JSON syntax, serial numbers, schema versions, or dependency pointers corrupt the state file. Teams should always use safe CLI commands like `state mv` and `state rm`.",
    referenceUrl: "https://developer.hashicorp.com/terraform/cli/commands/state",
    tags: ["Terraform State", "Manual Edits", "State Safety"]
  },
  {
    id: "hashicorp-tfa-158",
    difficulty: "medium",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "Automatic Local State Backups (terraform.tfstate.backup)",
    scenario: "A developer executes terraform apply using the default local backend.",
    question: "Before writing an updated state file, what safety mechanism does Terraform use to prevent data loss on local backends?",
    options: [
      { id: 'A', text: "It writes the prior state to a backup file" },
      { id: 'B', text: "It emails the state to the user" },
      { id: 'C', text: "It uploads the state to GitHub automatically" },
      { id: 'D', text: "It prompts the user to print the state file on paper" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "For the local backend, whenever Terraform modifies state, it first saves the previous state snapshot to `terraform.tfstate.backup`. If an apply fails or corrupts state, administrators can recover by restoring the backup file.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/state/backends#local",
    tags: ["Terraform State", "State Backup", "Disaster Recovery"]
  },
  {
    id: "hashicorp-tfa-159",
    difficulty: "easy",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "Remote State Security: Encryption at Rest in Amazon S3",
    scenario: "A security architect configures an Amazon S3 bucket for storing Terraform state files.",
    question: "Which two S3 configurations must be enabled to protect state confidentiality and prevent accidental deletion?",
    options: [
      { id: 'A', text: "Public website hosting and a CORS configuration" },
      { id: 'B', text: "Default server-side encryption and bucket versioning" },
      { id: 'C', text: "S3 Glacier Deep Archive and Transfer Acceleration" },
      { id: 'D', text: "Requester pays and S3 Object Lambda access points" }
    ],
    correctAnswers: ['B'],
    type: "single",
    explanation: "Best practices for S3 state storage mandate: enabling `Server-Side Encryption` (protecting plaintext secrets at rest) and `Bucket Versioning` (allowing recovery of older state files if corrupted or accidentally overwritten).",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/settings/backends/s3",
    tags: ["Terraform State", "S3 Backend", "Encryption and Versioning"]
  },
  {
    id: "hashicorp-tfa-160",
    difficulty: "medium",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "Partial State in Failed Deployments",
    scenario: "A `terraform apply` provisions a VPC, subnets, and internet gateway successfully, but fails on the 10th resource (an EC2 instance) due to an invalid AMI ID.",
    question: "What is recorded in the state file when an apply encounters an error midway through execution?",
    options: [
      { id: 'A', text: "The state file is corrupted and deleted" },
      { id: 'B', text: "The state file is rolled back completely to empty" },
      { id: 'C', text: "A partial state holding the nine resources that were created" },
      { id: 'D', text: "None of the resources are recorded" }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "Terraform writes state iteratively. If a run fails partway through, Terraform records `partial state` containing all resources successfully created before the failure. On the next run, Terraform does not recreate the first 9 resources; it only attempts the failed resource.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/state/purpose#metadata",
    tags: ["Terraform State", "Partial State", "Resilience"]
  },
  {
    id: "hashicorp-tfa-161",
    difficulty: "easy",
    certId: "hashicorp-tfa",
    domainId: "d7",
    domainName: "Maintain infrastructure with Terraform",
    title: "Terraform State Push Command (terraform state push)",
    scenario: "An administrator needs to restore a recovered state file (`recovered_state.json`) to a remote backend after resolving a state corruption event.",
    question: "Which command uploads a specific local state file directly to the configured remote backend?",
    options: [
      { id: 'A', text: "terraform state push recovered_state.json" },
      { id: 'B', text: "terraform state upload recovered_state.json" },
      { id: 'C', text: "terraform push state recovered_state.json" },
      { id: 'D', text: "terraform apply -state=recovered_state.json" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "`terraform state push &lt;file&gt;` uploads a local state file directly to the configured remote backend. It validates serial numbers to prevent accidental overwrites unless `-force` is explicitly provided.",
    referenceUrl: "https://developer.hashicorp.com/terraform/cli/commands/state/push",
    tags: ["Terraform CLI", "terraform state push", "Disaster Recovery"]
  },
  {
    id: "hashicorp-tfa-162",
    difficulty: "medium",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "Disaster Recovery: S3 Bucket Versioning State Recovery",
    scenario: "A catastrophic network drop corrupts the remote state file stored in an Amazon S3 backend. The S3 bucket has versioning enabled.",
    question: "How can the operations team recover the previous valid state file?",
    options: [
      { id: 'A', text: "The state file is permanently destroyed" },
      { id: 'B', text: "By contacting HashiCorp support to generate a new state file" },
      { id: 'C', text: "By running terraform uncorrupt" },
      { id: 'D', text: "By restoring the prior version of the object from the bucket" }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "With `S3 Bucket Versioning` enabled, every state modification writes a new object version. If state is corrupted, administrators simply restore the immediate previous object version from S3, restoring complete infrastructure tracking instantaneously.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/settings/backends/s3",
    tags: ["Terraform State", "S3 Versioning", "Disaster Recovery"]
  },
  {
    id: "hashicorp-tfa-163",
    difficulty: "easy",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "Terraform Backend Configuration Blocks Cannot Access Named Values",
    scenario: "A developer attempts to dynamically configure the S3 backend bucket name using a variable: `bucket = var.state_bucket_name` inside `terraform { backend \"s3\" { ... } }`.",
    question: "Why does terraform init fail with: 'Variables not allowed'?",
    options: [
      { id: 'A', text: "The backend block accepts only literal string values" },
      { id: 'B', text: "Variables resolve only inside resource and output blocks" },
      { id: 'C', text: "The bucket name must be a literal for the lock to work" },
      { id: 'D', text: "The backend is initialised before variables are parsed" }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Backend initialization occurs at the very beginning of the Terraform lifecycle, before variables or functions are evaluated. Consequently, `backend` blocks cannot interpolate `var.*`, `local.*`, or any dynamic HCL expressions.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/settings/backends/configuration#partial-configuration",
    tags: ["Terraform Backends", "Backend Variables", "Partial Configuration"]
  },
  {
    id: "hashicorp-tfa-164",
    difficulty: "medium",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "Partial Backend Configuration (-backend-config)",
    scenario: "To keep backend configuration dynamic across environments without hardcoding bucket names, a developer omits the `bucket` argument from `main.tf`.",
    question: "How can backend parameters be supplied dynamically during terraform init?",
    options: [
      { id: 'A', text: "By passing `-backend-config` flags or a file to `terraform init`" },
      { id: 'B', text: "By setting TF_VAR_backend_bucket before running init" },
      { id: 'C', text: "By passing -var=\"bucket=my-bucket\" to terraform init" },
      { id: 'D', text: "By writing the bucket name into the dependency lock file" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "`Partial configuration` allows omitting backend attributes from `.tf` files. The missing parameters are supplied at `init` time via `-backend-config=\"key=value\"`, `-backend-config=path.hcl`, or environment variables, keeping configurations portable.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/settings/backends/configuration#partial-configuration",
    tags: ["Terraform Backends", "-backend-config", "Partial Configuration"]
  },
  {
    id: "hashicorp-tfa-165",
    difficulty: "easy",
    certId: "hashicorp-tfa",
    domainId: "d8",
    domainName: "HCP Terraform",
    title: "HCP Terraform/Enterprise cloud Block (Terraform 1.1+)",
    scenario: "A platform team configures remote execution and state management with HCP Terraform.",
    question: "Starting in Terraform v1.1, which dedicated configuration block simplifies connecting a configuration to HCP Terraform or Enterprise workspaces?",
    options: [
      { id: 'A', text: "terraform { remote { host = \"app.terraform.io\" workspace = \"my-app\" } }" },
      { id: 'B', text: "terraform { cloud { organization = \"my-org\" workspaces { name = \"my-app\" } } }" },
      { id: 'C', text: "terraform { tfe { organization = \"my-org\" workspace = \"my-app\" } }" },
      { id: 'D', text: "terraform { hashicorp { cloud = true workspace = \"my-app\" } }" }
    ],
    correctAnswers: ['B'],
    type: "single",
    explanation: "The `cloud` block (introduced in Terraform 1.1) is the modern successor to the `remote` backend. It cleanly declares the HCP Terraform `organization`, `workspaces` (single workspace or tags), and optional custom `hostname` for Terraform Enterprise.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/settings/terraform-cloud",
    tags: ["Terraform 1.1+", "cloud Block", "HCP Terraform"]
  },
  {
    id: "hashicorp-tfa-166",
    difficulty: "medium",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "State File Resource Metadata: schema_version Attribute",
    scenario: "A developer inspects the raw JSON state file and notices that each resource has a `schema_version` number.",
    question: "What is the purpose of schema_version inside the state file?",
    options: [
      { id: 'A', text: "It records the provider schema version, so the provider can upgrade the entry" },
      { id: 'B', text: "It tracks how many times the resource was updated in the cloud" },
      { id: 'C', text: "It tracks the user's IAM permission tier" },
      { id: 'D', text: "It represents the operating system patch level of the VM" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Providers define schema versions for resources. When a provider updates its internal attribute schema, it uses `schema_version` in state to run automated upgrade logic (state migrations) that transforms older state schemas to current schemas seamlessly.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/state",
    tags: ["Terraform State", "schema_version", "State Migration"]
  },
  {
    id: "hashicorp-tfa-167",
    difficulty: "easy",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "Terraform State Storage: Google Cloud Storage (GCS Backend)",
    scenario: "An enterprise deploys workloads on Google Cloud Platform.",
    question: "Which backend block configures state storage in a Google Cloud Storage bucket with native locking?",
    options: [
      { id: 'A', text: "terraform { backend \"gcs\" { bucket = \"my-tf-state-bucket\" prefix = \"terraform/state\" } }" },
      { id: 'B', text: "terraform { backend \"google\" { storage_bucket = \"my-tf-state\" prefix = \"state\" } }" },
      { id: 'C', text: "terraform { backend \"cloud_storage\" { bucket = \"my-tf-state\" path = \"state\" } }" },
      { id: 'D', text: "terraform { backend \"gcp\" { bucket = \"my-tf-state\" prefix = \"state\" } }" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "The standard GCP backend is named `gcs`. It requires a `bucket` name and an optional `prefix` path. GCS natively supports object locking using preconditions, providing distributed state locking without external databases.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/settings/backends/gcs",
    tags: ["Terraform Backends", "GCS Backend", "Google Cloud"]
  },
  {
    id: "hashicorp-tfa-168",
    difficulty: "medium",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "Terraform State Storage: Azure Blob Storage (azurerm Backend)",
    scenario: "A team standardizes on Microsoft Azure infrastructure.",
    question: "Which backend block configures state storage in Azure Blob Storage with automated blob lease locking?",
    options: [
      { id: 'A', text: "A `backend \"azurerm\"` block with the account, container and key" },
      { id: 'B', text: "terraform { backend \"azure\" { blob_container = \"tfstate\" } }" },
      { id: 'C', text: "terraform { backend \"microsoft\" { account = \"sa\" } }" },
      { id: 'D', text: "terraform { backend \"blob\" { container = \"tfstate\" } }" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "The `azurerm` backend stores state in Azure Blob Storage within a specified Storage Account and container. Azure Blob Leases provide native, distributed state locking automatically.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/settings/backends/azurerm",
    tags: ["Terraform Backends", "azurerm Backend", "Azure"]
  },
  {
    id: "hashicorp-tfa-169",
    difficulty: "easy",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "Terraform State File Structure: JSON Format",
    scenario: "An engineer writes an automated audit script to parse Terraform state files.",
    question: "In what data serialization format does Terraform write all state files?",
    options: [
      { id: 'A', text: "JSON (JavaScript Object Notation)" },
      { id: 'B', text: "YAML (YAML Ain't Markup Language)" },
      { id: 'C', text: "XML (Extensible Markup Language)" },
      { id: 'D', text: "Protobuf binary format" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Terraform state files (`.tfstate`) are stored in standard `JSON` format. The JSON schema includes root keys like `version`, `terraform_version`, `serial`, `lineage`, `outputs`, and `resources`.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/state",
    tags: ["Terraform State", "JSON Format", "Internals"]
  },
  {
    id: "hashicorp-tfa-170",
    difficulty: "medium",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "State File Lock Identification and Information",
    scenario: "A developer encounters a state lock error when running terraform apply.",
    question: "When a state lock collision occurs, what vital troubleshooting information does Terraform display to the blocked user?",
    options: [
      { id: 'A', text: "The credentials the locking run authenticated with" },
      { id: 'B', text: "The lock id, the operation, who took it, and when it was created" },
      { id: 'C', text: "A generic error code with no further detail in it" },
      { id: 'D', text: "The login password of the user holding the lock" }
    ],
    correctAnswers: ['B'],
    type: "single",
    explanation: "The lock information output contains: `ID` (used with `force-unlock`), `Path`, `Operation` (Apply/Plan), `Who` (username@hostname), `Version`, and `Created` timestamp, making it easy to identify the colleague or CI runner holding the lock.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/state/locking",
    tags: ["Terraform State", "Lock Info", "Troubleshooting"]
  },
  {
    id: "hashicorp-tfa-171",
    difficulty: "easy",
    certId: "hashicorp-tfa",
    domainId: "d6",
    domainName: "Terraform state management",
    title: "Handling State Drift: Out-of-Band Tag Updates",
    scenario: "An operations engineer adds an environment tag (`CostCenter = 1234`) to an EC2 instance directly in the AWS Console. The Terraform configuration file does not declare this tag.",
    question: "What occurs when terraform plan is executed?",
    options: [
      { id: 'A', text: "It refreshes, sees the tag, and plans an in-place update to remove it" },
      { id: 'B', text: "Terraform ignores the tag difference completely" },
      { id: 'C', text: "Terraform crashes and refuses to plan" },
      { id: 'D', text: "Terraform automatically copies the tag back into main.tf for you" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "During the refresh phase, Terraform discovers the out-of-band tag. Because the declared HCL configuration does not include the tag, Terraform plans an in-place update (`~`) to strip the tag from the cloud resource, maintaining code as the desired state.",
    referenceUrl: "https://developer.hashicorp.com/terraform/tutorials/state/drift",
    tags: ["Terraform State", "Drift Remediation", "In-Place Updates"]
  },
  {
    id: "hashicorp-tfa-172",
    difficulty: "medium",
    certId: "hashicorp-tfa",
    domainId: "d3",
    domainName: "Core Terraform workflow",
    title: "State Replacement Actions: Required Attributes",
    scenario: "A developer modifies the `cidr_block` of an existing `aws_vpc` resource in HCL. AWS APIs do not support changing the primary IPv4 CIDR block of an existing VPC in-place.",
    question: "How does Terraform plan this change in the CLI output?",
    options: [
      { id: 'A', text: "Terraform rejects the plan with an API error" },
      { id: 'B', text: "+ create only" },
      { id: 'C', text: "-/+ replace (destroy and then create replacement)" },
      { id: 'D', text: "~ update in-place" }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "When a modified attribute cannot be updated in-place due to cloud API constraints (known as `ForceNew` in provider schemas), Terraform plans a `- / + replace` action: destroying the existing resource and creating a brand-new replacement instance.",
    referenceUrl: "https://developer.hashicorp.com/terraform/cli/commands/plan#resource-actions",
    tags: ["Terraform Plan", "Resource Replacement", "ForceNew"]
  },
  {
    id: "hashicorp-tfa-173",
    difficulty: "easy",
    certId: "hashicorp-tfa",
    domainId: "d4",
    domainName: "Terraform configuration",
    title: "create_before_destroy Lifecycle Rule to Prevent Outages",
    scenario: "A web server is replaced due to an AMI update. Under standard behavior, Terraform destroys the old web server first, causing 5 minutes of downtime before the new server is ready.",
    question: "Which lifecycle meta-argument instructs Terraform to provision the replacement instance before terminating the old instance?",
    options: [
      { id: 'A', text: "lifecycle { create_before_destroy = true }" },
      { id: 'B', text: "lifecycle { rolling_update = true }" },
      { id: 'C', text: "lifecycle { replace_first = true }" },
      { id: 'D', text: "lifecycle { zero_downtime = true }" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Setting `create_before_destroy = true` in a resource's `lifecycle` block inverts the replacement order: Terraform provisions the new replacement resource first, verifies creation, and only then tears down the old resource, preventing service outages.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/meta-arguments/lifecycle#create_before_destroy",
    tags: ["Terraform Lifecycle", "create_before_destroy", "Zero Downtime"]
  },
  {
    id: "hashicorp-tfa-174",
    difficulty: "medium",
    certId: "hashicorp-tfa",
    domainId: "d4",
    domainName: "Terraform configuration",
    title: "prevent_destroy Lifecycle Rule for Mission-Critical Resources",
    scenario: "A developer accidentally runs `terraform destroy` or changes a ForceNew attribute on a mission-critical production database.",
    question: "Which lifecycle meta-argument acts as a safety latch that causes Terraform to immediately reject any plan attempting to destroy the resource?",
    options: [
      { id: 'A', text: "lifecycle { prevent_destroy = true }" },
      { id: 'B', text: "lifecycle { no_destroy = true }" },
      { id: 'C', text: "lifecycle { protect = true }" },
      { id: 'D', text: "lifecycle { immutable = true }" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "`prevent_destroy = true` causes Terraform to throw an immediate error and reject any plan (or `terraform destroy`) that would result in the destruction of the resource, preventing catastrophic accidental data loss.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/meta-arguments/lifecycle#prevent_destroy",
    tags: ["Terraform Lifecycle", "prevent_destroy", "Safety Latch"]
  },
  {
    id: "hashicorp-tfa-175",
    difficulty: "easy",
    certId: "hashicorp-tfa",
    domainId: "d4",
    domainName: "Terraform configuration",
    title: "ignore_changes Lifecycle Rule for External Autoscaling",
    scenario: "An external Kubernetes Horizontal Pod Autoscaler (HPA) or AWS Auto Scaling group dynamically adjusts the `desired_capacity` of a cluster between 5 and 50 nodes. Every time Terraform runs, it attempts to reset the capacity back to the hardcoded `desired_capacity = 5` in HCL.",
    question: "Which lifecycle rule instructs Terraform to ignore differences in specific attributes during planning?",
    options: [
      { id: 'A', text: "lifecycle { ignore_changes = [desired_capacity] }" },
      { id: 'B', text: "lifecycle { skip = [desired_capacity] }" },
      { id: 'C', text: "lifecycle { dynamic = [desired_capacity] }" },
      { id: 'D', text: "lifecycle { unmanaged = [desired_capacity] }" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "The `ignore_changes` lifecycle argument accepts a list of attributes that Terraform should ignore when comparing desired state against live infrastructure, allowing external autoscalers to adjust values without Terraform reverting them.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/meta-arguments/lifecycle#ignore_changes",
    tags: ["Terraform Lifecycle", "ignore_changes", "Autoscaling"]
  }
];

export default HASHICORP_TFA_QUESTIONS_7;
