export const HASHICORP_TFA_QUESTIONS_2 = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "on_destroy = true" },
      { id: 'C', text: "action = delete" },
      { id: 'D', text: "type = destroy" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Official Providers" },
      { id: 'B', text: "Partner Providers" },
      { id: 'C', text: "Community Providers" },
      { id: 'D', text: "Enterprise Providers" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "waitFor = [aws_iam_role_policy_attachment.app]" },
      { id: 'B', text: "after = [aws_iam_role_policy_attachment.app]" },
      { id: 'C', text: "depends_on = [aws_iam_role_policy_attachment.app]" },
      { id: 'D', text: "before = [aws_iam_role_policy_attachment.app]" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    title: "HCP Terraform: Remote Execution vs Local CLI Execution",
    scenario: "An organization adopts HCP Terraform, and developers execute commands from their local workstations.",
    ...
    options: [
      ...
      { id: 'C', text: "Remotely in a secure, isolated container runner managed by HCP Terraform" },
      { id: 'D', text: "Locally on the developer's own laptop, using its memory and credentials" }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "In standard HCP Terraform remote execution mode, the local CLI acts as a thin client: it bundles the local configuration files, transmits them to HCP Terraform, and executes `plan` and `apply` inside a remote container runner with centralized environment variables and state locking.",
    referenceUrl: "https://developer.hashicorp.com/terraform/cloud-docs/run",
    tags: ["HCP Terraform", "Remote Execution", "Enterprise"]
  },
  {
    id: "hashicorp-tfa-39",
    difficulty: "hard",
    ...
    domainId: "d8",
    domainName: "HCP Terraform",
    title: "HCP Terraform Private Module Registry",
    ...
    question: "Which HCP Terraform feature hosts private, version-controlled reusable modules for organizational members?",
    options: [
      { id: 'A', text: "HCP Terraform Private Module Registry" },
      { id: 'B', text: "Public Terraform Registry" },
      { id: 'C', text: "GitHub Releases public bucket" },
      { id: 'D', text: "Local filesystem folders alone" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "`HCP Terraform Private Module Registry` provides a secure, centralized repository for sharing vetted, versioned infrastructure modules within an organization, supporting semantic versioning, automated documentation rendering, and access controls.",
    referenceUrl: "https://developer.hashicorp.com/terraform/cloud-docs/registry",
    tags: ["HCP Terraform", "Private Registry", "Module Governance"]
  },
  {
    id: "hashicorp-tfa-40",
    difficulty: "medium",
    ...
    domainId: "d8",
    domainName: "HCP Terraform",
    title: "HCP Terraform Workspaces vs CLI Workspaces",
    scenario: "An engineer familiar with open-source CLI workspaces transitions to HCP Terraform.",
    question: "In HCP Terraform, what does a Workspace represent?",
    options: [
    ...
    correctAnswers: ['A'],
    type: "single",
    explanation: "In HCP Terraform, a `Workspace` is a comprehensive operational unit. Unlike CLI workspaces that only isolate state files, an HCP Terraform workspace encapsulates VCS repository links, environment variables, cloud credentials, execution history, and team RBAC.",
    referenceUrl: "https://developer.hashicorp.com/terraform/cloud-docs/workspaces",
    tags: ["HCP Terraform", "Workspaces", "Governance"]
  },
  {
    id: "hashicorp-tfa-41",
    difficulty: "hard",
    ...
    domainId: "d8",
    domainName: "HCP Terraform",
    title: "VCS-Driven Workflow in HCP Terraform",
    scenario: "A company links an HCP Terraform workspace to a GitHub repository's main branch.",
    ...
    options: [
      ...
      { id: 'B', text: "HCP Terraform immediately provisions all resources to production" },
      { id: 'C', text: "HCP Terraform deletes the branch" },
      { id: 'D', text: "HCP Terraform merges the pull request without human review" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "In the `VCS-driven workflow`, opening a pull request automatically triggers a `speculative plan`. HCP Terraform plans the changes against live state and posts the plan summary as a commit status check on GitHub, enabling peer review before merging.",
    referenceUrl: "https://developer.hashicorp.com/terraform/cloud-docs/vcs",
    tags: ["HCP Terraform", "VCS Workflow", "GitOps"]
  },
  {
    id: "hashicorp-tfa-42",
    difficulty: "medium",
    ...
    domainId: "d8",
    domainName: "HCP Terraform",
    title: "Speculative Plans in HCP Terraform",
    scenario: "A speculative plan is triggered on a pull request in HCP Terraform.",
    ...
    correctAnswers: ['A'],
    type: "single",
    explanation: "`Speculative plans` in HCP Terraform are strictly ephemeral, read-only plan outputs. They cannot be applied to live infrastructure. Their sole purpose is providing feedback on the plan output and Sentinel policy compliance prior to merge.",
    referenceUrl: "https://developer.hashicorp.com/terraform/cloud-docs/run/states#speculative-plans",
    tags: ["HCP Terraform", "Speculative Plans", "Safety"]
  },
  {
    id: "hashicorp-tfa-43",
    difficulty: "hard",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "TERRAFORM_CACHE_PATH" },
      { id: 'B', text: "TF_PLUGIN_CACHE_DIR" },
      { id: 'C', text: "TF_GLOBAL_DIR" },
      { id: 'D', text: "TF_PROVIDER_CACHE" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "terraform upgrade --all" },
      { id: 'B', text: "terraform init -upgrade" },
      { id: 'C', text: "terraform refresh" },
      { id: 'D', text: "terraform update" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Through a lock object written beside the state, so the second run is refused" },
    ...
    correctAnswers: ['A'],
    type: "single",
    explanation: "Since Terraform 1.10 the `s3` backend locks state natively: with `use_lockfile = true` it writes a `.tflock` object next to the state using S3 conditional writes, and a second run that cannot acquire it fails with a lock error. The older `dynamodb_table` locking still works but has been deprecated since Terraform 1.11.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/settings/backends/s3#dynamodb-state-locking",
    tags: ["Terraform", "State Locking", "S3 Native Locking"]
  },
  {
    id: "hashicorp-tfa-49",
    difficulty: "hard",
  ...
  },
  {
    ...
    domainId: "d6",
    domainName: "Terraform state management",
    title: "The cloud Block vs a State Backend",
    scenario: "A cloud platform architect compares a `backend \"s3\"` block with a `cloud` block that points at HCP Terraform.",
    question: "What is the main functional difference between them?",
    options: [
      { id: 'A', text: "The s3 backend runs in memory; the cloud block runs on disk" },
      { id: 'B', text: "The s3 backend cannot lock state at all" },
      { id: 'C', text: "The s3 backend is free; the cloud block needs an Enterprise license" },
      { id: 'D', text: "A backend stores and locks state; the cloud block can also run operations remotely" }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "A `backend` block (such as `s3`, `gcs` or `azurerm`) only tells Terraform where to store and lock state; plan and apply still run on the local machine. The `cloud` block integrates the working directory with HCP Terraform or Terraform Enterprise, which stores state and can execute runs remotely with policy checks. HCP Terraform has a free tier, and the S3 backend supports locking. Older documentation called these standard and enhanced backends; the `remote` backend is the legacy form of the `cloud` block.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/settings/backends/configuration#types",
    tags: ["Terraform", "Backends", "cloud Block"]
  }
];
