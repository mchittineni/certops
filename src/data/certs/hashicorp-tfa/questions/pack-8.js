export const HASHICORP_TFA_QUESTIONS_8 = [
  {
    ...
    options: [
      { id: 'A', text: "lifecycle { ignore_changes = all }" },
      { id: 'B', text: "lifecycle { ignore_changes = [\"*\"] }" },
      { id: 'C', text: "lifecycle { freeze = true }" },
      { id: 'D', text: "lifecycle { static = true }" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Consul Key-Value sessions and locks" },
      { id: 'B', text: "External DynamoDB tables" },
      { id: 'C', text: "Consul Connect service mesh" },
      { id: 'D', text: "Consul Raft log rotation" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "http backend" },
      { id: 'B', text: "custom backend" },
      { id: 'C', text: "rest backend" },
      { id: 'D', text: "api backend" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A smaller blast radius, and faster plan and apply runs" },
      { id: 'B', text: "Cloud providers discount infrastructure costs by 50%" },
      { id: 'C', text: "Eliminates the need for writing HCL code" },
      { id: 'D', text: "Automates cloud provider account creation" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Because state holds every value in clear, including the secrets" },
      { id: 'B', text: "Because remote state files can execute arbitrary shell scripts" },
      { id: 'C', text: "Because reading state deletes the remote resources" },
      { id: 'D', text: "Remote state data sources do not access state files" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    scenario: "An engineer runs `terraform taint aws_instance.app`, a command deprecated since Terraform 0.15.2.",
    question: "What does this command modify?",
    options: [
      { id: 'A', text: "It marks the instance tainted in state, leaving the real resource alone" },
      { id: 'B', text: "It terminates the cloud virtual machine there and then, before any apply" },
      { id: 'C', text: "It deletes the resource block from main.tf" },
      { id: 'D', text: "It corrupts the dependency lock file" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "`terraform taint` modifies only the state file: it flags the resource record as tainted, and the next `terraform apply` destroys and recreates it. HashiCorp now recommends `terraform apply -replace=\"aws_instance.app\"`, which shows the replacement in a reviewable plan instead of changing state ahead of time.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "-lock-timeout=30s on the command" },
      { id: 'B', text: "-wait-lock=30s on the command" },
      { id: 'C', text: "-retry-lock=30s on the command" },
      { id: 'D', text: "-lock-retry=3 on the command" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Through resource addresses of type, name and an index or key" },
      { id: 'B', text: "Through random cloud UUIDs assigned by AWS" },
      { id: 'C', text: "Through the line number where the resource is defined in main.tf" },
      { id: 'D', text: "By the file creation date" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "With `moved` blocks in the code, or `terraform state mv`" },
      { id: 'B', text: "Running terraform apply -force" },
      { id: 'C', text: "Deleting the state file and running init" },
      { id: 'D', text: "Renaming the cloud resources in the cloud web console" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The bucket is still in state, so comparing state with the code reveals it" },
      { id: 'B', text: "Terraform searches Git commit history" },
      { id: 'C', text: "Cloud providers alert Terraform via webhooks" },
      { id: 'D', text: "Terraform cannot know; the bucket remains orphaned forever" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    domainId: "d6",
    domainName: "Terraform state management",
    title: "State Compatibility Across Terraform 1.x Releases",
    ...
    question: "What does the Terraform v1.x compatibility promise say about this?",
    options: [
      { id: 'A', text: "Downgrading is not guaranteed; newer 1.x state may use features 1.0.0 cannot read" },
      { id: 'B', text: "Terraform converts the state to the 1.0.0 format automatically on first read" },
      { id: 'C', text: "Every 1.x release is guaranteed to read and write state from every other 1.x release" },
      { id: 'D', text: "Terraform discards the state and rebuilds it from the provider APIs" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "HashiCorp promises that you can upgrade from any v1.x release to a later one without special steps. Downgrading to an earlier v1.x release might work but is not guaranteed, because later releases can introduce features, including new state storage formats, that older releases cannot understand. Terraform never silently converts or discards state; keep the team on the version recorded by the last apply or newer.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Keep the state in a central security account, or isolate it per environment" },
      { id: 'B', text: "Keep one state file for all fifty accounts in a single bucket" },
      { id: 'C', text: "Keep each account's state on the engineer's own workstation" },
      { id: 'D', text: "Keep one state per account with no separation by environment" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A standard plan modifies the cloud resources directly, whereas refresh-only does not touch them." },
      { id: 'B', text: "There is no difference beyond the wording the command prints at the end of the run." },
      { id: 'C', text: "A standard plan proposes changes to the infrastructure; refresh-only proposes only state updates." },
      { id: 'D', text: "A standard plan never queries the provider, whereas refresh-only calls its read APIs." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "A standard `terraform plan` queries APIs to refresh state and then calculates actions needed to bring cloud infrastructure into alignment with declared HCL code. A refresh-only plan only proposes updating the state file to match cloud reality; nothing is written until `terraform apply -refresh-only` is approved, and infrastructure is never changed.",
    referenceUrl: "https://developer.hashicorp.com/terraform/cli/commands/plan#refresh-only",
  ...
  },
  {
    ...
    domainId: "d6",
    domainName: "Terraform state management",
    title: "State Locking: S3 Backend Native Lockfile",
    scenario: "An administrator configures a new S3 backend on Terraform 1.12 and wants state locking without provisioning a separate DynamoDB table.",
    question: "Which S3 backend argument enables that locking?",
    options: [
      { id: 'A', text: "`use_lockfile = true`" },
      { id: 'B', text: "`encrypt = true`" },
      { id: 'C', text: "`use_path_style = true`" },
      { id: 'D', text: "`lock_table = \"terraform-locks\"`" }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Since Terraform 1.10 the S3 backend can lock state natively: `use_lockfile = true` makes Terraform write a `.tflock` object beside the state file using S3 conditional writes. The DynamoDB-based `dynamodb_table` argument is deprecated. `encrypt` controls server-side encryption, `use_path_style` changes the request URL format, and there is no `lock_table` argument.",
    referenceUrl: "https://developer.hashicorp.com/terraform/language/settings/backends/s3#dynamodb_table",
    tags: ["Terraform Backends", "S3 Native Locking", "use_lockfile"]
  },
  {
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "State caches the attributes, so the graph is computed locally without thousands of API calls." },
      { id: 'B', text: "Terraform ignores the provider's rate limits and retries the calls until they succeed." },
      { id: 'C', text: "Terraform batches its reads into one call per resource type at the start of a run." },
      { id: 'D', text: "Terraform bypasses the provider APIs and reads the cloud's own inventory service." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "It asks the operator interactively whether the file changed" },
      { id: 'B', text: "It compares the file's modification time with the last run" },
      { id: 'C', text: "It re-uploads the file on every apply, changed or not" },
      { id: 'D', text: "It hashes the file, compares it with the stored etag, and plans an update" }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "State holds secrets in clear, and git offers no locking between writers" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "The backend's distributed state locking mechanism" },
