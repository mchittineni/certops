export const HASHICORP_TFA_FLASHCARDS_8 = [
  ...
  },
  {
    ...
    back: "Tainting a resource flags it as <strong>tainted in state</strong>, so the next apply destroys and recreates it. <code>taint</code> is deprecated; use <code>terraform apply -replace=ADDR</code> instead.",
  ...
  },
  {
    ...
    certId: "hashicorp-tfa",
    domainId: "d6",
    front: "Terraform State: 1.x Version Compatibility",
    hint: "Upgrades are promised; downgrades are not.",
    back: "Any later v1.x release can use state from an earlier one. Going back to an <strong>older</strong> v1.x release is <strong>not guaranteed</strong> to work, because newer releases may write state features the older CLI cannot read.",
  ...
  },
  {
    ...
    certId: "hashicorp-tfa",
    domainId: "d6",
    front: "S3 Backend: Native State Locking",
    hint: "One boolean, no DynamoDB table.",
    back: "Set <code>use_lockfile = true</code> in the S3 backend (Terraform 1.10+) to lock state with a <code>.tflock</code> object in the bucket. The older DynamoDB table (<code>LockID</code> key) approach is deprecated.",
