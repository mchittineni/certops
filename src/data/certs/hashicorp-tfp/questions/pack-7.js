export const HASHICORP_TFP_QUESTIONS_7 = [
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Pass the provider explicitly to the grandchild with its own providers argument, because implicit inheritance only carries default provider configurations." },
      { id: 'B', text: "Nothing - aliased providers are inherited automatically at every level." },
      { id: 'C', text: "Define a new provider block in the grandchild." },
      { id: 'D', text: "Use depends_on to link the provider." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The private registry publication log." },
      { id: 'B', text: "The state file history alone." },
      { id: 'C', text: "The organisation audit trail, streamed or exported for retention, complemented by per-workspace run history." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Use timestamp() in the name to avoid collisions." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Fork the module for production." },
      { id: 'B', text: "Pass the production value as an argument in the module block of the production root configuration." },
      { id: 'C', text: "Edit the default inside the module for production." },
      { id: 'D', text: "Set an environment variable named after the module input." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "TF_LOG_PROVIDER, which sets the level for provider logs independently of TF_LOG_CORE." },
      { id: 'B', text: "TF_DEBUG_PROVIDER=1" },
      { id: 'C', text: "TF_LOG_PATH=provider.log, which filters by component." },
      { id: 'D', text: "TF_LOG=PROVIDER" }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Marking the test file with dry_run = true." },
      { id: 'B', text: "Setting parallelism to zero in the test file." },
      { id: 'C', text: "Mock providers together with run blocks using command = plan, so resources are never really created." },
      { id: 'D', text: "Running terraform validate instead, which evaluates outputs." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The command always succeeds and state is archived." },
      { id: 'C', text: "The workspace is deleted and its state merged into default." },
      { id: 'D', text: "Terraform refuses unless the state is empty, because deleting it would orphan the managed resources; -force overrides at that cost." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The plan is identical but cached." },
      { id: 'B', text: "State is never written again." },
      { id: 'C', text: "The plan refuses to run without a saved plan file." },
      { id: 'D', text: "The plan is faster because provider reads are skipped, but it is computed against possibly stale state and will not show drift." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Publish the version and let consumers discover issues." },
      { id: 'B', text: "Bump every consumer at once so problems are found quickly." },
      { id: 'C', text: "Skip testing because semantic versioning guarantees compatibility." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Increase the backend timeout and continue." },
      { id: 'B', text: "Split the configuration into several states along ownership and lifecycle boundaries, and remove resources that no longer need Terraform management." },
      { id: 'C', text: "Switch to the local backend, which has no size limit." },
      { id: 'D', text: "Compress the state file in the backend." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Both as environment variables." },
      { id: 'B', text: "The credential in the configuration provider block and the size as an environment variable." },
      { id: 'C', text: "The instance size as a Terraform variable and the credential as an environment variable marked sensitive." },
      { id: 'D', text: "Both as Terraform variables." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Fail the pipeline and block the apply, because an unavailable check must not be treated as a pass." },
      { id: 'B', text: "Retry indefinitely until the check responds." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The working directory is pointed at the wrong state - a wrong backend key, workspace, or cloud workspace - so stop, confirm the backend and workspace, and inspect state before running anything." },
      { id: 'B', text: "Destroy always includes every resource in the account." },
      { id: 'C', text: "The plan is showing resources from data sources." },
      { id: 'D', text: "Terraform destroys resources by tag, so the tags overlap." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Only a required_version constraint, leaving providers undeclared." },
      { id: 'B', text: "Exact version pins for both providers and full provider configuration blocks." },
      { id: 'C', text: "A backend block plus required_providers." },
