export const HASHICORP_TFP_QUESTIONS_7 = [
  {
    ...
    options: [
      { id: 'A', text: "Use ${{ var.name }} instead." },
      { id: 'B', text: "Interpolation is required for all variable references." },
      { id: 'C', text: "Wrap it in the tostring function." },
      { id: 'D', text: "Write var.name directly; wrapping a single reference in a template string is redundant and terraform fmt style guidance discourages it." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Use depends_on to link the provider." },
      { id: 'D', text: "Pass the provider explicitly to the grandchild with its own providers argument, because implicit inheritance only carries default provider configurations." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "terraform state show module.network" },
      { id: 'B', text: "terraform show module.network" },
      { id: 'C', text: "terraform state list --filter=module.network" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Disable state locking to speed the run up." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Run terraform init to rebuild the reference index." },
      { id: 'B', text: "Every reference to the removed address must be updated or removed too, since Terraform resolves references statically before planning." },
      { id: 'C', text: "Run terraform refresh to drop stale references." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Provider debug logs." },
      { id: 'B', text: "The private registry publication log." },
      { id: 'C', text: "The state file history alone." },
      { id: 'D', text: "The organisation audit trail, streamed or exported for retention, complemented by per-workspace run history." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Concatenate the parts inline in every resource and hope they fit." },
      { id: 'B', text: "Use timestamp() in the name to avoid collisions." },
      { id: 'C', text: "Use uuid() so uniqueness is guaranteed." },
      { id: 'D', text: "Compose the name in a local with join and lower, truncate with substr, and add a variable validation asserting the final pattern and length." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Pass the production value as an argument in the module block of the production root configuration." },
      { id: 'B', text: "Set an environment variable named after the module input." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Restore the correct state version from backend versioning or the HCP Terraform state history, verify with terraform plan showing no changes, and investigate how the push was permitted." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Inconsistent tool and provider versions, unaudited use of personal credentials, no shared run history, and no enforced review or policy gate." },
      { id: 'D', text: "Nothing, provided everyone uses the same Terraform version." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "TF_DEBUG_PROVIDER=1" },
      { id: 'D', text: "TF_LOG_PROVIDER, which sets the level for provider logs independently of TF_LOG_CORE." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Declare it with required = true." },
      { id: 'B', text: "Declare it as sensitive, which forces the caller to supply it." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Mock providers together with run blocks using command = plan, so resources are never really created." },
      { id: 'B', text: "Marking the test file with dry_run = true." },
      { id: 'C', text: "Running terraform validate instead, which evaluates outputs." },
      { id: 'D', text: "Setting parallelism to zero in the test file." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Terraform refuses unless the state is empty, because deleting it would orphan the managed resources; -force overrides at that cost." },
      { id: 'B', text: "The workspace is deleted and its state merged into default." },
      { id: 'C', text: "The workspace and all its resources are destroyed." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The plan is faster because provider reads are skipped, but it is computed against possibly stale state and will not show drift." },
      { id: 'B', text: "The plan is identical but cached." },
      { id: 'C', text: "The plan refuses to run without a saved plan file." },
      { id: 'D', text: "State is never written again." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Use count with twenty conditional expressions." },
      ...
      { id: 'C', text: "Wrap the twenty blocks in a module and call it once." },
      { id: 'D', text: "Generate the HCL with a shell script before each run." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The plan file caches credentials that then expire." },
      { id: 'B', text: "Plan mostly performs read operations, so an identity with read but not write permissions can plan successfully and only fail when the write is attempted." },
      { id: 'C', text: "Apply uses a different provider version." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Skip testing because semantic versioning guarantees compatibility." },
      { id: 'B', text: "Publish the version and let consumers discover issues." },
      { id: 'C', text: "Bump every consumer at once so problems are found quickly." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Split the configuration into several states along ownership and lifecycle boundaries, and remove resources that no longer need Terraform management." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The instance size as a Terraform variable and the credential as an environment variable marked sensitive." },
      { id: 'B', text: "The credential in the configuration provider block and the size as an environment variable." },
      { id: 'C', text: "Both as Terraform variables." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Use both forms together for redundancy." },
      { id: 'B', text: "It makes no difference to behaviour." },
      { id: 'C', text: "Use inline blocks, because they plan faster." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Skip the check and continue, since the plan was already reviewed." },
      { id: 'C', text: "Apply first and evaluate the policy afterwards." },
      { id: 'D', text: "Fail the pipeline and block the apply, because an unavailable check must not be treated as a pass." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Destroy always includes every resource in the account." },
      { id: 'B', text: "Terraform destroys resources by tag, so the tags overlap." },
      { id: 'C', text: "The working directory is pointed at the wrong state - a wrong backend key, workspace, or cloud workspace - so stop, confirm the backend and workspace, and inspect state before running anything." },
      { id: 'D', text: "The plan is showing resources from data sources." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "required_providers entries with source addresses and permissive lower-bound constraints, and no provider configuration blocks." },
      ...
      { id: 'D', text: "Exact version pins for both providers and full provider configuration blocks." }
    ],
    correctAnswers: ['A'],
    type: "single",
