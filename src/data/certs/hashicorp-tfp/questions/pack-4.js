export const HASHICORP_TFP_QUESTIONS_4 = [
  {
    ...
    options: [
      ...
      { id: 'C', text: "merge(local.default_tags, var.tags)" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Move every resource into one large module." },
      { id: 'C', text: "Keep one state file but always use -target." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The directory in which Terraform is run - its .tf files form the root module, and any module blocks in them call child modules." },
      { id: 'B', text: "The top level of the git repository." },
      { id: 'C', text: "The directory named root or modules/root." },
      { id: 'D', text: "Whichever directory contains the backend block, regardless of where Terraform runs." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Committing the .terraform directory to the repository." },
      { id: 'B', text: "A provider plugin cache directory (TF_PLUGIN_CACHE_DIR) persisted between jobs, or a provider mirror." },
      { id: 'C', text: "Removing version constraints so any cached version is accepted." },
      { id: 'D', text: "Running terraform init -upgrade on every job." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Instance keys appear in plan output and state addresses, which would expose the sensitive value; derive the keys from a non-sensitive source instead." },
      { id: 'B', text: "Wrap the value in nonsensitive() as the standard fix." },
      { id: 'C', text: "Sensitive values are always null at plan time; add a default." },
      { id: 'D', text: "The provider does not support sensitive inputs; upgrade it." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Give the application workspace write access to the networking state." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "terraform output -raw endpoint" },
      { id: 'B', text: "terraform show endpoint" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Keep nesting shallow - typically one or two levels - because each layer adds pass-through variables and obscures where values come from." },
      { id: 'B', text: "Nesting depth has no effect because the graph is flattened." },
      { id: 'C', text: "Terraform forbids more than two levels of nesting." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Run terraform refresh and diff the state files by hand." },
      { id: 'B', text: "Run terraform plan -detailed-exitcode on a schedule and alert when it exits 2, or use HCP Terraform health assessments." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The resource exists in the cloud but not in state." },
      ...
      { id: 'D', text: "The state file has not been refreshed." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The bucket is marked for replacement on the next apply." },
      { id: 'B', text: "Terraform forgets the bucket; the real bucket still exists but is no longer managed, and the next plan would try to create it if the configuration still declares it." },
      { id: 'C', text: "The configuration block is removed from the .tf files." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Fork the module into two permanent copies." },
      { id: 'C', text: "Add the new behaviour behind an optional input with a backwards-compatible default, release it as a minor version, and deprecate the old behaviour before removing it in a major release." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A for_each on the provider block." },
      ...
      { id: 'C', text: "One provider block whose region is a list." },
      { id: 'D', text: "Two required_providers entries for the same provider." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Init, then apply, then plan to verify." },
      { id: 'C', text: "Write, then init, then plan and review, then apply." },
      { id: 'D', text: "Plan, then init, then apply." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Reduce unnecessary data sources and avoid re-reading large collections on every run." },
      { id: 'B', text: "Split the configuration into smaller state files aligned to ownership and rate of change." },
      { id: 'C', text: "Remove the dependency lock file." },
      { id: 'D', text: "Set -parallelism=1 to reduce provider load." }
    ],
    correctAnswers: ['A', 'B'],
    type: "multiple",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "variable \"terraform_version\" { default = \"1.5.0\" }" },
      { id: 'C', text: "terraform { required_version = \">= 1.5.0\" }" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Every attribute of every resource, so nothing is missing." },
      { id: 'B', text: "The identifiers and attributes consumers must reference, such as vpc_id and subnet_ids, described and typed clearly." },
      { id: 'C', text: "The provider credentials used during the run." },
      { id: 'D', text: "The input variables, echoed back." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A run task that generates configuration." },
      { id: 'C', text: "A Sentinel policy that creates resources." },
      { id: 'D', text: "No-code modules published in the private registry, which teams instantiate through the UI into new workspaces." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Disable refresh during apply." },
      ...
      { id: 'D', text: "Use -auto-approve so no human is involved." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Terraform restores the deleted object from state." },
      { id: 'C', text: "Terraform silently removes the resource from the configuration." },
      { id: 'D', text: "Refresh detects the object is gone and the plan proposes to create it again, since the configuration still declares it." }
    ],
    correctAnswers: ['D'],
    type: "single",
