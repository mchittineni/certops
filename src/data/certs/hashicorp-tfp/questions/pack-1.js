export const HASHICORP_TFP_QUESTIONS_1 = [
  {
    ...
    options: [
      ...
      { id: 'B', text: "for_each = toset(var.names), referencing each.value" },
      { id: 'C', text: "A dynamic block over var.names" },
      { id: 'D', text: "count = length(var.names), indexing with count.index" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Apply the rename and accept the replacement." },
      { id: 'C', text: "Run terraform state mv on every workstation and in CI." },
      { id: 'D', text: "Import the instance under the new address and remove the old state entry." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "terraform show followed by terraform apply." },
      { id: 'B', text: "terraform plan then terraform apply -auto-approve in the same job." },
      { id: 'C', text: "terraform plan -out=tfplan, store the artifact, then terraform apply tfplan." },
      { id: 'D', text: "terraform apply -refresh=false twice." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A validation block inside the variable with a condition and an error_message." },
      { id: 'B', text: "A local value that calls the fail function." },
      { id: 'C', text: "Setting the variable type to string and documenting the allowed values." },
      { id: 'D', text: "A precondition block on every resource that uses the variable." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "It prevents reading state while an apply runs, using S3 object versioning." },
      { id: 'C', text: "It encrypts the state file so simultaneous writes are rejected." },
      { id: 'D', text: "It prevents concurrent writes from corrupting state or duplicating resources; the S3 backend supports locking via DynamoDB or S3 native lockfile support depending on the configuration." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "TF_VAR_log=debug and TF_OUTPUT=file" },
      { id: 'C', text: "TF_LOG=DEBUG (or TRACE) together with TF_LOG_PATH=./terraform.log" },
      { id: 'D', text: "TF_LOG_PROVIDER=on only" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A for expression assigned directly to the ingress argument." },
      { id: 'B', text: "count on the security group resource." },
      { id: 'C', text: "for_each on the security group resource itself." },
      { id: 'D', text: "A dynamic \"ingress\" block with for_each over the list and a content block." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A workspace variable marked sensitive." },
      { id: 'C', text: "A required manual apply approval." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Omitting version and relying on the lock file" },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "terraform import run once by an engineer on their laptop." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Initialises the backend, installs required providers and modules, and writes or verifies the dependency lock file." },
      { id: 'B', text: "Refreshes state against the real infrastructure and reports drift." },
      { id: 'C', text: "Creates the remote state bucket if it does not exist." },
      { id: 'D', text: "Validates variable values against their type constraints." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "A lifecycle block with prevent_destroy = true." },
      { id: 'D', text: "Setting the resource count to 1 permanently." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "The for_each keys derive from values unknown until apply; restructure so keys come from known inputs, or split the apply into stages." },
      { id: 'D', text: "The provider version is too old; upgrade it." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Publish the values as outputs of the networking workspace and consume them with terraform_remote_state or an HCP Terraform outputs data source." },
      { id: 'D', text: "Copy the ids into the application variables file after each networking apply." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Deleting the resource block from the configuration and applying." },
      { id: 'B', text: "Running terraform destroy -target on the resource." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Naming the output with a leading underscore." },
      { id: 'B', text: "Setting sensitive = true on the output block." },
      { id: 'C', text: "Wrapping the value in the nonsensitive function." },
      { id: 'D', text: "Storing the value in a local instead of an output." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "terraform taint aws_instance.web" },
      { id: 'B', text: "terraform refresh -target=aws_instance.web" },
      { id: 'C', text: "terraform apply -replace=\"aws_instance.web\"" },
      { id: 'D', text: "terraform state rm aws_instance.web" }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Parallelism is too high; rerun with -parallelism=1." },
      { id: 'B', text: "The state file is corrupt; restore it from a backup." },
      { id: 'C', text: "Two resources reference each other, so the graph has no valid order; break the cycle by extracting the mutual reference into a separate resource such as a standalone rule." },
      { id: 'D', text: "The provider is not initialised; run terraform init -upgrade." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Local execution, where HCP Terraform stores only the state." },
      { id: 'D', text: "Remote execution, where the CLI streams the run to HCP Terraform and the plan and apply happen there." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "It contains resource state and must never be committed." },
      { id: 'D', text: "It records module versions and is regenerated on every plan, so it should be ignored." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The module should define its own backend block so it can be applied directly." },
      { id: 'B', text: "All six should become required variables with no defaults." },
      { id: 'C', text: "Credentials, region, and backend settings do not belong in a module interface; they are provider and root-level concerns." },
      { id: 'D', text: "Region should stay but subnet_ids should be looked up with a data source inside the module." }
    ],
    correctAnswers: ['C'],
    type: "single",
