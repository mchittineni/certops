export const HASHICORP_TFP_QUESTIONS_1 = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "terraform show followed by terraform apply." },
      { id: 'C', text: "terraform plan then terraform apply -auto-approve in the same job." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Setting the variable type to string and documenting the allowed values." },
      { id: 'C', text: "A validation block inside the variable with a condition and an error_message." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "It stops concurrent writes corrupting state; S3 locks with a lockfile." },
    ...
    correctAnswers: ['B'],
    type: "single",
    explanation: "Locking serialises state-mutating operations so two applies cannot interleave and clobber each other, which would otherwise leave orphaned or duplicated resources. Since Terraform 1.10 the S3 backend locks with a `.tflock` object (`use_lockfile = true`); older configurations use a DynamoDB table, which is deprecated from 1.11. Versioning helps recovery rather than mutual exclusion, refreshing addresses drift rather than concurrency, and encryption protects confidentiality.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Setting the resource count to 1 permanently." },
      { id: 'B', text: "A lifecycle block with ignore_changes = all." },
      { id: 'C', text: "A depends_on entry pointing at the database." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Adding prevent_destroy and deleting the block." },
      { id: 'D', text: "A removed block with lifecycle { destroy = false } for the resource address." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "No execution mode change is needed; the CLI always runs remotely once a cloud block exists." },
      { id: 'B', text: "Agent execution, which is required for any remote run." },
