export const HASHICORP_TFP_QUESTIONS_2 = [
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Import them into the new configuration, then `terraform state rm` them from the old." },
    ...
    correctAnswers: ['A'],
    type: "single",
    explanation: "Cross-state moves are state surgery: back up both states, bring the resources under the new configuration with import blocks (or `terraform import`), then stop the old configuration tracking them with `terraform state rm` or, from Terraform 1.7, a `removed` block with `destroy = false`. `terraform state mv -state-out` only works with local state files, moved blocks only work inside one state, deleting and reapplying recreates the infrastructure, and copying the whole state brings resources the new configuration does not declare.",
  ...
  },
  {
    ...
    domainId: "d2",
    domainName: "Develop and troubleshoot dynamic configuration",
    title: "Asserting an Invariant on a Resolved Value",
  ...
  },
  {
    ...
    correctAnswers: ['A'],
    type: "single",
    explanation: "A map of objects declaring both attributes documents the expected shape and makes Terraform reject malformed input during evaluation, and keying by name also gives stable for_each keys. The object type without the zone attribute would silently drop availability_zone, a list of objects loses the stable keys, and map(list(string)) cannot name two distinct attributes per element.",
