export const HASHICORP_TFP_QUESTIONS_3 = [
  {
    ...
    options: [
      ...
      { id: 'B', text: "depends_on = [var.enable_monitoring]" },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Delete the lock table or lock object directly as the first step." },
      { id: 'B', text: "Delete the state file and reinitialise." },
      { id: 'C', text: "Run terraform apply -lock=false as the permanent workaround." },
      { id: 'D', text: "Confirm no operation is actually running, then run terraform force-unlock with the reported lock ID." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "var.network.vpc_id" },
      { id: 'B', text: "module.network.vpc_id" },
      { id: 'C', text: "data.module.network.vpc_id" },
      { id: 'D', text: "module.network.output.vpc_id" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Terraform installs the newest version and silently rewrites the lock file." },
      { id: 'B', text: "Terraform fails because the lock file is out of date." },
      { id: 'C', text: "Terraform installs 5.31.0 as recorded in the lock file; upgrading requires terraform init -upgrade and a lock file commit." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "object({ name = string, tags = optional(map(string), {}), enabled = optional(bool, true) })" },
      { id: 'C', text: "object({ name = string, tags = map(string), enabled = bool }) with a variable default" },
      { id: 'D', text: "map(string) with documented keys" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A run task that proxies provider traffic." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The resource needs prevent_destroy set." },
      { id: 'B', text: "The plan file is out of date; regenerate it." },
      { id: 'C', text: "The object is not in Terraform state - it was created outside Terraform or removed from state - so it must be imported." },
      { id: 'D', text: "The provider cache is stale; run terraform init -upgrade." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Automatic rollback of failed applies." },
      { id: 'B', text: "Storage of provider binaries alongside state." },
      { id: 'C', text: "Shared remote storage with access control and encryption." },
      { id: 'D', text: "State locking to serialise concurrent operations." }
    ],
    correctAnswers: ['C', 'D'],
    type: "multiple",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A provider block pinning the exact provider version." },
      { id: 'B', text: "A single main.tf file and any repository name." },
      { id: 'C', text: "A backend block declaring where the module state lives." },
      { id: 'D', text: "A repository named terraform-PROVIDER-NAME, a standard file layout with main.tf, variables.tf, and outputs.tf, and semver git tags such as v1.2.0." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "terraform apply -refresh=false" },
      { id: 'B', text: "terraform apply -input=false -no-color, applying a saved plan file" },
      { id: 'C', text: "terraform apply -auto-approve -input=true" },
      { id: 'D', text: "terraform apply -lock=false -no-color" }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "A resource block with lifecycle { ignore_changes = all }." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Read the plan diff carefully to see which attribute changed and why - typically a computed upstream value, a provider default change, or drift - then decide whether to fix the source or ignore it." },
      { id: 'B', text: "Immediately run terraform state rm on the resource." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Team access permissions on the workspace, granting plan-only to developers and apply to the platform team." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Run terraform destroy, change the backend, then apply again." },
      ...
      { id: 'D', text: "Add the new backend configuration and run terraform init, accepting the prompt to copy existing state to the new backend." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Moving the resources inside the module." },
      { id: 'C', text: "depends_on = [module.network] on the dependent resource." },
      { id: 'D', text: "Referencing module.network directly in a resource argument." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Add a longer timeout and retry loop to the provisioner." },
      { id: 'C', text: "Bake the software into a machine image or use cloud-init / user_data, treating provisioners as a last resort." },
      { id: 'D', text: "Run the provisioner with when = destroy instead." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "0 means no changes, 1 means changes, 2 means a policy failure." },
      { id: 'D', text: "Any non-zero code means an error." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The resource blocks, which must each declare credentials." },
      { id: 'C', text: "The credential chain available to the process - environment variables, shared config files, instance or workload identity - rather than the Terraform configuration itself." },
      { id: 'D', text: "The state file, which stores credentials." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Terraform rebuilds state automatically on the next refresh." },
      ...
      { id: 'D', text: "The provider blocks the next apply until state is restored." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A run task attached at the post-plan stage, configured as mandatory so a failing result stops the run." },
      { id: 'B', text: "A variable set holding the tool API key." },
      { id: 'C', text: "A notification configuration posting to the tool." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Ask consumers to stop pinning versions." },
      { id: 'B', text: "Force-push a new tag over 2.8.0." },
      { id: 'C', text: "Release it as 2.9.0 so consumers pick it up automatically and find out quickly." },
      { id: 'D', text: "Release it as 3.0.0 following semantic versioning, document the migration, and let consumers move their constraint deliberately." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "terraform.tfvars, because files are authoritative." },
      { id: 'D', text: "Whichever is evaluated first, which is non-deterministic." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A provider bug or a mismatch between the planned and applied value; workarounds include upgrading the provider, adding ignore_changes for that attribute, or reporting it upstream." },
      { id: 'C', text: "Terraform core is out of date and must be downgraded." },
      { id: 'D', text: "Two applies ran concurrently." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A locals block computing the key." },
