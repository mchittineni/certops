export const HASHICORP_TFP_QUESTIONS_16 = [
  {
    ...
    options: [
      { id: 'A', text: "Configure a native `encryption {}` block in the root module with a KMS key provider." },
    ...
    correctAnswers: ['B'],
    type: "single",
    explanation: "Configure the backend to encrypt state at rest with a customer-managed key, for example the S3 backend with `encrypt = true` and `kms_key_id` pointing at a KMS key that has automatic rotation enabled. Terraform has no client-side state encryption: the `encryption {}` block is an OpenTofu 1.7 feature, not part of Terraform. HCP Terraform encrypts state at rest itself. Marking variables `sensitive` only redacts CLI output and the values are still written to state, and bucket versioning helps recovery rather than confidentiality.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Configure a native `encryption {}` block in the root module with a KMS key provider." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "Configure the backend to encrypt state at rest with a customer-managed key, for example the S3 backend with `encrypt = true` and `kms_key_id` pointing at a KMS key that has automatic rotation enabled. Terraform has no client-side state encryption: the `encryption {}` block is an OpenTofu 1.7 feature, not part of Terraform. HCP Terraform encrypts state at rest itself. Marking variables `sensitive` only redacts CLI output and the values are still written to state, and bucket versioning helps recovery rather than confidentiality.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Configure a native `encryption {}` block in the root module with a KMS key provider." },
