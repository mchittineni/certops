export const HASHICORP_TFP_QUESTIONS_20 = [
  ...
  },
  {
    ...
    correctAnswers: ['B'],
    type: "single",
    explanation: "Assign the team the `Plan` permission on the workspace. HCP Terraform fixed workspace permission sets are `Read` (view state and outputs), `Plan` (queue speculative plans and review them), `Write` (also apply runs, lock the workspace, and read and write variables), and `Admin` (manage settings, VCS connections, team access, and delete the workspace). `Write` would let the team edit variables, which the requirement forbids, so `Plan` is the least-privilege fit.",
