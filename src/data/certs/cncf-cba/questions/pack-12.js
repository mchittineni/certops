export const CNCF_CBA_QUESTIONS_12 = [
  {
    ...
    options: [
      { id: 'A', text: "Forward the end user's own Backstage token on every inter-plugin request so that each plugin authorizes those requests as that user." },
      { id: 'B', text: "Share one static key across the backend plugins, injected through configuration as `${BACKEND_SECRET}`." },
      { id: 'C', text: "Use the `auth` and `httpAuth` core services to issue and verify self-signed plugin-to-plugin tokens per request." },
      { id: 'D', text: "Restrict inter-plugin traffic with a Kubernetes NetworkPolicy so that only backend pods can reach the plugin endpoints." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "Use the `auth` and `httpAuth` core services of the new backend system. They are secure by default: the caller gets a self-signed plugin request token from `auth.getPluginRequestToken`, optionally on behalf of the original user's credentials, and the receiving plugin verifies it against the caller's public key set endpoint. The legacy token manager service that used to sign these tokens was removed in Backstage 1.31. A NetworkPolicy proves which pod a call came from but not which plugin or on whose behalf, so a compromised plugin still reaches everything; forwarding the user token breaks scheduled and event-driven work, where there is no user in the request; and a shared static key authenticates the cluster as a whole rather than the individual caller, and rotating it means restarting every plugin at once.",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Use the `auth` and `httpAuth` core services to issue and verify self-signed plugin-to-plugin tokens per request." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Forward the end user's own Backstage token on every inter-plugin request so that each plugin authorizes those requests as that user." },
      { id: 'B', text: "Use the `auth` and `httpAuth` core services to issue and verify self-signed plugin-to-plugin tokens per request." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Implement a custom `PermissionPolicy` in `@backstage/plugin-permission-backend` that evaluates incoming requests against entity ownership and role attributes." },
      { id: 'B', text: "Adopt the Backstage RBAC plugin's administrative UI and assign roles there without implementing a policy in code." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Keep PostgreSQL but disable the Knex migrations and apply each schema change manually with SQL at release time." },
      { id: 'C', text: "Run SQLite through the `better-sqlite3` driver with the database file kept on a persistent volume mounted into the pod." },
      { id: 'D', text: "Point the catalog at a managed MySQL instance and let Knex build the schema on first boot." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    question: "Which Backstage catalog pattern or ingestion configuration handles this volume efficiently while preventing database contention?",
    options: [
      { id: 'A', text: "Keep PostgreSQL but disable the Knex migrations and apply each schema change manually with SQL at release time." },
      ...
      { id: 'C', text: "Point the catalog at a managed MySQL instance and let Knex build the schema on first boot." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Deploy the Backstage `EventsService` and `EventBroker` to publish incoming webhook payloads to subscribing plugins asynchronously." }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Deploy the Backstage `EventsService` and `EventBroker` to publish incoming webhook payloads to subscribing plugins asynchronously. The Backstage Events subsystem (`@backstage/plugin-events-node`) provides an `EventsService` and `EventBroker`. Ingress plugins (such as webhook receivers) publish CloudEvents onto topics, which subscribing plugins consume asynchronously. One webhook per plugin means the Git provider has to be reconfigured whenever a plugin is added, and each endpoint reimplements signature verification; an external broker delivers the same decoupling at the cost of running and securing another system for traffic this size; and a polled table trades the push for latency and a cleanup problem.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Publish the payloads onto an external broker such as Kafka and have each plugin consume from a topic there." },
      { id: 'C', text: "Give each interested plugin its own webhook endpoint registered with the Git provider, processing each payload inline as it arrives." },
      { id: 'D', text: "Persist incoming payloads in a Backstage database table and have each plugin poll that table on a scheduled task." }
    ],
    correctAnswers: ['A'],
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Address the backends by their in-cluster DNS names, such as `http://plugin-catalog.backstage.svc.cluster.local`." },
      { id: 'D', text: "Read `backend.baseUrl` from the config API in the frontend and append the plugin id to it for the endpoints each of the plugins calls." }
    ],
    correctAnswers: ['A'],
    type: "single",
