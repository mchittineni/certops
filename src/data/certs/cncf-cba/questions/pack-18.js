export const CNCF_CBA_QUESTIONS_18 = [
  {
    ...
    options: [
      ...
      { id: 'B', text: "Run each task in its own container with the workspace mounted from a shared volume that the runner provisions." },
      ...
      { id: 'D', text: "Give every scaffolder task the same working directory and clear it out before each new run begins." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Give every scaffolder task the same working directory and clear it out before each new run begins." },
      { id: 'B', text: "Execute each scaffolder task in a dedicated, isolated temporary directory that is automatically purged upon task completion." },
      { id: 'C', text: "Keep the workspace after completion so that the platform team can inspect tasks that failed, pruning the directories on a weekly schedule." },
      { id: 'D', text: "Run each task in its own container with the workspace mounted from a shared volume that the runner provisions." }
    ],
    correctAnswers: ['B'],
  ...
  },
  {
    ...
    question: "Which architectural approach or Backstage configuration satisfies these enterprise portal objectives?",
    options: [
      { id: 'A', text: "Write the task output to the backend's own log stream and have operators read it from Kubernetes." },
      ...
      { id: 'C', text: "Stream task logs and step lifecycle status from the backend to the frontend using Server-Sent Events (SSE) via the task event stream." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Write the task output to the backend's own log stream and have operators read it from Kubernetes." },
      { id: 'B', text: "Poll the `/tasks/:taskId` endpoint from the frontend every few seconds and re-render the log each time, stopping once the task reports a terminal status." },
      { id: 'C', text: "Open a WebSocket from the browser to the scaffolder backend for each task that is running, closing it when the task ends." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Read the token from `app-config.yaml` inside the action rather than passing it through the task at all, scoping it under the integrations block." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Declare the token as an ordinary template parameter with `ui:widget: password` so the form masks it." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Pass tokens using `ctx.secrets` and configure the task logger to mask secret values automatically in all task event logs." },
      { id: 'D', text: "Read the token from `app-config.yaml` inside the action rather than passing it through the task at all, scoping it under the integrations block." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    question: "Which Backstage catalog pattern or ingestion configuration handles this volume efficiently while preventing database contention?",
    options: [
      { id: 'A', text: "Provide curated 'Golden Path' templates that automatically scaffold container definitions, CI/CD pipelines, and security linters." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Mount the page as a `createCardExtension` on the entity page instead of as its own route." },
  ...
  },
  {
    ...
    question: "Which Backstage security mechanism or configuration satisfies these compliance controls?",
    options: [
      { id: 'A', text: "Create a frontend plugin using `createPlugin` and expose routable pages using `createRoutableExtension`." },
      { id: 'B', text: "Export the page component directly and import it into `App.tsx` without declaring a plugin at all." },
      { id: 'C', text: "Mount the page as a `createCardExtension` on the entity page instead of as its own route." },
      { id: 'D', text: "Create the plugin with `createPlugin` but expose the page through `createComponentExtension`." }
    ],
    correctAnswers: ['A'],
