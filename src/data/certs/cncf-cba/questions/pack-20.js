export const CNCF_CBA_QUESTIONS_20 = [
  {
    ...
    options: [
      ...
      { id: 'B', text: "Import the configuration singleton from `@backstage/config` directly inside the component." },
      { id: 'C', text: "Pass the config and storage objects down as props from the root `App` component." },
      { id: 'D', text: "Consume Backstage utility APIs using React hooks like `useApi(configApiRef)` and `useApi(storageApiRef)`." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Pass the config and storage objects down as props from the root `App` component." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Set the palette through an `app.theme` block in `app-config.yaml` so that it can be changed at runtime without rebuilding the frontend." },
      { id: 'D', text: "Fork `@backstage/theme` and publish an internal package with the palette already changed." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    question: "Which Backstage security mechanism or configuration satisfies these compliance controls?",
    options: [
      { id: 'A', text: "Set the palette through an `app.theme` block in `app-config.yaml` so that it can be changed at runtime without rebuilding the frontend." },
  ...
  },
  {
    ...
    question: "Which architectural approach or Backstage configuration satisfies these enterprise portal objectives?",
    options: [
      { id: 'A', text: "Deploy the community RBAC plugin (`@backstage-community/plugin-rbac`) to administer roles, permissions, and policies dynamically." },
      ...
      { id: 'D', text: "Implement a custom `PermissionPolicy` in code that reads its role assignments from a configuration file, reloading that file whenever the backend restarts." }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Deploy the community RBAC plugins: `@backstage-community/plugin-rbac-backend`, which plugs into the Backstage permission framework, and the `@backstage-community/plugin-rbac` frontend. There is no `@backstage/plugin-rbac` package. Administrators create roles, assign users and groups, and attach permission policies (such as template execution or catalog editing) through the RBAC UI or configuration files, without writing policy code or redeploying. A config-driven policy avoids recompiling for each role change but still needs a deployment to alter the file, and it gives administrators no interface; ownership-based permissions cover the common case and cannot express a role that spans entities nobody owns; and an ingress restriction decides who may enter the portal rather than what they may do once inside.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Deploy the community RBAC plugin (`@backstage-community/plugin-rbac`) to administer roles, permissions, and policies dynamically." },
      { id: 'C', text: "Leave the default allow-all policy in place and restrict who can reach the portal at the ingress layer." },
  ...
  },
  {
    ...
    certId: "cncf-cba",
    domainId: "d1",
    domainName: "Backstage Development Workflow",
    ...
    options: [
      ...
      { id: 'B', text: "Run `yarn upgrade-interactive` and select the `@backstage/*` packages to move them all forward together." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Work through the official Upgrade Helper diff and apply each change to the affected files by hand." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Run `yarn upgrade-interactive` and select the `@backstage/*` packages to move them all forward together." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "Run `yarn backstage-cli versions:bump` to update all `@backstage/*` packages to synchronized compatible versions. Because Backstage is composed of dozens of interdependent packages, official upgrades are managed via `yarn backstage-cli versions:bump`. This command queries the release manifest and synchronizes all `@backstage/*` packages to mutually compatible versions. `upgrade-interactive` moves each package to its own newest version with no notion of which versions were released as a compatible set; the Upgrade Helper is genuinely needed for changes to files the CLI does not own, such as `App.tsx`, so it complements the bump rather than replacing it; and `latest` on every package discards the release-set guarantee entirely.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Set every `@backstage/*` dependency to `latest` and reinstall to pick up the current releases." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Send audit events straight to an external SIEM from each plugin using its own HTTP client." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Treat the scaffolder's task event stream as the audit record rather than using the logger service." }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Configure structured JSON logging via the Backstage Winston logger service and capture scaffolder execution audit trails. Backstage utilizes Winston for structured logging (`RootLoggerService`). In production, log formatters emit structured JSON events including user identity, timestamps, and action metadata, providing an auditable log stream for enterprise SIEM ingestion. `console.log` reaches the collector but carries none of the plugin, service, and trace fields the logger service attaches, so entries cannot be correlated across a request; the task event stream covers scaffolder runs alone and expires with the task; and per-plugin SIEM clients duplicate delivery, retry, and credential handling in every plugin, and drop events whenever one of them fails.",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Configure structured JSON logging via the Backstage Winston logger service and capture scaffolder execution audit trails." },
      { id: 'C', text: "Treat the scaffolder's task event stream as the audit record rather than using the logger service." },
