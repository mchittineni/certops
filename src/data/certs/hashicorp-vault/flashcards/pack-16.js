export const HASHICORP_VAULT_FLASHCARDS_16 = [
  ...
  },
  {
    ...
    back: "<strong>Audit Devices and Failure Behavior</strong>: Enable multiple audit devices (e.g., file and syslog) and monitor them, so Vault keeps serving while at least one device can still write. Core architectural rationale: Vault refuses to serve a request it cannot log, so with one audit device that device becomes an availability dependency. Enabling a second device means Vault ke...",
  ...
  },
  {
    ...
    back: "<strong>Disaster Recovery vs Performance Replication</strong>: Use Disaster Recovery (DR) replication to maintain a warm-standby secondary cluster that can be promoted, or Performance Replication for localized secret reads. Core architectural rationale: Both replication modes are asynchronous and answer different questions: DR keeps a warm secondary carrying tokens and leases so it can be promoted, while performance replication ...",
