export const HASHICORP_VAULT_QUESTIONS_7 = [
  {
    ...
    title: "Tamper-Evident Hand-Off of a Bootstrap Credential: High-Frequency FinTech Trading",
    scenario: "A quantitative trading desk requires microsecond secrets delivery, zero packet loss, and deterministic authentication guarantees. The platform team is delivering an initial credential to a new client so that any interception is detected. The work is scoped to the staging environment.",
    ...
    options: [
      ...
      { id: 'B', text: "Store the credential at a KV v2 path readable only by the recipient's own policy and have the client fetch it from Vault on boot." },
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Indefinite Authentication for a Long-Running Daemon: Healthcare Patient Records & HIPAA",
    scenario: "A national hospital network requires strict cryptographic privacy, auditable access controls, and HIPAA compliance. The platform team is keeping a background daemon authenticated indefinitely without handing it privileged credentials. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "A Token That Outlives the Session That Created It: Global E-Commerce Black Friday Scale",
    scenario: "An international retail marketplace prepares for 100x traffic surges with zero downtime and instant failover. The platform team is provisioning a service token whose lifetime must not depend on the operator who created it. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      { id: 'A', text: "Create the token through a second auth method role so that it is bound to a different identity entity than its creator." },
      ...
      { id: 'D', text: "Create the token with 'vault token create -ttl' set longer than the parent token's remaining lifetime." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Enable the TLS certificate auth method and issue each workload a client certificate from Vault's PKI secrets engine for mutual TLS login." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Pod Identity Without Distributing Credentials: Multi-Tenant B2B SaaS Platform",
    scenario: "An enterprise cloud SaaS architecture mandates strict logical tenant isolation, data masking, and per-tenant resource quotas. The platform team is authenticating ephemeral cluster pods whose identity must be revocable the moment they are deleted. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Read-Only Access to a KV v2 Path: Media Streaming & Global CDN",
    scenario: "A global video streaming service distributes high-bitrate live media with distributed edge caching and tokenized DRM protection. The platform team is granting an application read access to its own secrets without allowing it to overwrite them. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "One Policy Rule Covering Many Teams: Aerospace Satellite Ground Systems",
    scenario: "An aerospace telemetry platform processes orbital downlinks with fault-tolerant queuing and asynchronous edge processing. The platform team is scaling per-team access control without authoring a separate rule for every team. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      ...
      { id: 'D', text: "Create a separate KV v2 mount for each team and grant every auth role a policy scoped to its own mount path." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Tamper-Evident Hand-Off of a Bootstrap Credential: Telecommunications 5G Core Network",
    scenario: "A national telecom operator manages high-density network slices with automated scaling and sub-millisecond service mesh routing. The platform team is delivering an initial credential to a new client so that any interception is detected. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Indefinite Authentication for a Long-Running Daemon: Renewable Energy Smart Grid IoT",
    scenario: "A smart electrical grid platform monitors millions of smart meters with low-latency time-series analysis and automated load shedding. The platform team is keeping a background daemon authenticated indefinitely without handing it privileged credentials. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      ...
      { id: 'C', text: "Issue a service token with a long explicit max TTL and have the daemon re-authenticate through its AppRole role once the token eventually reaches that limit." },
      { id: 'D', text: "Issue a batch token and have the daemon request a replacement each time its current token is rejected as expired." }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "A periodic token has no maximum TTL: as long as it is renewed within its period it lives forever, which is precisely what an always-on daemon needs. Every other option still runs into a ceiling. A long max TTL and a raised max_lease_ttl only move the expiry further out, so the daemon must eventually re-authenticate and needs stored credentials to do it, and batch tokens cannot be renewed at all.",
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "A Token That Outlives the Session That Created It: Supply Chain Cold-Chain Logistics",
    scenario: "A pharmaceutical distribution network tracks temperature-sensitive cargo with cryptographic provenance and automated breach alerts. The platform team is provisioning a service token whose lifetime must not depend on the operator who created it. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Machine Identity for Containerized Workloads: Banking Core Ledger & Payments",
    scenario: "A central banking consortium enforces ACID consistency, immutable transaction audit trails, and automated reconciliation. The platform team is configuring machine authentication for workloads that cannot be pre-seeded with credentials. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Pod Identity Without Distributing Credentials: Genomic Sequencing & Biotech Pipeline",
    scenario: "A genomics laboratory processes petabyte-scale FASTQ files with distributed batch computing and high-throughput POSIX storage. The platform team is authenticating ephemeral cluster pods whose identity must be revocable the moment they are deleted. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      ...
      { id: 'D', text: "Enable the AppRole auth method and deliver a Vault response-wrapped secret_id to every pod through an init container at startup." }
    ],
    correctAnswers: ['C'],
    type: "single",
    explanation: "The Kubernetes auth method calls the cluster TokenReview API on every login, so a service account token stops working the instant the account or pod is removed. JWT auth against the JWKS endpoint verifies the token signature offline and therefore keeps accepting a token after the pod is gone. AppRole works but reintroduces the credential-delivery problem this method exists to remove, and certificate auth puts long-lived key material back into a Secret.",
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Read-Only Access to a KV v2 Path: Defense-Grade Zero-Trust Network",
    scenario: "A defense intelligence system enforces continuous mutual TLS authentication, strict least privilege, and non-repudiation. The platform team is granting an application read access to its own secrets without allowing it to overwrite them. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "One Policy Rule Covering Many Teams: Online Multiplayer Gaming Engine",
    scenario: "A real-time competitive gaming cluster orchestrates match sessions with regional matchmaking and anti-cheat validation. The platform team is scaling per-team access control without authoring a separate rule for every team. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Tamper-Evident Hand-Off of a Bootstrap Credential: Insurance Risk & Actuarial Modeling",
    scenario: "An actuarial underwriting platform executes Monte Carlo simulations across millions of policy holder records with parallel workers. The platform team is delivering an initial credential to a new client so that any interception is detected. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      { id: 'A', text: "Store the credential at a KV v2 path readable only by the recipient's own policy and have the client fetch it from Vault on boot." },
      { id: 'B', text: "Issue the credential with a very short TTL so that an intercepted value expires before an attacker is able to use it." },
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Indefinite Authentication for a Long-Running Daemon: Pharmaceutical Clinical Trial Platform",
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "A Token That Outlives the Session That Created It: Smart City Traffic & Mobility Sensor Hub",
    scenario: "A metropolitan transit authority optimizes urban traffic signals with real-time video analytics and edge inference. The platform team is provisioning a service token whose lifetime must not depend on the operator who created it. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Machine Identity for Containerized Workloads: Digital Identity & Biometric Verification",
    scenario: "A cross-border passport control gateway validates identity credentials with zero-knowledge cryptographic proofs. The platform team is configuring machine authentication for workloads that cannot be pre-seeded with credentials. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      ...
      { id: 'B', text: "Configure the AppRole auth engine with a static role_id and dynamically generated secret_id bound to CIDR subnets and short TTLs." },
      { id: 'C', text: "Enable the Kubernetes auth method and bind roles to each workload's service account, validating pod identity through the cluster TokenReview API." },
      { id: 'D', text: "Enable the TLS certificate auth method and issue each workload a client certificate from Vault's PKI secrets engine for mutual TLS login." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Pod Identity Without Distributing Credentials: Legal Discovery & Semantic Document Search",
    scenario: "A global law firm conducts regulatory discovery across millions of scanned legal filings with vector-enhanced semantic retrieval. The platform team is authenticating ephemeral cluster pods whose identity must be revocable the moment they are deleted. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      { id: 'A', text: "Enable the JWT auth method, point it at the cluster's public JWKS issuer URL, and bind roles to the subject claim carried in each pod's projected service account token." },
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Read-Only Access to a KV v2 Path: AdTech Real-Time Bidding Exchange",
    scenario: "An advertising exchange processes 500,000 bids per second with a strict 20-millisecond SLA and distributed caching. The platform team is granting an application read access to its own secrets without allowing it to overwrite them. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "One Policy Rule Covering Many Teams: Precision Agriculture & Drone Scouting",
    scenario: "An agricultural drone fleet captures multispectral crop imagery with automated computer vision defect classification. The platform team is scaling per-team access control without authoring a separate rule for every team. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Tamper-Evident Hand-Off of a Bootstrap Credential: Industrial Robotics Predictive Maintenance",
    scenario: "A semiconductor fabrication facility detects vibration harmonics on manufacturing robots to prevent unplanned downtime. The platform team is delivering an initial credential to a new client so that any interception is detected. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      { id: 'A', text: "Issue the credential with a very short TTL so that an intercepted value expires before an attacker is able to use it." },
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Indefinite Authentication for a Long-Running Daemon: Educational Remote Proctoring Platform",
    scenario: "An online university platform enforces anti-plagiarism and biometric proctoring for high-stakes certification exams. The platform team is keeping a background daemon authenticated indefinitely without handing it privileged credentials. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      { id: 'A', text: "Issue a batch token and have the daemon request a replacement each time its current token is rejected as expired." },
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "A Token That Outlives the Session That Created It: Real Estate Valuation & Geo-Spatial Analytics",
    scenario: "A property appraisal engine fuses GIS parcel maps with real-time market transactions for automated valuation. The platform team is provisioning a service token whose lifetime must not depend on the operator who created it. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  },
  {
    ...
    domainId: "d1",
    domainName: "Compare authentication methods and manage Vault policies",
    title: "Machine Identity for Containerized Workloads: Disaster Emergency Dispatch & Operations",
