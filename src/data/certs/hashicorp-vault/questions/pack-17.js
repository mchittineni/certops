export const HASHICORP_VAULT_QUESTIONS_17 = [
  {
    id: "hashicorp-vault-401",
    difficulty: "easy",
    certId: "hashicorp-vault",
    domainId: "d9",
    domainName: "Access management architecture",
    title: "Delivering Secrets to Mixed Kubernetes and VM Workloads: High-Frequency FinTech Trading",
    scenario: "A quantitative trading desk requires microsecond secrets delivery, zero packet loss, and deterministic authentication guarantees. The platform team is delivering secrets into applications that run on both Kubernetes and plain virtual machines. The work is scoped to the staging environment.",
    ...
    options: [
      ...
      { id: 'C', text: "Deploy the Vault Secrets Operator so that secrets are synchronised into native Kubernetes Secret objects for pods to consume." },
  ...
  {
    id: "hashicorp-vault-402",
    difficulty: "easy",
    certId: "hashicorp-vault",
    domainId: "d8",
    domainName: "Vault deployment architecture",
    title: "Surviving the Loss of a Cluster Node: Healthcare Patient Records & HIPAA",
    scenario: "A national hospital network requires strict cryptographic privacy, auditable access controls, and HIPAA compliance. The platform team is designing a production cluster topology that tolerates the loss of a node. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      ...
      { id: 'D', text: "Deploy several independent Vault clusters behind a load balancer that spreads write requests evenly across all of them." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  {
    id: "hashicorp-vault-403",
    difficulty: "easy",
    certId: "hashicorp-vault",
    domainId: "d7",
    domainName: "Vault architecture fundamentals",
    title: "Restarting a Node Without an Operator Key Ceremony: Global E-Commerce Black Friday Scale",
    scenario: "An international retail marketplace prepares for 100x traffic surges with zero downtime and instant failover. The platform team is bringing nodes back into service after a restart without assembling key holders. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  {
    id: "hashicorp-vault-404",
    difficulty: "easy",
    certId: "hashicorp-vault",
    domainId: "d8",
    domainName: "Vault deployment architecture",
    title: "Choosing a Cross-Region Replication Mode: Autonomous Vehicle Telemetry",
    scenario: "A self-driving automotive fleet streams terabytes of sensor telemetry requiring real-time distributed ingestion and anomaly detection. The platform team is planning cross-region failover alongside low-latency reads in each region. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      ...
      { id: 'D', text: "Use Disaster Recovery (DR) replication to maintain a warm-standby secondary cluster that can be promoted, or Performance Replication for localized secret reads." }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Both replication modes are asynchronous and answer different questions: DR keeps a warm secondary carrying tokens and leases so it can be promoted, while performance replication serves local reads in each region. Performance secondaries deliberately do not replicate tokens and leases, so promoting one leaves every client re-authenticating. Snapshot-and-restore is a real recovery path but its recovery point is only as recent as the last snapshot, and template-based copying between clusters reimplements replication without its consistency guarantees.",
    referenceUrl: "https://developer.hashicorp.com/vault/tutorials/certification-vault-associate-003",
    tags: ["replication", "dr", "performance-replication", "Autonomous Vehicle Telemetry"]
  },
  {
    id: "hashicorp-vault-405",
    difficulty: "easy",
    certId: "hashicorp-vault",
    domainId: "d4",
    domainName: "Vault leases",
    title: "Withdrawing Credentials Already in Circulation: Multi-Tenant B2B SaaS Platform",
    scenario: "An enterprise cloud SaaS architecture mandates strict logical tenant isolation, data masking, and per-tenant resource quotas. The platform team is cutting off credentials that have already been issued after a suspected compromise. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  {
    id: "hashicorp-vault-406",
    difficulty: "medium",
    certId: "hashicorp-vault",
    domainId: "d8",
    domainName: "Vault deployment architecture",
    title: "Audit Logging That Cannot Fail Quietly: Media Streaming & Global CDN",
    scenario: "A global video streaming service distributes high-bitrate live media with distributed edge caching and tokenized DRM protection. The platform team is guaranteeing every request is recorded without the logging path halting the cluster. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      ...
      { id: 'B', text: "Enable multiple audit devices (e.g., file and syslog) and monitor them, so Vault keeps serving while at least one device can still write." },
    ...
    correctAnswers: ['B'],
    type: "single",
    explanation: "Vault refuses to serve a request it cannot log, so with one audit device that device becomes an availability dependency. Enabling a second device means Vault keeps serving as long as either one accepts writes, although it still blocks if every device fails. A lone socket device is the most fragile choice of all, because a brief network problem at the collector stalls the cluster, and rotation does not help a file device whose filesystem is full or read-only. Setting log_raw writes secrets to the log in the clear, trading a logging problem for a disclosure one.",
    referenceUrl: "https://developer.hashicorp.com/vault/tutorials/certification-vault-associate-003",
    tags: ["audit", "logging", "compliance", "Media Streaming & Global CDN"]
  },
  {
    id: "hashicorp-vault-407",
    difficulty: "medium",
    certId: "hashicorp-vault",
    domainId: "d3",
    domainName: "Vault tokens",
    title: "Token Type for Very High Request Volume: Aerospace Satellite Ground Systems",
    scenario: "An aerospace telemetry platform processes orbital downlinks with fault-tolerant queuing and asynchronous edge processing. The platform team is choosing a token type for short-lived functions invoked many thousands of times per second. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  {
    id: "hashicorp-vault-408",
    difficulty: "medium",
    certId: "hashicorp-vault",
    domainId: "d9",
    domainName: "Access management architecture",
    title: "Delivering Secrets to Mixed Kubernetes and VM Workloads: Telecommunications 5G Core Network",
    scenario: "A national telecom operator manages high-density network slices with automated scaling and sub-millisecond service mesh routing. The platform team is delivering secrets into applications that run on both Kubernetes and plain virtual machines. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      { id: 'A', text: "Deploy the Vault CSI provider so that secrets are mounted into each pod's filesystem through the secrets-store CSI driver." },
      { id: 'B', text: "Deploy Vault Agent as a sidecar or daemon to handle authentication, token renewal, and automated secret templating to local disk." },
      { id: 'C', text: "Deploy the Vault Secrets Operator so that secrets are synchronised into native Kubernetes Secret objects for pods to consume." },
  ...
  {
    id: "hashicorp-vault-409",
    difficulty: "medium",
    certId: "hashicorp-vault",
    domainId: "d8",
    domainName: "Vault deployment architecture",
    title: "Surviving the Loss of a Cluster Node: Renewable Energy Smart Grid IoT",
    scenario: "A smart electrical grid platform monitors millions of smart meters with low-latency time-series analysis and automated load shedding. The platform team is designing a production cluster topology that tolerates the loss of a node. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  {
    id: "hashicorp-vault-410",
    difficulty: "medium",
    certId: "hashicorp-vault",
    domainId: "d7",
    domainName: "Vault architecture fundamentals",
    title: "Restarting a Node Without an Operator Key Ceremony: Supply Chain Cold-Chain Logistics",
    scenario: "A pharmaceutical distribution network tracks temperature-sensitive cargo with cryptographic provenance and automated breach alerts. The platform team is bringing nodes back into service after a restart without assembling key holders. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      { id: 'A', text: "Reduce the Shamir key threshold to a single share and place that share in the orchestrator's secret store for startup." },
  ...
  {
    id: "hashicorp-vault-411",
    difficulty: "medium",
    certId: "hashicorp-vault",
    domainId: "d8",
    domainName: "Vault deployment architecture",
    title: "Choosing a Cross-Region Replication Mode: Banking Core Ledger & Payments",
    scenario: "A central banking consortium enforces ACID consistency, immutable transaction audit trails, and automated reconciliation. The platform team is planning cross-region failover alongside low-latency reads in each region. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      { id: 'A', text: "Use Disaster Recovery (DR) replication to maintain a warm-standby secondary cluster that can be promoted, or Performance Replication for localized secret reads." },
      { id: 'B', text: "Run an independent cluster in each region and keep their secrets aligned with Vault Agent templates that copy values between them." },
      { id: 'C', text: "Use Performance Replication alone in every region and promote whichever performance secondary is healthiest if the primary region becomes unavailable for an extended period." },
  ...
  {
    id: "hashicorp-vault-412",
    difficulty: "medium",
    certId: "hashicorp-vault",
    domainId: "d4",
    domainName: "Vault leases",
    title: "Withdrawing Credentials Already in Circulation: Genomic Sequencing & Biotech Pipeline",
    scenario: "A genomics laboratory processes petabyte-scale FASTQ files with distributed batch computing and high-throughput POSIX storage. The platform team is cutting off credentials that have already been issued after a suspected compromise. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  {
    id: "hashicorp-vault-413",
    difficulty: "medium",
    certId: "hashicorp-vault",
    domainId: "d8",
    domainName: "Vault deployment architecture",
    title: "Audit Logging That Cannot Fail Quietly: Defense-Grade Zero-Trust Network",
    scenario: "A defense intelligence system enforces continuous mutual TLS authentication, strict least privilege, and non-repudiation. The platform team is guaranteeing every request is recorded without the logging path halting the cluster. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      ...
      { id: 'B', text: "Enable multiple audit devices (e.g., file and syslog) and monitor them, so Vault keeps serving while at least one device can still write." },
      { id: 'C', text: "Enable a single file audit device and configure aggressive log rotation so the destination cannot fill its disk." },
      { id: 'D', text: "Enable a single socket audit device pointed at the central logging collector so that audit records leave the Vault host immediately at the moment they are written." }
    ],
    correctAnswers: ['B'],
    type: "single",
    explanation: "Vault refuses to serve a request it cannot log, so with one audit device that device becomes an availability dependency. Enabling a second device means Vault keeps serving as long as either one accepts writes, although it still blocks if every device fails. A lone socket device is the most fragile choice of all, because a brief network problem at the collector stalls the cluster, and rotation does not help a file device whose filesystem is full or read-only. Setting log_raw writes secrets to the log in the clear, trading a logging problem for a disclosure one.",
  ...
  {
    id: "hashicorp-vault-414",
    difficulty: "medium",
    certId: "hashicorp-vault",
    domainId: "d3",
    domainName: "Vault tokens",
    title: "Token Type for Very High Request Volume: Online Multiplayer Gaming Engine",
    scenario: "A real-time competitive gaming cluster orchestrates match sessions with regional matchmaking and anti-cheat validation. The platform team is choosing a token type for short-lived functions invoked many thousands of times per second. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      { id: 'A', text: "Use batch tokens with renewable set to true so that a long-running invocation is able to extend its own lifetime." },
      { id: 'B', text: "Use Batch tokens for high-volume, short-lived workloads to eliminate write operations to the storage backend." },
      { id: 'C', text: "Use service tokens with very short TTLs so that expired entries are pruned from the token store by routine cleanup operations." },
      { id: 'D', text: "Use service tokens obtained through Vault Agent's token cache so that repeat invocations avoid contacting Vault." }
    ],
    correctAnswers: ['B'],
    type: "single",
    explanation: "A batch token is an encrypted blob handed to the client rather than a record in storage, so issuing one costs no write and the storage backend stops being the bottleneck. Service tokens are persisted whatever their TTL, and a short TTL raises the write rate because clients return sooner. Vault Agent's cache reduces round trips for a repeated caller but every distinct invocation still needs a token that must be written. Batch tokens are not renewable at all, so the last option cannot be configured as described.",
  ...
  {
    id: "hashicorp-vault-415",
    difficulty: "medium",
    certId: "hashicorp-vault",
    domainId: "d9",
    domainName: "Access management architecture",
    title: "Delivering Secrets to Mixed Kubernetes and VM Workloads: Insurance Risk & Actuarial Modeling",
    scenario: "An actuarial underwriting platform executes Monte Carlo simulations across millions of policy holder records with parallel workers. The platform team is delivering secrets into applications that run on both Kubernetes and plain virtual machines. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  {
    id: "hashicorp-vault-416",
    difficulty: "medium",
    certId: "hashicorp-vault",
    domainId: "d8",
    domainName: "Vault deployment architecture",
    title: "Surviving the Loss of a Cluster Node: Pharmaceutical Clinical Trial Platform",
    scenario: "A global pharmaceutical research group manages double-blind clinical trial records with strict regulatory reporting and audit trails. The platform team is designing a production cluster topology that tolerates the loss of a node. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      { id: 'A', text: "Deploy a single Vault node against a highly available managed database backend and restart the node automatically on failure." },
  ...
  {
    id: "hashicorp-vault-417",
    difficulty: "medium",
    certId: "hashicorp-vault",
    domainId: "d7",
    domainName: "Vault architecture fundamentals",
    title: "Restarting a Node Without an Operator Key Ceremony: Smart City Traffic & Mobility Sensor Hub",
    scenario: "A metropolitan transit authority optimizes urban traffic signals with real-time video analytics and edge inference. The platform team is bringing nodes back into service after a restart without assembling key holders. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  {
    id: "hashicorp-vault-418",
    difficulty: "medium",
    certId: "hashicorp-vault",
    domainId: "d8",
    domainName: "Vault deployment architecture",
    title: "Choosing a Cross-Region Replication Mode: Digital Identity & Biometric Verification",
    scenario: "A cross-border passport control gateway validates identity credentials with zero-knowledge cryptographic proofs. The platform team is planning cross-region failover alongside low-latency reads in each region. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      ...
      { id: 'B', text: "Use Disaster Recovery (DR) replication to maintain a warm-standby secondary cluster that can be promoted, or Performance Replication for localized secret reads." },
  ...
  {
    id: "hashicorp-vault-419",
    difficulty: "medium",
    certId: "hashicorp-vault",
    domainId: "d4",
    domainName: "Vault leases",
    title: "Withdrawing Credentials Already in Circulation: Legal Discovery & Semantic Document Search",
    scenario: "A global law firm conducts regulatory discovery across millions of scanned legal filings with vector-enhanced semantic retrieval. The platform team is cutting off credentials that have already been issued after a suspected compromise. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      { id: 'A', text: "Delete the auth method role so that no further credentials can be issued, and allow the ones already outstanding to lapse when their own TTL expires naturally." },
  ...
  {
    id: "hashicorp-vault-420",
    difficulty: "medium",
    certId: "hashicorp-vault",
    domainId: "d8",
    domainName: "Vault deployment architecture",
    title: "Audit Logging That Cannot Fail Quietly: AdTech Real-Time Bidding Exchange",
    scenario: "An advertising exchange processes 500,000 bids per second with a strict 20-millisecond SLA and distributed caching. The platform team is guaranteeing every request is recorded without the logging path halting the cluster. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      { id: 'A', text: "Enable a single file audit device and configure aggressive log rotation so the destination cannot fill its disk." },
      ...
      { id: 'D', text: "Enable multiple audit devices (e.g., file and syslog) and monitor them, so Vault keeps serving while at least one device can still write." }
    ],
    correctAnswers: ['D'],
    type: "single",
    explanation: "Vault refuses to serve a request it cannot log, so with one audit device that device becomes an availability dependency. Enabling a second device means Vault keeps serving as long as either one accepts writes, although it still blocks if every device fails. A lone socket device is the most fragile choice of all, because a brief network problem at the collector stalls the cluster, and rotation does not help a file device whose filesystem is full or read-only. Setting log_raw writes secrets to the log in the clear, trading a logging problem for a disclosure one.",
  ...
  {
    id: "hashicorp-vault-421",
    difficulty: "hard",
    certId: "hashicorp-vault",
    domainId: "d3",
    domainName: "Vault tokens",
    title: "Token Type for Very High Request Volume: Precision Agriculture & Drone Scouting",
    scenario: "An agricultural drone fleet captures multispectral crop imagery with automated computer vision defect classification. The platform team is choosing a token type for short-lived functions invoked many thousands of times per second. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      ...
      { id: 'B', text: "Use batch tokens with renewable set to true so that a long-running invocation is able to extend its own lifetime." },
  ...
  {
    id: "hashicorp-vault-422",
    difficulty: "hard",
    certId: "hashicorp-vault",
    domainId: "d9",
    domainName: "Access management architecture",
    title: "Delivering Secrets to Mixed Kubernetes and VM Workloads: Industrial Robotics Predictive Maintenance",
    scenario: "A semiconductor fabrication facility detects vibration harmonics on manufacturing robots to prevent unplanned downtime. The platform team is delivering secrets into applications that run on both Kubernetes and plain virtual machines. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      { id: 'A', text: "Deploy the Vault Secrets Operator so that secrets are synchronised into native Kubernetes Secret objects for pods to consume." },
      { id: 'B', text: "Have each application import the Vault API client library and manage its own login, token renewal, and secret refresh logic." },
  ...
  {
    id: "hashicorp-vault-423",
    difficulty: "hard",
    certId: "hashicorp-vault",
    domainId: "d8",
    domainName: "Vault deployment architecture",
    title: "Surviving the Loss of a Cluster Node: Educational Remote Proctoring Platform",
    scenario: "An online university platform enforces anti-plagiarism and biometric proctoring for high-stakes certification exams. The platform team is designing a production cluster topology that tolerates the loss of a node. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  {
    id: "hashicorp-vault-424",
    difficulty: "hard",
    certId: "hashicorp-vault",
    domainId: "d7",
    domainName: "Vault architecture fundamentals",
    title: "Restarting a Node Without an Operator Key Ceremony: Real Estate Valuation & Geo-Spatial Analytics",
    scenario: "A property appraisal engine fuses GIS parcel maps with real-time market transactions for automated valuation. The platform team is bringing nodes back into service after a restart without assembling key holders. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
  ...
  {
    id: "hashicorp-vault-425",
    difficulty: "hard",
    certId: "hashicorp-vault",
    domainId: "d8",
    domainName: "Vault deployment architecture",
    title: "Choosing a Cross-Region Replication Mode: Disaster Emergency Dispatch & Operations",
    scenario: "A municipal 911 emergency response platform guarantees 99.999% uptime with multi-region hot-standby active failover. The platform team is planning cross-region failover alongside low-latency reads in each region. The work is scoped to the staging environment.",
    question: "Which HashiCorp Vault approach best meets these requirements?",
    options: [
      { id: 'A', text: "Use Disaster Recovery (DR) replication to maintain a warm-standby secondary cluster that can be promoted, or Performance Replication for localized secret reads." },
      { id: 'B', text: "Run an independent cluster in each region and keep their secrets aligned with Vault Agent templates that copy values between them." },
...
export default HASHICORP_VAULT_QUESTIONS_17;
