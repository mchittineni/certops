export const GITHUB_ACTIONS_FLASHCARDS_4 = [
  ...
  },
  {
    ...
    back: "Reusable workflows can be nested up to <strong>10 levels</strong>: the top-level caller plus up to 9 levels of reusable workflows. A single workflow file can also call at most <strong>50 unique reusable workflows</strong>, counting nested calls. Exceeding either limit fails validation.",
  ...
  },
  {
    ...
    hint: "Billing for Actions artifact storage beyond the plan allowance.",
    back: "Each plan includes an Actions artifact storage allowance, for example <strong>500 MB on Free</strong> and <strong>1 GB on Pro</strong>. Storage beyond the allowance is billed per GB per month; reduce it by tuning <code>retention-days</code>.",
  ...
  },
  {
    ...
    back: "<code>actions/download-artifact</code> v4 and later can download artifacts from <em>another</em> workflow run or another repository: set <code>run-id</code>, <code>repository</code>, and a <code>github-token</code> with <code>actions:read</code> permission. Without those inputs it downloads artifacts from the current run.",
