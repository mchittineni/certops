export const GITHUB_ACTIONS_QUESTIONS_4 = [
  {
    ...
    options: [
      { id: 'A', text: "A reference to github.sha, which changes only when files change." },
      { id: 'B', text: "A call to toJSON on the matrix context." },
      { id: 'C', text: "A call to hashFiles with a glob matching the lockfiles, which returns a single hash over the matched file contents." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Reduce max-parallel to one, which allows the uploads to append to the existing artifact." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The metadata must list the dependencies under a packages key so the runner installs them." },
      { id: 'B', text: "The consuming workflow must run a setup-node step and install the action dependencies before using it." },
      { id: 'C', text: "The action must commit its runtime dependencies, in practice by bundling the code and its dependencies into a single committed file that the metadata points at, because the runner does not install packages for an action." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "A JavaScript action, which is the only type able to invoke other actions programmatically." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Split the bundle across several secrets and reassemble it in a step." },
      { id: 'C', text: "Store the bundle as an organization variable, which has no size restriction." },
      { id: 'D', text: "Commit the bundle to the repository encrypted with a passphrase held as a secret, and decrypt it during the run." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Both should be secrets, because both relate to an authenticated service." },
      { id: 'C', text: "Both should be variables, because masking would make the logs unreadable." },
      { id: 'D', text: "The base URL should be a configuration variable and the API key should be a secret, because variables are stored and displayed in plain text while secrets are encrypted and masked in logs." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Enterprise Actions policies, which can enable or disable Actions for organizations and can be scoped by repository visibility, with organization-level policies refining but not exceeding the enterprise setting." },
