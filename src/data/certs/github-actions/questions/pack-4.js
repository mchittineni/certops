export const GITHUB_ACTIONS_QUESTIONS_4 = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Adding a docs label to the commit through the API before pushing." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "The github.sha value is the SHA of the previous run, because pull request events lag by one commit." },
      { id: 'D', text: "The github.sha value is a synthetic identifier with no relationship to any commit, so the head SHA must always be read from the API." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Storing the credentials as organization secrets, which prevents forks from starting the workflow." },
      { id: 'B', text: "Adding a concurrency group, which deduplicates runs across the fork network." },
      { id: 'C', text: "Removing the schedule trigger, since scheduled workflows always propagate to forks and cannot be constrained." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Set defaults.run.working-directory on the job and override it on the other seven steps." },
      { id: 'C', text: "Split the step into its own job with needs." },
      { id: 'D', text: "Prefix the command with a change-directory call joined by a logical operator." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The secrets context is not available in a workflow-level env block, so the reference must move to a job-level or step-level env entry." },
      { id: 'B', text: "The secrets context requires the secret name to be uppercase, and the rejection is a naming error." },
      { id: 'C', text: "Secrets may only be referenced inside a with block passed to an action." },
      { id: 'D', text: "The workflow must declare a permissions block containing secrets: read before the context resolves." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Reduce the schedule interval to monthly, which combines the intervening updates into one pull request." },
      { id: 'D', text: "Define a groups entry in the Dependabot configuration so related action updates are combined into a single pull request." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The workflow_dispatch and schedule triggers are only recognised once the workflow file exists on the default branch, though a dispatch can then select another branch to run from." },
      { id: 'B', text: "The Actions tab caches the workflow list for up to an hour, so the button will appear shortly." },
      { id: 'C', text: "The button appears only after the workflow has run at least once from another trigger." },
      { id: 'D', text: "Manual triggers require the workflow to declare a permissions block, which is missing." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Caches are evicted whenever any branch writes a new entry with a different key." },
      { id: 'B', text: "Caches are scoped per runner, so a different runner never sees an existing entry." },
      { id: 'C', text: "Caches are scoped per branch, and a branch can read caches from its own ref, its base branch and the default branch, but not from an unrelated sibling branch." },
      { id: 'D', text: "Caches are only readable by the workflow run that created them." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Move the upload into a composite action, which handles naming automatically." },
      { id: 'B', text: "Reduce max-parallel to one, which allows the uploads to append to the existing artifact." },
      { id: 'C', text: "Give each job a distinct artifact name incorporating a matrix value, then merge or download them together in a later job." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Move the action to a larger runner, which builds the image faster and is the only available remedy." },
      { id: 'B', text: "Reduce the number of declared inputs, since each one adds container startup overhead." },
      { id: 'C', text: "Add a cache step for the Docker layers before the action, which the runner reuses when building the action image." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "No action type can call another action, so this must be a reusable workflow." },
      { id: 'B', text: "A JavaScript action, which is the only type able to invoke other actions programmatically." },
      { id: 'C', text: "A composite action, whose steps list may contain both uses steps referencing other actions and run steps." },
      { id: 'D', text: "A Docker container action, which can declare the other actions in its metadata." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A CODEOWNERS entry mapping the workflows directory to the platform team, combined with branch protection requiring review from code owners." },
      { id: 'B', text: "A push protection rule blocking commits that touch the workflows directory." },
      { id: 'C', text: "A required reviewer on the production environment, which also covers workflow edits." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure the runner proxy environment variables, including the no-proxy exclusions, in the environment of the runner service rather than only in the interactive shell." },
      { id: 'B', text: "Add the proxy address as a repository secret so the runner reads it at job start." },
      { id: 'C', text: "Place the runners in a runner group, which routes traffic through the organization proxy." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A repository ruleset applied to every private repository that blocks the workflows directory." },
      { id: 'B', text: "Removing the Actions tab from private repositories through the API." },
