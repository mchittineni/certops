export const GITHUB_ACTIONS_QUESTIONS_4 = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Split the step into its own job with needs." },
      { id: 'C', text: "Prefix the command with a change-directory call joined by a logical operator." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "No action type can call another action, so this must be a reusable workflow." },
      { id: 'C', text: "A Docker container action, which can declare the other actions in its metadata." },
  ...
  },
  {
    ...
    domainId: "d4",
    domainName: "Manage GitHub Actions for the enterprise",
    title: "Limiting Where Actions Runs Across an Enterprise",
    scenario: "An enterprise wants GitHub Actions available in only three of its organizations, and wants workflows there limited to actions created within the enterprise, with both rules enforced centrally rather than organization by organization.",
    question: "Which capability supports this?",
    options: [
      { id: 'A', text: "Setting the default workflow token permissions to read-only enterprise-wide, which stops workflows running." },
      { id: 'B', text: "Enterprise Actions policies, which enable Actions for selected organizations and restrict allowed actions." },
      { id: 'C', text: "A repository ruleset applied to every repository that blocks changes to the workflows directory." },
      { id: 'D', text: "Removing the Actions tab from every repository in the other organizations through the REST API." }
    ],
    correctAnswers: ['B'],
    type: "single",
    explanation: "Enterprise Actions policies let an enterprise owner enable Actions for all organizations, for specific organizations, or disable it, and restrict which actions may run by source, such as only actions created within the enterprise or those plus a selected list. The policy is not scoped by repository visibility; only settings such as retention and fork pull request handling differ by visibility. A ruleset blocking the workflows directory would stop legitimate work rather than govern execution, the Actions tab is not removable as a policy control, and read-only token defaults change what a workflow may do rather than whether it runs.",
