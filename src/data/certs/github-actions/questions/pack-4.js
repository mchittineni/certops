export const GITHUB_ACTIONS_QUESTIONS_4 = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "The default token lacks contents: read, so history is truncated." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The step should use single quotes around the expression, which prevents the shell from interpreting the value." },
      ...
      { id: 'D', text: "The expression is substituted into the script before the shell runs, so a crafted title becomes shell syntax; the value should instead be passed through an env entry and referenced as a shell variable, quoted." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Pushing with the no-verify flag, which tells the server not to dispatch events." },
      { id: 'B', text: "Adding an empty file named .noci to the repository root." },
      { id: 'C', text: "Adding a docs label to the commit through the API before pushing." },
      { id: 'D', text: "Including a recognised skip directive such as the skip ci marker in the commit message, which suppresses push and pull request triggered runs." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The github.sha value is the SHA of the previous run, because pull request events lag by one commit." },
      { id: 'B', text: "The github.sha value is a synthetic identifier with no relationship to any commit, so the head SHA must always be read from the API." },
      { id: 'C', text: "The github.sha value is the SHA of the ephemeral merge commit of the pull request into the base branch; the branch tip is available as the head SHA in the pull request payload." },
      { id: 'D', text: "The github.sha value is the base branch tip; the contributor commit is only available by running a git command after checkout." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A job-level if comparing github.repository to the upstream owner and repository name, so the job only runs in the canonical repository." },
      ...
      { id: 'D', text: "Adding a concurrency group, which deduplicates runs across the fork network." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Set defaults.run.working-directory on the job and override it on the other seven steps." },
      { id: 'D', text: "Split the step into its own job with needs." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Secrets may only be referenced inside a with block passed to an action." },
      { id: 'B', text: "The secrets context requires the secret name to be uppercase, and the rejection is a naming error." },
      { id: 'C', text: "The workflow must declare a permissions block containing secrets: read before the context resolves." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Only the exit code of the last command determines the step result in every shell, so each command must be joined with a logical and operator." },
      { id: 'D', text: "The step must be split into three separate steps, because a multi-line run step never fails on an intermediate command." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Define a groups entry in the Dependabot configuration so related action updates are combined into a single pull request." },
      { id: 'B', text: "Reduce the schedule interval to monthly, which combines the intervening updates into one pull request." },
      { id: 'C', text: "Set open-pull-requests-limit to one, which merges the remaining updates into the allowed pull request." },
      { id: 'D', text: "Disable the github-actions ecosystem and rely on the moving major version tags instead." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The button appears only after the workflow has run at least once from another trigger." },
      { id: 'B', text: "The workflow_dispatch and schedule triggers are only recognised once the workflow file exists on the default branch, though a dispatch can then select another branch to run from." },
      { id: 'C', text: "Manual triggers require the workflow to declare a permissions block, which is missing." },
      { id: 'D', text: "The Actions tab caches the workflow list for up to an hour, so the button will appear shortly." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Caches are scoped per branch, and a branch can read caches from its own ref, its base branch and the default branch, but not from an unrelated sibling branch." },
      ...
      { id: 'D', text: "Caches are scoped per runner, so a different runner never sees an existing entry." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Allow all actions, combined with a required review of the workflows directory." },
      { id: 'B', text: "Allow select actions, enabling the options for actions created by GitHub and by Marketplace verified creators, plus an allow list for specific exceptions." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Re-run the workflow again and hope to reproduce the failure." },
      { id: 'B', text: "Download the artifact from the failed attempt, which contains the logs." },
      { id: 'C', text: "Failed attempts are discarded when a re-run succeeds, so the logs must be reconstructed from the annotations." },
      { id: 'D', text: "Select the earlier attempt from the attempt selector on the run page, or request that attempt number through the API or CLI." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "By owner, repository and the latest release tag, then compare the result to the working copy." },
      ...
      { id: 'D', text: "By owner, repository and the branch name of the pull request head." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Publish the image to a registry and point the metadata at that prebuilt image reference, so the runner pulls it instead of building it on every job." },
      { id: 'C', text: "Reduce the number of declared inputs, since each one adds container startup overhead." },
      { id: 'D', text: "Move the action to a larger runner, which builds the image faster and is the only available remedy." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An organization Actions policy restricting who may edit workflow files." },
      { id: 'B', text: "A CODEOWNERS entry mapping the workflows directory to the platform team, combined with branch protection requiring review from code owners." },
      { id: 'C', text: "A push protection rule blocking commits that touch the workflows directory." },
      { id: 'D', text: "A required reviewer on the production environment, which also covers workflow edits." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The base URL should be a secret and the API key a variable, so the endpoint is hidden from contributors." },
      ...
      { id: 'C', text: "Both should be secrets, because both relate to an authenticated service." },
      { id: 'D', text: "Both should be variables, because masking would make the logs unreadable." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Configure the runner proxy environment variables, including the no-proxy exclusions, in the environment of the runner service rather than only in the interactive shell." },
      { id: 'C', text: "Add the proxy address as a repository secret so the runner reads it at job start." },
