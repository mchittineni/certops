export const GITHUB_ACTIONS_QUESTIONS_1 = [
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Upload the dependency directory as a build artifact and download it at the start of each run." },
      { id: 'B', text: "Set the cache key to a constant string so it never changes." },
      { id: 'C', text: "Replace the lockfile hash in the key with the run identifier so the key is always unique." },
      { id: 'D', text: "Add restore-keys containing a shorter prefix of the same key so a partial match seeds the directory before installation." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "pull_request_target does not provide the pull request number, so the coverage comment cannot be posted." },
      { id: 'B', text: "pull_request_target only fires when the pull request is merged, so coverage would be reported too late." },
      { id: 'C', text: "pull_request_target runs in the context of the base repository with access to secrets and a writable token, so checking out and executing untrusted fork code lets a contributor exfiltrate those secrets." },
      { id: 'D', text: "pull_request_target cannot be combined with a permissions block, so the token always has full write access." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Removal of the needs keyword so the teardown job is independent." },
      { id: 'C', text: "An if condition evaluating always(), because a job with needs is skipped by default once an upstream job fails." },
      { id: 'D', text: "A continue-on-error setting on the integration job, which is enough on its own to let dependents run." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A push trigger with branches set to main and paths-ignore set to the services directory." },
      { id: 'C', text: "A push trigger with branches set to main and paths set to a pattern matching the services directory." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Both belong in the cache, because the cache persists across workflow runs." },
      { id: 'D', text: "Both belong in artifacts, because artifacts are the only storage retained after a run." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The digest can only be transferred by uploading it as an artifact and downloading it in the caller." },
      { id: 'C', text: "The reusable workflow writes the digest to GITHUB_ENV, which propagates to the caller." },
      { id: 'D', text: "The reusable workflow declares an outputs block mapping a job output, and the calling workflow reads it from the needs context of the calling job." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "The action.yml is missing a branding block, which is required for composite actions." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Write the value to a file named outputs.json in the workspace root." },
      { id: 'D', text: "Call core.setOutput from the toolkit and declare a matching entry under outputs in action.yml." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The consuming workflow must grant packages: read so the runner can pull the image on non-Linux platforms." },
      { id: 'C', text: "The action image needs a multi-architecture manifest, after which it will run on all three platforms." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Store the access key pair in an environment secret protected by required reviewers." },
      ...
      { id: 'D', text: "Configure OpenID Connect trust between the cloud provider and GitHub, grant the job id-token: write, and exchange the short-lived token for a scoped cloud role." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Require approval for first-time contributors, which by itself makes persistent self-hosted runners safe for public repositories." },
      ...
      { id: 'C', text: "Grant the workflow a read-only GITHUB_TOKEN, which prevents the job from executing contributor code." },
      { id: 'D', text: "Use ephemeral runners, so each runner processes a single job and is then discarded along with anything the job left behind." }
    ],
    correctAnswers: ['B', 'D'],
    type: "multiple",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Create an environment secret in one repository and reference it from the other six." },
      { id: 'B', text: "Create an organization secret with its repository access set to the seven approved repositories." },
      { id: 'C', text: "Commit the token to a private repository and check it out during each workflow run." },
      { id: 'D', text: "Create an organization variable holding the token so every repository can read it." }
    ],
    correctAnswers: ['B'],
    type: "single",
