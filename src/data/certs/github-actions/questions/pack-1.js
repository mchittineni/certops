export const GITHUB_ACTIONS_QUESTIONS_1 = [
  {
    ...
    options: [
      ...
      { id: 'B', text: "Set timeout-minutes to a value lower than the average run duration." },
      { id: 'C', text: "Set strategy.fail-fast to true on the build job." },
      { id: 'D', text: "Add a concurrency block with group set to a value derived from the workflow and github.ref, and cancel-in-progress set to true." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set the job to run on a self-hosted runner so it inherits the repository owner permissions." },
      { id: 'B', text: "Add the actions/checkout step with persist-credentials set to false." },
      { id: 'C', text: "Add a permissions block granting contents: write at the workflow or job level." },
      { id: 'D', text: "Create a personal access token and store it as an organization variable." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A continue-on-error setting scoped to the windows-latest runner." },
      { id: 'B', text: "A max-parallel value of five." },
      { id: 'C', text: "An exclude entry naming node-version 16 together with os windows-latest." },
      { id: 'D', text: "An include entry listing the five wanted combinations." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Write the tag to an environment variable with export so later jobs inherit it." },
      { id: 'D', text: "Write the tag to the file referenced by GITHUB_ENV, which is shared across all jobs in the workflow." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Add restore-keys containing a shorter prefix of the same key so a partial match seeds the directory before installation." },
      { id: 'B', text: "Replace the lockfile hash in the key with the run identifier so the key is always unique." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "pull_request_target cannot be combined with a permissions block, so the token always has full write access." },
      { id: 'B', text: "pull_request_target does not provide the pull request number, so the coverage comment cannot be posted." },
      { id: 'C', text: "pull_request_target only fires when the pull request is merged, so coverage would be reported too late." },
      { id: 'D', text: "pull_request_target runs in the context of the base repository with access to secrets and a writable token, so checking out and executing untrusted fork code lets a contributor exfiltrate those secrets." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Rely on the needs relationship, which copies the workspace from the upstream job automatically." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A timeout-minutes value on the teardown job." },
      { id: 'B', text: "An if condition evaluating always(), because a job with needs is skipped by default once an upstream job fails." },
      { id: 'C', text: "A continue-on-error setting on the integration job, which is enough on its own to let dependents run." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A push trigger on main with a job-level if condition comparing github.event.head_commit.message to a docs prefix." },
      { id: 'B', text: "A push trigger with branches set to main and paths set to a pattern matching the services directory." },
      { id: 'C', text: "A push trigger with branches set to main and paths-ignore set to the services directory." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Define the shared file with a workflow_call trigger and inputs, then reference it from each repository with the uses keyword at job level, passing secrets: inherit." },
      { id: 'B', text: "Publish the workflow as a composite action and reference it as a step in each repository." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Reference the action by its full commit SHA rather than a tag or branch." },
      ...
      { id: 'C', text: "Move the workflow onto self-hosted runners so the action executes inside the corporate network." },
      { id: 'D', text: "Replace the version tag with the main branch so the newest code is always used." }
    ],
    correctAnswers: ['A', 'B'],
    type: "multiple",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The installer belongs in the cache and the dependency directory belongs in an artifact." },
      { id: 'B', text: "Both belong in the cache, because the cache persists across workflow runs." },
      { id: 'C', text: "Both belong in artifacts, because artifacts are the only storage retained after a run." },
      { id: 'D', text: "The dependency directory belongs in the cache and the installer belongs in an artifact." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The digest can only be transferred by uploading it as an artifact and downloading it in the caller." },
      { id: 'B', text: "The reusable workflow writes the digest to GITHUB_ENV, which propagates to the caller." },
      { id: 'C', text: "The reusable workflow declares an outputs block mapping a job output, and the calling workflow reads it from the needs context of the calling job." },
      { id: 'D', text: "The calling workflow reads the digest from the env context, which reusable workflows populate for their caller." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Push an empty commit to trigger a fresh run of the whole workflow." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The annotations shown on the summary page of each individual workflow run." },
      ...
      { id: 'D', text: "The Actions usage metrics and the downloadable usage report available from the organization billing settings." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Composite actions require runs.using to name a specific runtime version rather than the word composite." },
      { id: 'B', text: "The run step omits the shell key, which is mandatory for every run step in a composite action." },
      { id: 'C', text: "Composite actions cannot contain run steps, only uses steps." },
      { id: 'D', text: "The action.yml is missing a branding block, which is required for composite actions." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Call core.setOutput from the toolkit and declare a matching entry under outputs in action.yml." },
      { id: 'B', text: "Call core.exportVariable and declare the name under inputs in action.yml." },
      { id: 'C', text: "Print the value to standard output; the runner captures the last line as the step output automatically." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Docker container actions require the runner to have Docker installed, so the fix is to add a Docker setup step before the action on each platform." },
      ...
      { id: 'D', text: "Docker container actions execute only on Linux runners, so the action should be rewritten as a JavaScript action for cross-platform use." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Publish each release as a separate repository named after its version." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Move the access key pair from repository secrets to organization secrets scoped to selected repositories." },
      { id: 'B', text: "Configure OpenID Connect trust between the cloud provider and GitHub, grant the job id-token: write, and exchange the short-lived token for a scoped cloud role." },
      { id: 'C', text: "Rotate the access key pair automatically every twenty-four hours with a scheduled workflow." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Register the runners at repository level in all four repositories individually and delete the organization registration." },
      { id: 'B', text: "Place the runners in a runner group whose access is limited to the selected repositories." },
      { id: 'C', text: "Apply a unique label to the runners and ask teams not to reference that label." },
      { id: 'D', text: "Set an organization ruleset that blocks the runs-on key in non-production repositories." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Keep fork pull requests on GitHub-hosted runners and reserve the self-hosted pool for trusted internal repositories." },
      ...
      { id: 'D', text: "Require approval for first-time contributors, which by itself makes persistent self-hosted runners safe for public repositories." }
    ],
    correctAnswers: ['B', 'C'],
    type: "multiple",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Create an organization secret with its repository access set to the seven approved repositories." },
      { id: 'C', text: "Create an organization variable holding the token so every repository can read it." },
      { id: 'D', text: "Create an environment secret in one repository and reference it from the other six." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Require a CODEOWNERS review on the workflows directory in every repository." },
      { id: 'B', text: "Disable Actions entirely for the enterprise and re-enable it per repository as requests arrive." },
      { id: 'C', text: "Set the Actions permissions policy to allow enterprise actions plus specified actions, and populate the allow list with the reviewed third-party references." },
