export const GITHUB_ACTIONS_QUESTIONS_6 = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "By splitting into three workflow files, since one workflow cannot behave differently per trigger." },
      { id: 'C', text: "By comparing github.ref, which differs per trigger type." },
      { id: 'D', text: "By reading a workflow_dispatch input, which is populated with the trigger name for every event." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Add a verbose flag to every step, which is the only supported mechanism." },
      { id: 'B', text: "Re-run the jobs with the debug logging option enabled, or set the step and runner debug flags as repository secrets or variables for a broader investigation." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Add a concurrency group shared by all four jobs." },
      { id: 'B', text: "Declare needs naming only the slowest of the three, since the others always finish first." },
      { id: 'C', text: "Declare needs on the packaging job as a list naming all three jobs, which waits for every one and inherits the implicit success requirement." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The job name key, which replaces the run title when there is a single job." },
      { id: 'B', text: "The run-name key at workflow level, which accepts an expression and sets the title shown for each run." },
      { id: 'C', text: "The name key at workflow level, which is re-evaluated per run." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "An empty permissions mapping on the job, which disables every scope of the automatic token for that job." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Lower the compression-level input on the upload step so the archive is created with little or no compression." },
      { id: 'B', text: "Raise the compression-level input so the archive is smaller and uploads faster." },
      { id: 'C', text: "Split the directory across ten artifacts, which parallelises compression." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The workflow run reached the maximum total run duration of thirty-five days." },
      { id: 'B', text: "The job timeout-minutes value elapsed, which counts queue time as execution time." },
      { id: 'C', text: "A job waiting for a self-hosted runner is terminated once it has been queued for the maximum queue time of twenty-four hours." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Read the artifact contents out of the run logs, where they are recorded." },
      { id: 'C', text: "Use the GitHub CLI run download command with the run identifier, optionally naming the artifact." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Add the public action repositories to the organization allow list, which mirrors them automatically." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A job condition comparing github.ref to the permitted branch names." },
      { id: 'B', text: "Branch protection on the release branches requiring review." },
      { id: 'C', text: "A deployment branch policy on the production environment restricting which branches and tags may deploy to it." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Keep secrets: inherit, since the called workflow can only read secrets it declares in its workflow_call block anyway, making the two equivalent." },
      { id: 'B', text: "Replace secrets: inherit with an explicit secrets mapping passing only the single secret the called workflow declares." },
      { id: 'C', text: "Pass the secret through the with block instead, which avoids the secrets context entirely." },
      { id: 'D', text: "Move the fourteen unrelated secrets to organization level, which excludes them from inheritance." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Windows and macOS runners are billed per job started rather than per minute." },
      { id: 'C', text: "Linux minutes are free on all plans, so only other platforms appear in the bill." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A setup entry in the runs block, executed once per job." },
      { id: 'B', text: "A first-run flag on the main entry point." },
      { id: 'C', text: "A pre entry in the runs block naming a script the runner executes before the main entry point, with pre-if controlling when it applies." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A release-triggered workflow that force-updates the major version tag to the newly published release commit, guarded so it only runs for non-prerelease publications and only when the release is compatible." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Tokens are only obtainable through published cloud login actions, so one must be written and published first." },
      { id: 'C', text: "Read the token from the secrets context, where the platform places it when the permission is granted." },
      { id: 'D', text: "Call the repository API with the automatic token to mint an identity token." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Up to six individuals or teams may be listed as required reviewers, only one of them needs to approve, and the environment offers a setting that prevents the user who started the run from approving it." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Set an organization policy that freezes the runner version for all self-hosted runners." },
      { id: 'D', text: "Nothing can be done; self-hosted runners always update themselves." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Rely on workflow run logs, which capture the same governance events." },
      { id: 'C', text: "Increase the audit log retention setting to the required number of years." },
      { id: 'D', text: "Export the audit log by hand each quarter from the web interface." }
    ],
    correctAnswers: ['A'],
    type: "single",
