export const GITHUB_ACTIONS_QUESTIONS_6 = [
  {
    ...
    options: [
      { id: 'A', text: "The conclusion property, which records the raw result, whereas outcome is adjusted by continue-on-error." },
      { id: 'B', text: "Neither, because a tolerated failure is discarded and cannot be observed later in the job." },
      { id: 'C', text: "The outcome property of the step, which records the result before continue-on-error is applied, whereas conclusion records the result after it." },
      { id: 'D', text: "The exit-code property, which is the only unmodified signal available." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "The two filters cannot both be used, so the intent should be written with branches alone listing every branch name explicitly." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Chain the three jobs with needs so they run in sequence, then have packaging depend on the last." },
      { id: 'D', text: "Add a concurrency group shared by all four jobs." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Setting the job to run on a self-hosted runner, which is not issued a token." },
      { id: 'B', text: "Setting permissions to contents: none, which is the only scope that can be removed." },
      { id: 'C', text: "Omitting the permissions key, which defaults to no permissions." },
      { id: 'D', text: "An empty permissions mapping on the job, which disables every scope of the automatic token for that job." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Replace secrets: inherit with an explicit secrets mapping passing only the single secret the called workflow declares." },
      { id: 'B', text: "Pass the secret through the with block instead, which avoids the secrets context entirely." },
      { id: 'C', text: "Move the fourteen unrelated secrets to organization level, which excludes them from inheritance." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Windows and macOS runners are billed per job started rather than per minute." },
      { id: 'B', text: "Jobs shorter than five minutes are rounded up to an hour on non-Linux platforms." },
      { id: 'C', text: "Minutes on Windows and macOS runners are billed at a multiplier relative to Linux, so a short job on those platforms can cost more than a long Linux job." },
      { id: 'D', text: "Linux minutes are free on all plans, so only other platforms appear in the bill." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Read the token from the secrets context, where the platform places it when the permission is granted." },
      { id: 'C', text: "Call the repository API with the automatic token to mint an identity token." },
      { id: 'D', text: "Tokens are only obtainable through published cloud login actions, so one must be written and published first." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Nothing can be done; self-hosted runners always update themselves." },
      { id: 'B', text: "Start the runner with the update-disabling option and take responsibility for installing new runner versions before the old one stops being supported, since an outdated runner eventually cannot connect." },
      { id: 'C', text: "Block the update endpoint at the firewall, which stops the update without any operational obligation." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Register each instance interactively with a personal access token held in the instance metadata." },
      { id: 'B', text: "Generate a just-in-time runner configuration through the API for each instance, which encodes a single-use ephemeral registration the runner consumes at start-up." },
      { id: 'C', text: "Register a persistent runner and have the last step of every job restart the host." },
      { id: 'D', text: "Bake a registration token into the instance image and rotate the image weekly." }
    ],
    correctAnswers: ['B'],
    type: "single",
