export const GITHUB_ACTIONS_QUESTIONS_7 = [
  {
    ...
    options: [
      { id: 'A', text: "The github.event_name value, which is set to the activity type when types are declared." },
      { id: 'B', text: "The github.event.type value, which is populated for every event." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Use the delimiter form, writing the name followed by a chosen delimiter, then the content, then the delimiter again, picking a delimiter that cannot occur in the content." },
      { id: 'B', text: "Replace the newlines with an escape sequence and rely on the runner to restore them." },
      { id: 'C', text: "Write the value to an artifact and set the output to the artifact name, since multi-line outputs are unsupported." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The automatic token covers every repository in the organization, so the failure indicates the repository name is misspelled." },
      { id: 'B', text: "The automatic token is scoped to the current repository only, so the second checkout needs a separate credential such as a GitHub App installation token or a fine-grained token with read access to that repository, supplied to the step." },
      { id: 'C', text: "A permissions block granting contents: read extends the automatic token to sibling repositories." },
      { id: 'D', text: "Cross-repository checkout requires the repositories to share a runner group." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The limit applies per job rather than per run, so raising max-parallel resolves it." },
      { id: 'C', text: "A matrix may produce at most two hundred and fifty-six jobs per workflow run, so the strategy must be reduced, typically by testing the full cross-product on a schedule and a representative subset on pull requests." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "A workflow_run trigger, with the portal starting a placeholder workflow first." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A concurrency group on the step with a three-minute limit." },
      { id: 'C', text: "A continue-on-error setting on the step, which stops it after the default step limit." },
      { id: 'D', text: "A timeout-minutes value on the job, which applies to its longest step." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The github.ref value for the source branch and github.ref_name for the target branch." },
      { id: 'C', text: "The github.base_ref value for the source branch and github.head_ref for the target branch." },
      { id: 'D', text: "Only github.ref is available, so both names must be parsed from it." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "An early non-zero exit, because a skipped job blocks any required status check of the same name." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Through a workflow-level env block, which is the only place the secrets context resolves." },
      { id: 'B', text: "By writing the secret to GITHUB_ENV in the preceding step, which is the only supported route for secrets." },
      { id: 'C', text: "Through an env block on the step, mapping the environment variable name to the secrets context reference." },
      { id: 'D', text: "Through the with block, since every action input becomes an environment variable of the same name." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Set continue-on-error to true on the job, and fail the run from a later job if a supported version failed." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The workflow_call trigger supports boolean, number and string input types but not choice, so the input should be a string with the permitted values validated inside the workflow or expressed through separate callers." },
      { id: 'B', text: "The workflow_call trigger supports choice, so the rejection indicates a syntax error elsewhere." },
      { id: 'C', text: "Reusable workflows cannot declare typed inputs at all; every value arrives as a string." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Each repository has its own fixed limit of five concurrent jobs regardless of plan." },
      { id: 'B', text: "Hosted runners are allocated to repositories in alphabetical order during periods of contention." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Workflows never run for pull requests from a fork of a private repository, and the setting cannot be changed." },
      { id: 'B', text: "Running workflows from private forks is off by default and is enabled by an explicit setting, which separately controls whether such runs may read secrets and whether the automatic token may have write access." },
      { id: 'C', text: "Workflows from private forks always run with full access, because the fork is inside the organization." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The GitHub CLI workflow list command, which enumerates workflows with their state and identifiers, with view and disable subcommands for the individual ones." },
      { id: 'B', text: "The GitHub CLI run list command, which enumerates workflows rather than runs." },
      { id: 'C', text: "A recursive directory listing of the workflows directory, which is the only inventory available." },
      { id: 'D', text: "The GitHub CLI repo view command with the workflows flag." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Add a retry with backoff around the step, since resolution failures are usually transient." },
      { id: 'C', text: "Raise the incident with the Marketplace, since deleted actions are restored on request." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Both are configuration errors caught when the call is evaluated, because a called workflow accepts only the inputs it declares and the supplied values must match the declared types." },
      { id: 'B', text: "Both are ignored silently, and the called workflow sees default values." },
      { id: 'C', text: "The undeclared key is accepted and reachable through the inputs context, while the type mismatch is coerced." },
      { id: 'D', text: "Both are runtime errors surfaced only when a step reads the value." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "An outputs entry whose value property is an expression referencing the inner step output by that step identifier." },
      { id: 'C', text: "A permissions block granting the action the ability to publish outputs." },
      { id: 'D', text: "Nothing; inner step outputs are exposed automatically under the identifier of the inner step." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Container actions require an absolute path to the Dockerfile." },
      { id: 'B', text: "The image path is resolved relative to the directory containing the action metadata file, so the Dockerfile must accompany action.yml in the same directory." },
      { id: 'C', text: "The image path is resolved relative to the root of the action repository regardless of where the metadata sits." },
      { id: 'D', text: "The image path is resolved relative to the consuming repository workspace." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Store the token as an environment secret on an environment named dependabot." },
      ...
      { id: 'C', text: "Store the token as a Dependabot secret, which is a separate store from Actions secrets, and reference it from the registries section of the Dependabot configuration." },
  ...
  },
  {
    ...
    question: "What is required?",
    options: [
      { id: 'A', text: "Grant the job packages: write in its permissions block, and ensure the package is configured to allow the repository to publish to it." },
      { id: 'B', text: "Replace the automatic token with a personal access token, since the automatic token can never publish packages." },
      { id: 'C', text: "Grant the job contents: write, which covers registry publication." },
      { id: 'D', text: "Move the job to a self-hosted runner inside the organization network." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "At the organization level, placed in a runner group whose access starts with the single repository and is widened as the others need it." },
      { id: 'B', text: "At the repository level, then re-registered at organization level when the second repository appears." },
      { id: 'C', text: "At the enterprise level, since that is the only level supporting runner groups." },
      { id: 'D', text: "At the repository level in all three repositories simultaneously, using the same machine." }
    ],
    correctAnswers: ['A'],
    type: "single",
