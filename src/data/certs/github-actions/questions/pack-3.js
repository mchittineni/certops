export const GITHUB_ACTIONS_QUESTIONS_3 = [
  {
    ...
    options: [
      { id: 'A', text: "A create trigger, with a job-level if comparing github.ref_type to tag." },
      { id: 'B', text: "A push trigger with a tags filter carrying the version pattern and no branches filter." },
      { id: 'C', text: "A push trigger with both a branches filter of the default branch and a tags filter." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set strategy.fail-fast to false so a failing combination no longer cancels its siblings." },
      { id: 'B', text: "Set continue-on-error to true on the test step in every combination." },
      { id: 'C', text: "Raise strategy.max-parallel so all six combinations start before any can fail." },
      { id: 'D', text: "Split the matrix into six separate jobs joined by needs." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "timeout-minutes on the job, replacing a default limit of three hundred and sixty minutes." },
      { id: 'B', text: "concurrency on the job, replacing an unlimited default." },
      { id: 'C', text: "retry-limit on the job, replacing a default of three attempts." },
      { id: 'D', text: "continue-on-error on the job, replacing a default of false." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A container key on the job naming the image, so the runner executes the job steps inside it." },
      { id: 'C', text: "A runs-on value naming the image instead of a runner label." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Emit a notice workflow command containing the markdown table." },
      ...
      { id: 'D', text: "Append markdown to the file path held in GITHUB_STEP_SUMMARY from a step." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A comparison of github.ref to the literal string release/, which matches by prefix." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Switch to a self-hosted runner, which is the only way to control installed packages." },
      { id: 'B', text: "Clear the Actions cache for the repository, which holds the previous image contents." },
      { id: 'C', text: "Pin runs-on to the specific Ubuntu image version to regain a known-good environment, and install the package explicitly so the workflow no longer depends on undocumented image contents." },
      { id: 'D', text: "Add a retry loop around the failing step, since hosted image contents vary run to run." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "It runs using the version of the workflow file on the default branch and in the context of the base repository with secrets available, so it must treat any artifact from the first workflow as untrusted input." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A second matrix dimension holding the flag, which doubles the combinations." },
      { id: 'C', text: "A step-level if comparing the Node version, since matrix values cannot be conditional." },
      { id: 'D', text: "An include entry whose keys match the existing Node 20 combination and which adds the extra key, expanding that combination rather than creating a new one." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A create trigger, which fires when a release is created." },
      { id: 'C', text: "A release trigger with no types filter, which defaults to published only." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A reusable workflow is referenced by a workflow_call entry in the calling workflow triggers." },
      { id: 'B', text: "A reusable workflow is referenced by adding its name to the needs list of the calling job." },
      { id: 'C', text: "The uses key belongs on a step, but the reference must omit the .github/workflows portion of the path." },
      { id: 'D', text: "The uses key belongs on the job itself, referencing owner, repository, the path under .github/workflows, and a ref, with no steps in that job." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "There is no limit on nesting depth, only on the total number of jobs in a run." },
      { id: 'B', text: "Reusable workflows cannot call other reusable workflows at all, so only one level is possible." },
      { id: 'C', text: "Nesting is capped at four levels of reusable workflows in a chain, so a five-deep chain must be flattened." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Add each consuming repository as a collaborator on the action repository." },
      { id: 'B', text: "Grant the consuming workflows a permissions block with contents: read for the action repository." },
      { id: 'C', text: "Make the action repository public, which is the only way to share an action." },
      { id: 'D', text: "In the action repository Actions settings, enable access so its actions and workflows may be used by other repositories in the organization." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "A repository or organization setting requires approval before workflows run for first-time contributors, and a maintainer approves each run from the pull request checks area." },
      { id: 'D', text: "Workflows never run for contributors without write access, and the pending state is permanent." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The Insights dependency graph for the deployed commit." },
      { id: 'B', text: "The artifact retention list for the repository." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A concurrency block with a group naming the environment and cancel-in-progress set to false, so a second run queues until the first completes." },
      { id: 'B', text: "A concurrency block with cancel-in-progress set to true, which is the safe default for deployments." },
      { id: 'C', text: "A max-parallel value of one on the deployment matrix." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Download the run logs archive from the run page, or fetch it through the API or CLI, and store it in the evidence system." },
      { id: 'C', text: "Increase the repository retention setting, which extends retention for runs already recorded." },
      { id: 'D', text: "Re-run the workflow so a fresh copy of the logs is created with a new retention window." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Declare the token under a secrets block in action.yml, mirroring reusable workflow syntax." },
      { id: 'C', text: "Read the token from an environment variable that the runner populates from repository secrets automatically." },
      { id: 'D', text: "Reference the secret through the env context instead of the secrets context." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "In a file named inputs.json placed in the container working directory by the runner." },
      { id: 'B', text: "As an environment variable whose name is the input name uppercased with non-alphanumeric characters replaced by underscores, prefixed with INPUT_." },
      { id: 'C', text: "As the first positional argument, since undeclared args default to the input order." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "An image file named icon.svg committed to the repository root." },
      { id: 'C', text: "The repository social preview image configured in repository settings." },
      { id: 'D', text: "A logo key inside the runs block of action.yml." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set the default workflow permissions for the organization to read-only, after which a workflow raises its own scope with a permissions block." },
      ...
      { id: 'D', text: "Enable required reviewers on every environment, which downgrades the token." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Secrets are withheld and the token is read-only for workflows triggered by a fork pull request, because the workflow definition and the code both come from an untrusted source." },
      { id: 'B', text: "Secrets are withheld only until a maintainer approves the run, after which the same run receives them." },
      { id: 'C', text: "The secrets exist but must be referenced through the env context in fork pull requests." },
      { id: 'D', text: "The fork must be added to the repository runner group before secrets are passed." }
    ],
    correctAnswers: ['A'],
    type: "single",
