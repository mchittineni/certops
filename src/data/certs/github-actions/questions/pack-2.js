export const GITHUB_ACTIONS_QUESTIONS_2 = [
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A separate job running the database, with the test job declaring needs on it." },
      { id: 'B', text: "A services block on the job declaring a postgres container image with health check options." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "continue-on-error set to true on the upload step." },
      { id: 'B', text: "if evaluating cancelled(), which covers any non-successful outcome." },
      { id: 'C', text: "if evaluating success(), which is the default and therefore explicit." },
      { id: 'D', text: "if evaluating failure(), which is true when a previous step in the job has failed." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A defaults block on the job setting run.working-directory to frontend." },
      { id: 'B', text: "A workflow-level env entry named WORKING_DIRECTORY set to frontend." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "In a repository named starter-workflows owned by the organization." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Put the GPU runners in their own group and grant only this repository access to it." },
      { id: 'C', text: "Give the GPU runners a higher priority value so the scheduler always prefers them." },
      { id: 'D', text: "Set `runs-on` to the hostname of one particular GPU machine in the runner pool." }
    ],
    correctAnswers: ['A'],
    type: "single",
    explanation: "Labels route jobs to self-hosted runners: giving runs-on a list means the job is dispatched only to a runner carrying every label in it, so combining self-hosted with a hardware label pins the job to that subset. A runner group by itself controls which repositories may use its runners; it routes a job only when runs-on names it with the group key, so merely granting the repository access does not stop the job starting on an ordinary runner. Runners have no priority field, and targeting a single hostname would remove redundancy even if it were supported.",
