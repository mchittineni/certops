export const GITHUB_ACTIONS_QUESTIONS_2 = [
  {
    ...
    options: [
      { id: 'A', text: "debug, because the most specific scope wins and step-level env overrides job and workflow level." },
      { id: 'B', text: "warn, because job-level env always takes priority over step-level env." },
      { id: 'C', text: "info, because workflow-level variables are applied last and override narrower scopes." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Add the value to the workflow-level env block using an expression that reads the earlier step." },
      { id: 'C', text: "Write the value to GITHUB_PATH, which the runner converts into an environment variable." },
      { id: 'D', text: "Use export in the first step, because all steps in a job share one shell session." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "A composite action that starts the database in the background with a run step." },
      { id: 'D', text: "A container key on the job, which runs the database image as the job container." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Convert build into a composite action so branch protection resolves it differently." },
      { id: 'D', text: "Remove build from the required checks list and rely on reviewers noticing failures." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A trap registered in the shell by the action, which the runner preserves across steps." },
      { id: 'B', text: "A cleanup key under runs, which the runner calls after the final step." },
      { id: 'C', text: "A post entry under runs, naming a script the runner executes at the end of the job, with post-if available to control when it runs." },
      { id: 'D', text: "A separate teardown job with an if condition of always()." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Move the action so its metadata file sits at the root of its own repository, since a listing maps to a single action at the repository root." },
      { id: 'B', text: "Add a workflow that runs the action, because the Marketplace validates behaviour before listing." },
      { id: 'C', text: "Convert the action to a Docker container action, because only container actions are listable." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Recompile the action bundle, which embeds the runtime version." },
      ...
      { id: 'D', text: "Update the runs.using value in action.yml to the supported Node major version and verify the bundled code runs on it." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "As three organization secrets scoped to the repository." },
      ...
      { id: 'C', text: "As encrypted values committed to the repository and decrypted at run time with a single master repository secret." },
      { id: 'D', text: "As three repository secrets with names prefixed by the target, and a naming convention enforced in review." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Add the runners to a runner group, which enables automatic scaling for that group." },
      { id: 'B', text: "Deploy Actions Runner Controller with a runner scale set, so ephemeral runner pods are created in response to queued jobs and removed afterwards." },
