export const GITHUB_ACTIONS_QUESTIONS_10 = [
  {
    ...
    options: [
      { id: 'A', text: "Schedules accept a seconds field, which allows sub-minute precision when needed." },
      { id: 'B', text: "Cron expressions are in coordinated universal time and runs are guaranteed to start within ten seconds of the scheduled moment." },
      { id: 'C', text: "Cron expressions follow the repository configured time zone, and the minimum interval is one minute." },
      { id: 'D', text: "Cron expressions are interpreted in coordinated universal time rather than a local zone, the shortest supported interval is five minutes, and runs may be delayed during periods of high load, so a schedule is a best-effort trigger rather than a guarantee." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Conditions are supported at workflow level but must appear before the trigger declaration." },
      { id: 'B', text: "Conditions are supported only on steps, so a job-level condition would also be rejected." },
      { id: 'C', text: "Conditions are supported on jobs and on steps but not at workflow level, so the intent is expressed either by trigger filters or by putting the same condition on every job." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A concurrency group on the job, which limits it to one at a time and is the only available control." },
      { id: 'B', text: "Reducing the matrix to six combinations and iterating over the rest inside each job." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Declare the output in a job outputs block, which removes the need to set it from the step." },
      { id: 'B', text: "Use the set-env command instead, which supersedes set-output." },
      { id: 'C', text: "Prefix the command with a double colon, which re-enables the legacy syntax." },
      { id: 'D', text: "Append a name and value pair to the file named by GITHUB_OUTPUT, which replaced the deprecated command form." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Rename the job identifier to include the matrix values." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "There is no way to emit the delimiter, so the file must be produced by an action written in a general purpose language." },
      ...
      { id: 'D', text: "Place the content in a single-quoted YAML block, which suppresses expression evaluation." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "As an artifact automatically attached to every run." },
      { id: 'C', text: "As an environment variable named GITHUB_EVENT containing the serialised payload." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The closed activity, because a push trigger also fires for pushes to other branches." },
      { id: 'B', text: "The two are exactly equivalent, so the choice is a matter of style." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "There is no tooling; each pipeline must be rewritten by hand after a manual inventory." },
      { id: 'C', text: "Starter workflows, which detect the previous system and convert its configuration on first use." },
      { id: 'D', text: "The GitHub Actions Importer, which audits an existing estate to report usage and complexity, forecasts runner needs, and performs dry-run and actual conversions of supported pipelines." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Subdirectories are supported only for reusable workflows." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The command line reads its credential from an environment variable, so the step must set the token variable from the secrets context; a permissions block scopes the token but does not hand it to the tool." },
      { id: 'C', text: "The command line cannot authenticate inside a workflow and requires a personal access token stored in a secret." },
      { id: 'D', text: "The command line must be installed with a setup step, which also configures authentication." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Use a runs-on value listing both the hosted label and the self-hosted labels, so whichever exists is matched." },
      { id: 'B', text: "Declare a runner label as a typed input of the reusable workflow with a sensible default, and reference that input in runs-on so each caller supplies the label its environment provides." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "Make the repositories private, which removes previously generated logs." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Increase artifact retention so builds can be reused instead of repeated." },
      ...
      { id: 'D', text: "Move every job to larger runners, since faster machines consume fewer minutes overall." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Yes, but only by setting a defaults block, which composite actions support." },
      { id: 'C', text: "No; composite actions support only bash, so other languages require a container action." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Set the action to composite, which serialises its steps globally." },
      { id: 'C', text: "Add a concurrency key to the runs block, which the runner honours across repositories." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "As a patch release with the old input kept as an undocumented alias, which is always preferable to a major version." },
      ...
      { id: 'D', text: "As a minor release, because the new input is additive and renames are considered internal." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "A composite action, because it expresses a sequence of steps in the workflow syntax the team already knows, adds no build or image pull, and runs on any platform the underlying tools support." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Registration is unrestricted but runners must be approved before their first job, which is where the control lies." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Configure a required status check that pages on failure." },
      ...
      { id: 'D', text: "Subscribe an organization-level webhook to workflow run and deployment status events and route failures to the alerting system, so no repository workflow has to change." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "The number of workflow runs per repository and the average run duration." },
      { id: 'C', text: "Workflow file formatting, job naming conventions and badge placement in each README." },
