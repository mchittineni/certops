export const GITHUB_ACTIONS_QUESTIONS_10 = [
  {
    ...
    options: [
      { id: 'A', text: "Cron expressions are interpreted in coordinated universal time rather than a local zone, the shortest supported interval is five minutes, and runs may be delayed during periods of high load, so a schedule is a best-effort trigger rather than a guarantee." },
      { id: 'B', text: "Cron expressions follow the repository configured time zone, and the minimum interval is one minute." },
      { id: 'C', text: "Cron expressions are in coordinated universal time and runs are guaranteed to start within ten seconds of the scheduled moment." },
      { id: 'D', text: "Schedules accept a seconds field, which allows sub-minute precision when needed." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The writing step has no id, so there is no key under which the steps context can expose its outputs." },
      { id: 'B', text: "The writing step has no name, which is what the steps context is keyed by." },
      { id: 'C', text: "The reading step must declare needs naming the writing step." },
      { id: 'D', text: "Step outputs are only readable from a different job through the needs context." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "Conditions are supported at workflow level only for scheduled workflows." },
      { id: 'C', text: "Conditions are supported at workflow level but must appear before the trigger declaration." },
      { id: 'D', text: "Conditions are supported on jobs and on steps but not at workflow level, so the intent is expressed either by trigger filters or by putting the same condition on every job." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A fail-fast setting, which staggers the jobs." },
      { id: 'B', text: "A concurrency group on the job, which limits it to one at a time and is the only available control." },
      { id: 'C', text: "Reducing the matrix to six combinations and iterating over the rest inside each job." },
      { id: 'D', text: "A max-parallel value of six on the matrix strategy, which limits how many of its jobs run at once without reducing the number of combinations." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The fromJSON function, which parses a JSON string from an expression into an object or array whose properties can then be addressed." },
      { id: 'B', text: "The toJSON function, which converts a string into an addressable object." },
      { id: 'C', text: "The format function, which extracts named fields from a JSON string." },
      { id: 'D', text: "There is no such function; JSON must be parsed by a shell step and re-emitted as separate outputs." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Set the job name to an expression incorporating the meaningful matrix values, which becomes the displayed name for each leg." },
      { id: 'B', text: "Rename the job identifier to include the matrix values." },
      { id: 'C', text: "Set run-name, which controls how each matrix leg is displayed." },
      { id: 'D', text: "Add a display key to each matrix entry." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Prefix each brace with a backslash, which the expression parser treats as an escape." },
      { id: 'B', text: "Place the content in a single-quoted YAML block, which suppresses expression evaluation." },
      { id: 'C', text: "Wrap a quoted string containing the delimiter in an expression, so the expression evaluates to the literal text." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "As an artifact automatically attached to every run." },
      { id: 'B', text: "As an environment variable named GITHUB_EVENT containing the serialised payload." },
      { id: 'C', text: "As a JSON file on disk whose path is given by the GITHUB_EVENT_PATH environment variable, which the step can read and parse, treating the contents as untrusted input." },
      { id: 'D', text: "Only through individual expressions, since the payload is never written to disk." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The two are exactly equivalent, so the choice is a matter of style." },
      { id: 'B', text: "The closed activity, because a push trigger also fires for pushes to other branches." },
      { id: 'C', text: "Neither; deployment can only be triggered by a release event." },
      { id: 'D', text: "The push trigger on the default branch, because it fires for every way a change lands there including direct pushes and merge queue merges, whereas the closed activity misses those paths." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Starter workflows, which detect the previous system and convert its configuration on first use." },
      { id: 'B', text: "The GitHub Actions Importer, which audits an existing estate to report usage and complexity, forecasts runner needs, and performs dry-run and actual conversions of supported pipelines." },
      { id: 'C', text: "The actions synchronisation tool, which mirrors pipelines from other systems." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Workflow files are read only from directly within the .github/workflows directory, and files placed in subdirectories beneath it are ignored." },
      { id: 'B', text: "Subdirectories are supported but each needs an index file listing its workflows." },
      { id: 'C', text: "Subdirectories are supported only for reusable workflows." },
      { id: 'D', text: "The files are read but their names must be unique across all subdirectories, which the move violated." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The command line reads its credential from an environment variable, so the step must set the token variable from the secrets context; a permissions block scopes the token but does not hand it to the tool." },
      ...
      { id: 'D', text: "The permissions block must additionally grant a cli scope." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Declare a runner label as a typed input of the reusable workflow with a sensible default, and reference that input in runs-on so each caller supplies the label its environment provides." },
      ...
      { id: 'D', text: "Omit runs-on, which makes the platform choose an available runner." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Delete the logs and artifacts of the identified runs through the API endpoints for run logs and artifacts, or delete the runs themselves, driving it from a script across the repositories." },
      { id: 'B', text: "Reduce the retention setting to one day, which retroactively removes existing data." },
      { id: 'C', text: "Runs cannot be deleted before their retention expires, so the request cannot be satisfied." },
      { id: 'D', text: "Make the repositories private, which removes previously generated logs." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Convert all workflows into reusable workflows, which are billed at a lower rate." },
      { id: 'B', text: "Move every job to larger runners, since faster machines consume fewer minutes overall." },
      { id: 'C', text: "Cancel superseded runs with concurrency groups, restrict expensive non-Linux legs to the schedule rather than every pull request, filter triggers by path so unaffected work does not run, and cache dependencies effectively." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "No; every run step in a composite action must use the same shell, declared once for the action." },
      { id: 'C', text: "Yes, but only by setting a defaults block, which composite actions support." },
      { id: 'D', text: "No; composite actions support only bash, so other languages require a container action." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Add a concurrency key to the runs block, which the runner honours across repositories." },
      { id: 'B', text: "Declare an exclusive input defaulting to true, which the runner interprets as a mutual exclusion request." },
      { id: 'C', text: "Set the action to composite, which serialises its steps globally." },
      { id: 'D', text: "Serialisation is a property of runs rather than of actions, so the metadata cannot express it; the action should document that callers must place it in a concurrency group, and where possible take a lock in the external system so correctness does not depend on the caller." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "As a patch release, because two of the three changes are backwards compatible." },
      { id: 'B', text: "As a minor release, because the new input is additive and renames are considered internal." },
      { id: 'C', text: "As a new major version, because renaming an input breaks every caller passing the old name, and the major version tag should only be moved forward for callers once they have migrated." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A composite action, because it expresses a sequence of steps in the workflow syntax the team already knows, adds no build or image pull, and runs on any platform the underlying tools support." },
      { id: 'B', text: "A Docker container action, because it guarantees the tools are present." },
      { id: 'C', text: "A JavaScript action, because only that type can shape arguments." },
      { id: 'D', text: "None; a sequence of tool invocations must remain inline in each workflow." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Registering a runner requires a registration token that only someone with administrative access at the corresponding level can obtain, and the registration is recorded in the audit log." },
      { id: 'B', text: "Any member with write access to a repository can register an organization runner." },
      { id: 'C', text: "Registration requires only the organization name, since tokens are needed solely for removal." },
      { id: 'D', text: "Registration is unrestricted but runners must be approved before their first job, which is where the control lies." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Declare the environment on the job inside the reusable workflow, so its protection rules and environment secrets apply to every caller; the environment and its rules are resolved in the calling repository, so each repository must define it." },
      { id: 'B', text: "Declare the environment in each calling workflow, because a reusable workflow cannot declare one." },
      { id: 'C', text: "Declare the environment in the repository that hosts the reusable workflow, whose rules then apply to every caller." },
      { id: 'D', text: "Environments cannot be combined with reusable workflows, so approvals must be implemented as a manual dispatch gate." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Subscribe an organization-level webhook to workflow run and deployment status events and route failures to the alerting system, so no repository workflow has to change." },
      { id: 'B', text: "Enable email notifications for the platform team on each repository." },
      { id: 'C', text: "Add a scheduled workflow in one repository that polls the API for every other repository." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Workflow file formatting, job naming conventions and badge placement in each README." },
      { id: 'B', text: "The list of Marketplace actions by popularity, and which teams use the most minutes." },
      { id: 'C', text: "The number of workflow runs per repository and the average run duration." },
      { id: 'D', text: "The allowed actions policy and its allow list, default token permissions, runner group membership and repository access, stored secrets and their scope and age, and the cloud trust policies that accept identity tokens." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Run the untrusted build in a job with no secrets and a token holding no write scopes, hand its result forward as an artifact, and perform signing and publishing in a separate job that holds the credentials and never executes contributor code." },
      { id: 'B', text: "Run both in one job, granting the write scopes only from the publishing step onward using a step-level permissions block." },
      { id: 'C', text: "Run both in one job on an ephemeral runner, since the runner being discarded prevents credential reuse." },
      { id: 'D', text: "Run the build with continue-on-error and publish regardless, checking signatures afterwards." }
    ],
    correctAnswers: ['A'],
    type: "single",
