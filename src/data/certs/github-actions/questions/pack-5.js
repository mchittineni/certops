export const GITHUB_ACTIONS_QUESTIONS_5 = [
  {
    ...
    options: [
      ...
      { id: 'B', text: "A concurrency group keyed on the pull request number, which suppresses draft runs." },
      { id: 'C', text: "A paths filter excluding files typically changed during drafting." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The trigger must list activity types explicitly, adding labeled to the default set of opened, synchronize and reopened, because naming any types replaces the default set." },
      { id: 'B', text: "Nothing, because labelling counts as an update and already triggers the synchronize activity." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A line appended to GITHUB_STEP_SUMMARY containing a link to the file and line." },
      ...
      { id: 'D', text: "An error workflow command carrying file, line and column properties along with the message." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'D', text: "Upload the debug bundle to the cache instead, which expires sooner." }
    ],
    correctAnswers: ['A'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "As a list of all three labels, from which the runner picks the matching one." },
      { id: 'B', text: "As the literal string matrix.os, which the runner resolves at dispatch." },
      { id: 'C', text: "As ubuntu-latest, with a container key selecting the operating system per combination." },
      { id: 'D', text: "As an expression referencing the os value of the matrix context." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "The needs context exposes only the job result, so any value must travel as an artifact." },
      { id: 'D', text: "Job outputs containing a value registered as a secret are redacted on the runner and never reach the platform, so a credential has to be re-obtained in the consuming job or stored in a secret manager instead." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'C', text: "The Dependabot configuration file must be present on the default branch to take effect, and a copy on a feature branch is ignored." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A starter workflow published in the organization .github repository." },
      { id: 'B', text: "A repository ruleset defined at organization level that requires the specified workflow to pass, applied to the target repositories." },
      { id: 'C', text: "A reusable workflow that each repository is asked to call from its own pipeline." },
      { id: 'D', text: "A branch protection rule in each repository requiring a status check with the agreed name." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "A composite action for the multi-job piece and a reusable workflow for the setup sequence." },
      { id: 'B', text: "A composite action for both, since composite actions can declare jobs when required." },
      { id: 'C', text: "A reusable workflow for both, since a reusable workflow can be referenced from a step." },
      { id: 'D', text: "A reusable workflow for the multi-job piece and a composite action for the setup sequence." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "The reporting workflow must declare needs on the build workflow, which makes the artifact visible." },
      { id: 'B', text: "Artifacts cannot cross runs under any circumstances, so the coverage data must be committed to a branch." },
      { id: 'C', text: "Artifacts are scoped to the run that created them, so the reporting workflow must fetch the artifact of the triggering run by its run identifier, taken from the workflow_run event payload, using the API or an action that supports a run identifier." },
      { id: 'D', text: "Artifacts are deleted as soon as their run completes, so the build workflow must write to the cache instead." }
    ],
    correctAnswers: ['C'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Whether the source can be reviewed and pinned to an immutable reference, what permissions and secrets the workflow will expose it to, and whether the publisher is maintained and verified." },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'B', text: "A path relative to the consuming repository root, since actions are copied into it." },
      { id: 'C', text: "An absolute path under the runner tool cache, where actions are staged." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Add a since key to the new input naming the version at which the old one stops working." },
      { id: 'B', text: "Mark the old input required false, which emits a deprecation warning automatically." },
      { id: 'C', text: "Remove the old input and rely on the action code to read the raw environment variable, which produces a warning." },
      { id: 'D', text: "Keep the old input declared and attach a deprecationMessage to it, so the runner warns when a caller supplies it while the value still reaches the action." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Call core.warning with the message, which fails the step when the workflow runs in strict mode." },
      { id: 'B', text: "Write the message to standard error, which the runner treats as a failure." },
      { id: 'C', text: "Call core.setFailed with the message, which records the error and sets a failing exit code for the step." },
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Secrets are only exposed if a workflow explicitly disables log masking, which requires administrator rights." },
      { id: 'B', text: "Secrets are safe because masking prevents a secret from ever leaving the runner." },
      { id: 'C', text: "The reviewer is correct: since values are never displayed, write access carries no secret exposure." },
      { id: 'D', text: "Anyone able to merge a change to a workflow file can cause a run to use a secret and print or transmit a transformed version of it, so write access should be treated as equivalent to access to every secret the repository holds; narrowing exposure means environment scoping with reviewers, and protecting the workflow path." }
    ],
    correctAnswers: ['D'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Run the runner as an administrator so it can install whatever a build needs, and rely on the trusted repository boundary." },
      { id: 'B', text: "Run the runner service under a dedicated unprivileged account on a machine that hosts nothing else, restrict its outbound network reach to what builds require, and prefer ephemeral instances so state does not carry between jobs." },
      { id: 'C', text: "Co-locate the runner with the artifact registry to reduce network exposure between them." },
      { id: 'D', text: "Grant the runner host broad outbound access so that dependency resolution never fails, and audit the logs afterwards." }
    ],
    correctAnswers: ['B'],
    type: "single",
  ...
  },
  {
    ...
    options: [
      { id: 'A', text: "Neither, because a name collision makes the reference ambiguous and the workflow fails." },
      ...
      { id: 'C', text: "The organization secret, because organization settings override repository settings." },
