...
### Content
- Added **AWS Certified Generative AI Developer - Professional** (`aws-aip`, AIP-C01) and **AWS Certified AI Business Strategist** (`aws-aib`, AIB-C01, beta) as `planned`, with verified blueprints. Removed the placeholders for **Database - Specialty** (`aws-dbs`) and **Machine Learning - Specialty** (`aws-mls`), whose exams AWS has retired. Their role-path steps were replaced (ML path) or dropped (data path). The catalogue now tracks every exam in AWS's current exam-guide index.
...
### Fixed
- **AWS blueprints checked against every official exam guide** (docs.aws.amazon.com, 2026-09-23).
  - `aws-scs` moves from SCS-C02 to **SCS-C03**, which has six new domains, including separate Detection and Incident Response domains. All 1,000 items were reclassified.
  - `aws-mla` moves to **MLA-C02** (the last English sitting of MLA-C01 is 2026-09-28). Each domain was renamed one-to-one, and the time is now 170 minutes for the beta.
  - `aws-saa` was carrying one SAA-C02 domain name and a 65-minute time; the real exam is 130 minutes.
  - `aws-clf` said 45 minutes instead of 90.
  - One `aws-dop` domain name is shortened to match the guide.
  - `aws-dva` and `aws-sap` already matched; their C03 versions open for registration on 2026-10-27.
  - Planned `aws-soa` becomes **SOA-C03, CloudOps Engineer - Associate**.
  - Planned `aws-ans` gets its real blueprint, a passing score of 700 as the guide states, and a note that it retires on 2026-12-31.
  - **Answer keys fixed:** `aws-scs-73` keyed `PutBucketPolicy` for detecting lifecycle deletion (the answer is `DeleteBucketLifecycle`), and `aws-scs-173` said "other account's root principal" in a single-account scenario. The DDoS Response Team (DRT) is now called the Shield Response Team (SRT).
  - **Still to fix or author:** the SCS classifier flagged more items for doc verification, including ACM exportable certificates, tag-policy enforcement, IRSA vs EKS Pod Identity, the KMS FIPS level and several service names. It also counted only 198 distinct questions in `aws-scs`, with C03 content missing on Incident Response, IAM (Cognito, Verified Permissions, Roles Anywhere), Security Lake, resource control policies, and more. MLA-C02's new foundation-model content is uncovered.
- The app test for the exam clock assumed two-digit minutes, which only held while `aws-saa` was wrongly set to 65 minutes. It now accepts three-digit minutes, as every exam of 100 minutes or more already shows.
- **Docs updated to the current catalogue.** The README, CONTRIBUTING, the roadmap, CONTENT-ARCHITECTURE, CI-AND-SECURITY and DISTRACTOR-REMEDIATION now say 73 certifications, 33 live, 40 planned, 21 still on placeholder blueprints, and 16,500 live questions. Several had said 52, 68, 27 or 32. EXAM-READINESS lists every blueprint verified since September (HashiCorp, GitHub and AWS), and the README links to it.
